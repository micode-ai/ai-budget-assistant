...
/**
 * Mirrors the `useLocalSearchParams` shape `app/story.tsx` used to declare
 * inline, before this body moved here (Task 7) so the desktop Spending Story
 * dialog can host the same component instead of navigating.
 */
export type SpendingStoryPrefill = {
  month?: string;
  year?: string;
};

interface SpendingStoryViewProps {
  initial?: SpendingStoryPrefill;
}

/**
 * The entire body of `app/story.tsx`, moved here unchanged so `StoryDialog`
 * (desktop) and the route (mobile) share one definition. The `initial` prop
 * replaces the route's own `useLocalSearchParams` read — the only
 * substitution made. `loadStory()` still fires from the same mount effect it
 * always did, so hosting it in a dialog changes neither when nor how often it
 * generates: opening the dialog mounts this component exactly the way
 * navigating to the route did, and closing it unmounts, same as navigating
 * away.
 */
export function SpendingStoryView({ initial }: SpendingStoryViewProps) {
  ...
  const [selectedMonth, setSelectedMonth] = useState(
    initial?.month ? Number(initial.month) : now.getMonth() + 1,
  );
  const [selectedYear, setSelectedYear] = useState(
    initial?.year ? Number(initial.year) : now.getFullYear(),
  );
