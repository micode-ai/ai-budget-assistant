...
export interface SetBalanceViewProps {
  /**
   * Which balance is being edited; absent or `''` is the create form. On the
   * mobile route this is the `editId` search param, so a deep link into edit
   * mode keeps working; a dialog holds it in its own state.
   */
  editId?: string;
  /**
   * Asks the owner to change `editId` (`''` clears it). The view switches
   * between create and edit mode by requesting this rather than holding the id
   * itself, so on the route the param — and the URL on web — stays the source
   * of truth exactly as it was before this body left the route file.
   */
  onEditIdChange: (editId: string) => void;
  /**
   * The Cancel button, which only renders in edit mode: `router.back()`-or-
   * clear-the-param on the mobile route today, a dialog's close on desktop.
   * Deliberately NOT fired on a successful save — saving shows an alert and
   * leaves the view open on its balance list, which is what the route does.
   */
  onDone: () => void;
}

/**
 * The entire body of `app/wallet/set-balance.tsx`, moved here unchanged so a
 * desktop dialog and the route can share one definition — nothing under `src/`
 * may import from `app/`. Follows `BudgetCreateForm`: the only substitutions
 * are this screen's three `router` calls, which became the two callbacks above.
 */
export function SetBalanceView({ editId, onEditIdChange, onDone }: SetBalanceViewProps) {
                ...
                <View key={balance.id} style={styles.balanceRow}>
                  ...
                  <View style={styles.rowActions}>
                    <TouchableOpacity
                      ...
                      onPress={() => onEditIdChange(balance.id)}
