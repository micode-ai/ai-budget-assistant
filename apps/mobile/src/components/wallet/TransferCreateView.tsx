...
export interface TransferCreateViewProps {
  /**
   * Called after the transfer is recorded: `router.back()` on the route, a
   * dialog's close on desktop. Threaded straight into `useTransferForm`, which
   * is where the decision actually has to be made — see `TransferFormOptions`.
   */
  onSaved: () => void;
}

/**
 * The entire body of `app/wallet/transfer.tsx`, moved here unchanged so a
 * desktop dialog and the route can share one definition — nothing under `src/`
 * may import from `app/`. Follows `SetBalanceView`.
 *
 * The screen was already a thin composition over `useTransferForm` plus three
 * `src/components/wallet/*` components (ABA-469), so the only substitutions
 * were its two `router` touchpoints: `handleSubmit`'s `router.back()`, which
 * moved into the hook's `onSaved`, and the `Stack.Screen` header.
 *
 * **That header stayed on the route.** Its `headerRight` is a shortcut to
 * `/wallet/transfers` (the full history), and a `Stack.Screen` rendered inside
 * a dialog would try to configure whichever route happens to be underneath it.
 * `TransferDialog` reproduces the same shortcut in its own header, so the
 * dialog still hosts the screen in full.
 */
export function TransferCreateView({ onSaved }: TransferCreateViewProps) {
  const { t } = useTranslation();
  const theme = useTheme();
  const styles = useStyles(createStyles);
  const form = useTransferForm({ onSaved });
