...
export interface ExchangeViewProps {
  /**
   * Prefill for the pair, as raw strings, validated here by `isCurrency`
   * exactly as the route validated its own search params before this body left
   * the route file. On the route these are the `fromCurrency`/`toCurrency`
   * params a `rate_watch_hit` push deep-link supplies (see
   * `src/services/notifications.ts`); a dialog passes neither and gets the
   * USD -> EUR default.
   */
  initialFromCurrency?: string;
  initialToCurrency?: string;
  /**
   * Called after a successful `addExchange`: `router.back()` on the route, a
   * dialog's close on desktop. The exchange is already recorded by the time
   * this fires — it is "we are done here", not "save".
   */
  onSaved: () => void;
}

/**
 * The entire body of `app/wallet/exchange/index.tsx`, moved here unchanged so a
 * desktop dialog and the route can share one definition — nothing under `src/`
 * may import from `app/`. Follows `SetBalanceView`: the only substitutions are
 * this screen's two `router` touchpoints, which became the props above.
 *
 * **The route's `Stack.Screen` header stayed on the route.** Its `headerRight`
 * is a shortcut to `/wallet/exchanges` (the full history), and a `Stack.Screen`
 * rendered inside a dialog would try to configure whichever route happens to
 * be underneath it. `ExchangeDialog` reproduces that same shortcut in its own
 * header instead, so the dialog still hosts the screen in full.
 */
export function ExchangeView({ initialFromCurrency, initialToCurrency, onSaved }: ExchangeViewProps) {
  ...
  const [fromCurrency, setFromCurrency] = useState<Currency>(
    isCurrency(initialFromCurrency) ? initialFromCurrency : 'USD',
  );
  const [toCurrency, setToCurrency] = useState<Currency>(
    isCurrency(initialToCurrency) ? initialToCurrency : 'EUR',
  );
