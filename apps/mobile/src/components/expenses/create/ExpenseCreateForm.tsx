 ...
 * `expense/[id].tsx`'s `handleCopy` (duplicate), `useReceiptSave.ts`'s
 * `handleEditExpense` (receipt scan → manual edit), `IncomeDetailsCard.tsx`'s
 * "Record repayment" row, and `AddDebtSheet.tsx`'s "lend money" shortcut.
 * Mirrors the `useLocalSearchParams` shape the route used to declare inline.
 */
export type ExpenseCreatePrefill = {
  amount?: string;
  description?: string;
  merchant?: string;
  categoryId?: string;
...
export function ExpenseCreateForm({ initial, onDone }: ExpenseCreateFormProps) {
  useEffect(() => {
  ...
  const debt = useDebtExpenseFields({
    isDebt: initial?.isDebt === 'true',
    debtContactName: initial?.debtContactName,
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const newExpense = await addExpense({
        ...
        relatedDebtIncomeId: isDebtRepayment ? initial?.relatedDebtIncomeId : undefined,
      ...
      >
        <ScrollView style={styles.flex} contentContainerStyle={styles.scrollContent}>
          ...
          <DebtExpenseFields
            ...
            repaymentContactName={initial?.debtContactName}
          />
