...
<html>
  ...
  <body>
    ...
    <script>
      ...
      window.__renderShield = function (payloadJson) {
        try {
...
/**
 * Hidden WebView that renders a 1080x1920 "story card" PNG from an Inflation Shield payload
...
export const InflationShieldShareCard = forwardRef<InflationShieldShareCardHandle>(function InflationShieldShareCard(_props, ref) {
  ...
  const tagRef = useRef<string>('share');
  ...
  const onMessage = (event: WebViewMessageEvent) => {
    ...
    (async () => {
      try {
        ...
        const fileName = `inflation-shield-${tagRef.current}.png`;
  ...
  useImperativeHandle(ref, () => ({
    share: (payload: ShieldSharePayload) =>
      new Promise<boolean>((resolve) => {
        ...
        const js = `window.__renderShield(${JSON.stringify(JSON.stringify(payload))}); true;`;
