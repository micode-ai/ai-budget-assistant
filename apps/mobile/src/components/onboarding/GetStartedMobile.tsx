...
/**
 * Imperative escape hatch consumed ONLY by the desktop wrapper
 * (`onboarding/desktop/GetStartedDesktop.tsx`) — the phone never takes a ref.
 *
 * On the phone (and on narrow web) every entry is a `router.push`: this
 * screen stays on the stack underneath, and when the pushed entry screen pops
 * back, `useFocusEffect` below fires `finish()` if a transaction landed while
 * it was away. On desktop an entry opens a DIALOG over this same screen
 * instead — nothing ever leaves and nothing ever regains focus, so that
 * mechanism can never run. `finishIfPending` is the same check, callable from
 * outside once a dialog closes.
 */
export interface GetStartedHandle {
  finishIfPending: () => void;
}

interface GetStartedMobileProps {
  /**
   * How an entry card resolves a tap. Defaults to `router.push` — today's
   * behaviour, unchanged for the phone and for narrow web. The desktop
   * wrapper passes a handler that consults `resolveDialogAction`'s table
   * instead, so three of the four entries open a dialog over this screen
   * rather than replacing it. `markSeen()` still fires here regardless of
   * which path is taken, so every entry — dialog or navigation — stops this
   * screen from ever being shown to an established user again.
   */
  onSelect?: (route: string) => void;
}

export const GetStartedMobile = forwardRef<GetStartedHandle, GetStartedMobileProps>(
  function GetStartedMobile({ onSelect }, ref) {
    ...
    //
    // This is the navigate-path mechanism only. The dialog path (desktop) never
    // triggers a focus event — see `GetStartedHandle` above.
    useFocusEffect(
      useCallback(() => {
