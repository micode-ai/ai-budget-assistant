...
/**
 * Prefill fields the various "hand off to the create form" flows pass today —
 * `AddDebtSheet.tsx`'s "borrow money" shortcut, `ExpenseDetailsCard.tsx`'s
 * "Record repayment" row, and `income/receipt.tsx`'s `handleEditIncome`
 * (receipt scan → manual edit). Mirrors the `useLocalSearchParams` shape the
 * route used to declare inline.
 */
...
interface IncomeCreateFormProps {
  initial?: IncomeCreatePrefill;
  onDone: () => void;
  onOpenVoice?: () => void;
  onOpenReceipt?: () => void;
}

export function IncomeCreateForm({ initial, onDone, onOpenVoice, onOpenReceipt }: IncomeCreateFormProps) {
  useEffect(() => {
  ...
  const isDebtRepayment = initial?.isDebtRepayment === 'true';
  ...
  const [currencyCode, setCurrencyCode] = useState<Currency>(
    (initial?.currencyCode as Currency) || user?.currencyCode || 'USD',
  );
  ...
  // Debt state
  const [isDebt, setIsDebt] = useState(initial?.isDebt === 'true');
  ...
  const handleSubmit = async () => {
    ...
    try {
      await addIncome({
        ...
        relatedDebtExpenseId: isDebtRepayment ? initial?.relatedDebtExpenseId : undefined,
      });
        ...
        <ScrollView style={styles.flex} contentContainerStyle={styles.scrollContent}>
          ...
          {isDebtRepayment ? (
            <View style={styles.debtBanner}>
              ...
              {initial?.debtContactName ? (
                <Text style={styles.debtBannerContact}>{initial.debtContactName}</Text>
              ) : null}
            </View>
          ) : (
            <View style={styles.fieldContainer}>
