...
export interface BudgetCreateFormProps {
  /** `router.back()` on the mobile route today; a dialog's close on desktop. */
  onDone: () => void;
}

/**
 * The entire body of `app/budget/new.tsx`, moved here unchanged so
 * `BudgetCreateDialog` (desktop) and the route (mobile) share one
 * definition. The route has no `useLocalSearchParams` to prefill from, so
 * unlike `ExpenseCreateForm`/`IncomeCreateForm` this takes no `initial`
 * prop — `onDone` is the only substitution made, replacing the route's own
 * `router.back()`.
 */
export function BudgetCreateForm({ onDone }: BudgetCreateFormProps) {
  useEffect(() => {
