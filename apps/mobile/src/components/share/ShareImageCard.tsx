...
export interface ShareCardPayload {
  /** Used only to name the generated file (`<fileNamePrefix>-<fileTag>.png`). */
...
export interface ShareImageCardProps {
  /** Name of the `window.<name>` bridge the injected JS calls — must be unique per mounted card. */
  renderFnName: string;
  gradientFrom: string;
  gradientTo: string;
  /** Generated file is named `<fileNamePrefix>-<payload.fileTag>.png`. */
...
function buildCardHtml({ gradientFrom, gradientTo, renderFnName }: Pick<ShareImageCardProps, 'gradientFrom' | 'gradientTo' | 'renderFnName'>): string {
  return `<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8" />
  ...
  <body>
    ...
    <script>
      ...
      window.${renderFnName} = function (payloadJson) {
        try {
...
/**
 * Hidden WebView that renders a 1080x1920 "story card" PNG from a share payload
 ...
 *
 * Generic mechanism shared by feature-specific wrappers (e.g. `WrappedShareCard`,
 * `InflationShieldShareCard`) — those own only their gradient/payload/copy.
 */
export const ShareImageCard = forwardRef<ShareImageCardHandle, ShareImageCardProps>(function ShareImageCard(
  { renderFnName, gradientFrom, gradientTo, fileNamePrefix },
  ref
) {
  ...
  const tagRef = useRef<number | string>('share');

  const cardHtml = useMemo(
    () => buildCardHtml({ gradientFrom, gradientTo, renderFnName }),
    [gradientFrom, gradientTo, renderFnName]
  );
  ...
  const onMessage = (event: WebViewMessageEvent) => {
    ...
    (async () => {
      try {
        ...
        const fileName = `${fileNamePrefix}-${tagRef.current}.png`;
  ...
  useImperativeHandle(ref, () => ({
    share: (payload: ShareCardPayload) =>
      new Promise<boolean>((resolve) => {
        ...
        tagRef.current = payload.fileTag;
        resolverRef.current = resolve;
        timeoutRef.current = setTimeout(() => finish(false), SHARE_TIMEOUT_MS);

        const js = `window.${renderFnName}(${JSON.stringify(JSON.stringify(payload))}); true;`;
