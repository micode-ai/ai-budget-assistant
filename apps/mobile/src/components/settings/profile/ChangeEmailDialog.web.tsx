...
/**
 * Kept in step by hand with the native no-op's copy in `ChangeEmailDialog.tsx`
 * — one prop, so a `.types.ts` third file (the `SettingsRoute` convention)
 * would cost more than it protects. `WebShell` does the same.
 */
interface Props {
  onClose: () => void;
}
 ...
 *
 * ## Why this is a `.web.tsx` and not a `desktop` branch
 *
 * The scrim below is a raw `<div>`, and a `<div>` in a native render is not
 * dead weight — it throws. The four shared components that reflow through
 * §5a's runtime `desktop?: boolean` flag (`SafeToSpendSheet`,
 * `InflationIndexSection`, `AccountSwitcher`, `FinancialHealthWidget`) all
 * render React Native primitives on both sides, so the worst their unused
 * branch can be is unused. This one is different in kind.
 *
 * Shipping it to the phone was safe only by a chain of reasoning — the
 * provider that sets `desktop: true` lives in `SettingsShell`, imported solely
 * by `SettingsRoute.web.tsx` — and that chain has to be re-derived by anyone
 * who touches either end of it. Metro resolving this file on web and the real
 * no-op `ChangeEmailDialog.tsx` everywhere else needs no reasoning at all.
 * Same split, same reason, as `WebShell` and `ShareImageCard`: the native file
 * is genuinely nothing, never a re-export of this one, which would drag the
 * whole tree back into the graph it was removed from.
 *
