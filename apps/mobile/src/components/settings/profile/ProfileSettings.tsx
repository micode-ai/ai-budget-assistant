 ...
 * subscription plan.
 *
 * Lifted out of `app/settings/profile.tsx` unchanged so the desktop settings
 * shell can host it as a pane — `src/` may not import from `app/`, so a route
 * file is not somewhere a pane can render from. The route is now a thin
 * wrapper around `SettingsRoute`, the one file that decides mobile vs desktop.
 * The frame owns the `SafeAreaView` this file used to own, and
 * `SettingsScreenKeyboardScroll` — not `SettingsScreenScroll` — is what
 * replaces the `KeyboardAwareScreen`: this screen has four text inputs, two of
 ...
 *
 * Every write here ends in a `showAlert`, never in a navigation — `handleSaveName`,
 * `handleTimezoneChange` and `handleSavePaymentMethods` all alert on success, and
 * `handleCurrencyChange` delegates to `authStore.setCurrency`, which is optimistic
 * and deliberately silent. Checked rather than assumed, because a screen that
 * signals by navigating has nothing to say inside a pane, where there is no back.
 * None of these do. `showAlert`, not `Alert.alert`: the latter is a no-op on
 * react-native-web, so on desktop the user would get no confirmation at all.
 *
 * ## The email row
 *
 * On mobile it pushes `/settings/change-email`, as it always has. In a pane it
...
export function ProfileSettings() {
  ...
  /**
   * Mounting the dialog is what makes a half-finished change resumable: it is
   * rendered conditionally, so it unmounts on close and remounts on open, and
   * `ChangeEmailView`'s mount effect re-reads the pending record from
   * `secureStorage` — the same path an app restart takes. Nothing on this side
   * clears that record, and nothing here should start.
   */
  const handleChangeEmail = () => {
    ...
    router.push('/settings/change-email' as any);
  };
      ...
      <SettingsScreenKeyboardScroll style={styles.scrollView} contentContainerStyle={styles.content}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.fieldRow}>
            ...
            <TouchableOpacity style={styles.fieldValueRow} onPress={handleChangeEmail}>
