export default {
  common: {
    total: 'Razem',
    save: 'Zapisz',
    cancel: 'Anuluj',
    delete: 'Usuń',
    edit: 'Edytuj',
    update: 'Aktualizuj',
    error: 'Błąd',
    back: 'Wróć',
    done: 'Gotowe',
    loading: 'Ładowanie...',
    success: 'Sukces',
    of: 'z',
    uncategorized: 'Bez kategorii',
    unknownProject: 'Nieznany projekt',
    unknownTag: 'Nieznany tag',
    ok: 'OK',
    remove: 'Usuń',
    copy: 'Kopiuj',
    share: 'Udostępnij',
    retry: 'Ponów',
    clear: 'Wyczyść',
    yes: 'Tak',
    no: 'Nie',
    showAll: 'Pokaż wszystko',
    duplicate: 'Duplikuj',
    selectMultiple: 'Zaznacz wiele',
    import: 'Importuj',
    addedBy: 'Dodane przez {{name}}',
    deleteConfirmTitle: 'Usuń transakcję',
    deleteConfirmMessage: 'Czy na pewno? Tej akcji nie można cofnąć.',
    confirm: 'Potwierdź',
    search: 'Szukaj',
    noResults: 'Brak wyników',
  },
  dates: {
    today: 'Dzisiaj',
    yesterday: 'Wczoraj',
    daysAgo: '{{count}} dni temu',
    weeksAgo_one: '{{count}} tydzień temu',
    weeksAgo_other: '{{count}} tyg. temu',
    monthsAgo_one: '{{count}} miesiąc temu',
    monthsAgo_other: '{{count}} mies. temu',
    yearsAgo_one: '{{count}} rok temu',
    yearsAgo_other: '{{count}} lat temu',
  },
  nav: {
    dashboard: 'Pulpit',
    expenses: 'Transakcje',
    budgets: 'Budżet',
    analytics: 'Analityka',
    aiChat: 'Czat AI',
    newExpense: 'Nowy wydatek',
    expenseDetails: 'Szczegóły wydatku',
    newBudget: 'Nowy budżet',
    budgetDetails: 'Szczegóły budżetu',
    accounts: 'Konta',
    createAccount: 'Utwórz konto',
    accountSettings: 'Ustawienia konta',
    settings: 'Ustawienia',
    inviteMember: 'Zaproś',
    joinAccount: 'Dołącz',
    adminPanel: 'Panel administratora',
    income: 'Przychody',
    newIncome: 'Nowy przychód',
    incomeDetails: 'Szczegóły przychodu',
    investmentDashboard: 'Inwestycje',
    assetSearch: 'Szukaj aktywów',
    assetDetails: 'Szczegóły aktywa',
    investmentTransaction: 'Transakcja',
    investmentAnalytics: 'Analityka',
    calendar: 'Kalendarz',
  },
  auth: {
    appName: 'AI Budget',
    appTagline: 'Śledź swoje wydatki z pomocą AI',
    signIn: 'Zaloguj się',
    signUp: 'Zarejestruj się',
    email: 'E-mail',
    password: 'Hasło',
    confirmPassword: 'Potwierdź hasło',
    fullName: 'Imię i nazwisko',
    createAccount: 'Utwórz konto',
    startTracking: 'Zacznij śledzić swoje finanse',
    noAccount: 'Nie masz konta? ',
    haveAccount: 'Masz już konto? ',
    useBiometric: 'Użyj {{type}}',
    quickLogin: 'Szybkie logowanie',
    currency: 'Waluta',
    language: 'Język',
    forgotPassword: 'Zapomniałeś hasła?',
    resetPassword: 'Zresetuj hasło',
    sendCode: 'Wyślij kod',
    newPassword: 'Nowe hasło',
    confirmNewPassword: 'Potwierdź nowe hasło',
    codeSent: 'Jeśli ten email jest zarejestrowany, kod resetujący został wysłany.',
    passwordResetSuccess: 'Hasło zostało zresetowane. Zaloguj się ponownie.',
    backToLogin: 'Powrót do logowania',
    resendCode: 'Wyślij kod ponownie',
    enterResetCode: 'Wprowadź 6-cyfrowy kod wysłany na Twój email',
    enterEmail: 'Podaj swój email, aby otrzymać kod resetujący',
    verifyEmail: 'Weryfikacja e-mail',
    verify: 'Weryfikuj',
    verificationSuccess: 'E-mail zweryfikowany pomyślnie!',
    enterVerificationCode: 'Wprowadź 6-cyfrowy kod wysłany na {{email}}',
    signingIn: 'Logowanie...',
    loadingData: 'Ładowanie Twoich danych...',
    continueWithGoogle: 'Kontynuuj z Google',
    or: 'lub',
    googleTermsNote: 'Kontynuując, akceptujesz nasz Regulamin i Politykę prywatności',
  },
  dashboard: {
    hello: 'Cześć, {{name}}!',
    monthlyBudget: 'Budżet miesięczny',
    used: '{{percent}}% wykorzystano',
    addExpense: 'Dodaj wydatek',
    voiceInput: 'Głosowo',
    scanReceipt: 'Skanuj paragon',
    voiceIncome: 'Głosowy przychód',
    scanInvoice: 'Skanuj fakturę',
    recentExpenses: 'Ostatnie wydatki',
    seeAll: 'Zobacz wszystko',
    noExpenses: 'Brak wydatków',
    addFirstExpense: 'Dodaj swój pierwszy wydatek, aby rozpocząć',
    expense: 'Wydatek',
    activeBudgets: 'Aktywne budżety',
    noBudgets: 'Brak budżetu',
    createBudgetHint: 'Utwórz budżet, aby śledzić swoje wydatki',
    walletBalances: 'Salda portfela',
    exchangeCurrency: 'Kantor',
    currencyConverter: 'Konwerter',
    transfers: 'Przelewy',
    totalIncome: 'Przychody',
    totalExpenses: 'Wydatki',
    netProfit: 'Zysk netto',
    netProfitSubtitle: 'Ostatnie 6 miesięcy',
    netCapital: 'Kapitał netto',
    netCapitalSubtitle: 'Łączny majątek',
    purchaseRequest: 'Wniosek o zakup',
    shoppingList: 'Zakupy',
    shoppingListFull: 'Lista zakupów',
  },
  expenses: {
    noExpenses: 'Brak wydatków',
    addFirst: 'Dodaj swój pierwszy wydatek, naciskając przycisk +',
    addExpense: 'Dodaj wydatek',
    manualEntry: 'Ręczne wprowadzanie',
    voiceInput: 'Głosowo',
    scanReceipt: 'Skanuj paragon',
    tabExpenses: 'Wydatki',
    tabIncome: 'Przychody',
    periodWeek: 'Tydzień',
    periodMonth: 'Miesiąc',
    periodYear: 'Rok',
    periodAll: 'Wszystko',
    periodCustom: 'Własny',
    categoryAll: 'Wszystkie',
    categoryNone: 'Bez kategorii',

    searchPlaceholder: 'Szukaj transakcji...',
    merchant: 'Sprzedawca',
    merchantPlaceholder: 'np. Biedronka, Amazon',
    merchantAll: 'Wszyscy sprzedawcy',
    merchantNone: 'Brak sprzedawców',
    merchantsSelected: 'Wybrano: {{count}}',
    bulkSelect: 'Zaznacz',
    bulkCancel: 'Anuluj',
    bulkSelectAll: 'Zaznacz wszystko',
    bulkSelected: '{{count}} zaznaczonych',
    bulkSetCategory: 'Ustaw kategorię',
    bulkAddTag: 'Dodaj tag',
    bulkDelete: 'Usuń',
    bulkDeleteConfirm: 'Usunąć {{count}} wydatki?',
    bulkDeleteConfirmMessage: 'Tej akcji nie można cofnąć.',
    bulkCategoryApplied: 'Kategoria zastosowana do {{count}} wydatków',
    bulkTagsApplied: 'Tagi dodane do {{count}} wydatków',
    bulkDeleted: '{{count}} wydatków usuniętych',
    receivable: 'Należność',
    merge: {
      title: 'Scal transakcje',
      subtitle: 'Wybierz transakcję do zachowania i pola do połączenia.',
      survivorQuestion: 'Którą transakcję zachować?',
      keep: 'Zachowaj',
      keepFields: 'Pola do zachowania z drugiej transakcji',
      fieldMerchant: 'Sprzedawca',
      fieldNotes: 'Notatki',
      fieldCategory: 'Kategoria',
      fieldProject: 'Projekt',
      fieldTags: 'Tagi',
      fieldReceipt: 'Paragon',
      confirm: 'Scal',
      merged: 'Transakcje scalone.',
      cannotMergeSelf: 'Nie można scalić transakcji z samą sobą.',
      currencyNote: 'Różne waluty — zostanie zachowany tylko jeden wpis.',
    },
  },
  categorize: {
    bannerText: 'Wydatki bez kategorii: {{count}}',
    bannerTextIncome: 'Przychody bez kategorii: {{count}}',
    bannerAction: 'Zaproponuj kategorie',
    bannerActionExpense: 'Rozłóż wydatki',
    bannerActionIncome: 'Rozłóż przychody',
    bannerShort: 'Bez kategorii: {{count}}',
    summary: 'Przypisano {{assigned}} z {{total}}',
    summaryNew: 'nowe kategorie: {{count}}',
    newBadge: 'Nowa',
    change: 'Zmień',
    searchPlaceholder: 'Szukaj kategorii',
    applyShort: 'Zastosuj · {{count}}',
    title: 'Kategoryzuj wydatki',
    titleIncome: 'Kategoryzuj przychody',
    analyzing: 'Analizuję wydatki…',
    analyzingIncome: 'Analizuję przychody…',
    newCategory: 'Nowa kategoria',
    existingCategory: 'Istniejąca kategoria',
    unassigned: 'Nie udało się ustalić',
    choose: 'Wybierz',
    createNew: '+ Utwórz nową kategorię',
    newNamePlaceholder: 'Nazwa kategorii',
    apply: 'Zastosuj ({{count}})',
    applyWithNew: 'Zastosuj ({{count}}) · nowe kategorie: {{created}}',
    done: 'Skategoryzowano: {{count}} · nowe kategorie: {{created}}',
    nothingToSuggest: 'Na razie nie ma czego proponować.',
    limitReached: 'Limit podpowiedzi AI na dziś wyczerpany — pokazano tylko te z reguł.',
    skippedEncrypted: 'Pominięto zaszyfrowane wydatki: {{count}}',
    skippedEncryptedIncome: 'Pominięto zaszyfrowane przychody: {{count}}',
    error: 'Nie udało się wczytać propozycji.',
    retry: 'Spróbuj ponownie',
  },
  expensesDesktop: {
    colDate: 'Data',
    colDescription: 'Opis',
    colCategory: 'Kategoria',
    colAccount: 'Dodane przez',
    colAmount: 'Kwota',
    summaryCount: 'Transakcje',
    moreCurrencies_one: '+{{count}} inna waluta',
    moreCurrencies_few: '+{{count}} inne waluty',
    moreCurrencies_many: '+{{count}} innych walut',
    moreCurrencies_other: '+{{count}} innych walut',
    emptyState: 'Brak transakcji',
    facetsHeading: 'Filtry',
    clearAllFilters: 'Wyczyść wszystko',
    facetPeriod: 'Okres',
    facetKind: 'Pokaż',
    facetCategory: 'Kategoria',
    facetMerchant: 'Sprzedawca',
    kindAll: 'Wszystko',
    noMerchant: 'Brak sprzedawcy',
    clearFacet: 'Wyczyść',
    noOptions: 'Brak wyników dla bieżących filtrów',
    showMore: '+{{count}} więcej',
    showLess: 'Pokaż mniej',
    filtersLabel: 'Filtry ({{count}})',
    dialogClose: 'Zamknij',
    discardChangesTitle: 'Odrzucić zmiany?',
    discardChangesMessage: 'Masz niezapisane zmiany. Zamknięcie teraz je odrzuci.',
    discardChangesConfirm: 'Odrzuć',
    selectRow: 'Zaznacz wiersz',
    selectAllRows: 'Zaznacz wszystkie widoczne wiersze',
    clearSelection: 'Wyczyść zaznaczenie',
    rowActions: 'Akcje wiersza',
  },
  incomes: {
    noIncomes: 'Brak przychodów',
    addFirst: 'Dodaj swój pierwszy przychód, naciskając przycisk +',
    addIncome: 'Dodaj przychód',
  },
  incomeNew: {
    amountPlaceholder: '0,00',
    description: 'Opis',
    descriptionPlaceholder: 'Skąd pochodzi ten przychód?',
    date: 'Data',
    category: 'Kategoria',
    notes: 'Notatki',
    notesPlaceholder: 'Opcjonalne notatki...',
    saving: 'Zapisywanie...',
    saveIncome: 'Zapisz przychód',
  },
  incomeDetail: {
    notFound: 'Nie znaleziono przychodu',
    currency: 'Waluta',
    noDescription: 'Brak opisu',
    description: 'Opis',
    date: 'Data',
    category: 'Kategoria',
    notes: 'Notatki',
    deleteTitle: 'Usuń przychód',
    deleteConfirm: 'Czy na pewno chcesz usunąć ten przychód?',
  },
  budgets: {
    noBudgets: 'Brak budżetu',
    createHint: 'Utwórz budżet, aby zacząć śledzić swoje limity wydatków',
    createBudget: 'Utwórz budżet',
    onTrack: 'W normie',
    overBudget: 'Przekroczono budżet',
    spent: 'wydano',
    remaining: 'pozostało',
    periods: {
      daily: 'Dzienny',
      weekly: 'Tygodniowy',
      monthly: 'Miesięczny',
      yearly: 'Roczny',
      custom: 'Własny',
    },
  },
  analytics: {
    week: 'Tydzień',
    month: 'Miesiąc',
    year: 'Rok',
    viewExpenses: 'Zobacz wydatki',
    totalSpent: 'Łącznie wydano',
    avgPerDay: 'Średnio dziennie',
    transactions: 'transakcje',
    this_week: 'Ten Tydzień',
    this_month: 'Ten Miesiąc',
    this_year: 'Ten Rok',
    spendingTrend: 'Trend wydatków',
    spendingByMonth: 'Wydatki wg miesiąca',
    spendingByCategory: 'Wydatki wg kategorii',
    noData: 'Brak danych o wydatkach',
    addExpensesHint: 'Dodaj wydatki, aby zobaczyć podział na kategorie',
    categoryDetails: 'Szczegóły kategorii',
    quickInsights: 'Szybki przegląd',
    topCategory: 'Główna kategoria',
    topCategoryText: '„{{category}}" to Twoja najwyższa kategoria wydatków w tym {{range}}.',
    peakSpendingDay: 'Dzień największych wydatków',
    peakSpendingText: 'Twoje największe wydatki były dnia {{date}}.',
    dailyBudgetTip: 'Dzienna wskazówka budżetowa',
    dailyBudgetText: 'Aby utrzymać się w budżecie, staraj się nie wydawać dziennie więcej niż {{amount}}.',
    allCurrencies: 'Wszystkie',
    exportReport: 'Eksportuj raport',
    topItems: 'Najczęściej kupowane',
    itemName: 'Artykuł',
    itemTotalSpent: 'Łącznie wydano',
    itemPurchaseCount: 'Zakupów',
    months: {
      jan: 'Sty', feb: 'Lut', mar: 'Mar', apr: 'Kwi',
      may: 'Maj', jun: 'Cze', jul: 'Lip', aug: 'Sie',
      sep: 'Wrz', oct: 'Paź', nov: 'Lis', dec: 'Gru',
    },
    days: {
      sun: 'Ndz', mon: 'Pon', tue: 'Wt', wed: 'Śr',
      thu: 'Czw', fri: 'Pt', sat: 'Sob',
    },
    budgetVsActual: 'Budżet vs Rzeczywistość',
    noBudgetsHint: 'Utwórz budżety, aby śledzić postępy',
    budgetLabel: 'Budżet',
    actualLabel: 'Rzeczywiste',
    onTrack: 'W normie',
    overBudget: 'Przekroczony',
    dayOfWeekTitle: 'Wydatki wg dnia tygodnia',
    peakDayInsight: 'Najwięcej wydajesz w {{day}}',
    periodUp: 'więcej niż poprzedni {{period}}',
    periodDown: 'mniej niż poprzedni {{period}}',
    vsAvgUp: '{{pct}}% powyżej średniej 3 miesięcy',
    vsAvgDown: '{{pct}}% poniżej średniej 3 miesięcy',
    totalSavings: 'Oszczędności z paragonów',
    totalSavingsText: 'Zaoszczędziłeś {{amount}} na rabatach w tym {{range}}.',
    totalDepositsPaid: 'Zapłacona kaucja',
    totalDepositsPaidText: 'Zapłaciłeś {{amount}} kaucji za opakowania zwrotne w tym {{range}}.',
    savingsDetailDiscountTitle: 'Oszczędności z rabatów',
    savingsDetailDepositTitle: 'Zapłacona kaucja',
    savingsDetailByStore: 'Według sklepu',
    savingsDetailRecent: 'Ostatnie paragony',
    savingsDetailEncrypted: 'To konto jest w pełni zaszyfrowane, więc nie możemy tego rozbić na szczegóły.',
    savingsDetailEmpty: 'Nic jeszcze nie zarejestrowano w tym okresie.',
    savingsDetailApprox: 'Niektóre kwoty wykorzystują przybliżony kurs wymiany.',
    savingsDetailDiscountNote: 'Rabaty już zastosowane na Twoich paragonach — to nie jest obietnica przyszłych ofert.',
    savingsDetailDepositNote: 'To, co zapłaciłeś przy kasie za opakowania zwrotne — to nie zwrot; zwrócone butelki i puszki nie są śledzone.',
    byMerchant: 'Według sprzedawców',
    merchantOther: 'Inne',
    byTag: 'Według tagów',
    byProject: 'Według projektów',
    byIncomeCategory: 'Przychody według kategorii',
    incomeCategoryOther: 'Inne',
    tagBreakdown: 'Podział tagów',
    tagTrends: 'Trendy tagów',
    projectComparison: 'Porównanie projektów',
    topTags: 'Najpopularniejsze tagi',
    filterByTag: 'Filtruj według tagu',
    filterByProject: 'Filtruj według projektu',
    tagAnalyticsComingSoon: 'Analityka tagów wkrótce',
    projectAnalyticsComingSoon: 'Analityka projektów wkrótce',
  },
  chat: {
    title: 'Asystent budżetowy AI',
    subtitle: 'Zapytaj mnie o swoje finanse, wzorce wydatków lub poproś o spersonalizowane porady budżetowe.',
    topExpenses: 'Największe wydatki',
    topExpensesQ: 'Na co wydałem najwięcej w tym miesiącu?',
    budgetStatus: 'Status budżetu',
    budgetStatusQ: 'Czy mam budżet pod kontrolą?',
    savingTips: 'Porady oszczędnościowe',
    savingTipsQ: 'Daj mi wskazówki, jak oszczędzać pieniądze',
    placeholder: 'Zapytaj o swój budżet...',
    thinking: 'Myślę...',
    processingVoice: 'Przetwarzanie głosu...',
    errorMessage: 'Przepraszam, wystąpił błąd. Sprawdź połączenie i spróbuj ponownie.',
    copySuccess: 'Skopiowano!',
    confirmAction: 'Potwierdź',
    rejectAction: 'Anuluj',
    actionConfirmed: 'Akcja potwierdzona',
    actionRejected: 'Akcja anulowana',
    confirming: 'Przetwarzanie...',
    actionCreateExpense: 'Dodaj wydatek',
    actionCreateIncome: 'Dodaj przychód',
    actionCreateBudget: 'Utwórz budżet',
    actionCreateCategory: 'Utwórz kategorię',
    actionGetExpenses: 'Wydatki',
    actionGetBudgetStatus: 'Status budżetu',
    actionGetCategoryBreakdown: 'Podział na kategorie',
    actionAddShoppingList: 'Dodano do listy zakupów',
    actionRemoveShoppingList: 'Usunięto z listy zakupów',
    actionShoppingNotFound: 'Nie znaleziono: {{names}}',
    actionShoppingSuggestions: 'Sugestie zakupowe',
    shoppingSuggestionsRestock: 'Kończy się',
    shoppingSuggestionsDeals: 'Okazje',
    actionInflationShield: 'Tarcza antyinflacyjna',
    actionDepositTotal: 'Zapłacona kaucja',
    actionDiscountTotal: 'Uzyskane rabaty',
    andMore: '+{{count}} więcej',
    confirmTitle: 'Potwierdź akcję',
    resultSuccess: 'Zakończono pomyślnie',
    resultFailed: 'Akcja nie powiodła się',
    resultUndone: 'Cofnięto',
    amount: 'Kwota',
    category: 'Kategoria',
    date: 'Data',
    period: 'Okres',
    spent: 'wydano',
    remaining: 'pozostało',
    budget: 'Budżet',
    history: 'Historia',
    historyTitle: 'Rozmowy',
    historyEmpty: 'Brak rozmów',
    newConversation: 'Nowa rozmowa',
    loadingHistory: 'Ładowanie...',
    conversationUntitled: 'Rozmowa',
    shared: 'Wspólny',
    private: 'Prywatny',
    renameConversation: 'Zmień nazwę rozmowy',
    deleteConversationTitle: 'Usuń rozmowę',
    deleteSharedConversationMessage: 'Ta rozmowa jest wspólna. Jeśli ją usuniesz, zniknie ona wszystkim członkom konta, a nie tylko Tobie. Tej akcji nie można cofnąć.',
    conversationActions: 'Akcje rozmowy',
    pinConversation: 'Przypnij',
    unpinConversation: 'Odepnij',
  },
  expenseNew: {
    amountPlaceholder: '0,00',
    description: 'Opis',
    descriptionPlaceholder: 'Na co był ten wydatek?',
    date: 'Data',
    category: 'Kategoria',
    saving: 'Zapisywanie...',
    saveExpense: 'Zapisz wydatek',
  },
  expenseDetail: {
    moveTitle: 'Przenieś na konto',
    moveSubtitle: 'Wybierz konto, na które przenieść ten wydatek. Tagi, projekt i kategoria nie zostaną przeniesione.',
    moveSuccessTitle: 'Wydatek przeniesiony',
    moveSuccess: 'Przeniesiono na {{account}}.',
    moveError: 'Nie udało się przenieść wydatku. Spróbuj ponownie.',
    notFound: 'Nie znaleziono wydatku',
    amount: 'Kwota',
    currency: 'Waluta',
    noDescription: 'Brak opisu',
    description: 'Opis',
    date: 'Data',
    category: 'Kategoria',
    notes: 'Notatki',
    syncStatus: 'Status synchronizacji',
    sourceManual: 'Ręczne wprowadzanie',
    sourceVoice: 'Głosowo',
    sourceOcr: 'Skan paragonu',
    sourceImport: 'Zaimportowano',
    sourceNotification: 'Autozapis',
    extractItems: 'Wyodrębnij pozycje',
    extractingItems: 'Odczytujemy paragon…',
    extractItemsTitle: 'Zamień pozycje',
    extractItemsConfirm: 'Ten wydatek ma już {{count}} pozycji. Zastąpić je pozycjami odczytanymi z paragonu?',
    extractItemsNoItems: 'Nie znaleziono pozycji na paragonie.',
    extractItemsDone: 'Odczytano pozycje: {{count}}',
    extractItemsFailed: 'Nie udało się odczytać paragonu. Spróbuj ponownie.',
    deleteTitle: 'Usuń wydatek',
    deleteConfirm: 'Czy na pewno chcesz usunąć ten wydatek?',
    copy: 'Kopiuj',
    receiptItems: 'Pozycje paragonu',
    noItems: 'Brak pozycji',
    addItem: 'Dodaj pozycję',
    editItem: 'Edytuj pozycję',
    itemDescription: 'Opis',
    itemQuantity: 'Ilość',
    itemUnitPrice: 'Cena jedn.',
    itemTotalPrice: 'Suma',
    itemSaved: 'Pozycja zapisana',
    itemDeleted: 'Pozycja usunięta',
    confirmDeleteItem: 'Czy na pewno chcesz usunąć tę pozycję?',
    receiptImage: 'Zdjęcie paragonu',
    viewImage: 'Wyświetl',
    shareImage: 'Udostępnij',
    saveImage: 'Zapisz',
    imageSaved: 'Obraz zapisany w galerii',
    galleryPermissionDenied: 'Dostęp do galerii jest wymagany do zapisywania obrazów',
    tapToOpen: 'Dotknij, aby otworzyć',
    deleteImage: 'Usuń zdjęcie',
    replaceImage: 'Zmień zdjęcie',
    confirmDeleteImage: 'Czy na pewno usunąć zdjęcie paragonu?',
    attachReceipt: 'Dołącz paragon',
    attachFromCamera: 'Zrób zdjęcie',
    attachFromGallery: 'Z galerii',
    attachAsPdf: 'Wybierz PDF',
    cameraPermissionDenied: 'Wymagany dostęp do kamery',
  },
  recurring: {
    repeat: 'Powtarzaj',
    repeatDesc: 'Automatycznie dodawaj ten wydatek każdy okres',
    period: 'Okres',
    weekly: 'Co tydzień',
    monthly: 'Co miesiąc',
    yearly: 'Co rok',
    seriesBanner: 'Część serii cyklicznej',
    seriesPeriod: 'Powtarza się {{period}}',
    stopRecurring: 'Zatrzymaj powtarzanie',
    stopRecurringConfirm: 'Zatrzymać automatyczne dodawanie? Istniejące rekordy nie zostaną usunięte.',
    stopped: 'Powtarzanie zatrzymane',
  },
  voice: {
    title: 'Wydatek głosowy',
    listening: 'Słucham... Dotknij, aby zatrzymać',
    tapToStart: 'Dotknij, aby zacząć mówić',
    processing: 'Przetwarzanie...',
    example: 'Przykład: „Kawa w Starbucks, pięć złotych"',
    analyzing: 'Analizowanie Twojego wydatku...',
    youSaid: 'Powiedziałeś:',
    confirmTitle: 'Potwierdź wydatek',
    amount: 'Kwota',
    description: 'Opis',
    category: 'Kategoria',
    merchant: 'Sprzedawca',
    highConfidence: 'Wysoka pewność',
    mediumConfidence: 'Średnia pewność',
    saveExpense: 'Zapisz wydatek',
    tryAgain: 'Spróbuj ponownie',
    success: 'Wydatek dodany pomyślnie!',
    addAnother: 'Dodaj kolejny',
    saveFailed: 'Nie udało się zapisać wydatku',
  },
  receipt: {
    // ABA-603: the receipt was already scanned
    duplicateExactTitle: 'Ten paragon został już dodany',
    duplicateExactBody: 'Został już zeskanowany i zapisany: {{what}}. Zeskanować go mimo to?',
    duplicateLikelyTitle: 'Pasujący wydatek już istnieje',
    duplicateOpen: 'Otwórz',
    scanAnyway: 'Skanuj mimo to',
    title: 'Skanuj paragon',
    instructions: 'Zrób zdjęcie, wybierz z galerii lub wgraj PDF',
    hint: 'Skieruj aparat na paragon, aby uzyskać najlepsze wyniki',
    takePhoto: 'Zrób zdjęcie',
    chooseGallery: 'Wybierz z galerii',
    analyzing: 'Analizowanie paragonu...',
    extracting: 'Wyodrębnianie danych za pomocą AI...',
    scannedTitle: 'Paragon zeskanowany',
    totalAmount: 'Łączna kwota',
    description: 'Opis',
    merchant: 'Sprzedawca',
    category: 'Kategoria',
    date: 'Data',
    items: 'Pozycje ({{count}})',
    moreItems: '+{{count}} więcej pozycji',
    highConfidence: 'Wysoka pewność',
    mediumConfidence: 'Średnia pewność',
    saveExpense: 'Zapisz wydatek',
    scanAgain: 'Skanuj ponownie',
    success: 'Wydatek dodany pomyślnie!',
    scanAnother: 'Skanuj kolejny',
    saveFailed: 'Nie udało się zapisać wydatku',
    discount: 'Rabat',
    deposit: 'Kaucja',
    saveImage: 'Zapisz zdjęcie paragonu',
    choosePdf: 'Wgraj PDF',
    analyzingPdf: 'Analizowanie paragonu PDF...',
    userPromptPlaceholder: 'Dodatkowe instrukcje dla AI (opcjonalnie)',
    sessionCount_one: '{{count}} paragon zeskanowany w tej sesji',
    sessionCount_few: '{{count}} paragony zeskanowane w tej sesji',
    sessionCount_many: '{{count}} paragonów zeskanowanych w tej sesji',
    sessionCount_other: '{{count}} paragonów zeskanowanych w tej sesji',
    sessionCapTitle: 'Dobre tempo!',
    sessionCapBody: 'W tej sesji zeskanowano {{count}} paragonów. Możesz kontynuować lub zrobić przerwę — Twój postęp jest już zapisany.',
    addItem: '+ Dodaj pozycję',
    itemQuantity: 'Ilość',
    itemUnitPrice: 'Cena jednostkowa',
    itemTotalPrice: 'Cena łączna',
    itemDescriptionPlaceholder: 'Nazwa pozycji',
    itemRequiredError: 'Podaj nazwę i cenę większą od 0',
    removeItemA11y: 'Usuń pozycję',
    shoppingListChecked_one: '{{count}} pozycja z listy zakupów została oznaczona jako kupiona',
    shoppingListChecked_few: '{{count}} pozycje z listy zakupów zostały oznaczone jako kupione',
    shoppingListChecked_many: '{{count}} pozycji z listy zakupów zostało oznaczonych jako kupione',
    shoppingListChecked_other: '{{count}} pozycji z listy zakupów zostało oznaczonych jako kupione',
    undoShoppingListCheck: 'Cofnij',
  },
  incomeVoice: {
    title: 'Głosowy przychód',
    listening: 'Słucham... Dotknij, aby zatrzymać',
    tapToStart: 'Dotknij, aby zacząć mówić',
    processing: 'Przetwarzanie...',
    example: 'Przykład: „Otrzymałem 500 od klienta, honorarium konsultingowe"',
    analyzing: 'Analizowanie przychodu...',
    youSaid: 'Powiedziałeś:',
    confirmTitle: 'Potwierdź przychód',
    amount: 'Kwota',
    description: 'Opis',
    descriptionPlaceholder: 'Za co była ta płatność?',
    category: 'Kategoria',
    highConfidence: 'Wysoka pewność',
    mediumConfidence: 'Średnia pewność',
    saveIncome: 'Zapisz przychód',
    tryAgain: 'Spróbuj ponownie',
    success: 'Przychód dodany pomyślnie!',
    addAnother: 'Dodaj kolejny',
    saveFailed: 'Nie udało się zapisać przychodu',
  },
  incomeReceipt: {
    title: 'Skanuj fakturę',
    instructions: 'Zrób zdjęcie, wybierz z galerii lub wgraj PDF',
    hint: 'Skieruj aparat na fakturę lub dokument płatności',
    takePhoto: 'Zrób zdjęcie',
    chooseGallery: 'Wybierz z galerii',
    analyzingPdf: 'Analizowanie PDF...',
    analyzing: 'Analizowanie dokumentu...',
    extracting: 'Wyodrębnianie danych za pomocą AI...',
    scannedTitle: 'Dokument zeskanowany',
    totalAmount: 'Łączna kwota',
    description: 'Opis',
    category: 'Kategoria',
    date: 'Data',
    highConfidence: 'Wysoka pewność',
    mediumConfidence: 'Średnia pewność',
    scanAgain: 'Skanuj ponownie',
    success: 'Przychód dodany pomyślnie!',
    scanAnother: 'Skanuj kolejny',
    saveFailed: 'Nie udało się zapisać przychodu',
    choosePdf: 'Wgraj PDF',
    userPromptPlaceholder: 'Dodatkowe instrukcje dla AI (opcjonalnie)',
  },
  budgetNew: {
    name: 'Nazwa budżetu',
    namePlaceholder: 'np. Miesięczne zakupy spożywcze',
    amount: 'Kwota',
    amountPlaceholder: '0,00',
    period: 'Okres',
    categoryOptional: 'Kategoria (opcjonalnie)',
    categoryHint: 'Pozostaw puste dla budżetu ogólnego',
    alertAt: 'Powiadom przy',
    noAlert: 'Brak',
    creating: 'Tworzenie...',
    createBudget: 'Utwórz budżet',
    errorName: 'Proszę podać nazwę budżetu',
    errorAmount: 'Proszę podać prawidłową kwotę',
    errorFailed: 'Nie udało się utworzyć budżetu',
    budgetMode: 'Tryb budżetu',
    modeOverall: 'Ogólny',
    modeByCategory: 'Według kategorii',
    addCategory: 'Dodaj kategorię',
    totalBudget: 'Budżet łączny',
    errorNoCategories: 'Dodaj przynajmniej jedną kategorię',
    byCategoryHint: 'Przypisz limity wydatków do każdej kategorii',
  },
  budgetDetail: {
    notFound: 'Nie znaleziono budżetu',
    onTrack: 'W normie',
    overBudget: 'Przekroczono budżet',
    period: 'Okres',
    category: 'Kategoria',
    alertThreshold: 'Próg powiadomienia',
    daysRemaining: 'Pozostało dni',
    projectedTotal: 'Prognozowana suma',
    status: 'Status',
    active: 'Aktywny',
    inactive: 'Nieaktywny',
    deleteTitle: 'Usuń budżet',
    deleteConfirm: 'Czy na pewno chcesz usunąć ten budżet?',
    categoryBreakdown: 'Podział na kategorie',
    editBudget: 'Edytuj budżet',
    save: 'Zapisz',
    saving: 'Zapisywanie...',
    history: {
      title: 'Historia',
      spent: 'Wydano',
      limit: 'Limit',
      overCount: 'Przekroczono {{count}} z {{total}} okresów',
      avgOverage: 'Śr. nadwyżka: +{{amount}}',
      avgSavings: 'Śr. oszczędności: {{amount}}',
    },
  },
  budgetsDesktop: {
    summaryTotal: 'Wszystkie budżety',
    needsAttention: 'Wymaga uwagi',
    categoriesOverAllocation: 'Kategorie ponad limit',
    needsAttentionHeading: 'Wymaga uwagi ({{count}})',
    onTrackHeading: 'W normie ({{count}})',
    nearingLimit: 'Zbliża się do limitu',
    exceedsBy: 'Przekracza o {{amount}}',
    projectedExceedBy: 'Prognozowane przekroczenie o {{amount}} (~{{date}})',
    emptyHint: 'Budżety ostrzegają cię, zanim wydasz za dużo — zacznij od jednej kategorii lub ogólnego miesięcznego limitu',
  },
  dashboardDesktop: {
    focusEmptyTitle: 'Twój główny widok jest wyłączony',
    focusEmptyBody: 'Włącz {{safeToSpend}}, {{netProfit}}, {{incomeExpenses}}, {{monthlyBudget}} lub {{wallets}}, aby zobaczyć to tutaj.',
    manageWidgets: 'Zarządzaj widżetami',
  },
  validation: {
    fillAllFields: 'Proszę wypełnić wszystkie pola',
    invalidEmail: 'Proszę podać prawidłowy adres e-mail',
    passwordMin8: 'Hasło musi mieć co najmniej 8 znaków',
    passwordLatin: 'Hasło musi zawierać co najmniej jedną wielką literę łacińską (A-Z), jedną małą literę łacińską (a-z) i jedną cyfrę (0-9)',
    passwordsNoMatch: 'Hasła nie są zgodne',
    nameMin2: 'Imię musi mieć co najmniej 2 znaki',
    invalidAmount: 'Proszę podać prawidłową kwotę',
    noDescription: 'Proszę podać opis',
  },
  accounts: {
    switchAccount: 'Przełącz konto',
    displayCurrency: 'Waluta wyświetlania',
    manage: 'Zarządzaj kontami',
    create: 'Utwórz konto',
    personal: 'Osobiste',
    name: 'Nazwa konta',
    namePlaceholder: 'np. Mój budżet',
    nameRequired: 'Wprowadź nazwę konta',
    type: 'Typ konta',
    currency: 'Waluta',
    details: 'Szczegóły',
    notFound: 'Konto nie znalezione',
    deleteConfirm: 'Usuń konto',
    deleteConfirmMessage: 'Czy na pewno chcesz usunąć "{{name}}"? Wszystkie dane zostaną utracone.',
    deleteAccount: 'Usuń konto',
    leaveConfirm: 'Opuść konto',
    leaveConfirmMessage: 'Czy na pewno chcesz opuścić "{{name}}"?',
    leaveAccount: 'Opuść konto',
    leave: 'Opuść',
    members: 'Członkowie',
    pendingInvitations: 'Oczekujące zaproszenia',
    noPendingInvitations: 'Brak oczekujących zaproszeń',
    cancelInvitation: 'Anuluj zaproszenie',
    cancelInvitationMessage: 'Czy na pewno chcesz anulować to zaproszenie?',
    removeMemberConfirm: 'Usuń członka',
    removeMemberMessage: 'Czy na pewno chcesz usunąć {{name}}?',
    changeRole: 'Zmień rolę',
    selectRole: 'Wybierz nową rolę dla tego członka',
    inviteByLink: 'Przez link',
    inviteByEmail: 'Przez email',
    inviteEmail: 'Adres email',
    emailPlaceholder: 'user@example.com',
    emailRequired: 'Wprowadź adres email',
    inviteRole: 'Rola',
    sendInvite: 'Wyślij zaproszenie',
    inviteSent: 'Zaproszenie wysłane!',
    inviteSentEmail: 'Zaproszenie zostało wysłane na {{email}}.',
    shareCode: 'Udostępnij ten kod osobie, którą chcesz zaprosić.',
    codeCopied: 'Kod skopiowany',
    inviteShareMessage: 'Dołącz do mojego budżetu! Kod: {{code}}',
    joinAccount: 'Dołącz do konta',
    joinDescription: 'Wprowadź kod zaproszenia, aby dołączyć do wspólnego konta.',
    enterCode: 'Wprowadź kod',
    codeRequired: 'Wprowadź kod zaproszenia',
    join: 'Dołącz',
    joinedSuccess: 'Pomyślnie dołączono!',
    inviteBySearch: 'Szukaj',
    searchLabel: 'Szukaj po imieniu lub e-mailu',
    searchPlaceholder: 'Wpisz imię lub e-mail…',
    searchNoResults: 'Nie znaleziono użytkowników.',
    selectUserRequired: 'Wybierz użytkownika do zaproszenia',
    inviteSentPush: '{{name}} otrzyma powiadomienie, aby zaakceptować.',
    types: {
      personal: 'Osobiste',
      business: 'Firmowe',
      shared: 'Wspólne',
      investment: 'Inwestycja',
    },
    roles: {
      owner: 'Właściciel',
      editor: 'Edytor',
      viewer: 'Obserwator',
    },
    roleDescriptions: {
      editor: 'Może dodawać i edytować wydatki',
      viewer: 'Może tylko przeglądać dane',
    },
    financialMonth: 'Miesiąc finansowy',
    financialMonthStartsOn: 'Miesiąc zaczyna się {{day}}. dnia',
    financialMonthCalendar: 'Miesiąc kalendarzowy (od 1.)',
    financialMonthPickerTitle: 'Miesiąc finansowy zaczyna się',
    financialMonthHint:
      'Jeśli wypłata wpływa 10. dnia miesiąca, ustaw 10 — budżety i raporty będą liczone od 10. do 9. dnia następnego miesiąca. Poprzednie okresy również zostaną przeliczone; dane pozostają bez zmian.',
    financialMonthClamped: 'W krótszych miesiącach okres zacznie się w ostatnim dniu.',
  },
  settings: {
    title: 'Ustawienia',
    profile: 'Profil',
    name: 'Imię',
    email: 'E-mail',
    currency: 'Waluta',
    timezone: 'Strefa czasowa',
    appSettings: 'Ustawienia aplikacji',
    language: 'Język',
    appearance: 'Wygląd',
    theme: 'Motyw',
    system: 'Systemowy',
    light: 'Jasny',
    dark: 'Ciemny',
    accentColor: 'Kolor akcentu',
    accentDefault: 'Domyślny',
    customColor: 'Własny',
    pickColor: 'Wybierz kolor',
    applyColor: 'Zastosuj',
    resetAccent: 'Przywróć domyślny',
    invalidColor: 'Nieprawidłowy kolor',
    paymentConsequenceHint: 'Revolut i PayPal dodają prawdziwy przycisk płatności do Twoich linków podziału rachunku; BLIK pokazuje zamiast tego Twój numer wraz z instrukcją (brak linku międzybankowego); Gotówka i Inne pokazują wpisane dane jako instrukcję zamiast przycisku.',
    invalidPaymentHandle: 'Nieprawidłowy identyfikator płatności',
    addPaymentMethod: 'Dodaj metodę płatności',
    about: 'O aplikacji',
    version: 'Wersja',
    support: 'Wsparcie i opinie',
    logout: 'Wyloguj się',
    logoutConfirm: 'Czy na pewno chcesz się wylogować?',
    profileUpdated: 'Profil zaktualizowany',
    timezoneSearch: 'Szukaj strefy czasowej...',
    timezoneUpdated: 'Strefa czasowa zaktualizowana',
    dataSync: 'Dane i synchronizacja',
    lastSynced: 'Ostatnia synchronizacja',
    syncNow: 'Synchronizuj teraz',
    syncing: 'Synchronizacja...',
    syncComplete: 'Dane zsynchronizowane pomyślnie',
    syncFailed: 'Synchronizacja nie powiodła się. Sprawdź połączenie.',
    neverSynced: 'Nigdy',
    justNow: 'Przed chwilą',
    minutesAgo: '{{count}} min temu',
    hoursAgo: '{{count}} godz. temu',
    aiResponseMode: 'Styl odpowiedzi AI',
    aiResponseModeSimple: 'Prosty',
    aiResponseModeBalanced: 'Zrównoważony',
    aiResponseModeExpert: 'Ekspert',
    aiResponseModeSimpleDesc: 'Prosty język, bez żargonu',
    aiResponseModeBalancedDesc: 'Mix prostego i technicznego',
    aiResponseModeExpertDesc: 'Terminologia finansowa, szczegółowa analiza',
    aiModel: 'Model AI',
    aiModelFast: 'Szybki',
    aiModelBalanced: 'Zrównoważony',
    aiModelQuality: 'Jakość',
    aiModelCost: 'koszt',
    telegram: {
      title: 'Bot Telegram',
      connect: 'Połącz Telegram',
      disconnect: 'Rozłącz',
      disconnectConfirm: 'Czy na pewno chcesz rozłączyć bota Telegram?',
      linked: 'Telegram połączony',
      description: 'Połącz bota Telegram, aby dodawać wydatki, przychody, skanować paragony i rozmawiać z asystentem AI przez Telegram.',
      tapToCopy: 'Dotknij, aby skopiować kod',
      codeCopied: 'Kod skopiowany do schowka',
      linkInstructions: 'Otwórz @{{botUsername}} w Telegramie i wyślij:\n/link YOUR_CODE',
    },
    bots: {
      title: 'Boty czatu',
      subtitle: 'Połącz Telegram i WhatsApp',
    },
    dashboardWidgets: 'Widżety pulpitu',
    widgetsReorderHint: 'Przeciągnij uchwyt, aby zmienić kolejność',
    widgetsResetOrder: 'Przywróć domyślną kolejność',
    quickActionsTitle: 'Szybkie akcje',
    widgetsTitle: 'Widżety',
    widget: {
      gamification: 'Grywalizacja',
      monthlyBudget: 'Miesięczny budżet',
      incomeExpenses: 'Przychody i wydatki',
      debts: 'Długi i pożyczki',
      netProfit: 'Wykres zysku netto',
      netCapital: 'Kapitał netto',
      fatFinder: 'Fat Finder (AI)',
      calendar: 'Kalendarz',
      goals: 'Cele',
      wallets: 'Salda portfela',
      financialHealth: 'Kondycja Finansowa',
    },
    changeEmail: {
      title: 'Zmień e-mail',
      step1Title: 'Nowy adres e-mail',
      newEmailLabel: 'Nowy e-mail',
      newEmailPlaceholder: 'Wprowadź nowy adres e-mail',
      passwordLabel: 'Aktualne hasło',
      passwordPlaceholder: 'Wprowadź aktualne hasło',
      sendCode: 'Wyślij kod weryfikacyjny',
      step2Title: 'Zweryfikuj nowy e-mail',
      step2Subtitle: 'Kod 6-cyfrowy został wysłany na {{email}}',
      codePlaceholder: 'Wprowadź kod 6-cyfrowy',
      confirm: 'Potwierdź zmianę e-maila',
      resend: 'Wyślij kod ponownie',
      success: 'E-mail zmieniony pomyślnie',
      successMessage: 'Twój adres e-mail został zaktualizowany.',
      pendingNotice: 'Kod weryfikacyjny został wcześniej wysłany na {{email}}.',
    },
  },
  notifications: {
    title: 'Powiadomienia',
    pushNotifications: 'Powiadomienia Push',
    pushNotificationsDesc: 'Włącz lub wyłącz wszystkie powiadomienia push',
    budgetAlerts: 'Alerty Budżetowe',
    budgetAlertsDesc: 'Otrzymuj powiadomienia po osiągnięciu progów budżetowych',
    sharedAccountActivity: 'Aktywność Wspólnego Konta',
    sharedAccountActivityDesc: 'Otrzymuj powiadomienia gdy inni dodają wydatki do wspólnych kont',
    debtReminders: 'Przypomnienia o długach',
    debtRemindersDesc: 'Powiadomienie 3 dni przed terminem spłaty i po jego przekroczeniu',
    recurringExpenses: 'Wydatki cykliczne',
    recurringExpensesDesc: 'Powiadomienie gdy cykliczny wydatek jest automatycznie rejestrowany',
    subscriptionRenewals: 'Odnowienia subskrypcji',
    subscriptionRenewalsDesc: 'Powiadomienie 3 dni przed odnowieniem śledzonej subskrypcji',
    anomalyAlerts: 'Anomalie',
    anomalyAlertsDesc: 'Nietypowe wydatki, podwyżki cen i podwójne płatności',
    trackingReminder: 'Przypomnienie o śledzeniu',
    trackingReminderDesc: 'Otrzymuj powiadomienie, gdy nie zapisałeś wydatków przez 3+ dni',
    shoppingReminder: 'Przypomnienia o uzupełnieniu zapasów',
    shoppingReminderDesc: 'Otrzymuj przypomnienie, gdy nadejdzie czas na ponowny zakup stałego produktu',
    shoppingDeals: 'Powiadomienia o promocjach',
    shoppingDealsDesc: 'Otrzymuj powiadomienie, gdy cena stałego produktu spadnie',
    inflationShield: 'Powiadomienia o zapasach',
    inflationShieldDesc: 'Otrzymuj powiadomienie, gdy cena stałego produktu ma wkrótce wzrosnąć',
  },
  alerts: {
    title: 'Alerty',
    empty: 'Brak alertów. Powiadomimy Cię, gdy zauważymy coś nietypowego w Twoich finansach.',
    markAllRead: 'Oznacz jako przeczytane',
    duplicateTitle: 'Możliwa podwójna płatność',
    duplicateBody: '{{merchant}} pobrał {{amount}} {{currency}} dwa razy w ciągu dwóch dni.',
    priceIncreaseTitle: '{{merchant}} podrożał',
    priceIncreaseBody: 'Było {{oldAmount}}, teraz {{newAmount}} {{currency}} (+{{percent}}%).',
    spikeTitle: 'Nietypowe wydatki',
    spikeBody: '{{category}}: {{percent}}% powyżej średniej z 3 miesięcy.',
    recurringTitle: '{{merchant}} wygląda na subskrypcję',
    recurringBody: 'Pobrano {{amount}} {{currency}} {{cycle}} co najmniej 3 razy. Dotknij, aby śledzić.',
    cycleMonthly: 'co miesiąc',
    cycleWeekly: 'co tydzień',
    markAsRecurring: 'Oznacz jako cykliczne',
    mergeTitle: 'Ten sam zakup, dwie waluty?',
    mergeBody: '{{amountA}} {{currencyA}} i {{amountB}} {{currencyB}} u {{merchant}} wyglądają jak ta sama transakcja. Scalić je?',
    alreadyResolvedTitle: 'Już rozwiązane',
    alreadyResolvedBody: 'Wygląda na to, że to już załatwione — tego wydatku już tu nie ma.',
    tabAlerts: 'Alerty',
    tabInvitations: 'Zaproszenia',
    invitationTitle: 'Zaproszenie do {{accountName}}',
    invitationSubtitle: '{{inviterName}} zaprosił(a) Cię jako {{role}}',
    invitationAccept: 'Akceptuj',
    invitationDecline: 'Odrzuć',
    invitationsEmpty: 'Brak oczekujących zaproszeń',
    priceCheckTitle: 'Warto sprawdzić ten paragon',
    priceCheckBody: 'U {{merchant}}: powyżej zwykłej ceny — produkty: {{count}}, różnica: około {{amount}} {{currency}}.',
  },
  whatsappBot: {
    title: 'Bot WhatsApp',
    subtitle: 'Śledź wydatki przez WhatsApp',
    connectButton: 'Połącz WhatsApp',
    disconnectButton: 'Rozłącz',
    codeInstructions: 'Otwórz WhatsApp i wyślij ten kod:',
    openButton: 'Otwórz WhatsApp',
    copyCode: 'Kopiuj kod',
    linkedAs: 'Połączono jako {{name}}',
    notLinked: 'Niepołączony',
    confirmDisconnect: 'Rozłączyć WhatsApp?',
    refresh: 'Odśwież status',
  },
  slackBot: {
    title: 'Bot Slack',
    subtitle: 'Śledź wydatki przez Slack',
    connectButton: 'Połącz Slack',
    disconnectButton: 'Rozłącz',
    codeInstructions: 'Otwórz Slack i wyślij botowi wiadomość bezpośrednią: `link {{code}}`',
    openButton: 'Otwórz Slack',
    copyCode: 'Kopiuj kod',
    linkedAs: 'Połączono jako {{name}}',
    linked: 'Połączono',
    notLinked: 'Niepołączony',
    confirmDisconnect: 'Rozłączyć Slack?',
    refresh: 'Odśwież status',
    addToSlack: 'Dodaj do Slack',
    addToSlackHint: 'Administratorzy workspace: dodaj bota do swojego workspace Slack (jednorazowo). Następnie każdy użytkownik łączy swoje konto za pomocą kodu.',
  },
  wiseImport: {
    title: 'Import z Wise',
    subtitle: 'Importuj wydatki i przychody z wyciągu CSV z Wise',
    pickFile: 'Wybierz plik CSV',
    parsing: 'Przetwarzanie CSV…',
    importButton: 'Importuj {{count}} wierszy',
    importAnother: 'Importuj kolejny',
    success: 'Import zakończony',
    summary: '{{expenses}} wydatków · {{incomes}} przychodów · {{exchanges}} wymian',
    help: {
      where: 'W Wise: Transakcje → Wyciągi i raporty → CSV. Wybierz zakres dat i walutę, pobierz plik, a następnie wybierz go tutaj.',
    },
    preview: {
      selected: 'Wybrano: {{count}}',
      skipped: 'Już zaimportowano: {{count}}',
      alreadyImported: 'Już zaimportowano',
      fxPair: 'Wymiana walut',
    },
    error: {
      parseFailed: 'Nie udało się przetworzyć pliku CSV. Upewnij się, że pobrałeś wyciąg z Wise w formacie CSV.',
    },
  },
  bankImport: {
    title: 'Importuj transakcje',
    subtitle: 'Importuj plik CSV ze swojego banku',
    quickImportHeader: 'Szybki import',
    autoDetect: 'Rozpoznaj automatycznie (dowolny bank)',
    analysing: 'Czytamy Twój wyciąg…',
    migrationHeader: 'Przenosisz się z innej aplikacji?',
    migrationHint: 'Zaimportuj historię z innej aplikacji budżetowej — Twoje kategorie przyjdą razem z nią.',
    savedMappingsHeader: 'Zapisane mapowania',
    noSavedMappings: 'Brak zapisanych mapowań',
    detectedBank: 'Wykryto: {{name}}',
    pickBankTitle: 'Z jakiego banku pochodzi ten plik?',
    pickBankSubtitle: 'Nie udało się automatycznie wykryć Twojego banku. Wybierz go poniżej lub użyj opcji Inne, aby ręcznie przypisać kolumny.',
    other: 'Inne (własny CSV)',
    mapperTitle: 'Przypisz kolumny',
    mapperSubtitle: 'Wskaż, która kolumna zawiera jakie dane',
    mapperDate: 'Kolumna z datą',
    mapperAmount: 'Kolumna z kwotą',
    mapperDescription: 'Kolumna z opisem',
    mapperCurrency: 'Kolumna z walutą (opcjonalna)',
    mapperCounterparty: 'Kolumna z kontrahentem (opcjonalna)',
    mapperSplitDebitCredit: 'Rozdziel kolumny debetową i kredytową',
    mapperDebit: 'Kolumna debetowa (wydatki)',
    mapperCredit: 'Kolumna kredytowa (przychody)',
    mapperDelimiter: 'Separator',
    mapperEncoding: 'Kodowanie',
    mapperAmountFormat: 'Format kwoty',
    mapperDateFormat: 'Format daty',
    mapperContinue: 'Kontynuuj',
    mapperSaveMapping: 'Zapisz to mapowanie na przyszłość',
    mapperSaveName: 'Nazwa mapowania',
    parseErrorsBanner: '{{count}} wierszy pominięto z powodu błędów parsowania',
    deleteMapping: 'Usuń mapowanie',
    deleteMappingConfirm: 'Usunąć to zapisane mapowanie?',
    summary: 'Zaimportowano {{expenses}} wydatków, {{incomes}} przychodów, {{exchanges}} wymian',
    requestCardTitle: 'Nie ma Twojego banku?',
    requestCardSubtitle: 'Wyślij nam nazwę banku i przykładowy wyciąg — dodamy jego obsługę.',
    requestTitle: 'Zgłoś bank',
    requestIntro: 'Powiedz nam, z jakiego banku korzystasz, i dołącz przykład wyciągu (CSV lub PDF). Użyjemy go, żeby dodać obsługę tego banku.',
    requestBankName: 'Nazwa banku',
    requestBankNamePlaceholder: 'np. Santander, Revolut',
    requestNotes: 'Uwagi (opcjonalnie)',
    requestNotesPlaceholder: 'Cokolwiek przydatnego — format, skąd wyeksportowano itp.',
    requestAttach: 'Dołącz przykładowy wyciąg',
    requestAttachHint: 'CSV lub PDF, do 5 MB',
    requestAttached: 'Załączono: {{name}}',
    requestSend: 'Wyślij zgłoszenie',
    requestSent: 'Dziękujemy! Twoje zgłoszenie zostało wysłane — przyjrzymy się temu.',
    requestErrorNoName: 'Proszę podać nazwę banku.',
    requestError: 'Nie udało się wysłać zgłoszenia. Spróbuj ponownie później.',
    pastImportsHeader: 'Historia importów',
    noPastImports: 'Brak importów',
    transactionCount_one: '{{count}} transakcja',
    transactionCount_few: '{{count}} transakcje',
    transactionCount_many: '{{count}} transakcji',
    transactionCount_other: '{{count}} transakcji',
    undoImportTitle: 'Cofnij import',
    undoImportConfirm: 'Spowoduje to usunięcie {{count}} transakcji z tego importu. Możesz ponownie zaimportować ten sam plik.',
    undoImportAction: 'Cofnij import',
    possibleMerge: 'Możliwy duplikat',
    mergeWithExisting: 'Scal z istniejącą',
    importAsNew: 'Importuj jako nową',
    aiConsentTitle: 'Pozwolić AI odczytać ten wyciąg?',
    aiConsentBody:
      'Nie rozpoznajemy formatu tego banku. AI może ustalić, która kolumna to data, kwota i opis — resztę zrobi Twoje urządzenie.',
    aiConsentWhatLeaves: 'Co zostanie wysłane',
    aiConsentWhatLeavesCsv: 'Wiersz nagłówka i do 10 przykładowych wierszy z Twojego pliku.',
    aiConsentWhatLeavesPdf: 'Pierwsze 20 linii tekstu z Twojego wyciągu.',
    aiConsentThirdParty: 'Te dane trafiają do zewnętrznego dostawcy AI w celu analizy i nie są przez niego przechowywane do trenowania modeli.',
    aiConsentOnce: 'Pytamy raz na konto. Nadal możesz samodzielnie zmapować kolumny.',
    aiConsentAccept: 'Użyj AI',
    aiConsentDecline: 'Zmapuję to sam',
    aiConsentFailed: 'Nie udało się zapisać Twojego wyboru. Spróbuj ponownie.',
    aiInferredBy: 'Kolumny dopasowane przez AI',
    aiInferredEdit: 'Źle? Dotknij, aby poprawić',
    aiCurrencyAssumed: 'Ten plik nie ma kolumny waluty, więc każdy wiersz odczytano jako {{currency}}. Sprawdź przed importem.',
    aiWarningNoBalance: 'Nie mogliśmy potwierdzić, że znaleziono wszystkie transakcje — ten wyciąg nie ma salda końcowego do porównania. Sprawdź listę.',
    aiWarningMismatch: 'Znalezione transakcje nie sumują się do salda końcowego tego wyciągu. Niektórych wierszy może brakować.',
    aiWarningTruncated: 'Ten wyciąg był zbyt długi, aby odczytać go w całości — {{count}} stron nie zostało przetworzonych.',
    aiPdfPaywall: 'Odczytywanie wyciągów PDF za pomocą AI',
    aiBankGuess: 'Wygląda na: {{bank}}',
    aiCurrencyFix: 'Zmień walutę',
    aiRoleDate: 'Data',
    aiRoleAmount: 'Kwota',
    aiRoleDebit: 'Obciążenie',
    aiRoleCredit: 'Uznanie',
    aiRoleDescription: 'Opis',
    aiRoleCurrency: 'Waluta',
    aiRoleCounterparty: 'Odbiorca',
    error: {
      encodingUnknown: 'Nie udało się wykryć kodowania pliku. Spróbuj zapisać plik CSV jako UTF-8.',
      parseFailed: 'Nie udało się przetworzyć pliku. Sprawdź format i spróbuj ponownie.',
      tooLarge: 'Plik jest za duży (maks. 5 MB).',
    },
  },
  wallet: {
    title: 'Portfel',
    balances: 'Salda',
    setInitialBalance: 'Ustaw saldo',
    currentBalance: 'Bieżące saldo',
    initialBalance: 'Saldo początkowe',
    totalBalance: 'Saldo całkowite ({{currency}})',
    totalIncome: 'Całkowity dochód',
    totalSpent: 'Łącznie wydano',
    exchangedIn: 'Otrzymano z wymiany',
    exchangedOut: 'Wymieniono',
    noBalances: 'Brak Salda',
    noBalancesHint: 'Ustaw salda początkowe, aby zacząć śledzić portfel',
    addBalance: 'Dodaj saldo',
    currency: 'Waluta',
    amount: 'Kwota',
    deleteBalance: 'Usuń saldo',
    deleteConfirm: 'Czy na pewno chcesz usunąć to saldo?',
    insufficientBalance: 'Niewystarczające saldo',
    existingBalances: 'Istniejące salda',
    balanceSaved: 'Saldo ustawione pomyślnie',
    editInitialBalance: 'Edytuj saldo początkowe',
    balanceUpdated: 'Saldo zaktualizowane pomyślnie',
    transferredIn: 'Otrzymano przelewem',
    transferredOut: 'Przelano',
    balanceHistory: 'Historia salda',
    monthsWindow: '{{count}}M',
    monthlyChange: 'w {{month}}',
    displayCurrency: 'Waluta wyświetlania',
    noHistoryYet: 'Dodaj transakcje, aby zobaczyć trend salda',
  },
  exchange: {
    title: 'Kantor',
    from: 'Z',
    to: 'Na',
    rate: 'Kurs wymiany',
    date: 'Data',
    notes: 'Notatki',
    notesPlaceholder: 'Opcjonalne notatki...',
    recentExchanges: 'Ostatnie wymiany',
    sameCurrencyError: 'Nie można wymienić tej samej waluty',
    exchange: 'Wymień',
    allExchanges: 'Wszystkie wymiany',
    rateAlerts: 'Alerty kursowe',
    notifyMe: 'Powiadom mnie, gdy kurs osiągnie mój cel',
    targetRate: 'Docelowy kurs',
    above: 'powyżej',
    below: 'poniżej',
    addAlert: 'Dodaj alert',
    alertCreated: 'Alert utworzony — powiadomimy Cię, gdy kurs zostanie osiągnięty.',
    deleteAlertConfirm: 'Usunąć ten alert kursowy?',
    noAlerts: 'Brak alertów dla tej pary.',
    alertWhen: 'Powiadom, gdy {{direction}} {{rate}} {{toCurrency}}',
    newAlert: 'Nowy alert',
    alertsWaiting: 'Oczekujące',
    alertsFired: 'Już uruchomione',
    noAlertsYet: 'Nie masz jeszcze alertów. Ustaw cel — powiadomimy, gdy kurs go osiągnie.',
    alertFiredAt: 'Uruchomiony przy {{rate}} · {{date}}',
    allCurrencies: 'Wszystkie waluty',
    allTime: 'Cały czas',
    thisMonth: 'Ten miesiąc',
    last3Months: 'Ostatnie 3 miesiące',
    thisYear: 'Ten rok',
    noExchanges: 'Brak wymian',
    noExchangesHint: 'Wymiany walut pasujące do wybranych filtrów pojawią się tutaj.',
    notFound: 'Wymiana nie znaleziona',
    deleteTitle: 'Usuń wymianę',
    deleteConfirm: 'Czy na pewno chcesz usunąć tę wymianę?',
    editTitle: 'Szczegóły wymiany',
  },
  converter: {
    title: 'Przelicznik walut',
    result: 'Wynik',
    enterAmount: 'Wpisz kwotę',
    rateInfo: '1 {{from}} = {{rate}} {{to}}',
  },
  transfer: {
    title: 'Przelew',
    fromAccount: 'Z konta',
    toAccount: 'Na konto',
    amount: 'Kwota',
    rate: 'Kurs wymiany',
    notes: 'Notatki',
    notesPlaceholder: 'Opcjonalne notatki...',
    sameAccountError: 'Źródło i cel muszą być różnymi kontami',
    submit: 'Przelej',
    recentTransfers: 'Ostatnie przelewy',
    success: 'Przelew zakończony',
    countAsIncome: 'Licz jako przychód',
    countAsIncomeHint: 'Tworzy zapis przychodu na koncie docelowym',
    date: 'Data',
    notFound: 'Przelew nie znaleziony',
    deleteTitle: 'Usuń przelew',
    deleteConfirm: 'Czy na pewno chcesz usunąć ten przelew?',
    allTransfers: 'Wszystkie przelewy',
    allAccounts: 'Wszystkie konta',
    periodAll: 'Cały czas',
    periodThisMonth: 'Ten miesiąc',
    periodLast3Months: 'Ostatnie 3 miesiące',
    periodThisYear: 'Ten rok',
    noTransfers: 'Brak przelewów',
    noTransfersHint: 'Przelewy między kontami pojawią się tutaj',
    frequent: 'Częste',
    available: 'Dostępne:',
    max: 'Maks',
    insufficientHint: 'Kwota przekracza znane saldo tego konta',
    saveFailed: 'Zmiana nie została przyjęta',
    saveFailedHint: 'Przelew pozostał bez zmian. Możliwe, że nie masz dostępu do jednego z wybranych kont.',
  },
  errors: {
    error: 'Błąd',
    loginFailed: 'Logowanie nie powiodło się',
    invalidCredentials: 'Nieprawidłowy e-mail lub hasło',
    accountDeactivated: 'Konto jest dezaktywowane',
    emailAlreadyExists: 'Użytkownik z tym adresem e-mail już istnieje',
    registrationFailed: 'Rejestracja nie powiodła się',
    biometricFailed: 'Uwierzytelnianie biometryczne nie powiodło się',
    biometricLoginFailed: 'Logowanie biometryczne nie powiodło się',
    quickLoginFailed: 'Szybkie logowanie nie powiodło się',
    noSavedSession: 'Nie znaleziono zapisanej sesji',
    sessionExpired: 'Sesja wygasła',
    invalidResetCode: 'Nieprawidłowy lub wygasły kod',
    tooManyResetAttempts: 'Zbyt wiele prób. Spróbuj ponownie później.',
    resetFailed: 'Nie udało się zresetować hasła. Spróbuj ponownie.',
    micPermissionDenied: 'Odmówiono dostępu do mikrofonu',
    startRecordingFailed: 'Nie udało się rozpocząć nagrywania',
    processRecordingFailed: 'Nie udało się przetworzyć nagrania',
    cameraPermissionDenied: 'Odmówiono dostępu do aparatu',
    galleryPermissionDenied: 'Odmówiono dostępu do galerii',
    captureImageFailed: 'Nie udało się zrobić zdjęcia',
    pickImageFailed: 'Nie udało się wybrać zdjęcia',
    processReceiptFailed: 'Nie udało się przetworzyć paragonu',
    pdfTooLarge: 'Plik PDF jest za duży (maks. 10 MB)',
    saveFailed: 'Nie udało się zapisać wydatku',
    createBudgetFailed: 'Nie udało się utworzyć budżetu',
    sendMessageFailed: 'Nie udało się wysłać wiadomości',
    loadConversationFailed: 'Nie udało się wczytać konwersacji',
    chatError: 'Przepraszam, wystąpił błąd. Sprawdź połączenie i spróbuj ponownie.',
    invalidVerificationCode: 'Nieprawidłowy kod weryfikacyjny',
    verificationCodeExpired: 'Kod weryfikacyjny wygasł',
    verificationFailed: 'Weryfikacja nie powiodła się',
    unknown: 'Wystąpił nieoczekiwany błąd',
    googleSignInFailed: 'Logowanie przez Google nie powiodło się. Spróbuj ponownie.',
    googleEmailNotVerified: 'Adres e-mail Twojego konta Google nie został zweryfikowany.',
    usePasswordLogin: 'To konto korzysta z logowania Google. Naciśnij „Kontynuuj z Google".',
  },
  insights: {
    title: 'Analiza predykcyjna',
    anomalies: 'Anomalie wydatków',
    anomalyText: '{{percent}}% więcej niż zwykle na {{category}}',
    avgLabel: 'śr.',
    predictions: 'Prognozy budżetowe',
    exhaustionText: 'Wyczerpie się ok. {{date}}',
    projectedTotal: 'Prognoza: {{amount}}',
    noAnomalies: 'Twoje wydatki wyglądają normalnie',
    noPredictions: 'Brak aktywnych budżetów do prognozy',
    categoryLearned: 'Na podstawie Twojej historii',
    aiSuggested: 'Wgląd AI',
    generatedAt: 'Zaktualizowano {{time}}',
    dismiss: 'Odrzuć',
    seeAll: 'Zobacz wszystkie wglądy',
    anomalySpike: 'Skok wydatków',
    categoryComparison: 'Porównanie kategorii',
    trendChange: 'Zmiana trendu',
    budgetBurndown: 'Spalanie budżetu',
    savingsOpportunity: 'Możliwość oszczędności',
    loadingInsights: 'Analizuję Twoje wydatki...',
    noInsights: 'Brak nowych wglądów w tej chwili',
    proRequired: 'Ulepsz do Pro, aby uzyskać wglądy AI',
  },
  subscription: {
    title: 'Subskrypcja',
    managePlan: 'Zarządzaj planem',
    month: 'miesiąc',
    year: 'rok',
    upgrade: 'Ulepsz',
    viewDetails: 'Zobacz szczegóły',
    maybeLater: 'Może później',
    trial: '7-dniowy bezpłatny okres próbny',
    trialActive: 'Okres próbny aktywny',
    trialEndsAt: 'Okres próbny kończy się {{date}}',
    limitReached: 'Osiągnięto limit AI',
    limitReachedBody: 'Wykorzystałeś darmowy limit AI w tym miesiącu. Przejdź na Pro, aby mieć więcej zapytań AI i funkcje premium.',
    startTrial: 'Rozpocznij 7-dniowy darmowy okres próbny',
    aiRequests: 'Zapytania AI',
    usageThisMonth: 'Wykorzystanie w tym miesiącu',
    availablePlans: 'Dostępne plany',
    manage: 'Zarządzaj subskrypcją',
    upgraded: 'Plan zaktualizowany',
    upgradeSuccess: 'Twoja subskrypcja została zaktualizowana.',
    includesProFeatures: 'Zawiera wszystkie funkcje Pro, plus:',
    cancelsAt: 'Anulowanie {{date}}',
    status: {
      active: 'Aktywny',
      canceled: 'Anulowany',
      past_due: 'Zaległe płatności',
      incomplete: 'Niekompletny',
      trialing: 'Okres próbny',
      paused: 'Wstrzymany',
    },
    plans: {
      pro: { name: 'Pro' },
      business: { name: 'Business' },
    },
    features: {
      aiRequests300: '300 zapytań AI miesięcznie',
      accounts5: 'Do 5 kont',
      members5: 'Do 5 członków na konto',
      predictiveAnalytics: 'Analityka predykcyjna',
      anomalyDetection: 'Wykrywanie anomalii wydatków',
      unlimitedCurrencies: 'Nieograniczone waluty',
      unlimitedAi: 'Nieograniczone zapytania AI',
      unlimitedAccounts: 'Nieograniczone konta',
      unlimitedMembers: 'Nieograniczeni członkowie',
      advancedReporting: 'Zaawansowane raporty',
    },
    trialLimitNote: 'Plan próbny — ograniczone zapytania AI. Zaktualizuj plan, aby uzyskać pełny dostęp.',
  },
  welcome: {
    title: 'Witamy',
    heading: 'Witamy w AI Budget!',
    subheading: 'Twój inteligentny towarzysz finansowy',
    freeIncluded: 'Twój darmowy plan zawiera:',
    freeAiRequests: '50 zapytań AI miesięcznie',
    freeAccount: '3 konta osobiste',
    freeMember: '1 członek',
    freeTracking: 'Pełne śledzenie wydatków',
    wantMore: 'Chcesz więcej? Zaktualizuj w dowolnym momencie',
    continueWithFree: 'Kontynuuj za darmo',
  },
  onboarding: {
    heading: 'Od czego chcesz zacząć?',
    subheading: 'Dodaj jedną rzecz, a aplikacja zacznie działać dla Ciebie.',
    scanReceipt: 'Zeskanuj paragon',
    scanReceiptHint: 'Pozycje i kategorie odczytane za Ciebie',
    useVoice: 'Użyj głosu',
    typeManually: 'Wpisz ręcznie',
    bringHistory: 'Zaimportuj historię',
    bringHistoryHint: 'Z banku lub innej aplikacji budżetowej',
    later: 'Zrobię to później',
  },
  admin: {
    title: 'Panel administratora',
    openPanel: 'Panel administratora',
    systemStats: 'Statystyki systemu',
    totalUsers: 'Użytkownicy ogółem',
    totalAccounts: 'Konta ogółem',
    totalExpenses: 'Wydatki ogółem',
    subscriptionBreakdown: 'Subskrypcje',
    trialing: 'Okres próbny',
    aiUsageTitle: 'Użycie AI w tym miesiącu',
    estimatedCost: 'Szacowany koszt',
    totalCostUnits: 'Jednostki kosztów',
    totalRequests: '{{count}} zapytań',
    noUsage: 'Brak użycia AI w tym okresie',
    features: {
      chat: 'Czat',
      voice: 'Głos',
      parse: 'Analiza',
      categorization: 'Kategoryzacja',
      ocr: 'OCR / Skan paragonu',
      story: 'Historia',
      insights: 'AI Analiza',
      tag_suggestion: 'Sugestia tagów',
      project_suggestion: 'Sugestia projektu',
      split_suggestion: 'Sugestia podziału',
      goal_plan: 'Plan celów',
      investment_insights: 'Analiza inwestycji',
      fat_finder: 'Szukacz wydatków',
    },
  },
  drillDown: {
    title: 'Eksploruj dane',
    year: 'Rok',
    month: 'Miesiąc',
    week: 'Tydzień',
    day: 'Dzień',
    transactions: 'Transakcje',
    tapToExplore: 'Dotknij słupka, aby zbadać głębiej',
    noTransactions: 'Brak transakcji w tym okresie',
    back: 'Wróć',
    monthlySpending: 'Wydatki miesięczne',
    weeklySpending: 'Wydatki: {{month}}',
    dailySpending: 'Wydatki dzienne',
    weekLabel: 'Tydzień {{n}}',
    noData: 'Brak danych',
    noDataAvailable: 'Brak dostępnych danych',
    total: 'Łącznie',
  },
  story: {
    title: 'Twoja historia wydatków',
    viewStory: 'Zobacz swoją historię wydatków',
    generating: 'Piszę Twoją historię...',
    regenerate: 'Wygeneruj ponownie',
    lastUpdated: 'Zaktualizowano {{time}}',
    share: 'Udostępnij historię',
    week: 'Tydzień',
    month: 'Miesiąc',
    proRequired: 'Ulepsz do Pro, aby uzyskać historie wydatków',
    noData: 'Za mało danych, aby wygenerować historię',
    achievementTitle: 'Osiągnięcie!',
  },
  goals: {
    title: 'Cele oszczędnościowe',
    subtitle: 'Wyznacz cele i śledź swoje postępy',
    create: 'Utwórz cel',
    name: 'Nazwa celu',
    targetAmount: 'Kwota docelowa',
    deadline: 'Termin',
    currentAmount: 'Bieżąca kwota',
    status: {
      active: 'Aktywny',
      paused: 'Wstrzymany',
      completed: 'Ukończony',
      failed: 'Nieudany',
    },
    planTitle: 'Plan AI',
    planMonthly: 'Wpłata miesięczna',
    planWeekly: 'Wpłata tygodniowa',
    feasibility: {
      easy: 'Łatwy',
      moderate: 'Umiarkowany',
      challenging: 'Wymagający',
      unrealistic: 'Nierealistyczny',
    },
    checkpoints: 'Kamienie milowe',
    categoryLimits: 'Limity kategorii',
    current: 'Bieżący',
    suggested: 'Sugerowany',
    savings: 'Oszczędności',
    progressOnTrack: 'W normie',
    progressBehind: 'Z opóźnieniem',
    percentComplete: '{{percent}}% ukończono',
    projectedDate: 'Prognoza: {{date}}',
    monthlyNeeded: 'Potrzebne miesięcznie',
    empty: 'Brak celów oszczędnościowych. Utwórz jeden, aby rozpocząć!',
    regenerate: 'Wygeneruj plan ponownie',
    deleteConfirm: 'Czy na pewno chcesz usunąć ten cel?',
    edit: 'Edytuj cel',
    generating: 'Generowanie Twojego planu...',
    behindBy: 'Opóźnienie',
    savingsGoals: 'Cele oszczędnościowe',
    noActiveGoals: 'Brak aktywnych celów',
    createYourFirstGoal: 'Ustaw cel oszczędnościowy, aby rozpocząć',
    daysRemaining_one: '{{count}} dzień pozostał',
    daysRemaining_few: '{{count}} dni pozostały',
    daysRemaining_other: '{{count}} dni pozostało',
    overdue: 'Zaległe',
    andMore_one: '+{{count}} więcej',
    andMore_other: '+{{count}} więcej',
    threeMonths: '3 miesiące',
    sixMonths: '6 miesięcy',
    oneYear: '1 rok',
    twoYears: '2 lata',
    goalName: 'Nazwa celu',
    goalNamePlaceholder: 'np. Samochód, Wakacje...',
    currency: 'Waluta',
    quickSelect: 'Szybki wybór:',
    createGoal: 'Utwórz cel',
    errorName: 'Proszę wprowadzić nazwę celu',
    errorAmount: 'Proszę wprowadzić prawidłową kwotę docelową',
    selectDate: 'Wybierz datę',
    errorDeadline: 'Proszę ustawić termin',
    errorInvalidDate: 'Proszę wprowadzić prawidłową datę (RRRR-MM-DD)',
    errorPastDate: 'Termin musi być w przyszłości',
    errorCreate: 'Nie udało się utworzyć celu. Proszę spróbować ponownie.',
    noGoals: 'Brak celów oszczędnościowych',
    noGoalsHint: 'Utwórz swój pierwszy cel, aby zacząć oszczędzać!',
    deleteTitle: 'Usuń cel',
    errorDelete: 'Nie udało się usunąć celu',
    errorRegenerate: 'Nie udało się ponownie wygenerować planu',
    goalDetail: 'Cel',
    notFound: 'Cel nie znaleziony',
    progress: 'Postęp',
    remaining: 'pozostało',
    onTrack: 'W normie',
    behindSchedule: 'Z opóźnieniem',
    projectedCompletion: 'Przewidywane zakończenie',
    aiPlan: 'Plan AI',
    monthly: 'Miesięcznie',
    weekly: 'Tygodniowo',
    category: 'Kategoria',
    regenerating: 'Ponowne generowanie...',
    regeneratePlan: 'Wygeneruj plan AI ponownie',
    deleteGoal: 'Usuń cel',
    addFunds: 'Dodaj kwotę',
    addFundsDescription: 'Ile udało ci się zaoszczędzić?',
    addFundsConfirm: 'Dodaj',
    currentSaved: 'Już zaoszczędzono',
    errorAddFunds: 'Nie udało się dodać kwoty',
    contributionsTitle: 'Historia wpłat',
    noContributions: 'Brak zarejestrowanych wpłat.',
    contributions: 'Wpłaty',
    contributionNote: '{{note}}',
  },
  fatFinder: {
    title: 'Audyt wydatków',
    tapToAnalyze: 'Dotknij, aby przeanalizować wydatki',
    subtitle: 'Znajdź możliwości oszczędzania',
    totalSavings: 'Potencjalne miesięczne oszczędności',
    finding: 'Wynik',
    findings: 'Wyniki',
    typeSubscription: 'Subskrypcja',
    typeRecurringSplurge: 'Powtarzalny wydatek',
    typeLargeOneOff: 'Duży wydatek jednorazowy',
    typeCategoryExcess: 'Nadmiar w kategorii',
    typeServiceOveruse: 'Nadmierne korzystanie z usługi',
    severityLow: 'Niski',
    severityMedium: 'Średni',
    severityHigh: 'Wysoki',
    currentMonthly: 'Bieżący',
    suggestedMonthly: 'Sugerowany',
    potentialSavings: 'Możesz zaoszczędzić',
    regenerate: 'Wygeneruj raport ponownie',
    proRequired: 'Ulepsz do Pro, aby uzyskać audyty wydatków',
    noFindings: 'Świetna robota! Nie znaleziono znaczących oszczędności.',
    generating: 'Analizowanie Twoich wydatków...',
    viewFullReport: 'Zobacz pełny raport',
    relatedExpenses: 'Powiązane wydatki',
    upgradeMessage: 'Przejdź na Pro, aby odblokować miesięczne audyty wydatków',
    loading: 'Analizujemy Twoje wydatki...',
    perMonth: 'miesięcznie',
    emptyTitle: 'Wygląda dobrze!',
    emptyDescription: 'W tym miesiącu nie znaleziono znaczących możliwości oszczędności.',
    findingsCount: '{{count}} znalezisk',
    generatedAt: 'Wygenerowano o {{time}}',
    trackSubscription: 'Śledź tę subskrypcję',
    severity: {
      low: 'Niski',
      medium: 'Średni',
      high: 'Wysoki',
    },
  },
  subscriptionManager: {
    title: 'Subskrypcje',
    settingsSubtitle: 'Śledź cykliczne opłaty i odnowienia',
    addTitle: 'Dodaj subskrypcję',
    editTitle: 'Edytuj subskrypcję',
    monthlyTotal: 'Miesięczny koszt',
    activeCount: '{{count}} aktywna(e)',
    activeTitle: 'Aktywne subskrypcje',
    emptyTitle: 'Brak śledzonych subskrypcji',
    emptyDescription: 'Śledź swoje opłaty cykliczne i nigdy nie daj się zaskoczyć odnowieniem.',
    addFirst: 'Dodaj pierwszą subskrypcję',
    deleteTitle: 'Usuń subskrypcję',
    deleteMessage: 'Usunąć "{{name}}" z trackera?',
    inactive: 'Nieaktywna',
    renewalToday: 'Odnawia się dzisiaj',
    renewalTomorrow: 'Odnawia się jutro',
    renewalInDays: 'Odnawia się za {{count}} dni',
    renewalOverdue: 'Odnowienie przeterminowane',
    notFound: 'Subskrypcja nie znaleziona',
    fieldName: 'Nazwa',
    fieldNamePlaceholder: 'np. Netflix',
    fieldAmount: 'Kwota',
    fieldCycle: 'Cykl rozliczeniowy',
    fieldNextRenewal: 'Następna data odnowienia (RRRR-MM-DD)',
    fieldNotes: 'Notatki (opcjonalne)',
    fieldNotesPlaceholder: 'np. Plan rodzinny',
    fieldActive: 'Aktywna',
    detectedFrom: 'Wykryta z: {{source}}',
    errorName: 'Podaj nazwę subskrypcji.',
    errorAmount: 'Podaj prawidłową kwotę większą od 0.',
    cycle: {
      monthly: 'Miesięcznie',
      yearly: 'Rocznie',
      quarterly: 'Kwartalnie',
      weekly: 'Tygodniowo',
    },
    calendarView: 'Kalendarz',
    listView: 'Lista',
    noRenewalsThisMonth: 'Brak odnowień w tym miesiącu',
    renewalsOnDay: 'Odnowienia {{day}}.',
  },
  tags: {
    title: 'Tagi',
    addTag: 'Dodaj tag',
    createTag: 'Utwórz tag',
    editTag: 'Edytuj tag',
    deleteTag: 'Usuń tag',
    tagName: 'Nazwa tagu',
    tagColor: 'Kolor tagu',
    noTags: 'Brak tagów',
    suggestedTags: 'Sugerowane tagi',
    aiSuggestions: 'Sugestie AI',
    suggestTags: 'Zaproponuj tagi',
    descriptionRequired: 'Najpierw wprowadź opis (co najmniej 3 znaki)',
    recentTags: 'Ostatnie tagi',
    popularTags: 'Popularne tagi',
    manageTags: 'Zarządzaj tagami',
    confirmDelete: 'Czy na pewno chcesz usunąć ten tag?',
    usedInExpenses: 'Użyty w {{count}} wydatkach',
  },
  projects: {
    title: 'Projekty',
    createProject: 'Utwórz projekt',
    editProject: 'Edytuj projekt',
    projectName: 'Nazwa projektu',
    description: 'Opis',
    startDate: 'Data rozpoczęcia',
    endDate: 'Data zakończenia',
    budget: 'Budżet',
    activeProjects: 'Aktywne projekty',
    archivedProjects: 'Zarchiwizowane projekty',
    noProjects: 'Brak projektów',
    addToProject: 'Dodaj do projektu',
    suggestedProject: 'Sugerowany projekt',
    totalSpent: 'Łączne wydatki',
    budgetRemaining: 'Pozostały budżet',
    archiveProject: 'Zarchiwizuj projekt',
    deleteProject: 'Usuń projekt',
    confirmDelete: 'Czy na pewno chcesz usunąć ten projekt?',
    confirmArchive: 'Czy na pewno chcesz zarchiwizować ten projekt?',
    color: 'Kolor',
    noExpenses: 'Brak wydatków w tym projekcie',
    expensesByCategory: 'Wydatki według kategorii',
  },
  purchaseRequests: {
    title: 'Wnioski o zakup',
    new: 'Nowy wniosek',
    approve: 'Zatwierdź',
    reject: 'Odrzuć',
    abstain: 'Wstrzymaj się',
    approved: 'Zatwierdzono',
    rejected: 'Odrzucono',
    pending: 'Oczekujące',
    purchased: 'Kupiono',
    expired: 'Wygasłe',
    active: 'Aktywne',
    history: 'Historia',
    votes: 'Głosy',
    addToPlan: 'Dodaj do planu zakupów',
    convertSuccess: 'Dodano do zaplanowanych wydatków',
    noRequests: 'Brak wniosków o zakup',
    createFirst: 'Zaproponuj zakup rodzinie do głosowania',
    voteComment: 'Dodaj komentarz (opcjonalnie)',
    deadline: 'Termin głosowania (opcjonalnie)',
    approvalRule: 'Zasada zatwierdzania',
    majority: 'Większość głosów',
    unanimous: 'Wszyscy muszą się zgodzić',
    ownerOnly: 'Właściciel decyduje',
    settingsTitle: 'Wnioski o zakup',
    settingsSubtitle: 'Rodzinne głosowanie nad zakupami',
    cancelRequest: 'Anuluj wniosek',
    markAsPurchased: 'Oznacz jako kupione',
    plannedBanner: 'Zaplanowany zakup · Oznacz jako kupiony',
    yourVote: 'Twój głos',
    of: 'z',
    membersVoted: 'członków zagłosowało',
    notifPurchaseRequests: 'Wnioski o zakup',
    notifPurchaseRequestsDesc: 'Nowe wnioski, głosy i decyzje',
    titleLabel: 'Tytuł',
    amountLabel: 'Kwota',
    editRequest: 'Edytuj wniosek',
    editTitle: 'Edytuj wniosek',
    deleteRequest: 'Usuń wniosek',
    deleteConfirm: 'Wniosek zostanie trwale usunięty',
  },
  splits: {
    title: 'Podział kategorii',
    splitExpense: 'Podziel wydatek',
    selectCategory: 'Wybierz kategorię',
    addCategory: 'Dodaj kategorię',
    amount: 'Kwota',
    percentage: 'Procent',
    remainingAmount: 'Pozostało: {{amount}}',
    totalMustMatch: 'Kwoty podziału muszą być równe sumie wydatku',
    suggestSplit: 'Zasugeruj podział',
    aiSuggestion: 'Sugestia AI',
    confirmSplit: 'Potwierdź podział',
    removeSplit: 'Usuń podział',
  },
  trip: {
    splitBetween: 'Podziel między',
    splitEqual: 'Po równo',
    splitExact: 'Dokładne kwoty',
    splitPercentage: 'Procentowo',
    splitShares: 'Udziały',
    splitSelectMember: 'Wybierz co najmniej jednego uczestnika',
    splitExactMismatch: 'Kwoty muszą sumować się do {{amount}}',
    splitPercentageMismatch: 'Procenty muszą sumować się do 100%',
    createTrip: 'Utwórz podróż',
    tripName: 'Nazwa podróży',
    tripNamePlaceholder: 'np. Lato we Włoszech',
    nameRequired: 'Podaj nazwę podróży',
    endDate: 'Data zakończenia',
    currency: 'Waluta',
    settleUp: 'Rozliczenie',
    balances: 'Salda',
    suggestedTransfers: 'Proponowane przelewy',
    allSettled: 'Wszystko rozliczone!',
    allSettledHint: 'Wszyscy uczestnicy tej podróży są rozliczeni — żadne płatności nie są potrzebne.',
    payVia: 'Zapłać',
    manualBlikInstructions: 'Wyślij tę płatność ręcznie na:',
    markAsPaid: 'Oznacz jako opłacone',
    markAsPaidBody: 'Poinformuj drugą osobę, że wysłano płatność. Potwierdzi ona otrzymanie.',
    confirmReceived: 'Potwierdź otrzymanie',
    awaitingConfirmation: 'Oczekiwanie na potwierdzenie',
    awaitingPayment: 'Oczekiwanie na płatność',
    paymentSettingsTitle: 'Ustawienia płatności',
    paymentMethod: 'Metoda płatności',
    paymentMethodRevolut: 'Revolut',
    paymentMethodPaypal: 'PayPal',
    paymentMethodBlik: 'BLIK',
    paymentMethodCash: 'Gotówka',
    paymentMethodOther: 'Inne',
    paymentHandle: 'Identyfikator płatności',
    paymentHandleRequired: 'Podaj identyfikator płatności',
    paymentHandlePlaceholderRevolut: 'twoja-nazwa-revolut',
    paymentHandlePlaceholderPaypal: 'twoj-slug-paypal.me',
    paymentHandlePlaceholderBlik: '+48 XXX XXX XXX',
    paymentHandlePlaceholderGeneric: 'np. gotówka, osobiście',
    paymentHandleBlikHint: 'Wpisz numer telefonu w formacie międzynarodowym, np. +48 XXX XXX XXX.',
    paymentInfoSaved: 'Twoje dane do płatności zostały zapisane.',
    daysLeft: 'Pozostało dni: {{count}}',
    daysLeft_one: 'Pozostał {{count}} dzień',
    tripEnded: 'Podróż zakończona — rozliczcie się',
    pastTrips: 'Poprzednie podróże',
    notifyTripSettleUp: 'Rozliczenie podróży',
    notifyTripSettleUpDesc: 'Przypomnienia o rozliczeniu sald, gdy podróż się kończy',
    archiveTrip: 'Archiwizuj podróż',
    archiveTripConfirm: 'Zarchiwizować tę podróż? Zostanie przeniesiona do Poprzednich podróży i stanie się tylko do odczytu.',
    archiveTripUnconfirmedWarning:
      'Są niepotwierdzone rozliczenia płatności. Zarchiwizować mimo to?',
    archiveTripForce: 'Zarchiwizuj mimo to',
    tripMap: 'Mapa podróży',
  },
  gamification: {
    title: 'Osiągnięcia',
    level: 'Poziom {{level}}',
    xpToNext: '{{xp}} XP do następnego poziomu',
    totalXp: '{{xp}} XP',
    streak: {
      title: 'Codzienna seria',
      days_one: '{{count}} dzień',
      days_other: '{{count}} dni',
      longest: 'Najdłuższa: {{count}} dni',
      broken: 'Zacznij swoją serię już dziś!',
      keepGoing: 'Tak trzymaj!',
    },
    badges: {
      unlocked: 'Osiągnięcie odblokowane!',
      earned: 'Zdobyto {{date}}',
      locked: 'Zablokowane',
      progress: '{{percent}}% ukończono',
      xpEarned: '+{{xp}} XP',
    },
    categories: {
      all: 'Wszystkie',
      budget: 'Budżet',
      tracking: 'Śledzenie',
      streak: 'Seria',
      milestone: 'Kamień milowy',
      savings: 'Oszczędności',
      social: 'Społeczne',
    },
    achievements: {
      firstExpense: { title: 'Pierwszy krok', description: 'Zarejestruj swój pierwszy wydatek' },
      expenses10: { title: 'Dobry początek', description: 'Zarejestruj 10 wydatków' },
      expenses50: { title: 'Konsekwentny śledzący', description: 'Zarejestruj 50 wydatków' },
      expenses100: { title: 'Mistrz wydatków', description: 'Zarejestruj 100 wydatków' },
      expenses500: { title: 'Legenda budżetu', description: 'Zarejestruj 500 wydatków' },
      firstBudget: { title: 'Twórca budżetu', description: 'Utwórz swój pierwszy budżet' },
      budgetMonthNoExceed: { title: 'Mistrz budżetu', description: 'Zakończ miesiąc bez przekroczenia budżetu' },
      budget3MonthsNoExceed: { title: 'Ekspert budżetowy', description: 'Utrzymaj się w budżecie przez 3 miesiące' },
      streak3: { title: 'Trzy z rzędu', description: '3-dniowa seria śledzenia' },
      streak7: { title: 'Wojownik tygodnia', description: '7-dniowa seria śledzenia' },
      streak30: { title: 'Miesięczne poświęcenie', description: '30-dniowa seria śledzenia' },
      streak100: { title: 'Setna seria', description: '100-dniowa seria śledzenia' },
      firstIncome: { title: 'Śledzący przychody', description: 'Zarejestruj swój pierwszy przychód' },
      netPositiveMonth: { title: 'Na plusie', description: 'Zakończ miesiąc z przychodami wyższymi niż wydatki' },
      referrals5: { title: 'Budowniczy społeczności', description: 'Poleć 5 znajomych, którzy pozostaną aktywni' },
      ambassador: { title: 'Ambasador', description: 'Poleć 10 znajomych — jesteś legendą!' },
    },
    dashboardWidget: {
      viewAll: 'Zobacz osiągnięcia',
    },
  },
  categoryCreate: {
    title: 'Nowa kategoria',
    namePlaceholder: 'Nazwa kategorii',
    nameRequired: 'Wprowadź nazwę kategorii',
    creating: 'Tworzenie...',
    create: 'Utwórz',
  },
  investments: {
    portfolio: 'Portfel',
    totalValue: 'Łączna wartość',
    dayChange: 'Niezrealizowany zysk/strata',
    holdingsCount_one: '{{count}} pozycja',
    holdingsCount_other: '{{count}} pozycji',
    holdings: 'Pozycje',
    addAsset: 'Dodaj aktywo',
    searchAsset: 'Szukaj aktywów',
    searchPlaceholder: 'Szukaj akcji, krypto, ETF...',
    noResults: 'Nie znaleziono aktywów',
    recommended: 'Zalecane',
    buy: 'Kup',
    sell: 'Sprzedaj',
    quantity: 'Ilość',
    pricePerUnit: 'Cena za jednostkę',
    fee: 'Prowizja',
    total: 'Suma',
    date: 'Data',
    notes: 'Notatki',
    notesPlaceholder: 'Opcjonalne notatki...',
    addTransaction: 'Dodaj transakcję',
    transactions: 'Transakcje',
    noTransactions: 'Brak transakcji',
    currentPrice: 'Aktualna cena',
    avgCost: 'Średni koszt',
    totalInvested: 'Łącznie zainwestowano',
    marketValue: 'Wartość rynkowa',
    unrealizedPnl: 'Niezrealizowany zysk/strata',
    pnl: 'Zysk/strata',
    returnPercent: 'Zwrot',
    allocation: 'Alokacja',
    analytics: 'Analityka',
    performance: 'Wyniki',
    topGainers: 'Największe wzrosty',
    topLosers: 'Największe spadki',
    benchmark: 'Benchmark',
    proRequired: 'Przejdź na Pro dla porównania z benchmarkiem',
    periods: {
      week: '1T',
      month: '1M',
      quarter: '3M',
      year: '1R',
      all: 'Wszystko',
    },
    assetTypes: {
      stock: 'Akcja',
      crypto: 'Krypto',
      etf: 'ETF',
      bond: 'Obligacja',
      commodity: 'Surowiec',
    },
    removeHolding: 'Usuń pozycję',
    removeHoldingConfirm: 'Czy na pewno chcesz usunąć tę pozycję? Wszystkie transakcje zostaną usunięte.',
    deleteTransaction: 'Usuń transakcję',
    deleteTransactionConfirm: 'Czy na pewno chcesz usunąć tę transakcję?',
    refreshPrices: 'Odśwież ceny',
    lastUpdated: 'Zaktualizowano {{time}}',
    noHoldings: 'Brak pozycji',
    noHoldingsHint: 'Wyszukaj i dodaj aktywa, aby rozpocząć śledzenie portfela',
    searchHint: 'Szukaj po symbolu lub nazwie firmy',
    tryDifferentQuery: 'Spróbuj innego zapytania',
    transactionType: 'Typ transakcji',
    feeOptional: 'Opcjonalnie',
    invalidQuantity: 'Wprowadź prawidłową ilość',
    invalidPrice: 'Wprowadź prawidłową cenę',
    holdingNotFound: 'Pozycja nie znaleziona',
    deleteHolding: 'Usuń pozycję',
    deleteHoldingConfirm: 'Czy na pewno chcesz usunąć tę pozycję? Wszystkie transakcje zostaną usunięte.',
    pnlPercent: 'Rentowność %',
    noHoldingsSubtitle: 'Wyszukaj i dodaj aktywa, aby rozpocząć śledzenie portfela',
    totalPnl: 'Łączny P&L',
    noPerformanceData: 'Brak dostępnych danych o wynikach',
    priceChartComingSoon: 'Wykres cen wkrótce',
    noPriceData: 'Brak danych cenowych',
    lastTxPrice: 'cena ost. trans.',
    allocationByType: 'Alokacja według typu',
    noAllocationData: 'Brak danych o alokacji',
    benchmarkComparison: 'Porównanie z benchmarkiem',
    benchmarkProOnly: 'Przejdź na Pro+ dla porównania z benchmarkiem',
    noBenchmarkData: 'Brak danych do porównania z benchmarkiem',
    yourPortfolio: 'Twój portfel',
    difference: 'Różnica',
    formulas: {
      formula: 'Formuła',
      example: 'Twoje dane',
      performanceTitle: 'Obliczenie rentowności',
      performanceDesc: 'Rentowność portfela pokazuje procentową zmianę wartości portfela w wybranym okresie. Porównuje obecną wartość z początkową.',
      performanceExample: 'Początek: ${{start}} → Koniec: ${{end}} = {{result}}% zwrot',
      allocationTitle: 'Obliczenie alokacji',
      allocationDesc: 'Alokacja aktywów pokazuje, jaki procent całkowitego portfela stanowi każdy typ aktywów. Pomaga to zrozumieć dywersyfikację.',
      allocationExample: 'Wartość każdego typu jest dzielona przez całkowitą wartość portfela i mnożona przez 100',
      gainersTitle: 'Obliczenie zysku/straty',
      gainersDesc: 'Procent zysku/straty (P&L) pokazuje, ile każda pozycja zarobiła lub straciła w porównaniu ze średnią ceną zakupu.',
      gainersExample: 'Jeśli kupiłeś po $100, a obecna cena to $120, Twój P&L = +20%',
      benchmarkTitle: 'Porównanie z benchmarkiem',
      benchmarkDesc: 'Porównanie pokazuje, jak Twój portfel wypadł względem indeksu rynkowego (np. S&P 500). Dodatnia różnica oznacza, że pobiłeś rynek.',
      benchmarkExample: 'Portfel: {{portfolio}}% vs Benchmark: {{benchmark}}% = {{diff}}% różnica',
    },
    insights: {
      title: 'Analizy AI portfela',
      loading: 'Analizowanie Twojego portfela...',
      error: 'Nie udało się załadować analiz',
      empty: 'Brak dostępnych analiz',
    },
  },
  help: {
    title: 'Pomoc',
    articleTitle: 'Pomoc',
    widgets: {
      title: 'Widżety ekranu głównego',
      subtitle: 'Tylko Android',
      intro: 'Dodaj widżety do ekranu głównego Androida, aby szybko uzyskać dostęp do danych o wydatkach lub rejestrować wydatki bez otwierania aplikacji.',
      howToAdd: 'Jak dodać widżet?',
      howToAddSteps: 'Przytrzymaj ekran główny → Widżety → AI Budget Assistant → wybierz widżet',
      available: 'Dostępne widżety',
      widgetSmallTitle: 'Budżet – Dziś',
      widgetSmallDesc: 'Łączne wydatki dnia z porównaniem do wczoraj.',
      widgetMediumTitle: 'Budżet – Tydzień',
      widgetMediumDesc: 'Wykres słupkowy wydatków z ostatnich 7 dni plus suma dnia.',
      widgetLargeTitle: 'Budżet – Przegląd',
      widgetLargeDesc: 'Paski postępu budżetu i główne kategorie wydatków bieżącego okresu.',
      widgetQuickTitle: 'Budżet – Szybkie dodanie',
      widgetQuickDesc: 'Trzy przyciski: 🎤 głos, 📷 skan paragonu lub ✏️ ręcznie — otwiera aplikację bezpośrednio.',
      faqAndroid: 'Czy widżety są dostępne na iOS?',
      faqAndroidAnswer: 'Widżety działają tylko na Androidzie. Użytkownicy iOS mogą korzystać z tych samych funkcji przez aplikację.',
      faqUpdate: 'Jak często dane widżetu są aktualizowane?',
      faqUpdateAnswer: 'Dziś, Tydzień i Przegląd odświeżają się co 30 min. Szybkie dodanie jest statyczne.',
    },
  },
  encryption: {
    title: 'Bezpieczeństwo',
    e2ee: 'Szyfrowanie end-to-end',
    e2eeDesc: 'Zaszyfruj swoje dane finansowe, aby tylko Ty mógł je odczytać',
    setupE2EE: 'Skonfiguruj szyfrowanie',
    setupDesc: 'Utwórz hasło szyfrujące, aby chronić swoje dane za pomocą szyfrowania end-to-end',
    passphrase: 'Hasło szyfrujące',
    passphrasePlaceholder: 'Wprowadź silne hasło szyfrujące...',
    passphraseConfirm: 'Potwierdź hasło szyfrujące',
    passphraseConfirmPlaceholder: 'Wprowadź ponownie hasło szyfrujące...',
    passphraseHint: 'To hasło jest niezależne od hasła logowania. Przechowuj je bezpiecznie — nie można go odzyskać.',
    passphraseMismatch: 'Hasła szyfrujące nie są zgodne',
    passphraseMin: 'Hasło szyfrujące musi mieć co najmniej 8 znaków',
    setup: 'Włącz szyfrowanie',
    settingUp: 'Konfigurowanie szyfrowania...',
    setupSuccess: 'Szyfrowanie włączone pomyślnie!',
    recoveryKey: 'Klucz odzyskiwania',
    recoveryKeyDesc: 'Zapisz ten klucz w bezpiecznym miejscu. Będziesz go potrzebować, jeśli zapomnisz hasła szyfrującego.',
    recoveryKeyCopied: 'Klucz odzyskiwania skopiowany do schowka',
    recoveryKeySaved: 'Zapisałem klucz odzyskiwania',
    unlock: 'Odblokuj szyfrowanie',
    unlockDesc: 'Wprowadź hasło szyfrujące, aby odszyfrować swoje dane',
    unlockButton: 'Odblokuj',
    unlocking: 'Odblokowywanie...',
    unlockFailed: 'Nieprawidłowe hasło szyfrujące. Spróbuj ponownie.',
    locked: 'Zablokowane',
    lockedDesc: 'Twoje zaszyfrowane dane są zablokowane. Wprowadź hasło szyfrujące, aby uzyskać dostęp.',
    unlocked: 'Odblokowane',
    lock: 'Zablokuj',
    lockDesc: 'Zablokuj zaszyfrowane dane',
    accountEncryption: 'Szyfrowanie konta',
    enableForAccount: 'Włącz dla tego konta',
    tier1: 'Standardowy',
    tier1Desc: 'Szyfruje opisy, notatki i nazwy. Analityka i AI pozostają dostępne.',
    tier2: 'Maksymalna prywatność',
    tier2Desc: 'Szyfruje wszystkie dane, w tym kwoty. Analityka serwerowa i AI zostają wyłączone.',
    enabled: 'Włączone',
    disabled: 'Wyłączone',
    tierLabel: 'Poziom szyfrowania',
    keyRotation: 'Rotacja klucza',
    keyRotationNeeded: 'Wymagana rotacja klucza (usunięto członka)',
    rotateKey: 'Rotuj klucz',
    rotating: 'Rotowanie klucza...',
    rotateSuccess: 'Klucz zrotowany pomyślnie',
    pendingGrants: 'Oczekujące przyznania klucza',
    pendingGrantsDesc: 'Ci członkowie potrzebują klucza szyfrującego, aby uzyskać dostęp do danych',
    grantKey: 'Przyznaj klucz',
    granting: 'Przyznawanie...',
    grantSuccess: 'Klucz przyznany pomyślnie',
    recovery: 'Odzyskiwanie',
    setupRecovery: 'Skonfiguruj klucz odzyskiwania',
    recoverAccess: 'Odzyskaj dostęp',
    recoverDesc: 'Wprowadź klucz odzyskiwania, aby odzyskać dostęp do zaszyfrowanych danych',
    recoveryKeyPlaceholder: 'XXXX-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX',
    newPassphrase: 'Nowe hasło szyfrujące',
    recovering: 'Odzyskiwanie...',
    recoverSuccess: 'Dostęp odzyskany! Zapisz nowy klucz odzyskiwania.',
    noMembers: 'Brak oczekujących przyznań klucza',
    resetTitle: 'Resetuj szyfrowanie',
    resetConfirm: 'Spowoduje to usunięcie Twojego profilu szyfrowania. Będziesz musiał ponownie skonfigurować szyfrowanie. Wcześniej zaszyfrowane dane, które nie zostały zsynchronizowane, zostaną utracone. Kontynuować?',
    resetButton: 'Resetuj szyfrowanie',
    resetSuccess: 'Szyfrowanie zostało zresetowane. Możesz skonfigurować je ponownie.',
  },
  reports: {
    title: 'Raporty',
    settingsTitle: 'Raporty i e-mail',
    generateReport: 'Generuj raport',
    format: 'Format',
    dateRange: 'Zakres dat',
    lastWeek: 'Ostatni tydzień',
    thisMonth: 'Ten miesiąc',
    lastQuarter: 'Ostatni kwartał',
    thisYear: 'Ten rok',
    specificMonth: 'Konkretny miesiąc',
    customRange: 'Własny zakres',
    selectMonth: 'Wybierz miesiąc',
    selectMonthHint: 'Wybierz miesiąc powyżej',
    from: 'Od',
    to: 'Do',
    pickDate: 'Wybierz datę',
    rangeIncomplete: 'Wybierz datę początkową i końcową — początek nie może być po końcu',
    monthlyDigestIndependent: 'Bieżący miesiąc, niezależnie od zakresu powyżej',
    generate: 'Generuj',
    generating: 'Generowanie...',
    reportGenerated: 'Raport wygenerowany pomyślnie!',
    reportSavedTo: 'Raport zapisany w:\n{{location}}',
    recentReports: 'Ostatnie raporty',
    noReports: 'Brak raportów',
    monthlyDigest: 'Podsumowanie miesięczne',
    monthlyDigestDesc: 'Otrzymuj miesięczny przegląd finansów na e-mail',
    income: 'Przychody',
    expenses: 'Wydatki',
    savingsRate: 'Wskaźnik oszczędności',
    topCategories: 'Główne kategorie',
    weeklyEmail: 'Tygodniowy e-mail',
    weeklyEmailDesc: 'Otrzymuj tygodniowe podsumowanie finansów na e-mail',
    weeklyEmailBusiness: 'Wymaga planu Business',
    sendOn: 'Wyślij w',
    sunday: 'Ndz',
    monday: 'Pon',
    tuesday: 'Wt',
    wednesday: 'Śr',
    thursday: 'Czw',
    friday: 'Pt',
    saturday: 'Sob',
    exportBackup: 'Utwórz kopię zapasową',
    backupExported: 'Kopia zapasowa utworzona pomyślnie!',
    backupSavedTo: 'Kopia zapasowa zapisana w:\n{{location}}',
    backupShared: 'Kopia zapasowa utworzona. Wybierz w menu, gdzie ją zapisać.',
    restoreBackup: 'Importuj kopię zapasową',
    exporting: 'Tworzenie...',
    restoring: 'Importowanie...',
    backupRestored: 'Kopia zapasowa zaimportowana pomyślnie!',
    restoreConfirmTitle: 'Importuj kopię zapasową',
    restoreConfirmMerge: 'Połączyć z istniejącymi danymi (pominąć duplikaty)?',
    restoreConfirmOverwrite: 'Nadpisać wszystkie istniejące dane? Tej operacji nie można cofnąć.',
    merge: 'Połącz',
    overwrite: 'Nadpisz',
    encryptionRestricted: 'Raporty nie są dostępne dla w pełni zaszyfrowanych kont',
    deleteTitle: 'Usuń raport',
    deleteConfirm: 'Czy na pewno chcesz usunąć ten raport?',
    share: 'Udostępnij',
    download: 'Pobierz',
  },
  legal: {
    privacyPolicy: 'Polityka Prywatności',
    termsOfService: 'Regulamin',
    agreeToTerms: 'Akceptuję',
    and: 'oraz',
    bySubscribing: 'Subskrybując, akceptujesz nasz',
    mustAcceptTerms: 'Musisz zaakceptować Regulamin i Politykę Prywatności',
  },
  location: {
    sectionTitle: 'Lokalizacja',
    attachToggle: 'Dołączaj lokalizację do nowych wydatków',
    attachToggleDesc: 'Zapisuje pozycję GPS, gdy dodajesz wydatek na miejscu, i sprawdza, gdzie jesteś, gdy otwierasz Pulpit',
    permissionDenied: 'Wymagane jest uprawnienie do lokalizacji. Włącz je w ustawieniach systemu.',
    title: 'Lokalizacja',
    addLocation: 'Dodaj lokalizację',
    editLocation: 'Edytuj lokalizację',
    removeLocation: 'Usuń lokalizację',
    myLocation: 'Moja lokalizacja',
    tapToPlace: 'Dotknij mapy, aby ustawić pinezkę',
    searchPlaceholder: 'Szukaj adresu lub miejsca',
    searchNoResults: 'Brak wyników',
    recentSearches: 'Ostatnie',
    pickerTitle: 'Lokalizacja wydatku',
  },
  map: {
    mapView: 'Mapa',
    listView: 'Lista',
    noLocationCount_one: '{{count}} wydatek bez lokalizacji',
    noLocationCount_few: '{{count}} wydatki bez lokalizacji',
    noLocationCount_many: '{{count}} wydatków bez lokalizacji',
    open: 'Otwórz',
  },
  debt: {
    debtsAndLoans: 'Długi i Pożyczki',
    debtDetails: 'Szczegóły długu',
    lendMoney: 'Pożycz komuś',
    borrowMoney: 'Pożycz od kogoś',
    contactNamePlaceholder: 'Imię osoby',
    setDueDate: 'Termin zwrotu (opcjonalnie)',
    dueDate: 'Termin zwrotu',
    contact: 'Kontakt',
    lent: 'Pożyczone',
    borrowed: 'Zadłużone',
    recordRepayment: 'Zapisz spłatę',
    repaymentHistory: 'Historia spłat',
    noRepayments: 'Brak zarejestrowanych spłat',
    originalAmount: 'Kwota początkowa',
    totalRepaid: 'Łącznie spłacono',
    remaining: 'Pozostało',
    status: 'Status',
    statusActive: 'Aktywny',
    statusPaid: 'Spłacony',
    statusOverdue: 'Zaległy',
    peopleOweYou: 'Są ci winni',
    youOwe: 'Jesteś winien',
    moneyLent: 'Pożyczone pieniądze',
    moneyBorrowed: 'Zadłużone pieniądze',
    noDebts: 'Brak długów',
    noDebtsHint: 'Oznacz wydatek lub przychód jako dług, aby go śledzić',
    addDebt: 'Dodaj dług',
    lendMoneyHint: 'Zapisz pieniądze, które pożyczyłeś',
    borrowMoneyHint: 'Zapisz pieniądze, które pożyczyłeś od kogoś',
    filterAll: 'Wszystkie',
    filterActive: 'Aktywne',
    filterOverdue: 'Zaległe',
    filterPaid: 'Spłacone',
    isDebtRepayment: 'Spłata długu',
    repaymentFor: 'Spłata długu',
  },
  calendar: {
    title: 'Kalendarz',
    income: 'Przychody',
    expenses: 'Wydatki',
    netProfit: 'Zysk netto',
    categories: 'Kategorie',
    wallets: 'Konta',
    transactions: 'Transakcje',
    noTransactions: 'Brak transakcji w tym okresie',
    tapToViewDetails: 'Dotknij, aby zobaczyć szczegóły',
    allMonth: 'Cały miesiąc',
  },
  scenarioSimulator: {
    title: 'Symulator scenariuszy',
    subtitle: 'Dostosuj wydatki i dochody, aby zobaczyć zmiany oszczędności',
    currentSavings: 'Bieżący miesiąc',
    scenarioSavings: 'Scenariusz',
    projectionTitle: 'Projekcja oszczędności',
    months: 'mies.',
    expenseAdjustments: 'Korekty wydatków',
    incomeAdjustments: 'Korekty dochodów',
    addExtraIncome: 'Dodaj dodatkowy dochód',
    extraIncomeDescription: 'Opis',
    extraIncomeAmount: 'Kwota miesięcznie',
    resetAll: 'Zresetuj wszystko',
    perMonth: '/mies.',
    unchanged: 'bez zmian',
    noData: 'Za mało danych',
    noDataHint: 'Dodaj wydatki i dochody, aby zobaczyć prognozy',
    currentPath: 'Obecny',
    scenarioPath: 'Scenariusz',
    saveScenario: 'Zapisz scenariusz',
    savedScenarios: 'Zapisane scenariusze',
    saveNamePlaceholder: 'np. Restauracje -30%',
    scenarioSaved: 'Scenariusz zapisany',
    noSavedScenarios: 'Brak zapisanych scenariuszy',
    loadScenario: 'Załaduj',
    deleteScenario: 'Usuń',
    shareScenario: 'Udostępnij',
    scenarioLimitFree: 'Osiągnięto limit 5 scenariuszy. Przejdź na Pro, aby zapisywać bez limitu.',
  },
  widgets: {
    today: 'Dzisiaj',
    openApp: 'Otwórz aplikację',
    week: 'Tydzień',
    budgets: 'Budżety',
    topCategories: 'Top kategorie',
    voice: 'Głos',
    scan: 'Skanuj',
    add: 'Dodaj',
    dayNames: ['Nd', 'Pon', 'Wt', 'Śr', 'Czw', 'Pt', 'Sob'],
  },
  settingsNav: {
    profile: 'Profil i konto',
    profileDesc: 'Imię, e-mail, strefa czasowa, waluta',
    appearance: 'Wygląd i język',
    appearanceDesc: 'Motyw, język',
    ai: 'Ustawienia AI',
    aiDesc: 'Styl odpowiedzi, wybór modelu',
    widgets: 'Widżety pulpitu',
    widgetsDesc: 'Pokaż lub ukryj widżety na ekranie głównym',
    notifications: 'Powiadomienia i integracje',
    notificationsDesc: 'Powiadomienia push, bot Telegram',
    security: 'Bezpieczeństwo i szyfrowanie',
    securityDesc: 'Szyfrowanie end-to-end (E2EE)',
    data: 'Dane i raporty',
    dataDesc: 'Synchronizacja, raporty e-mail, kopia zapasowa',
    about: 'O aplikacji',
    aboutDesc: 'Wersja, pomoc, wsparcie, informacje prawne',
    wallet: 'Portfel',
    walletDesc: 'Salda, długi, saldo początkowe',
    categories: 'Kategorie',
    categoriesDesc: 'Zarządzanie i usuwanie kategorii',
    merchants: 'Sprzedawcy',
    merchantsDesc: 'Zmień nazwy, scal lub usuń sprzedawców',
    referenceData: 'Słowniki',
    referenceDataDesc: 'Kategorie, sprzedawcy, tagi i projekty',
    accountsDesc: 'Przełączanie kont, członkowie, miesiąc finansowy',
    tags: 'Tagi',
    tagsDesc: 'Twórz, zmieniaj nazwy lub usuwaj tagi',
    projects: 'Projekty',
    projectsDesc: 'Zarządzaj projektami',
    subscriptionDesc: 'Plan, zużycie, ulepszenie',
    products: 'Produkty',
    productsDesc: 'Zarządzaj nazwami i grupami produktów',
    whatsNew: 'Co nowego',
    whatsNewDesc: 'Zobacz, co ta aplikacja już potrafi dla Ciebie zrobić',
  },
  categories: {
    title: 'Kategorie',
    expenseCategories: 'Kategorie wydatków',
    incomeCategories: 'Kategorie dochodów',
    delete: 'Usuń',
    deleteConfirmTitle: 'Usuń kategorię',
    deleteConfirmMessage: 'Czy na pewno chcesz usunąć "{{name}}"?',
    deleteErrorHasRecords: 'Nie można usunąć kategorii. Jest używana w {{expenses}} wydatkach, {{incomes}} dochodach, {{budgets}} budżetach i {{other}} innych wpisach.',
    nameTaken: 'Kategoria o nazwie „{{name}}” już istnieje. Wybierz inną nazwę.',
    nameTakenByDeleted: 'Usunięta kategoria o nazwie „{{name}}” wciąż istnieje. Wybierz inną nazwę.',
    deleteSuccess: 'Kategoria usunięta',
    iconLabel: 'Ikona',
    empty: 'Brak kategorii',
    system: 'Systemowa',
    edit: 'Edytuj kategorię',
    add: 'Dodaj kategorię',
    save: 'Zapisz',
    expense: {
      foodAndDining: 'Jedzenie i restauracje',
      transport: 'Transport',
      shopping: 'Zakupy',
      entertainment: 'Rozrywka',
      healthAndFitness: 'Zdrowie i fitness',
      billsAndUtilities: 'Rachunki i media',
      education: 'Edukacja',
      travel: 'Podróże',
      groceries: 'Artykuły spożywcze',
      coffeeAndDrinks: 'Kawa i napoje',
      subscriptions: 'Subskrypcje',
      clothing: 'Odzież',
      personalCare: 'Higiena osobista',
    },
    income: {
      salary: 'Wynagrodzenie',
      freelance: 'Freelance',
      investments: 'Inwestycje',
      gifts: 'Prezenty',
      otherIncome: 'Inne przychody',
    },
  },
  merchants: {
    title: 'Sprzedawcy',
    rename: 'Zmień nazwę',
    delete: 'Usuń',
    renameTitle: 'Zmień nazwę sprzedawcy',
    renamePlaceholder: 'Nazwa sprzedawcy',
    nameRequired: 'Wpisz nazwę sprzedawcy',
    expensesCount: '{{count}} wydatków',
    deleteConfirm: 'Usunąć sprzedawcę z {{count}} wydatków?',
    renamed: 'Zaktualizowano {{count}} wydatków',
    deleted: 'Wyczyszczono {{count}} wydatków',
    empty: 'Brak sprzedawców',
    select: 'Wybierz',
    selected: '{{count}} zaznaczonych',
    merge: 'Scal',
    mergeTitle: 'Scal sprzedawców',
    mergeInto: 'Scal w',
    mergeCount: '{{count}} wydatków zostanie przeniesionych',
    merged: 'Scalono {{count}} wydatków do {{name}}',
    selectToMerge: 'Zaznacz 2 lub więcej sprzedawców do scalenia',
    suggestionTitle: 'Wygląda jak ten sam sprzedawca',
    suggestionMerge: 'Scal w {{name}}',
    dismiss: 'Odrzuć',
    categoryRules: 'Reguły kategorii',
    noRules: 'Brak zapamiętanych reguł',
    ruleDeleteConfirm: 'Usunąć regułę auto-kategorii dla „{{merchant}}"?',
    ruleDeleted: 'Reguła usunięta',
    reapplyNothingToDo: 'Wszystkie wydatki {{merchant}} są już w odpowiedniej kategorii.',
    reapplyTitle: 'Zastosuj regułę ponownie',
    reapplyGroupLabel: '{{count}} z {{category}}',
    reapplySummary: '{{count}} wydatków zostanie przeniesionych do {{category}}',
    reapplyApplied: 'Przeniesiono {{count}} wydatków do {{category}}',
    reapplyApply: 'Zastosuj ({{count}})',
  },
  aiUsage: {
    confirmTitle: 'Użycie AI',
    confirmMessage: 'Ta akcja używa {{cost}} zapytań AI.\nWykorzystano: {{used}}/{{limit}} (pozostało {{remaining}})',
    continue: 'Kontynuuj',
    dontAskAgain: 'Nie pytaj ponownie',
    totalUsed: 'Wykorzystano',
    byFeature: 'Według funkcji',
    history: 'Historia',
    noUsage: 'Brak użycia AI w tym miesiącu',
    features: {
      chat: 'Chat AI',
      voice: 'Wprowadzanie głosowe',
      parse: 'Analiza tekstu',
      categorization: 'Kategoryzacja',
      ocr: 'Skan paragonu',
      story: 'Historia wydatków',
      fat_finder: 'Fat Finder',
      insights: 'AI Insights',
      investment_insights: 'Insights inwestycyjne',
      tag_suggestion: 'Sugestia tagów',
      project_suggestion: 'Sugestia projektów',
      split_suggestion: 'Sugestia podziału',
      goal_plan: 'Planowanie celów',
    },
  },
  referral: {
    title: 'Zaproś znajomych',
    yourCode: 'Twój kod polecenia',
    share: 'Udostępnij',
    copy: 'Kopiuj',
    copied: 'Skopiowano!',
    codeOptional: 'Kod polecenia (opcjonalnie)',
    stats: {
      total: 'Łącznie',
      qualified: 'Aktywne',
      pending: 'Oczekujące',
    },
    bonusAiRequests: 'Bonusowe zapytania AI',
    requests: 'zapytań',
    milestone5: 'Darmowy miesiąc Pro',
    milestone10: 'Odznaka Ambasadora',
    status: {
      pending: 'Oczekujące',
      qualified: 'Aktywne',
      expired: 'Wygasło',
    },
    extendedTrial: '14-dniowy przedłużony okres próbny',
    inviteCardTitle: 'Zyskacie oboje',
    inviteCardBody: "Znajomy właśnie oddał Ci pieniądze przez aplikację. Wyślij mu zaproszenie: on dostanie dłuższy okres próbny, Ty bonusowe zapytania AI.",
    inviteCardAction: 'Zaproś go',
    shareText: 'Dołącz do AI Budget Assistant z moim kodem {{code}}! Otrzymaj 14-dniowy okres próbny.',
    settingsTitle: 'Zaproś znajomych',
    settingsSubtitle: 'Udostępnij kod, zdobądź bonusowe zapytania AI',
    referredUsers: 'Poleceni użytkownicy',
    empty: {
      title: 'Brak poleceń',
      subtitle: 'Udostępnij swój kod, aby zacząć zdobywać nagrody',
    },
    reward: {
      free_pro_month: 'Darmowy miesiąc Pro',
      ambassador_badge: 'Odznaka Ambasadora',
    },
    howItWorks: {
      title: 'Jak to działa',
      step1: 'Udostępnij swój kod znajomym',
      step2: 'Rejestrują się i korzystają z aplikacji przez 7 dni',
      step3: 'Polecenie staje się aktywne — otrzymujesz +30 zapytań AI',
    },
    nextMilestone: 'Następny kamień milowy',
    friendJoined: '{{name}} dołączył z twoim kodem!',
    friendQualified: '{{name}} jest teraz aktywny! +30 zapytań AI zdobytych.',
  },
  update: {
    titleAvailable: 'Dostępna aktualizacja',
    titleRequired: 'Wymagana aktualizacja',
    bodyAvailable: 'Nowa wersja AI Budget Assistant jest dostępna.',
    bodyRequired: 'Ta wersja nie jest już wspierana. Zaktualizuj, aby kontynuować.',
    actionUpdate: 'Aktualizuj',
    actionLater: 'Później',
    releaseNotesLabel: 'Co nowego',
  },
  whatsNew: {
    title: 'Co nowego',
    spotlightNewBadge: 'Nowość',
    tierPro: 'Pro',
    tierBusiness: 'Business',
    actionLearnMore: 'Powiedz mi więcej',
    actionGotIt: 'Rozumiem',
    emptyState: 'Nic tu jeszcze nie ma — sprawdź ponownie po kolejnej aktualizacji.',
    entries: {
      'fat-finder': {
        title: 'Fat Finder',
        body: 'Audyt wydatków przez AI: znajduje zapomniane subskrypcje, podwyżki i jednorazowe obciążenia warte drugiego spojrzenia.',
      },
      'financial-health-score': {
        title: 'Wskaźnik kondycji finansowej',
        body: 'Jedna ocena od 0 do 100 na ekranie głównym: trzymanie budżetu, stopa oszczędności, postęp celów i stan długów.',
      },
      'safe-to-spend': {
        title: 'Bezpieczna kwota na dziś',
        body: 'Ile możesz dziś wydać, nie ryzykując rachunków, celów ani subskrypcji — to liczba na górze ekranu głównego.',
      },
      'personal-inflation-index': {
        title: 'Osobisty wskaźnik inflacji',
        body: 'Jak zmieniały się ceny dokładnie tych produktów, które kupujesz — na podstawie twoich zeskanowanych paragonów.',
      },
      'shopping-list-compare': {
        title: 'Gdzie taniej?',
        body: 'Porównaj listę zakupów między sklepami, do których naprawdę chodzisz — po realnych cenach z twoich paragonów.',
      },
      'community-price-map': {
        title: 'Mapa cen społeczności',
        body: 'Gdzie w okolicy znaleziono najlepszą cenę produktu: dane zbierane społecznościowo i anonimizowane — nie da się po nich dotrzeć do nikogo.',
      },
      'financial-wrapped': {
        title: 'Finansowe podsumowanie roku',
        body: 'Rok z twoimi pieniędzmi w stylu Spotify Wrapped: najczęstszy sklep, najdroższy miesiąc, stopa oszczędności i więcej.',
      },
      'inflation-shield': {
        title: 'Tarcza antyinflacyjna',
        body: 'Przewiduje, które z twoich regularnych zakupów zaraz zdrożeją, i podpowiada, by zrobić zapas teraz.',
      },
      'ai-chat-shopping-tools': {
        title: 'Lista zakupów przez czat',
        body: 'Powiedz czatowi „dodaj mleko i jajka” albo „co uzupełnić?” — sam doda, usunie i podpowie.',
      },
      'receipt-price-check': {
        title: 'Kontrola cen na paragonie',
        body: 'Każdy zeskanowany paragon jest po cichu porównywany z tym, ile płaciłeś wcześniej w tym samym sklepie — jeśli coś jest droższe niż zwykle, zobaczysz adnotację.',
      },
      'exchange-rate-alerts': {
        title: 'Alerty kursowe',
        body: 'Ustaw cel dla pary walutowej i dostań powiadomienie w godzinie, w której kurs go osiągnie — koniec ze sprawdzaniem w aplikacji.',
      },
      'shopping-list-auto-check': {
        title: 'Lista zakupów odhacza się sama',
        body: 'Zeskanuj paragon, a to, co naprawdę kupiłeś, samo odhaczy się z listy. Gdyby coś zaznaczyło się niepotrzebnie, obok jest „Cofnij”.',
      },
    },
  },
  wrapped: {
    title: 'Podsumowanie roku',
    notEnoughData: 'Wciąż za mało danych',
    notEnoughDataDesc: 'Śledź wydatki i przychody przez cały rok, aby odblokować swoje podsumowanie.',
    swipeHint: 'Przesuń, aby kontynuować →',
    introTitle: 'Twój {{year}}\nw liczbach',
    introSub: 'Spójrzmy razem na twój rok',
    totalTracked: 'Zarejestrowałeś',
    spentLabel: 'wydane w tym roku',
    earnedLabel: 'Zarobione',
    transactions: 'Transakcje',
    topMerchant: 'Twoje ulubione miejsce',
    merchantVisits: '{{count}} wizyt',
    merchantSpent: '{{value}} łącznie',
    biggestMonth: 'Twój najlepszy miesiąc',
    topCategory: 'Najczęstsza kategoria',
    ofSpending: '{{percent}}% wydatków',
    categoryMix: 'Na co poszły pieniądze',
    receiptsScanned: 'Zeskanowane paragony',
    receiptsScannedDesc: 'Każde skanowanie precyzuje twoje statystyki',
    saved: 'Zaoszczędziłeś',
    overspent: 'Wydałeś więcej, niż zarobiłeś',
    savingsRate: '{{percent}}% stopa oszczędności',
    savedVsLastYear: '{{direction}} {{value}} w porównaniu z zeszłym rokiem',
    personalInflation: 'Twoja osobista inflacja',
    inflationDesc: 'Jak zmieniły się twoje własne ceny w tym roku',
    streakDays: '{{count}} dni',
    longestStreak: 'Najdłuższa passa śledzenia',
    outroTitle: 'Taki był twój {{year}}!',
    outroDesc: 'Podziel się swoim rokiem w liczbach',
    hideAmounts: 'Ukryj kwoty',
    share: 'Udostępnij',
    shareTitle: '🎁 Moje podsumowanie {{year}} — AI Budget',
    shareSpent: 'Zarejestrowano {{value}} wydatków',
    shareMerchant: 'Ulubione miejsce: {{name}} ({{count}} wizyt)',
    shareMonth: 'Najlepszy miesiąc: {{month}}',
    shareCategory: 'Najczęstsza kategoria: {{name}} ({{percent}}%)',
    shareReceipts: 'Zeskanowane paragony: {{count}}',
    shareSaved: 'Zaoszczędzono: {{value}}',
    shareInflation: 'Moja osobista inflacja: {{percent}}%',
    shareStreak: 'Najdłuższa passa: {{count}} dni',
    shareCta: 'Śledź swój rok → ai-budget.pl',
  },
  communityPrices: {
    sectionTitle: 'Ceny społeczności',
    contributeToggle: 'Udostępniaj anonimowe ceny',
    contributeToggleDesc: 'Przekazuj zanonimizowane ceny produktów z paragonów, aby pomóc innym znaleźć najtańszy sklep. Żadne dane osobowe nie są udostępniane.',
    screenTitle: 'Gdzie najtaniej',
    entryTitle: 'Gdzie najtaniej?',
    entrySubtitle: 'Porównaj ceny społeczności w różnych sklepach',
    searchPlaceholder: 'Szukaj produktu...',
    searchHint: 'Wyszukaj produkt, aby porównać ceny w różnych sklepach',
    noResults: 'Brak jeszcze cen społeczności dla tego produktu',
    regionsAvailable: '{{count}} regionów',
    cheapest: 'Najtaniej',
    backedBy: '{{receipts}} paragonów · {{contributors}} osób',
    week1: '1 tydzień',
    week4: '4 tygodnie',
    listView: 'Lista',
    mapView: 'Mapa',
    mapEmpty: 'Brak jeszcze sklepów na mapie',
    paywall: 'Gdzie najtaniej',
  },
  safeToSpend: {
    title: 'Bezpieczne wydatki',
    today: 'Bezpieczne wydatki na dziś',
    subtitle: 'Po pokryciu zobowiązań',
    breakdownTitle: 'Jak to jest obliczane',
    wallet: 'Saldo portfela',
    expectedIncome: 'Oczekiwany przychód',
    subscriptions: 'Nadchodzące subskrypcje',
    recurring: 'Nadchodzące wydatki cykliczne',
    goals: 'Wpłaty na cele',
    buffer: 'Bufor bezpieczeństwa',
    daysLeft: 'Pozostałe dni',
    noIncomeAssumed: 'Przyjęto brak dodatkowych przychodów w tym miesiącu',
    approxRate: 'Zastosowano przybliżony kurs wymiany',
    widgetLabel: 'Bezpieczne wydatki',
  },
  inflationShield: {
    title: 'Tarcza antyinflacyjna',
    savedSoFar: 'Dotychczas zaoszczędzono',
    estimated: 'szacunkowo',
    basketForecast: 'Twój koszyk: ~{{pct}}% w przyszłym miesiącu',
    buyAheadTitle: 'Kup z wyprzedzeniem i oszczędzaj',
    buyAheadTip: 'Kup {{product}} teraz — oszczędź ~{{save}}',
    units_one: '{{count}} szt',
    units_few: '{{count}} szt',
    units_many: '{{count}} szt',
    atStore: 'w {{store}}',
    affordable: 'Dziś przystępne cenowo',
    approxRate: 'Kwoty wykorzystują przybliżone kursy wymiany',
    empty: 'Na razie nie ma czego się zaopatrywać',
    emptySub: 'Zeskanuj kilka paragonów, abyśmy mogli wykryć dla Ciebie rosnące ceny',
    share: 'Udostępnij',
    hideAmounts: 'Ukryj kwoty',
    shareTitle: '🛡️ Moja Tarcza antyinflacyjna — AI Budget',
    shareSaved: 'Dotychczas zaoszczędzono: ~{{value}}',
    shareBasket: 'Prognoza mojego koszyka: +{{pct}}%/mies.',
    shareItem: '{{product}} drożeje o +{{pct}}% — oszczędź ~{{save}}, kupując z wyprzedzeniem',
    shareTotal: 'Możesz zaoszczędzić ~{{value}}, kupując z wyprzedzeniem',
    shareCta: 'Pokonaj inflację → ai-budget.pl',
  },
  affordability: {
    yes: 'Tak, na to stać',
    no: 'Brak środków na teraz',
    tight: 'Na to stać, ale będzie ciasno',
    delaysGoal: 'Na to stać, ale opóźni cel',
    waitUntil: 'Na to stać po {{date}}',
  },
  healthScore: {
    title: 'Kondycja Finansowa',
    tapForDetails: 'Kliknij po szczegóły',
    notEnoughData: 'Za mało danych',
    breakdown: 'Rozkład wyniku',
    note: 'Wynik jest obliczany lokalnie na podstawie budżetów, dochodów, celów i długów.',
    label: {
      green: 'Świetnie',
      yellow: 'Wystarczająco',
      red: 'Wymaga uwagi',
    },
    component: {
      budgetAdherence: 'Przestrzeganie budżetu',
      savingsRate: 'Stopa oszczędności',
      goalProgress: 'Postęp celów',
      debtHealth: 'Stan zadłużenia',
    },
    detail: {
      budgetsOnTrack: 'Wszystkie budżety w normie',
      budgetsOverLimit: '{{overCount}} z {{totalCount}} budżetów przekroczonych',
      noBudgets: 'Brak aktywnych budżetów',
      savingsRateGood: 'Oszczędzasz {{pct}}% dochodów — świetnie!',
      savingsRateLow: 'Oszczędzasz {{pct}}% dochodów (cel: 20%)',
      noIncome: 'Brak dochodów w tym miesiącu',
      goalsOnTrack: 'Wszystkie cele na dobrej drodze',
      goalsBehind: '{{onTrackCount}} z {{totalCount}} celów na dobrej drodze',
      noGoals: 'Brak aktywnych celów oszczędnościowych',
      noOverdueDebts: 'Brak przeterminowanych długów',
      overdueDebts: '{{overdueCount}} przeterminowany(-ch) dług(-ów)',
    },
  },
  autoCapture: {
    title: 'Automatyczne przechwytywanie z powiadomień',
    subtitle: 'Automatycznie rejestruj płatności kartą z powiadomień push banku',
    privacy: 'Treść powiadomienia jest przetwarzana na urządzeniu. Tylko wynikowy wydatek jest synchronizowany — surowe dane nigdy nie opuszczają telefonu.',
    enable: 'Włącz automatyczne przechwytywanie',
    grantPermission: 'Przyznaj dostęp do powiadomień',
    permissionGranted: 'Dostęp do powiadomień przyznany',
    banks: 'Obsługiwane banki',
    androidOnly: 'Automatyczne przechwytywanie jest dostępne tylko na Androidzie. Na iOS użyj skanera paragonów.',
    captured: 'Przechwycono {{amount}} {{currency}} · {{merchant}}',
    reviewLast: 'Ostatnio przechwycone wydatki',
    testTitle: 'Brak automatycznie przechwyconych wydatków. Włącz przechwytywanie i dokonaj płatności kartą.',
  },
  familyFeed: {
    title: 'Rodzinna tablica',
    showAll: 'Pokaż wszystkie',
    today: 'Dzisiaj',
    yesterday: 'Wczoraj',
    expenses: '{{count}} wydatki · {{amount}}',
    incomes: '{{count}} przychody · {{amount}}',
    proposedPurchase: 'zaproponował/a zakup',
    purchaseApproved: 'Zakup zatwierdzony',
    purchaseMade: 'Zakupiono!',
    purchaseShort: 'Wniosek',
    purchaseRejected: 'Odrzucono',
    typeExpense: 'Wydatek',
    typeIncome: 'Przychód',
    typeRequest: 'Wniosek zakupu',
    noActivity: 'Brak aktywności',
    noActivityDesc: 'Tutaj pojawią się wydatki i zakupy wszystkich członków konta',
    expand: 'Pokaż szczegóły',
    collapse: 'Ukryj szczegóły',
    reactAdded: 'Reakcja dodana',
    reactRemoved: 'Reakcja usunięta',
  },
  priceHistory: {
    title: 'Twoja inflacja',
    headline: '+{{pct}}%',
    trackedProducts: '{{count}} produktów · {{period}}',
    period3m: '3M',
    period6m: '6M',
    period12m: '12M',
    periodAll: 'Cały',
    notEnoughData: 'Zeskanuj kilka paragonów, aby zobaczyć swoją inflację',
    noDataForPeriod: 'Za mało historii dla tego okresu. Spróbuj krótszego zakresu.',
    scanReceiptCta: 'Skanuj paragon',
    storeComparison: 'Porównanie sklepów',
    priceHistoryChart: 'Historia cen',
    cheapestStore: 'Najtaniej',
    manageProducts: 'Zarządzaj produktami',
    renameProduct: 'Zmień nazwę produktu',
    mergeProducts: 'Scal produkty',
    mergeInto: 'Scal w',
    merged: 'Scalono',
    deleteAlias: 'Przywróć oryginalną nazwę',
    ignoreProduct: 'Usuń ze śledzenia',
    ignoreConfirm: 'Usunąć "{{name}}" ze śledzenia cen? Możesz przywrócić go później resetując alias.',
    removePrice: 'Usuń tę cenę',
    removePriceConfirm: 'Usunąć cenę z {{date}} ({{price}})? Przydatne dla jednorazowych promocji, które zakłócają indeks inflacji.',
    noProducts: 'Brak śledzonych produktów',
    showMore: '+{{count}} więcej',
    productsHint: 'Produkty są wykrywane ze skanów paragonów. Kliknij produkt, aby zmienić jego nazwę, lub użyj Scal, aby połączyć warianty tego samego produktu.',
    resetAliasTitle: 'Zresetuj nazwę',
    resetAliasBody: 'Zresetuj do oryginalnej nazwy "{{name}}"?',
    resetAlias: 'Zresetuj',
    mergeSelected: 'Scal {{count}} produktów',
    purchasesCount: '{{count}} zakupów',
    reanalyzeWithAi: 'Przenalizuj z AI',
    reanalyzing: 'Analizuję...',
    reanalyzeSuccess: 'Zaktualizowano {{count}} produktów',
    reanalyzeConfirm: 'AI wygeneruje nowe nazwy produktów na podstawie opisów z paragonów. To może chwilę potrwać.',
  },
  shoppingList: {
    title: 'Lista zakupów',
    settingsSubtitle: 'Planuj zakupy, znajdź najtańszy sklep',
    planAShop: 'Zaplanuj zakupy',
    addItem: 'Dodaj produkt',
    addFreeText: 'Dodaj „{{text}}"',
    frequentlyBought: 'Często kupowane',
    searchProducts: 'Szukaj produktów',
    emptyList: 'Dodaj pierwszy produkt',
    noLists: 'Brak list zakupów',
    createFirstList: 'Utwórz listę',
    clearChecked: 'Wyczyść zaznaczone',
    compareCta: 'Porównaj ceny',
    compareTitle: 'Gdzie najtaniej',
    comparePaywall: 'Porównywanie cen koszyka między sklepami to funkcja Pro.',
    cheapest: 'Najtaniej',
    coverage: '{{covered}}/{{total}} pozycji',
    stalePrices: 'Niektóre ceny są nieaktualne',
    missingCount: '{{count}} bez ceny w tym sklepie',
    cheapestPerItem: 'Najniższa cena za produkt',
    notTracked: 'Jeszcze nieśledzone',
    noPriceData: 'Brak historii cen — najpierw zeskanuj kilka paragonów',
    comparing: 'Porównywanie…',
    newList: 'Nowa lista',
    deleteList: 'Usuń listę',
    deleteListConfirm: 'Usunąć tę listę i wszystkie jej produkty?',
    switchList: 'Przełącz listę',
    renameList: 'Zmień nazwę listy',
    archiveList: 'Zarchiwizuj listę',
    archiveListConfirm: 'Zarchiwizować tę listę? Zostanie ukryta, ale nie usunięta.',
    listName: 'Nazwa listy',
    manageLists: 'Zarządzaj listami',
    quantity: 'Ilość',
    restockTitle: 'Czas na uzupełnienie zapasów',
    mapTitle: 'Mapa sklepów',
    sortCheapest: 'Najtaniej',
    sortNearby: 'Najbliżej',
    noStoreLocations: 'Brak lokalizacji sklepów — pojawią się tutaj, gdy paragony będą zawierać adresy',
    findNearby: 'Znajdź w pobliżu',
    distanceKm: '{{km}} km stąd',
    dealsTitle: 'Promocje dla Ciebie',
    dealDrop: '-{{pct}}%',
    autoCheckSectionTitle: 'Lista zakupów',
    autoCheckFromReceipts: 'Automatyczne zaznaczanie z paragonów',
    autoCheckFromReceiptsDesc: 'Po zeskanowaniu odpowiadającego paragonu pozycje już obecne na liście zakupów zostaną automatycznie oznaczone jako kupione.',
    templates: 'Szablony',
    saveAsTemplate: 'Zapisz bieżącą listę jako szablon',
    templateNamePlaceholder: 'Nazwa szablonu',
    noTemplates: 'Brak zapisanych szablonów',
    templateItemCount: '{{count}} pozycji',
    templateApplied: 'Zastosowano szablon',
    templateAllAlreadyOnList: 'Wszystko z tego szablonu jest już na Twojej liście',
    templateAppliedSomeSkipped: 'Dodano {{added}}, {{skipped}} było już na liście',
    renameTemplate: 'Zmień nazwę szablonu',
    deleteTemplate: 'Usuń szablon',
    deleteTemplateConfirm: 'Usunąć ten szablon? Tej operacji nie można cofnąć.',
    shareList: 'Udostępnij listę',
    shareListAction: 'Udostępnij link',
    revokeLinkAction: 'Cofnij link',
    linkRevoked: 'Link cofnięty',
    linkCopied: 'Link skopiowany do schowka',
    shareLinkFailed: 'Nie udało się utworzyć ani zaktualizować linku do udostępniania. Spróbuj ponownie.',
  },
  receiptCheck: {
    cardTitle_one: '{{count}} produkt kosztuje więcej niż zwykle',
    cardTitle_few: '{{count}} produkty kosztują więcej niż zwykle',
    cardTitle_many: '{{count}} produktów kosztuje więcej niż zwykle',
    cardSubtitle: 'Około {{amount}} więcej niż zwykle płacisz tutaj — warto sprawdzić paragon.',
    usually: 'zwykle',
    youPaid: 'zapłacono',
    difference: 'różnica',
    lowConfidence: 'na podstawie tylko dwóch wcześniejszych zakupów',
    foundTotal: 'Znaleziono {{amount}} powyżej Twoich zwykłych cen w tym roku',
  },
  receiptSplit: {
    title: 'Podziel ten paragon',
    entryAction: 'Podziel',
    addPerson: 'Dodaj osobę',
    recentPeople: 'Ostatnie',
    personName: 'Imię',
    yourShare: 'Twój udział',
    shareByAmount: 'Kwota',
    shareReset: 'Po równo',
    shareYou: 'Ja',
    shareOverAllocated: 'Udziały tej pozycji przekraczają 100%',
    assignEveryone: 'Przypisz każdej osobie co najmniej jedną pozycję',
    assignHint: 'Dotknij pozycję, a potem zaznacz wszystkich, którzy ją dzielili',
    tapPersonHint: 'Zaznacz wszystkich, którzy dzielili tę pozycję',
    chipItemCount_one: '{{count}} pozycja',
    chipItemCount_few: '{{count}} pozycje',
    chipItemCount_many: '{{count}} pozycji',
    chipItemCount_other: '{{count}} pozycji',
    equalMode: 'Podziel po równo',
    equalHint: 'Ten wydatek nie ma pozycji, więc zostanie podzielony po równo.',
    createSplit: 'Utwórz linki',
    shareWith: 'Wyślij do {{name}}',
    copyAll: 'Skopiuj wszystkie linki',
    statusSent: 'Wysłano',
    statusOpened: 'Otwarto',
    statusClaimed: 'Mówi, że zapłacił(a)',
    statusSettled: 'Rozliczone',
    confirmPaid: 'Potwierdź otrzymanie',
    confirmedToast: 'Oznaczono jako rozliczone',
    cancelSplit: 'Anuluj podział',
    cancelConfirm: 'Anulować ten podział? Linki przestaną działać, a długi zostaną usunięte.',
    tooMany: 'Możesz podzielić z maksymalnie 20 osobami.',
    overBill: 'Suma udziałów przekracza kwotę rachunku.',
    encrypted: 'Podział nie jest dostępny dla kont z szyfrowaniem end-to-end, ponieważ serwer nie może odczytać paragonu.',
    offline: 'Potrzebujesz połączenia, aby utworzyć linki.',
    showQr: 'Pokaż kod QR',
    qrTitle: 'Zeskanuj, aby podzielić',
    qrHint: 'Każdy przy stole może to zeskanować, aby zobaczyć swoją część',
    qrShare: 'Udostępnij link',
    qrShareMessage: 'Zeskanuj to, aby zobaczyć swoją część rachunku:',
    flagUnknownItem: 'Pozycja',
    flagWholeShare: 'Cała jego/jej część',
    flagResolve: 'Rozwiązano',
    flagFixHint: 'Aby zmienić, kto ile jest winien, anuluj i utwórz podział od nowa.',
    editAssignment: 'Edytuj przypisanie',
    editAssignmentTitle: 'Kto to miał?',
    editAssignmentSave: 'Zapisz',
    editBlockedConfirmed: 'Nie można tego zmienić — ktoś już potwierdził płatność w tym podziale. Zamiast tego anuluj i utwórz podział od nowa.',
  },
  receiptCategorySplit: {
    title: 'Podział na kategorie',
    edit: 'Zmień kategorie',
    itemCategory: 'Kategoria tej pozycji',
    unassigned: 'Nieprzypisana',
    dropped: 'Nie udało się dopasować kategorii — zapisano bez podziału',
    notSplit: 'Nie podzielono automatycznie — kategorie możesz przypisać samodzielnie',
    newCategory: 'Nowa kategoria',
  },
  shortcuts: {
    helpTitle: 'Skróty klawiszowe',
    showHelp: 'Pokaż skróty',
    searchFocus: 'Przejdź do wyszukiwania',
    newExpense: 'Nowy wydatek',
    navigateRows: 'Poruszanie się między wierszami',
    openRow: 'Otwórz wiersz',
    toggleRow: 'Zaznacz / odznacz wiersz',
    closeDialog: 'Zamknij okno',
  },
} as const;
