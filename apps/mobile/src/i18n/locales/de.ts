export default {
  common: {
    total: 'Gesamt',
    save: 'Speichern',
    cancel: 'Abbrechen',
    delete: 'Löschen',
    edit: 'Bearbeiten',
    update: 'Aktualisieren',
    error: 'Fehler',
    back: 'Zurück',
    done: 'Fertig',
    loading: 'Laden...',
    success: 'Erfolg',
    of: 'von',
    uncategorized: 'Nicht kategorisiert',
    unknownProject: 'Unbekanntes Projekt',
    unknownTag: 'Unbekannter Tag',
    ok: 'OK',
    remove: 'Entfernen',
    copy: 'Kopieren',
    share: 'Teilen',
    retry: 'Wiederholen',
    clear: 'Leeren',
    yes: 'Ja',
    no: 'Nein',
    showAll: 'Alle anzeigen',
    duplicate: 'Duplizieren',
    selectMultiple: 'Mehrere auswählen',
    import: 'Importieren',
    addedBy: 'Hinzugefügt von {{name}}',
    deleteConfirmTitle: 'Transaktion löschen',
    deleteConfirmMessage: 'Sind Sie sicher? Diese Aktion kann nicht rückgängig gemacht werden.',
    confirm: 'Bestätigen',
    search: 'Suchen',
    noResults: 'Keine Ergebnisse',
  },
  dates: {
    today: 'Heute',
    yesterday: 'Gestern',
    daysAgo: 'Vor {{count}} Tagen',
    weeksAgo_one: 'Vor {{count}} Woche',
    weeksAgo_other: 'Vor {{count}} Wochen',
    monthsAgo_one: 'Vor {{count}} Monat',
    monthsAgo_other: 'Vor {{count}} Monaten',
    yearsAgo_one: 'Vor {{count}} Jahr',
    yearsAgo_other: 'Vor {{count}} Jahren',
  },
  nav: {
    dashboard: 'Übersicht',
    expenses: 'Transaktionen',
    budgets: 'Budget',
    analytics: 'Analysen',
    aiChat: 'KI-Chat',
    newExpense: 'Neue Ausgabe',
    expenseDetails: 'Ausgabendetails',
    newBudget: 'Neues Budget',
    budgetDetails: 'Budgetdetails',
    accounts: 'Konten',
    createAccount: 'Konto erstellen',
    accountSettings: 'Kontoeinstellungen',
    settings: 'Einstellungen',
    inviteMember: 'Einladen',
    joinAccount: 'Beitreten',
    adminPanel: 'Admin-Panel',
    income: 'Einkommen',
    newIncome: 'Neues Einkommen',
    incomeDetails: 'Einkommensdetails',
    investmentDashboard: 'Investitionen',
    assetSearch: 'Vermögenswerte suchen',
    assetDetails: 'Vermögensdetails',
    investmentTransaction: 'Transaktion',
    investmentAnalytics: 'Analytik',
    calendar: 'Kalender',
  },
  auth: {
    appName: 'AI Budget',
    appTagline: 'Verfolge deine Ausgaben mit KI',
    signIn: 'Anmelden',
    signUp: 'Registrieren',
    email: 'E-Mail',
    password: 'Passwort',
    confirmPassword: 'Passwort bestätigen',
    fullName: 'Vollständiger Name',
    createAccount: 'Konto erstellen',
    startTracking: 'Beginne deine Finanzen zu verfolgen',
    noAccount: 'Noch kein Konto? ',
    haveAccount: 'Bereits ein Konto? ',
    useBiometric: '{{type}} verwenden',
    quickLogin: 'Schnellanmeldung',
    currency: 'Währung',
    language: 'Sprache',
    forgotPassword: 'Passwort vergessen?',
    resetPassword: 'Passwort zurücksetzen',
    sendCode: 'Code senden',
    newPassword: 'Neues Passwort',
    confirmNewPassword: 'Neues Passwort bestätigen',
    codeSent: 'Wenn diese E-Mail registriert ist, wurde ein Zurücksetzungscode gesendet.',
    passwordResetSuccess: 'Passwort erfolgreich zurückgesetzt. Bitte melden Sie sich an.',
    backToLogin: 'Zurück zur Anmeldung',
    resendCode: 'Code erneut senden',
    enterResetCode: 'Geben Sie den 6-stelligen Code ein, der an Ihre E-Mail gesendet wurde',
    enterEmail: 'Geben Sie Ihre E-Mail-Adresse ein, um einen Zurücksetzungscode zu erhalten',
    verifyEmail: 'E-Mail verifizieren',
    verify: 'Verifizieren',
    verificationSuccess: 'E-Mail erfolgreich verifiziert!',
    enterVerificationCode: 'Geben Sie den 6-stelligen Code ein, der an {{email}} gesendet wurde',
    signingIn: 'Anmeldung...',
    loadingData: 'Ihre Daten werden geladen...',
    continueWithGoogle: 'Mit Google fortfahren',
    or: 'oder',
    googleTermsNote: 'Mit der Fortsetzung akzeptieren Sie unsere AGB und Datenschutzrichtlinie',
  },
  dashboard: {
    hello: 'Hallo, {{name}}!',
    monthlyBudget: 'Monatsbudget',
    used: '{{percent}}% verbraucht',
    addExpense: 'Ausgabe hinzufügen',
    voiceInput: 'Spracheingabe',
    scanReceipt: 'Beleg scannen',
    voiceIncome: 'Spracheingabe Einnahmen',
    scanInvoice: 'Rechnung scannen',
    recentExpenses: 'Letzte Ausgaben',
    seeAll: 'Alle anzeigen',
    noExpenses: 'Noch keine Ausgaben',
    addFirstExpense: 'Füge deine erste Ausgabe hinzu, um loszulegen',
    expense: 'Ausgabe',
    activeBudgets: 'Aktive Budgets',
    noBudgets: 'Kein Budget festgelegt',
    createBudgetHint: 'Erstelle ein Budget, um deine Ausgaben zu verfolgen',
    walletBalances: 'Geldbörsen-Salden',
    exchangeCurrency: 'Umtausch',
    currencyConverter: 'Konverter',
    transfers: 'Überweisungen',
    totalIncome: 'Einnahmen',
    totalExpenses: 'Ausgaben',
    netProfit: 'Nettogewinn',
    netProfitSubtitle: 'Letzte 6 Monate',
    netCapital: 'Nettokapital',
    netCapitalSubtitle: 'Gesamtvermögen',
    purchaseRequest: 'Kaufantrag',
    shoppingList: 'Einkauf',
    shoppingListFull: 'Einkaufsliste',
  },
  expenses: {
    noExpenses: 'Noch keine Ausgaben',
    addFirst: 'Füge deine erste Ausgabe hinzu, indem du auf die + Taste tippst',
    addExpense: 'Ausgabe hinzufügen',
    manualEntry: 'Manuelle Eingabe',
    voiceInput: 'Spracheingabe',
    scanReceipt: 'Beleg scannen',
    tabExpenses: 'Ausgaben',
    tabIncome: 'Einkommen',
    periodWeek: 'Woche',
    periodMonth: 'Monat',
    periodYear: 'Jahr',
    periodAll: 'Alle',
    periodCustom: 'Benutzerdefiniert',
    categoryAll: 'Alle',
    categoryNone: 'Ohne Kategorie',

    searchPlaceholder: 'Transaktionen suchen...',
    merchant: 'Händler',
    merchantPlaceholder: 'z. B. Biedronka, Amazon',
    merchantAll: 'Alle Händler',
    merchantNone: 'Noch keine Händler',
    merchantsSelected: '{{count}} ausgewählt',
    bulkSelect: 'Auswählen',
    bulkCancel: 'Abbrechen',
    bulkSelectAll: 'Alle auswählen',
    bulkSelected: '{{count}} ausgewählt',
    bulkSetCategory: 'Kategorie setzen',
    bulkAddTag: 'Tag hinzufügen',
    bulkDelete: 'Löschen',
    bulkDeleteConfirm: '{{count}} Ausgaben löschen?',
    bulkDeleteConfirmMessage: 'Diese Aktion kann nicht rückgängig gemacht werden.',
    bulkCategoryApplied: 'Kategorie auf {{count}} Ausgaben angewendet',
    bulkTagsApplied: 'Tags zu {{count}} Ausgaben hinzugefügt',
    bulkDeleted: '{{count}} Ausgaben gelöscht',
    receivable: 'Forderung',
    merge: {
      title: 'Ausgaben zusammenführen',
      subtitle: 'Diese Ausgaben könnten derselbe Kauf in verschiedenen Währungen sein.',
      survivorQuestion: 'Welchen Eintrag behalten?',
      keep: 'Behalten',
      keepFields: 'Felder vom anderen Eintrag übernehmen',
      fieldMerchant: 'Händler',
      fieldNotes: 'Notizen',
      fieldCategory: 'Kategorie',
      fieldProject: 'Projekt',
      fieldTags: 'Tags',
      fieldReceipt: 'Belegbild',
      confirm: 'Zusammenführen',
      merged: 'Ausgaben erfolgreich zusammengeführt.',
      cannotMergeSelf: 'Eine Ausgabe kann nicht mit sich selbst zusammengeführt werden.',
      currencyNote: 'Der verbleibende Eintrag behält seine eigene Währung und Betrag. Nur ein Eintrag bleibt erhalten.',
    },
  },
  categorize: {
    bannerText: 'Ausgaben ohne Kategorie: {{count}}',
    bannerTextIncome: 'Einnahmen ohne Kategorie: {{count}}',
    bannerAction: 'Kategorien vorschlagen',
    bannerShort: 'Ohne Kategorie: {{count}}',
    summary: '{{assigned}} von {{total}} zugeordnet',
    summaryNew: 'neue Kategorien: {{count}}',
    newBadge: 'Neu',
    change: 'Ändern',
    searchPlaceholder: 'Kategorien suchen',
    applyShort: 'Übernehmen · {{count}}',
    title: 'Ausgaben kategorisieren',
    titleIncome: 'Einnahmen kategorisieren',
    analyzing: 'Ausgaben werden analysiert…',
    analyzingIncome: 'Einnahmen werden analysiert…',
    newCategory: 'Neue Kategorie',
    existingCategory: 'Bestehende Kategorie',
    unassigned: 'Nicht zuordenbar',
    choose: 'Auswählen',
    createNew: '+ Neue Kategorie erstellen',
    newNamePlaceholder: 'Name der Kategorie',
    apply: 'Übernehmen ({{count}})',
    applyWithNew: 'Übernehmen ({{count}}) · neue Kategorien: {{created}}',
    done: 'Kategorisiert: {{count}} · neue Kategorien: {{created}}',
    nothingToSuggest: 'Im Moment gibt es nichts vorzuschlagen.',
    limitReached: 'KI-Vorschläge für heute aufgebraucht — nur regelbasierte werden angezeigt.',
    skippedEncrypted: 'Verschlüsselte Ausgaben übersprungen: {{count}}',
    skippedEncryptedIncome: 'Verschlüsselte Einnahmen übersprungen: {{count}}',
    error: 'Vorschläge konnten nicht geladen werden.',
    retry: 'Erneut versuchen',
  },
  expensesDesktop: {
    colDate: 'Datum',
    colDescription: 'Beschreibung',
    colCategory: 'Kategorie',
    colAccount: 'Hinzugefügt von',
    colAmount: 'Betrag',
    summaryCount: 'Transaktionen',
    moreCurrencies_one: '+{{count}} weitere Währung',
    moreCurrencies_other: '+{{count}} weitere Währungen',
    emptyState: 'Noch keine Transaktionen',
    facetsHeading: 'Filter',
    clearAllFilters: 'Alle zurücksetzen',
    facetPeriod: 'Zeitraum',
    facetKind: 'Anzeigen',
    facetCategory: 'Kategorie',
    facetMerchant: 'Händler',
    kindAll: 'Alle',
    noMerchant: 'Kein Händler',
    clearFacet: 'Zurücksetzen',
    noOptions: 'Keine Treffer für die aktuellen Filter',
    showMore: '+{{count}} mehr',
    showLess: 'Weniger anzeigen',
    filtersLabel: 'Filter ({{count}})',
    dialogClose: 'Schließen',
    discardChangesTitle: 'Änderungen verwerfen?',
    discardChangesMessage: 'Du hast ungespeicherte Änderungen. Wenn du jetzt schließt, gehen sie verloren.',
    discardChangesConfirm: 'Verwerfen',
    selectRow: 'Zeile auswählen',
    selectAllRows: 'Alle sichtbaren Zeilen auswählen',
    clearSelection: 'Auswahl aufheben',
    rowActions: 'Zeilenaktionen',
  },
  incomes: {
    noIncomes: 'Noch kein Einkommen',
    addFirst: 'Füge dein erstes Einkommen hinzu, indem du auf die + Taste tippst',
    addIncome: 'Einkommen hinzufügen',
  },
  incomeNew: {
    amountPlaceholder: '0,00',
    description: 'Beschreibung',
    descriptionPlaceholder: 'Woher stammt dieses Einkommen?',
    date: 'Datum',
    category: 'Kategorie',
    notes: 'Notizen',
    notesPlaceholder: 'Optionale Notizen...',
    saving: 'Speichern...',
    saveIncome: 'Einkommen speichern',
  },
  incomeDetail: {
    notFound: 'Einkommen nicht gefunden',
    currency: 'Währung',
    noDescription: 'Keine Beschreibung',
    description: 'Beschreibung',
    date: 'Datum',
    category: 'Kategorie',
    notes: 'Notizen',
    deleteTitle: 'Einkommen löschen',
    deleteConfirm: 'Bist du sicher, dass du dieses Einkommen löschen möchtest?',
  },
  budgets: {
    noBudgets: 'Kein Budget festgelegt',
    createHint: 'Erstelle ein Budget, um deine Ausgabenlimits zu verfolgen',
    createBudget: 'Budget erstellen',
    onTrack: 'Im Plan',
    overBudget: 'Budget überschritten',
    spent: 'ausgegeben',
    remaining: 'verbleibend',
    periods: {
      daily: 'Täglich',
      weekly: 'Wöchentlich',
      monthly: 'Monatlich',
      yearly: 'Jährlich',
      custom: 'Benutzerdefiniert',
    },
  },
  analytics: {
    week: 'Woche',
    month: 'Monat',
    year: 'Jahr',
    viewExpenses: 'Ausgaben anzeigen',
    totalSpent: 'Gesamtausgaben',
    avgPerDay: 'Durchschnitt pro Tag',
    transactions: 'Transaktionen',
    this_week: 'Diese Woche',
    this_month: 'Dieser Monat',
    this_year: 'Dieses Jahr',
    spendingTrend: 'Ausgabentrend',
    spendingByMonth: 'Ausgaben nach Monat',
    spendingByCategory: 'Ausgaben nach Kategorie',
    noData: 'Keine Ausgabendaten verfügbar',
    addExpensesHint: 'Füge Ausgaben hinzu, um die Kategorieübersicht zu sehen',
    categoryDetails: 'Kategoriedetails',
    quickInsights: 'Schnelle Einblicke',
    topCategory: 'Top-Kategorie',
    topCategoryText: '„{{category}}" ist deine höchste Ausgabenkategorie in dieser {{range}}.',
    peakSpendingDay: 'Ausgabenstärkster Tag',
    peakSpendingText: 'Deine höchsten Ausgaben waren am {{date}}.',
    dailyBudgetTip: 'Täglicher Budgettipp',
    dailyBudgetText: 'Um im Plan zu bleiben, versuche die täglichen Ausgaben unter {{amount}} zu halten.',
    allCurrencies: 'Alle',
    exportReport: 'Bericht exportieren',
    topItems: 'Häufigste Artikel',
    itemName: 'Artikel',
    itemTotalSpent: 'Insgesamt ausgegeben',
    itemPurchaseCount: 'Käufe',
    months: {
      jan: 'Jan', feb: 'Feb', mar: 'Mär', apr: 'Apr',
      may: 'Mai', jun: 'Jun', jul: 'Jul', aug: 'Aug',
      sep: 'Sep', oct: 'Okt', nov: 'Nov', dec: 'Dez',
    },
    days: {
      sun: 'So', mon: 'Mo', tue: 'Di', wed: 'Mi',
      thu: 'Do', fri: 'Fr', sat: 'Sa',
    },
    budgetVsActual: 'Budget vs. Tatsächlich',
    noBudgetsHint: 'Erstelle Budgets, um deinen Fortschritt zu sehen',
    budgetLabel: 'Budget',
    actualLabel: 'Tatsächlich',
    onTrack: 'Im Plan',
    overBudget: 'Über Budget',
    dayOfWeekTitle: 'Ausgaben nach Wochentag',
    peakDayInsight: 'Am meisten gibst du {{day}}s aus',
    periodUp: 'mehr als letzte {{period}}',
    periodDown: 'weniger als letzte {{period}}',
    vsAvgUp: '{{pct}}% über 3-Monats-Schnitt',
    vsAvgDown: '{{pct}}% unter 3-Monats-Schnitt',
    totalSavings: 'Quittungsersparnisse',
    totalSavingsText: 'Sie haben {{amount}} durch Rabatte in diesem {{range}} gespart.',
    totalDepositsPaid: 'Gezahltes Pfand',
    totalDepositsPaidText: 'Sie haben {{amount}} Pfand für Mehrwegverpackungen in diesem {{range}} gezahlt.',
    savingsDetailDiscountTitle: 'Rabattersparnis',
    savingsDetailDepositTitle: 'Gezahltes Pfand',
    savingsDetailByStore: 'Nach Geschäft',
    savingsDetailRecent: 'Letzte Belege',
    savingsDetailEncrypted: 'Dieses Konto ist vollständig verschlüsselt, daher können wir das nicht aufschlüsseln.',
    savingsDetailEmpty: 'Für diesen Zeitraum noch nichts erfasst.',
    savingsDetailApprox: 'Einige Beträge verwenden einen ungefähren Wechselkurs.',
    savingsDetailDiscountNote: 'Bereits auf Ihren Belegen angewandte Rabatte — kein Versprechen für zukünftige Angebote.',
    savingsDetailDepositNote: 'Das an der Kasse gezahlte Pfand für Mehrwegverpackungen — keine Rückerstattung; zurückgegebene Flaschen und Dosen werden nicht erfasst.',
    byMerchant: 'Nach Händlern',
    merchantOther: 'Sonstige',
    byTag: 'Nach Tags',
    byProject: 'Nach Projekten',
    byIncomeCategory: 'Einnahmen nach Kategorie',
    incomeCategoryOther: 'Sonstige',
    tagBreakdown: 'Tag-Übersicht',
    tagTrends: 'Tag-Trends',
    projectComparison: 'Projektvergleich',
    topTags: 'Top-Tags',
    filterByTag: 'Nach Tag filtern',
    filterByProject: 'Nach Projekt filtern',
    tagAnalyticsComingSoon: 'Tag-Analytik kommt bald',
    projectAnalyticsComingSoon: 'Projekt-Analytik kommt bald',
  },
  chat: {
    title: 'KI-Budgetassistent',
    subtitle: 'Frag mich alles über deine Finanzen, Ausgabenmuster oder erhalte persönliche Budgetberatung.',
    topExpenses: 'Top-Ausgaben',
    topExpensesQ: 'Wofür habe ich diesen Monat am meisten ausgegeben?',
    budgetStatus: 'Budgetstatus',
    budgetStatusQ: 'Bin ich mit meinem Budget auf Kurs?',
    savingTips: 'Spartipps',
    savingTipsQ: 'Gib mir Tipps zum Geldsparen',
    placeholder: 'Frag etwas zu deinem Budget...',
    thinking: 'Denke nach...',
    processingVoice: 'Verarbeite Sprache...',
    errorMessage: 'Entschuldigung, es ist ein Fehler aufgetreten. Bitte überprüfe deine Verbindung und versuche es erneut.',
    copySuccess: 'Kopiert!',
    confirmAction: 'Bestätigen',
    rejectAction: 'Abbrechen',
    actionConfirmed: 'Aktion bestätigt',
    actionRejected: 'Aktion abgebrochen',
    confirming: 'Wird verarbeitet...',
    actionCreateExpense: 'Ausgabe hinzufügen',
    actionCreateIncome: 'Einnahme hinzufügen',
    actionCreateBudget: 'Budget erstellen',
    actionCreateCategory: 'Kategorie erstellen',
    actionGetExpenses: 'Ausgaben',
    actionGetBudgetStatus: 'Budgetstatus',
    actionGetCategoryBreakdown: 'Kategorieaufschlüsselung',
    actionAddShoppingList: 'Zur Einkaufsliste hinzugefügt',
    actionRemoveShoppingList: 'Von der Einkaufsliste entfernt',
    actionShoppingNotFound: 'Nicht gefunden: {{names}}',
    actionShoppingSuggestions: 'Einkaufsvorschläge',
    shoppingSuggestionsRestock: 'Geht zur Neige',
    shoppingSuggestionsDeals: 'Angebote',
    actionInflationShield: 'Inflationsschutz',
    actionDepositTotal: 'Gezahltes Pfand',
    actionDiscountTotal: 'Erhaltener Rabatt',
    andMore: '+{{count}} mehr',
    confirmTitle: 'Aktion bestätigen',
    resultSuccess: 'Erfolgreich abgeschlossen',
    resultFailed: 'Aktion fehlgeschlagen',
    resultUndone: 'Rückgängig gemacht',
    amount: 'Betrag',
    category: 'Kategorie',
    date: 'Datum',
    period: 'Zeitraum',
    spent: 'ausgegeben',
    remaining: 'übrig',
    budget: 'Budget',
    history: 'Verlauf',
    historyTitle: 'Gespräche',
    historyEmpty: 'Noch keine Gespräche',
    newConversation: 'Neues Gespräch',
    loadingHistory: 'Wird geladen...',
    conversationUntitled: 'Gespräch',
    shared: 'Geteilt',
    private: 'Privat',
    renameConversation: 'Gespräch umbenennen',
    deleteConversationTitle: 'Gespräch löschen',
    deleteSharedConversationMessage: 'Dieses Gespräch ist geteilt. Wenn Sie es löschen, wird es für alle Kontomitglieder entfernt, nicht nur für Sie. Diese Aktion kann nicht rückgängig gemacht werden.',
    conversationActions: 'Gesprächsaktionen',
    pinConversation: 'Anheften',
    unpinConversation: 'Nicht mehr anheften',
  },
  expenseNew: {
    amountPlaceholder: '0,00',
    description: 'Beschreibung',
    descriptionPlaceholder: 'Wofür war diese Ausgabe?',
    date: 'Datum',
    category: 'Kategorie',
    saving: 'Speichern...',
    saveExpense: 'Ausgabe speichern',
  },
  expenseDetail: {
    moveTitle: 'In Konto verschieben',
    moveSubtitle: 'Wähle ein Konto, in das diese Ausgabe verschoben werden soll. Tags, Projekt und Kategorie werden nicht übernommen.',
    moveSuccessTitle: 'Ausgabe verschoben',
    moveSuccess: 'Nach {{account}} verschoben.',
    moveError: 'Ausgabe konnte nicht verschoben werden. Bitte versuche es erneut.',
    notFound: 'Ausgabe nicht gefunden',
    amount: 'Betrag',
    currency: 'Währung',
    noDescription: 'Keine Beschreibung',
    description: 'Beschreibung',
    date: 'Datum',
    category: 'Kategorie',
    notes: 'Notizen',
    syncStatus: 'Synchronisierungsstatus',
    sourceManual: 'Manuelle Eingabe',
    sourceVoice: 'Spracheingabe',
    sourceOcr: 'Belegscan',
    sourceImport: 'Importiert',
    sourceNotification: 'Automatisch erfasst',
    extractItems: 'Positionen extrahieren',
    extractingItems: 'Beleg wird gelesen…',
    extractItemsTitle: 'Positionen ersetzen',
    extractItemsConfirm: 'Diese Ausgabe hat bereits {{count}} Position(en). Durch die aus dem Beleg gelesenen ersetzen?',
    extractItemsNoItems: 'Keine Positionen auf diesem Beleg gefunden.',
    extractItemsDone: '{{count}} Position(en) aus dem Beleg extrahiert.',
    extractItemsFailed: 'Beleg konnte nicht gelesen werden. Bitte erneut versuchen.',
    deleteTitle: 'Ausgabe löschen',
    deleteConfirm: 'Bist du sicher, dass du diese Ausgabe löschen möchtest?',
    copy: 'Kopieren',
    receiptItems: 'Belegpositionen',
    noItems: 'Keine Positionen',
    addItem: 'Position hinzufügen',
    editItem: 'Position bearbeiten',
    itemDescription: 'Beschreibung',
    itemQuantity: 'Menge',
    itemUnitPrice: 'Stückpreis',
    itemTotalPrice: 'Gesamtpreis',
    itemSaved: 'Position gespeichert',
    itemDeleted: 'Position gelöscht',
    confirmDeleteItem: 'Diese Position löschen?',
    receiptImage: 'Kassenbon-Bild',
    viewImage: 'Bild anzeigen',
    shareImage: 'Teilen',
    saveImage: 'Speichern',
    imageSaved: 'Bild in der Galerie gespeichert',
    galleryPermissionDenied: 'Galeriezugriff ist zum Speichern von Bildern erforderlich',
    tapToOpen: 'Zum Öffnen tippen',
    deleteImage: 'Bild löschen',
    replaceImage: 'Bild ersetzen',
    confirmDeleteImage: 'Kassenbon-Bild löschen?',
    attachReceipt: 'Beleg anhängen',
    attachFromCamera: 'Foto aufnehmen',
    attachFromGallery: 'Aus Galerie',
    attachAsPdf: 'PDF auswählen',
    cameraPermissionDenied: 'Kamerazugriff erforderlich',
  },
  recurring: {
    repeat: 'Wiederholen',
    repeatDesc: 'Diese Ausgabe automatisch jeden Zeitraum hinzufügen',
    period: 'Zeitraum',
    weekly: 'Wöchentlich',
    monthly: 'Monatlich',
    yearly: 'Jährlich',
    seriesBanner: 'Teil einer wiederkehrenden Serie',
    seriesPeriod: 'Wiederholt sich {{period}}',
    stopRecurring: 'Wiederholung stoppen',
    stopRecurringConfirm: 'Automatisches Hinzufügen stoppen? Vorhandene Einträge werden nicht gelöscht.',
    stopped: 'Wiederholung gestoppt',
  },
  voice: {
    title: 'Sprachausgabe',
    listening: 'Zuhören... Tippen zum Stoppen',
    tapToStart: 'Tippen, um zu sprechen',
    processing: 'Verarbeitung...',
    example: 'Beispiel: „Kaffee bei Starbucks, fünf Euro"',
    analyzing: 'Analysiere deine Ausgabe...',
    youSaid: 'Du hast gesagt:',
    confirmTitle: 'Ausgabe bestätigen',
    amount: 'Betrag',
    description: 'Beschreibung',
    category: 'Kategorie',
    merchant: 'Händler',
    highConfidence: 'Hohe Zuverlässigkeit',
    mediumConfidence: 'Mittlere Zuverlässigkeit',
    saveExpense: 'Ausgabe speichern',
    tryAgain: 'Erneut versuchen',
    success: 'Ausgabe erfolgreich hinzugefügt!',
    addAnother: 'Weitere hinzufügen',
    saveFailed: 'Ausgabe konnte nicht gespeichert werden',
  },
  receipt: {
    title: 'Beleg scannen',
    instructions: 'Foto aufnehmen, aus Galerie wählen oder PDF hochladen',
    hint: 'Richte deine Kamera auf einen Beleg für beste Ergebnisse',
    takePhoto: 'Foto aufnehmen',
    chooseGallery: 'Aus Galerie wählen',
    analyzing: 'Beleg wird analysiert...',
    extracting: 'Daten werden mit KI extrahiert...',
    scannedTitle: 'Beleg gescannt',
    totalAmount: 'Gesamtbetrag',
    description: 'Beschreibung',
    merchant: 'Händler',
    category: 'Kategorie',
    date: 'Datum',
    items: 'Artikel ({{count}})',
    moreItems: '+{{count}} weitere Artikel',
    highConfidence: 'Hohe Zuverlässigkeit',
    mediumConfidence: 'Mittlere Zuverlässigkeit',
    saveExpense: 'Ausgabe speichern',
    scanAgain: 'Erneut scannen',
    success: 'Ausgabe erfolgreich hinzugefügt!',
    scanAnother: 'Weiteren scannen',
    saveFailed: 'Ausgabe konnte nicht gespeichert werden',
    discount: 'Rabatt',
    deposit: 'Pfand',
    saveImage: 'Kassenbon-Bild speichern',
    choosePdf: 'PDF hochladen',
    analyzingPdf: 'PDF-Beleg wird analysiert...',
    userPromptPlaceholder: 'Zusätzliche Anweisungen für KI (optional)',
    sessionCount_one: '{{count}} Beleg in dieser Sitzung gescannt',
    sessionCount_other: '{{count}} Belege in dieser Sitzung gescannt',
    sessionCapTitle: 'Gutes Tempo!',
    sessionCapBody: 'Sie haben in dieser Sitzung {{count}} Belege gescannt. Machen Sie weiter oder legen Sie eine Pause ein — Ihr Fortschritt ist bereits gespeichert.',
    addItem: '+ Artikel hinzufügen',
    itemQuantity: 'Menge',
    itemUnitPrice: 'Einzelpreis',
    itemTotalPrice: 'Gesamtpreis',
    itemDescriptionPlaceholder: 'Artikelname',
    itemRequiredError: 'Gib einen Namen und einen Preis größer als 0 ein',
    removeItemA11y: 'Artikel entfernen',
    shoppingListChecked_one: '{{count}} Artikel von deiner Einkaufsliste wurde abgehakt',
    shoppingListChecked_other: '{{count}} Artikel von deiner Einkaufsliste wurden abgehakt',
    undoShoppingListCheck: 'Rückgängig',
  },
  incomeVoice: {
    title: 'Spracheingabe Einnahmen',
    listening: 'Hört zu... Tippen zum Stoppen',
    tapToStart: 'Tippen um zu sprechen',
    processing: 'Verarbeitung...',
    example: 'Beispiel: "500 vom Kunden erhalten, Beratungshonorar"',
    analyzing: 'Einnahme wird analysiert...',
    youSaid: 'Sie sagten:',
    confirmTitle: 'Einnahme bestätigen',
    amount: 'Betrag',
    description: 'Beschreibung',
    descriptionPlaceholder: 'Wofür war diese Zahlung?',
    category: 'Kategorie',
    highConfidence: 'Hohe Konfidenz',
    mediumConfidence: 'Mittlere Konfidenz',
    saveIncome: 'Einnahme speichern',
    tryAgain: 'Erneut versuchen',
    success: 'Einnahme erfolgreich hinzugefügt!',
    addAnother: 'Weitere hinzufügen',
    saveFailed: 'Einnahme konnte nicht gespeichert werden',
  },
  incomeReceipt: {
    title: 'Rechnung scannen',
    instructions: 'Foto aufnehmen, aus Galerie wählen oder PDF hochladen',
    hint: 'Richten Sie die Kamera auf eine Rechnung oder ein Zahlungsdokument',
    takePhoto: 'Foto aufnehmen',
    chooseGallery: 'Aus Galerie wählen',
    analyzingPdf: 'PDF wird analysiert...',
    analyzing: 'Dokument wird analysiert...',
    extracting: 'Daten werden mit KI extrahiert...',
    scannedTitle: 'Dokument gescannt',
    totalAmount: 'Gesamtbetrag',
    description: 'Beschreibung',
    category: 'Kategorie',
    date: 'Datum',
    highConfidence: 'Hohe Konfidenz',
    mediumConfidence: 'Mittlere Konfidenz',
    scanAgain: 'Erneut scannen',
    success: 'Einnahme erfolgreich hinzugefügt!',
    scanAnother: 'Weitere scannen',
    saveFailed: 'Einnahme konnte nicht gespeichert werden',
    choosePdf: 'PDF hochladen',
    userPromptPlaceholder: 'Zusätzliche Anweisungen für KI (optional)',
  },
  budgetNew: {
    name: 'Budgetname',
    namePlaceholder: 'z.B. Monatliche Lebensmittel',
    amount: 'Betrag',
    amountPlaceholder: '0,00',
    period: 'Zeitraum',
    categoryOptional: 'Kategorie (Optional)',
    categoryHint: 'Leer lassen für ein Gesamtbudget',
    alertAt: 'Warnung bei',
    noAlert: 'Keine',
    creating: 'Erstellen...',
    createBudget: 'Budget erstellen',
    errorName: 'Bitte gib einen Budgetnamen ein',
    errorAmount: 'Bitte gib einen gültigen Betrag ein',
    errorFailed: 'Budget konnte nicht erstellt werden',
    budgetMode: 'Budget-Modus',
    modeOverall: 'Gesamt',
    modeByCategory: 'Nach Kategorie',
    addCategory: 'Kategorie hinzufügen',
    totalBudget: 'Gesamtbudget',
    errorNoCategories: 'Bitte mindestens eine Kategorie hinzufügen',
    byCategoryHint: 'Ausgabenlimits für jede Kategorie festlegen',
  },
  budgetDetail: {
    notFound: 'Budget nicht gefunden',
    onTrack: 'Im Plan',
    overBudget: 'Budget überschritten',
    period: 'Zeitraum',
    category: 'Kategorie',
    alertThreshold: 'Warnschwelle',
    daysRemaining: 'Verbleibende Tage',
    projectedTotal: 'Voraussichtliche Gesamtsumme',
    status: 'Status',
    active: 'Aktiv',
    inactive: 'Inaktiv',
    deleteTitle: 'Budget löschen',
    deleteConfirm: 'Bist du sicher, dass du dieses Budget löschen möchtest?',
    categoryBreakdown: 'Aufschlüsselung nach Kategorien',
    editBudget: 'Budget bearbeiten',
    save: 'Speichern',
    saving: 'Speichern...',
    history: {
      title: 'Verlauf',
      spent: 'Ausgegeben',
      limit: 'Limit',
      overCount: '{{count}} von {{total}} Zeiträumen überschritten',
      avgOverage: 'Ø Überschreitung: +{{amount}}',
      avgSavings: 'Ø Ersparnis: {{amount}}',
    },
  },
  budgetsDesktop: {
    summaryTotal: 'Budgets gesamt',
    needsAttention: 'Braucht Aufmerksamkeit',
    categoriesOverAllocation: 'Kategorien über Zuteilung',
    needsAttentionHeading: 'Braucht Aufmerksamkeit ({{count}})',
    onTrackHeading: 'Im Plan ({{count}})',
    nearingLimit: 'Nähert sich dem Limit',
    exceedsBy: 'Überschreitet um {{amount}}',
    projectedExceedBy: 'Voraussichtlich um {{amount}} überschritten (~{{date}})',
    emptyHint: 'Budgets warnen dich, bevor du zu viel ausgibst — starte mit einer Kategorie oder einem monatlichen Gesamtlimit',
  },
  dashboardDesktop: {
    focusEmptyTitle: 'Deine Hauptansicht ist ausgeschaltet',
    focusEmptyBody: 'Aktiviere {{safeToSpend}}, {{netProfit}}, {{incomeExpenses}}, {{monthlyBudget}} oder {{wallets}}, um es hier zu sehen.',
    manageWidgets: 'Widgets verwalten',
  },
  validation: {
    fillAllFields: 'Bitte fülle alle Felder aus',
    invalidEmail: 'Bitte gib eine gültige E-Mail-Adresse ein',
    passwordMin8: 'Das Passwort muss mindestens 8 Zeichen lang sein',
    passwordLatin: 'Das Passwort muss mindestens einen lateinischen Großbuchstaben (A-Z), einen lateinischen Kleinbuchstaben (a-z) und eine Zahl (0-9) enthalten',
    passwordsNoMatch: 'Passwörter stimmen nicht überein',
    nameMin2: 'Der Name muss mindestens 2 Zeichen lang sein',
    invalidAmount: 'Bitte gib einen gültigen Betrag ein',
    noDescription: 'Bitte gib eine Beschreibung ein',
  },
  accounts: {
    switchAccount: 'Konto wechseln',
    displayCurrency: 'Anzeigewährung',
    manage: 'Konten verwalten',
    create: 'Konto erstellen',
    personal: 'Persönlich',
    name: 'Kontoname',
    namePlaceholder: 'z.B. Mein Budget',
    nameRequired: 'Bitte geben Sie einen Kontonamen ein',
    type: 'Kontotyp',
    currency: 'Währung',
    details: 'Details',
    notFound: 'Konto nicht gefunden',
    deleteConfirm: 'Konto löschen',
    deleteConfirmMessage: 'Sind Sie sicher, dass Sie "{{name}}" löschen möchten? Alle Daten gehen verloren.',
    deleteAccount: 'Konto löschen',
    leaveConfirm: 'Konto verlassen',
    leaveConfirmMessage: 'Sind Sie sicher, dass Sie "{{name}}" verlassen möchten?',
    leaveAccount: 'Konto verlassen',
    leave: 'Verlassen',
    members: 'Mitglieder',
    pendingInvitations: 'Ausstehende Einladungen',
    noPendingInvitations: 'Keine ausstehenden Einladungen',
    cancelInvitation: 'Einladung stornieren',
    cancelInvitationMessage: 'Sind Sie sicher, dass Sie diese Einladung stornieren möchten?',
    removeMemberConfirm: 'Mitglied entfernen',
    removeMemberMessage: 'Sind Sie sicher, dass Sie {{name}} entfernen möchten?',
    changeRole: 'Rolle ändern',
    selectRole: 'Wählen Sie eine neue Rolle für dieses Mitglied',
    inviteByLink: 'Per Link',
    inviteByEmail: 'Per E-Mail',
    inviteEmail: 'E-Mail-Adresse',
    emailPlaceholder: 'user@example.com',
    emailRequired: 'Bitte geben Sie eine E-Mail-Adresse ein',
    inviteRole: 'Rolle',
    sendInvite: 'Einladung senden',
    inviteSent: 'Einladung gesendet!',
    inviteSentEmail: 'Eine Einladung wurde an {{email}} gesendet.',
    shareCode: 'Teilen Sie diesen Code mit der Person, die Sie einladen möchten.',
    codeCopied: 'Code kopiert',
    inviteShareMessage: 'Tritt meinem Budget bei! Code: {{code}}',
    joinAccount: 'Konto beitreten',
    joinDescription: 'Geben Sie den Einladungscode ein, um einem gemeinsamen Konto beizutreten.',
    enterCode: 'Code eingeben',
    codeRequired: 'Bitte geben Sie einen Einladungscode ein',
    join: 'Beitreten',
    joinedSuccess: 'Erfolgreich beigetreten!',
    inviteBySearch: 'Suche',
    searchLabel: 'Nach Name oder E-Mail suchen',
    searchPlaceholder: 'Name oder E-Mail eingeben…',
    searchNoResults: 'Keine passenden Nutzer gefunden.',
    selectUserRequired: 'Bitte einen Nutzer zum Einladen auswählen',
    inviteSentPush: '{{name}} erhält eine Benachrichtigung zum Annehmen.',
    types: {
      personal: 'Persönlich',
      business: 'Geschäftlich',
      shared: 'Gemeinsam',
      investment: 'Investition',
    },
    roles: {
      owner: 'Eigentümer',
      editor: 'Bearbeiter',
      viewer: 'Betrachter',
    },
    roleDescriptions: {
      editor: 'Kann Ausgaben hinzufügen und bearbeiten',
      viewer: 'Kann nur Daten anzeigen',
    },
    financialMonth: 'Finanzmonat',
    financialMonthStartsOn: 'Monat beginnt an Tag {{day}}',
    financialMonthCalendar: 'Kalendermonat (1.)',
    financialMonthPickerTitle: 'Finanzmonat beginnt am',
    financialMonthHint:
      'Wenn dein Gehalt am 10. eingeht, wähle 10 — Budgets und Berichte laufen dann vom 10. bis zum 9. des Folgemonats. Vergangene Zeiträume werden ebenfalls neu gruppiert; es werden keine Daten verändert.',
    financialMonthClamped: 'In kürzeren Monaten beginnt der Zeitraum am letzten Tag.',
  },
  settings: {
    title: 'Einstellungen',
    profile: 'Profil',
    name: 'Name',
    email: 'E-Mail',
    currency: 'Währung',
    timezone: 'Zeitzone',
    appSettings: 'App-Einstellungen',
    language: 'Sprache',
    appearance: 'Erscheinungsbild',
    theme: 'Design',
    system: 'System',
    light: 'Hell',
    dark: 'Dunkel',
    accentColor: 'Akzentfarbe',
    accentDefault: 'Standard',
    customColor: 'Eigene',
    pickColor: 'Farbe wählen',
    applyColor: 'Übernehmen',
    resetAccent: 'Auf Standard zurücksetzen',
    invalidColor: 'Ungültige Farbe',
    paymentConsequenceHint: 'Revolut und PayPal fügen deinen Split-Links einen echten Zahlen-Button hinzu; BLIK zeigt stattdessen deine Nummer mit Anweisungen (kein bankübergreifender Link); Bargeld und Andere zeigen deine Eingabe als Anweisung anstelle eines Buttons.',
    invalidPaymentHandle: 'Ungültige Zahlungskennung',
    addPaymentMethod: 'Zahlungsmethode hinzufügen',
    about: 'Über die App',
    version: 'Version',
    support: 'Support & Feedback',
    logout: 'Abmelden',
    logoutConfirm: 'Bist du sicher, dass du dich abmelden möchtest?',
    profileUpdated: 'Profil aktualisiert',
    timezoneSearch: 'Zeitzone suchen...',
    timezoneUpdated: 'Zeitzone aktualisiert',
    dataSync: 'Daten & Synchronisierung',
    lastSynced: 'Zuletzt synchronisiert',
    syncNow: 'Jetzt synchronisieren',
    syncing: 'Synchronisierung...',
    syncComplete: 'Daten erfolgreich synchronisiert',
    syncFailed: 'Synchronisierung fehlgeschlagen. Bitte Verbindung prüfen.',
    neverSynced: 'Nie',
    justNow: 'Gerade eben',
    minutesAgo: 'Vor {{count}} Min.',
    hoursAgo: 'Vor {{count}} Std.',
    aiResponseMode: 'KI-Antwortstil',
    aiResponseModeSimple: 'Einfach',
    aiResponseModeBalanced: 'Ausgewogen',
    aiResponseModeExpert: 'Experte',
    aiResponseModeSimpleDesc: 'Einfache Sprache, kein Fachjargon',
    aiResponseModeBalancedDesc: 'Mix aus einfach und fachlich',
    aiResponseModeExpertDesc: 'Finanzterminologie, detaillierte Analyse',
    aiModel: 'KI-Modell',
    aiModelFast: 'Schnell',
    aiModelBalanced: 'Ausgewogen',
    aiModelQuality: 'Qualität',
    aiModelCost: 'Kosten',
    telegram: {
      title: 'Telegram-Bot',
      connect: 'Telegram verbinden',
      disconnect: 'Trennen',
      disconnectConfirm: 'Sind Sie sicher, dass Sie den Telegram-Bot trennen möchten?',
      linked: 'Telegram verbunden',
      description: 'Verbinden Sie einen Telegram-Bot, um Ausgaben, Einnahmen hinzuzufügen, Belege zu scannen und mit dem KI-Assistenten über Telegram zu chatten.',
      tapToCopy: 'Tippen, um den Code zu kopieren',
      codeCopied: 'Code in die Zwischenablage kopiert',
      linkInstructions: 'Öffnen Sie @{{botUsername}} in Telegram und senden Sie:\n/link YOUR_CODE',
    },
    bots: {
      title: 'Chat-Bots',
      subtitle: 'Telegram & WhatsApp verbinden',
    },
    dashboardWidgets: 'Dashboard-Widgets',
    widgetsReorderHint: 'Griff ziehen zum Sortieren',
    widgetsResetOrder: 'Standardreihenfolge wiederherstellen',
    quickActionsTitle: 'Schnellaktionen',
    widgetsTitle: 'Widgets',
    widget: {
      gamification: 'Gamification',
      monthlyBudget: 'Monatsbudget',
      incomeExpenses: 'Einnahmen & Ausgaben',
      debts: 'Schulden & Darlehen',
      netProfit: 'Nettogewinn-Diagramm',
      netCapital: 'Nettokapital',
      fatFinder: 'Fat Finder (KI-Einblicke)',
      calendar: 'Kalender',
      goals: 'Ziele',
      wallets: 'Wallet-Salden',
      financialHealth: 'Finanzielle Gesundheit',
    },
    changeEmail: {
      title: 'E-Mail ändern',
      step1Title: 'Neue E-Mail-Adresse',
      newEmailLabel: 'Neue E-Mail',
      newEmailPlaceholder: 'Neue E-Mail-Adresse eingeben',
      passwordLabel: 'Aktuelles Passwort',
      passwordPlaceholder: 'Aktuelles Passwort eingeben',
      sendCode: 'Bestätigungscode senden',
      step2Title: 'Neue E-Mail bestätigen',
      step2Subtitle: 'Ein 6-stelliger Code wurde an {{email}} gesendet',
      codePlaceholder: '6-stelligen Code eingeben',
      confirm: 'E-Mail-Änderung bestätigen',
      resend: 'Code erneut senden',
      success: 'E-Mail erfolgreich geändert',
      successMessage: 'Ihre E-Mail-Adresse wurde aktualisiert.',
      pendingNotice: 'Ein Bestätigungscode wurde zuvor an {{email}} gesendet.',
    },
  },
  notifications: {
    title: 'Benachrichtigungen',
    pushNotifications: 'Push-Benachrichtigungen',
    pushNotificationsDesc: 'Alle Push-Benachrichtigungen aktivieren oder deaktivieren',
    budgetAlerts: 'Budget-Warnungen',
    budgetAlertsDesc: 'Benachrichtigung bei Erreichen von Budget-Schwellenwerten',
    sharedAccountActivity: 'Gemeinsame Kontoaktivität',
    sharedAccountActivityDesc: 'Benachrichtigung wenn andere Ausgaben zu gemeinsamen Konten hinzufügen',
    debtReminders: 'Schulden-Erinnerungen',
    debtRemindersDesc: 'Erinnerung 3 Tage vor Fälligkeit und bei Überschreitung',
    recurringExpenses: 'Wiederkehrende Ausgaben',
    recurringExpensesDesc: 'Benachrichtigung wenn eine wiederkehrende Ausgabe automatisch gebucht wird',
    subscriptionRenewals: 'Abonnement-Verlängerungen',
    subscriptionRenewalsDesc: '3 Tage vor der Verlängerung eines Abonnements benachrichtigt werden',
    anomalyAlerts: 'Anomalie-Hinweise',
    anomalyAlertsDesc: 'Ungewöhnliche Ausgaben, Preiserhöhungen und doppelte Abbuchungen',
    trackingReminder: 'Tracking-Erinnerung',
    trackingReminderDesc: 'Benachrichtigung, wenn du seit 3+ Tagen keine Ausgaben eingetragen hast',
    shoppingReminder: 'Nachkauf-Erinnerungen',
    shoppingReminderDesc: 'Erhalte eine Erinnerung, wenn ein regelmäßiger Artikel nachgekauft werden sollte',
    shoppingDeals: 'Preissenkungs-Hinweise',
    shoppingDealsDesc: 'Erhalte eine Benachrichtigung, wenn ein regelmäßiger Artikel im Preis fällt',
    inflationShield: 'Bevorratungs-Hinweise',
    inflationShieldDesc: 'Erhalte eine Benachrichtigung, wenn ein regelmäßiger Artikel im Preis steigen wird',
  },
  alerts: {
    title: 'Hinweise',
    empty: 'Noch keine Hinweise. Wir melden uns, wenn uns etwas Ungewöhnliches auffällt.',
    markAllRead: 'Alle als gelesen markieren',
    duplicateTitle: 'Mögliche doppelte Abbuchung',
    duplicateBody: '{{merchant}} hat {{amount}} {{currency}} zweimal in zwei Tagen abgebucht.',
    priceIncreaseTitle: '{{merchant}} ist teurer geworden',
    priceIncreaseBody: 'Vorher {{oldAmount}}, jetzt {{newAmount}} {{currency}} (+{{percent}} %).',
    spikeTitle: 'Ungewöhnliche Ausgaben',
    spikeBody: '{{category}}: {{percent}} % über deinem 3-Monats-Durchschnitt.',
    recurringTitle: '{{merchant}} sieht nach einem Abo aus',
    recurringBody: 'Mindestens 3-mal {{amount}} {{currency}} {{cycle}} abgebucht. Tippe zum Verfolgen.',
    cycleMonthly: 'monatlich',
    cycleWeekly: 'wöchentlich',
    markAsRecurring: 'Als wiederkehrend markieren',
    mergeTitle: 'Gleicher Kauf, zwei Währungen?',
    mergeBody: '{{amountA}} {{currencyA}} und {{amountB}} {{currencyB}} bei {{merchant}} scheinen eine Transaktion zu sein. Zusammenführen?',
    alreadyResolvedTitle: 'Bereits erledigt',
    alreadyResolvedBody: 'Das wurde offenbar schon erledigt – diese Ausgabe ist nicht mehr vorhanden.',
    tabAlerts: 'Hinweise',
    tabInvitations: 'Einladungen',
    invitationTitle: 'Einladung, {{accountName}} beizutreten',
    invitationSubtitle: '{{inviterName}} hat dich als {{role}} eingeladen',
    invitationAccept: 'Annehmen',
    invitationDecline: 'Ablehnen',
    invitationsEmpty: 'Keine offenen Einladungen',
    priceCheckTitle: 'Dieser Kassenbon ist einen Blick wert',
    priceCheckBody: 'Bei {{merchant}}: über dem üblichen Preis — Artikel: {{count}}, Differenz: etwa {{amount}} {{currency}}.',
  },
  whatsappBot: {
    title: 'WhatsApp-Bot',
    subtitle: 'Ausgaben über WhatsApp verfolgen',
    connectButton: 'WhatsApp verbinden',
    disconnectButton: 'Trennen',
    codeInstructions: 'Öffnen Sie WhatsApp und senden Sie diesen Code:',
    openButton: 'WhatsApp öffnen',
    copyCode: 'Code kopieren',
    linkedAs: 'Verknüpft als {{name}}',
    notLinked: 'Nicht verbunden',
    confirmDisconnect: 'WhatsApp trennen?',
    refresh: 'Status aktualisieren',
  },
  slackBot: {
    title: 'Slack-Bot',
    subtitle: 'Ausgaben über Slack verfolgen',
    connectButton: 'Slack verbinden',
    disconnectButton: 'Trennen',
    codeInstructions: 'Öffnen Sie Slack und senden Sie dem Bot eine Direktnachricht: `link {{code}}`',
    openButton: 'Slack öffnen',
    copyCode: 'Code kopieren',
    linkedAs: 'Verknüpft als {{name}}',
    linked: 'Verbunden',
    notLinked: 'Nicht verbunden',
    confirmDisconnect: 'Slack trennen?',
    refresh: 'Status aktualisieren',
    addToSlack: 'Zu Slack hinzufügen',
    addToSlackHint: 'Workspace-Admins: Fügen Sie den Bot einmalig zu Ihrem Slack-Workspace hinzu. Danach kann jedes Mitglied sein Konto mit einem Code verknüpfen.',
  },
  wiseImport: {
    title: 'Wise-Import',
    subtitle: 'Ausgaben und Einnahmen aus einem Wise-Kontoauszug (CSV) importieren',
    pickFile: 'CSV-Datei auswählen',
    parsing: 'CSV wird verarbeitet…',
    importButton: '{{count}} Zeilen importieren',
    importAnother: 'Weitere importieren',
    success: 'Import abgeschlossen',
    summary: '{{expenses}} Ausgaben · {{incomes}} Einnahmen · {{exchanges}} Umrechnungen',
    help: {
      where: 'In Wise: Transaktionen → Kontoauszüge und Berichte → CSV. Wählen Sie Zeitraum und Währung, laden Sie die Datei herunter und wählen Sie sie hier aus.',
    },
    preview: {
      selected: '{{count}} ausgewählt',
      skipped: '{{count}} bereits importiert',
      alreadyImported: 'Bereits importiert',
      fxPair: 'Währungsumrechnung',
    },
    error: {
      parseFailed: 'CSV konnte nicht verarbeitet werden. Stellen Sie sicher, dass Sie einen Wise-Kontoauszug im CSV-Format heruntergeladen haben.',
    },
  },
  bankImport: {
    title: 'Transaktionen importieren',
    subtitle: 'CSV-Datei Ihrer Bank importieren',
    quickImportHeader: 'Schnellimport',
    autoDetect: 'Automatisch erkennen (jede Bank)',
    analysing: 'Kontoauszug wird gelesen…',
    migrationHeader: 'Wechseln Sie von einer anderen App?',
    migrationHint: 'Importieren Sie Ihren Verlauf aus einer anderen Budget-App — Ihre Kategorien kommen mit.',
    savedMappingsHeader: 'Gespeicherte Zuordnungen',
    noSavedMappings: 'Noch keine gespeicherten Zuordnungen',
    detectedBank: 'Erkannt: {{name}}',
    pickBankTitle: 'Von welcher Bank stammt diese Datei?',
    pickBankSubtitle: 'Ihre Bank konnte nicht automatisch erkannt werden. Wählen Sie eine Bank unten aus oder verwenden Sie „Sonstige", um Spalten manuell zuzuordnen.',
    other: 'Sonstige (benutzerdefiniertes CSV)',
    mapperTitle: 'Spalten zuordnen',
    mapperSubtitle: 'Geben Sie an, welche Spalte welche Information enthält',
    mapperDate: 'Datumsspalte',
    mapperAmount: 'Betragsspalte',
    mapperDescription: 'Beschreibungsspalte',
    mapperCurrency: 'Währungsspalte (optional)',
    mapperCounterparty: 'Gegenseite-Spalte (optional)',
    mapperSplitDebitCredit: 'Soll/Haben-Spalten trennen',
    mapperDebit: 'Soll-Spalte (Ausgaben)',
    mapperCredit: 'Haben-Spalte (Einnahmen)',
    mapperDelimiter: 'Trennzeichen',
    mapperEncoding: 'Zeichenkodierung',
    mapperAmountFormat: 'Betragsformat',
    mapperDateFormat: 'Datumsformat',
    mapperContinue: 'Weiter',
    mapperSaveMapping: 'Zuordnung für das nächste Mal speichern',
    mapperSaveName: 'Name der Zuordnung',
    parseErrorsBanner: '{{count}} Zeilen aufgrund von Fehlern übersprungen',
    deleteMapping: 'Zuordnung löschen',
    deleteMappingConfirm: 'Diese gespeicherte Zuordnung löschen?',
    summary: '{{expenses}} Ausgaben, {{incomes}} Einnahmen, {{exchanges}} Umrechnungen importiert',
    requestCardTitle: 'Ihre Bank nicht dabei?',
    requestCardSubtitle: 'Schicken Sie uns den Banknamen und einen Beispielauszug — wir fügen die Unterstützung hinzu.',
    requestTitle: 'Bank anfragen',
    requestIntro: 'Teilen Sie uns mit, welche Bank Sie nutzen, und hängen Sie ein Beispiel des Kontoauszugs an (CSV oder PDF). Wir verwenden es, um die Unterstützung hinzuzufügen.',
    requestBankName: 'Bankname',
    requestBankNamePlaceholder: 'z. B. Santander, Revolut',
    requestNotes: 'Hinweise (optional)',
    requestNotesPlaceholder: 'Alles Nützliche — Format, wo Sie es exportiert haben usw.',
    requestAttach: 'Beispielauszug anhängen',
    requestAttachHint: 'CSV oder PDF, bis zu 5 MB',
    requestAttached: 'Angehängt: {{name}}',
    requestSend: 'Anfrage senden',
    requestSent: 'Danke! Ihre Anfrage wurde gesendet — wir schauen uns das an.',
    requestErrorNoName: 'Bitte geben Sie den Banknamen ein.',
    requestError: 'Die Anfrage konnte nicht gesendet werden. Bitte versuchen Sie es später erneut.',
    pastImportsHeader: 'Vergangene Importe',
    noPastImports: 'Noch keine Importe',
    transactionCount_one: '{{count}} Transaktion',
    transactionCount_other: '{{count}} Transaktionen',
    undoImportTitle: 'Import rückgängig machen',
    undoImportConfirm: 'Dadurch werden {{count}} Transaktionen aus diesem Import entfernt. Anschließend kann die gleiche Datei erneut importiert werden.',
    undoImportAction: 'Import rückgängig',
    possibleMerge: 'Existiert möglicherweise in einer anderen Währung',
    mergeWithExisting: 'Mit vorhandenem Eintrag zusammenführen',
    importAsNew: 'Als neu importieren',
    aiConsentTitle: 'KI diesen Kontoauszug lesen lassen?',
    aiConsentBody:
      'Wir erkennen das Format dieser Bank nicht. Die KI kann herausfinden, welche Spalte Datum, Betrag und Beschreibung enthält – den Rest erledigt Ihr Gerät.',
    aiConsentWhatLeaves: 'Was gesendet wird',
    aiConsentWhatLeavesCsv: 'Die Kopfzeile und bis zu 10 Beispielzeilen aus Ihrer Datei.',
    aiConsentWhatLeavesPdf: 'Die ersten 20 Textzeilen Ihres Kontoauszugs.',
    aiConsentThirdParty: 'Diese Daten werden zur Analyse an einen externen KI-Anbieter gesendet und dort nicht zu Trainingszwecken gespeichert.',
    aiConsentOnce: 'Wird nur einmal pro Konto gefragt. Sie können die Spalten weiterhin selbst zuordnen.',
    aiConsentAccept: 'KI verwenden',
    aiConsentDecline: 'Ich ordne es selbst zu',
    aiConsentFailed: 'Ihre Auswahl konnte nicht gespeichert werden. Bitte versuchen Sie es erneut.',
    aiInferredBy: 'Spalten von KI zugeordnet',
    aiInferredEdit: 'Falsch? Zum Korrigieren tippen',
    aiCurrencyAssumed: 'Diese Datei enthält keine Währungsspalte, daher wurde jede Zeile als {{currency}} gelesen. Bitte vor dem Import prüfen.',
    aiWarningNoBalance: 'Wir konnten nicht bestätigen, dass alle Transaktionen gefunden wurden – dieser Kontoauszug hat keinen Schlusssaldo zum Abgleich. Bitte überprüfen Sie die Liste.',
    aiWarningMismatch: 'Die gefundenen Transaktionen ergeben nicht den Schlusssaldo dieses Kontoauszugs. Möglicherweise fehlen einige Zeilen.',
    aiWarningTruncated: 'Dieser Kontoauszug war zu lang, um vollständig gelesen zu werden – {{count}} Seite(n) wurden nicht verarbeitet.',
    aiPdfPaywall: 'PDF-Kontoauszüge mit KI lesen',
    aiBankGuess: 'Sieht aus wie: {{bank}}',
    aiCurrencyFix: 'Währung ändern',
    aiRoleDate: 'Datum',
    aiRoleAmount: 'Betrag',
    aiRoleDebit: 'Soll',
    aiRoleCredit: 'Haben',
    aiRoleDescription: 'Beschreibung',
    aiRoleCurrency: 'Währung',
    aiRoleCounterparty: 'Empfänger',
    error: {
      encodingUnknown: 'Dateicodierung konnte nicht erkannt werden. Versuchen Sie, die CSV-Datei als UTF-8 zu speichern.',
      parseFailed: 'Die Datei konnte nicht verarbeitet werden. Überprüfen Sie das Format und versuchen Sie es erneut.',
      tooLarge: 'Datei zu groß (max. 5 MB).',
    },
  },
  wallet: {
    title: 'Geldbörse',
    balances: 'Salden',
    setInitialBalance: 'Saldo festlegen',
    currentBalance: 'Aktueller Saldo',
    initialBalance: 'Anfangssaldo',
    totalBalance: 'Gesamtsaldo ({{currency}})',
    totalIncome: 'Gesamteinkommen',
    totalSpent: 'Gesamtausgaben',
    exchangedIn: 'Eingetauscht',
    exchangedOut: 'Umgetauscht',
    noBalances: 'Kein Geldbörsen-Saldo',
    noBalancesHint: 'Lege Anfangssalden fest, um deine Geldbörse zu verfolgen',
    addBalance: 'Saldo hinzufügen',
    currency: 'Währung',
    amount: 'Betrag',
    deleteBalance: 'Saldo löschen',
    deleteConfirm: 'Bist du sicher, dass du diesen Saldo löschen möchtest?',
    insufficientBalance: 'Unzureichender Saldo',
    existingBalances: 'Vorhandene Salden',
    balanceSaved: 'Saldo erfolgreich festgelegt',
    editInitialBalance: 'Anfangsguthaben bearbeiten',
    balanceUpdated: 'Guthaben erfolgreich aktualisiert',
    transferredIn: 'Eingehende Überweisung',
    transferredOut: 'Ausgehende Überweisung',
    balanceHistory: 'Guthaben-Verlauf',
    monthsWindow: '{{count}}M',
    monthlyChange: 'im {{month}}',
    displayCurrency: 'Anzeigewährung',
    noHistoryYet: 'Füge Transaktionen hinzu, um den Guthabentrend zu sehen',
  },
  exchange: {
    title: 'Umtausch',
    from: 'Von',
    to: 'Nach',
    rate: 'Wechselkurs',
    date: 'Datum',
    notes: 'Notizen',
    notesPlaceholder: 'Optionale Notizen...',
    recentExchanges: 'Letzte Umtausche',
    sameCurrencyError: 'Gleiche Währung kann nicht umgetauscht werden',
    exchange: 'Umtauschen',
    allExchanges: 'Alle Umtausche',
    rateAlerts: 'Kursalarme',
    notifyMe: 'Benachrichtige mich, wenn der Kurs mein Ziel erreicht',
    targetRate: 'Zielkurs',
    above: 'über',
    below: 'unter',
    addAlert: 'Alarm hinzufügen',
    alertCreated: 'Alarm erstellt — wir benachrichtigen dich, wenn der Kurs erreicht ist.',
    deleteAlertConfirm: 'Diesen Kursalarm löschen?',
    noAlerts: 'Noch keine Alarme für dieses Paar.',
    alertWhen: 'Benachrichtigen wenn {{direction}} {{rate}} {{toCurrency}}',
    newAlert: 'Neuer Alarm',
    alertsWaiting: 'Wartend',
    alertsFired: 'Schon ausgelöst',
    noAlertsYet: 'Noch keine Kursalarme. Setze eine Zielmarke — wir melden uns, sobald der Kurs sie erreicht.',
    alertFiredAt: 'Ausgelöst bei {{rate}} · {{date}}',
    allCurrencies: 'Alle Währungen',
    allTime: 'Gesamte Zeit',
    thisMonth: 'Dieser Monat',
    last3Months: 'Letzte 3 Monate',
    thisYear: 'Dieses Jahr',
    noExchanges: 'Keine Umtausche',
    noExchangesHint: 'Währungsumtausche, die den ausgewählten Filtern entsprechen, werden hier angezeigt.',
    notFound: 'Umtausch nicht gefunden',
    deleteTitle: 'Umtausch löschen',
    deleteConfirm: 'Möchten Sie diesen Umtausch wirklich löschen?',
    editTitle: 'Umtausch-Details',
  },
  converter: {
    title: 'Währungsrechner',
    result: 'Ergebnis',
    enterAmount: 'Betrag eingeben',
    rateInfo: '1 {{from}} = {{rate}} {{to}}',
  },
  transfer: {
    title: 'Überweisung',
    fromAccount: 'Von Konto',
    toAccount: 'An Konto',
    amount: 'Betrag',
    rate: 'Wechselkurs',
    notes: 'Notizen',
    notesPlaceholder: 'Optionale Notizen...',
    sameAccountError: 'Quell- und Zielkonto müssen unterschiedlich sein',
    submit: 'Überweisen',
    recentTransfers: 'Letzte Überweisungen',
    success: 'Überweisung abgeschlossen',
    countAsIncome: 'Als Einkommen zählen',
    countAsIncomeHint: 'Erstellt einen Einkommensdatensatz auf dem Empfängerkonto',
    date: 'Datum',
    notFound: 'Überweisung nicht gefunden',
    deleteTitle: 'Überweisung löschen',
    deleteConfirm: 'Möchten Sie diese Überweisung wirklich löschen?',
    allTransfers: 'Alle Überweisungen',
    allAccounts: 'Alle Konten',
    periodAll: 'Gesamte Zeit',
    periodThisMonth: 'Dieser Monat',
    periodLast3Months: 'Letzte 3 Monate',
    periodThisYear: 'Dieses Jahr',
    noTransfers: 'Keine Überweisungen',
    noTransfersHint: 'Überweisungen zwischen Ihren Konten werden hier angezeigt',
    frequent: 'Häufig',
    available: 'Verfügbar:',
    max: 'Max',
    insufficientHint: 'Betrag übersteigt den bekannten Kontostand',
    saveFailed: 'Änderung nicht übernommen',
    saveFailedHint: 'Die Überweisung bleibt unverändert. Möglicherweise hast du keinen Zugriff auf eines der gewählten Konten.',
  },
  errors: {
    error: 'Fehler',
    loginFailed: 'Anmeldung fehlgeschlagen',
    invalidCredentials: 'Ungültige E-Mail oder Passwort',
    accountDeactivated: 'Konto ist deaktiviert',
    emailAlreadyExists: 'Ein Benutzer mit dieser E-Mail existiert bereits',
    registrationFailed: 'Registrierung fehlgeschlagen',
    biometricFailed: 'Biometrische Authentifizierung fehlgeschlagen',
    biometricLoginFailed: 'Biometrische Anmeldung fehlgeschlagen',
    quickLoginFailed: 'Schnellanmeldung fehlgeschlagen',
    noSavedSession: 'Keine gespeicherte Sitzung gefunden',
    sessionExpired: 'Sitzung abgelaufen',
    invalidResetCode: 'Ungültiger oder abgelaufener Code',
    tooManyResetAttempts: 'Zu viele Versuche. Bitte versuchen Sie es später erneut.',
    resetFailed: 'Passwort konnte nicht zurückgesetzt werden. Bitte versuchen Sie es erneut.',
    micPermissionDenied: 'Mikrofonberechtigung verweigert',
    startRecordingFailed: 'Aufnahme konnte nicht gestartet werden',
    processRecordingFailed: 'Aufnahme konnte nicht verarbeitet werden',
    cameraPermissionDenied: 'Kameraberechtigung verweigert',
    galleryPermissionDenied: 'Galerieberechtigung verweigert',
    captureImageFailed: 'Bild konnte nicht aufgenommen werden',
    pickImageFailed: 'Bild konnte nicht ausgewählt werden',
    processReceiptFailed: 'Beleg konnte nicht verarbeitet werden',
    pdfTooLarge: 'PDF-Datei ist zu groß (max. 10 MB)',
    saveFailed: 'Ausgabe konnte nicht gespeichert werden',
    createBudgetFailed: 'Budget konnte nicht erstellt werden',
    sendMessageFailed: 'Nachricht konnte nicht gesendet werden',
    loadConversationFailed: 'Konversation konnte nicht geladen werden',
    chatError: 'Entschuldigung, es ist ein Fehler aufgetreten. Bitte überprüfe deine Verbindung und versuche es erneut.',
    invalidVerificationCode: 'Ungültiger Verifizierungscode',
    verificationCodeExpired: 'Verifizierungscode abgelaufen',
    verificationFailed: 'Verifizierung fehlgeschlagen',
    unknown: 'Ein unerwarteter Fehler ist aufgetreten',
    googleSignInFailed: 'Google-Anmeldung fehlgeschlagen. Bitte versuchen Sie es erneut.',
    googleEmailNotVerified: 'Die E-Mail-Adresse Ihres Google-Kontos ist nicht verifiziert.',
    usePasswordLogin: 'Dieses Konto verwendet die Google-Anmeldung. Tippen Sie auf „Mit Google fortfahren".',
  },
  insights: {
    title: 'Vorausschauende Einblicke',
    anomalies: 'Ausgabenauffälligkeiten',
    anomalyText: '{{percent}}% mehr als üblich für {{category}}',
    avgLabel: 'Durchschn.',
    predictions: 'Budgetprognosen',
    exhaustionText: 'Aufgebraucht ca. {{date}}',
    projectedTotal: 'Prognose: {{amount}}',
    noAnomalies: 'Ihre Ausgaben sehen normal aus',
    noPredictions: 'Keine aktiven Budgets zur Prognose',
    categoryLearned: 'Basierend auf Ihrem Verlauf',
    aiSuggested: 'KI-Einblicke',
    generatedAt: 'Aktualisiert {{time}}',
    dismiss: 'Verwerfen',
    seeAll: 'Alle Einblicke',
    anomalySpike: 'Ausgabenspitze',
    categoryComparison: 'Kategorievergleich',
    trendChange: 'Trendwechsel',
    budgetBurndown: 'Budget-Burndown',
    savingsOpportunity: 'Sparmöglichkeit',
    loadingInsights: 'Analysiere deine Ausgaben...',
    noInsights: 'Derzeit keine neuen Einblicke',
    proRequired: 'Upgrade auf Pro für KI-Einblicke',
  },
  subscription: {
    title: 'Abonnement',
    managePlan: 'Plan verwalten',
    month: 'Monat',
    year: 'Jahr',
    upgrade: 'Upgraden',
    viewDetails: 'Details anzeigen',
    maybeLater: 'Vielleicht später',
    trial: '7-tägige kostenlose Testversion',
    trialActive: 'Testversion aktiv',
    trialEndsAt: 'Testversion endet am {{date}}',
    limitReached: 'KI-Limit erreicht',
    limitReachedBody: 'Du hast dein kostenloses KI-Limit für diesen Monat erreicht. Upgrade auf Pro für mehr KI-Anfragen und Premium-Funktionen.',
    startTrial: 'Starte deine 7-tägige kostenlose Testphase',
    aiRequests: 'KI-Anfragen',
    usageThisMonth: 'Nutzung diesen Monat',
    availablePlans: 'Verfügbare Pläne',
    manage: 'Abonnement verwalten',
    upgraded: 'Plan aktualisiert',
    upgradeSuccess: 'Ihr Abonnement wurde erfolgreich aktualisiert.',
    includesProFeatures: 'Enthält alle Pro-Funktionen, plus:',
    cancelsAt: 'Wird gekündigt am {{date}}',
    status: {
      active: 'Aktiv',
      canceled: 'Gekündigt',
      past_due: 'Überfällig',
      incomplete: 'Unvollständig',
      trialing: 'Testversion',
      paused: 'Pausiert',
    },
    plans: {
      pro: { name: 'Pro' },
      business: { name: 'Business' },
    },
    features: {
      aiRequests300: '300 KI-Anfragen pro Monat',
      accounts5: 'Bis zu 5 Konten',
      members5: 'Bis zu 5 Mitglieder pro Konto',
      predictiveAnalytics: 'Prädiktive Analysen',
      anomalyDetection: 'Erkennung von Ausgabenauffälligkeiten',
      unlimitedCurrencies: 'Unbegrenzte Währungen',
      unlimitedAi: 'Unbegrenzte KI-Anfragen',
      unlimitedAccounts: 'Unbegrenzte Konten',
      unlimitedMembers: 'Unbegrenzte Mitglieder',
      advancedReporting: 'Erweiterte Berichte',
    },
    trialLimitNote: 'Testversion — begrenzte KI-Anfragen. Upgraden für vollen Zugriff.',
  },
  welcome: {
    title: 'Willkommen',
    heading: 'Willkommen bei AI Budget!',
    subheading: 'Ihr intelligenter Finanzbegleiter',
    freeIncluded: 'Ihr kostenloser Plan beinhaltet:',
    freeAiRequests: '50 KI-Anfragen pro Monat',
    freeAccount: '3 persönliche Konten',
    freeMember: '1 Mitglied',
    freeTracking: 'Vollständige Ausgabenverfolgung',
    wantMore: 'Mehr gewünscht? Jederzeit upgraden',
    continueWithFree: 'Kostenlos fortfahren',
  },
  onboarding: {
    heading: 'Wo möchten Sie beginnen?',
    subheading: 'Fügen Sie eine Sache hinzu, und die App beginnt, für Sie zu arbeiten.',
    scanReceipt: 'Beleg scannen',
    scanReceiptHint: 'Positionen und Kategorien werden für Sie erkannt',
    useVoice: 'Stimme verwenden',
    typeManually: 'Manuell eingeben',
    bringHistory: 'Verlauf importieren',
    bringHistoryHint: 'Von Ihrer Bank oder einer anderen Budget-App',
    later: 'Das mache ich später',
  },
  admin: {
    title: 'Admin-Panel',
    openPanel: 'Admin-Dashboard',
    systemStats: 'Systemstatistiken',
    totalUsers: 'Benutzer gesamt',
    totalAccounts: 'Konten gesamt',
    totalExpenses: 'Ausgaben gesamt',
    subscriptionBreakdown: 'Abonnements',
    trialing: 'Testphase',
    aiUsageTitle: 'KI-Nutzung diesen Monat',
    estimatedCost: 'Geschätzte Kosten',
    totalCostUnits: 'Kosteneinheiten',
    totalRequests: '{{count}} Anfragen',
    noUsage: 'Keine KI-Nutzung in diesem Zeitraum',
    features: {
      chat: 'Chat',
      voice: 'Sprache',
      parse: 'Analyse',
      categorization: 'Kategorisierung',
      ocr: 'OCR / Belegscan',
      story: 'Geschichte',
      insights: 'KI-Einblicke',
      tag_suggestion: 'Tag-Vorschlag',
      project_suggestion: 'Projektvorschlag',
      split_suggestion: 'Aufteilungsvorschlag',
      goal_plan: 'Zielplan',
      investment_insights: 'Investitionsanalyse',
      fat_finder: 'Sparfinder',
    },
  },
  drillDown: {
    title: 'Daten erkunden',
    year: 'Jahr',
    month: 'Monat',
    week: 'Woche',
    day: 'Tag',
    transactions: 'Transaktionen',
    tapToExplore: 'Tippe auf einen Balken, um tiefer zu gehen',
    noTransactions: 'Keine Transaktionen für diesen Zeitraum',
    back: 'Zurück',
    monthlySpending: 'Monatliche Ausgaben',
    weeklySpending: 'Ausgaben: {{month}}',
    dailySpending: 'Tägliche Ausgaben',
    weekLabel: 'Woche {{n}}',
    noData: 'Keine Daten',
    noDataAvailable: 'Keine Daten verfügbar',
    total: 'Gesamt',
  },
  story: {
    title: 'Deine Ausgabengeschichte',
    viewStory: 'Deine Ausgabengeschichte ansehen',
    generating: 'Schreibe deine Geschichte...',
    regenerate: 'Neu generieren',
    lastUpdated: 'Aktualisiert {{time}}',
    share: 'Geschichte teilen',
    week: 'Woche',
    month: 'Monat',
    proRequired: 'Upgrade auf Pro für Ausgabengeschichten',
    noData: 'Noch nicht genug Daten für eine Geschichte',
    achievementTitle: 'Errungenschaft!',
  },
  goals: {
    title: 'Sparziele',
    subtitle: 'Setze Ziele und verfolge deinen Fortschritt',
    create: 'Ziel erstellen',
    name: 'Zielname',
    targetAmount: 'Zielbetrag',
    deadline: 'Frist',
    currentAmount: 'Aktueller Betrag',
    status: {
      active: 'Aktiv',
      paused: 'Pausiert',
      completed: 'Abgeschlossen',
      failed: 'Fehlgeschlagen',
    },
    planTitle: 'KI-Plan',
    planMonthly: 'Monatlicher Beitrag',
    planWeekly: 'Wöchentlicher Beitrag',
    feasibility: {
      easy: 'Einfach',
      moderate: 'Moderat',
      challenging: 'Herausfordernd',
      unrealistic: 'Unrealistisch',
    },
    checkpoints: 'Meilensteine',
    categoryLimits: 'Kategorielimits',
    current: 'Aktuell',
    suggested: 'Vorgeschlagen',
    savings: 'Ersparnisse',
    progressOnTrack: 'Im Plan',
    progressBehind: 'Im Rückstand',
    percentComplete: '{{percent}}% abgeschlossen',
    projectedDate: 'Voraussichtlich: {{date}}',
    monthlyNeeded: 'Monatlich benötigt',
    empty: 'Noch keine Sparziele. Erstelle eines, um loszulegen!',
    regenerate: 'Plan neu generieren',
    deleteConfirm: 'Bist du sicher, dass du dieses Ziel löschen möchtest?',
    edit: 'Ziel bearbeiten',
    generating: 'Dein Plan wird erstellt...',
    behindBy: 'Im Rückstand',
    savingsGoals: 'Sparziele',
    noActiveGoals: 'Keine aktiven Ziele',
    createYourFirstGoal: 'Setzen Sie ein Sparziel, um zu beginnen',
    daysRemaining_one: '{{count}} Tag übrig',
    daysRemaining_other: '{{count}} Tage übrig',
    overdue: 'Überfällig',
    andMore_one: '+{{count}} mehr',
    andMore_other: '+{{count}} mehr',
    threeMonths: '3 Monate',
    sixMonths: '6 Monate',
    oneYear: '1 Jahr',
    twoYears: '2 Jahre',
    goalName: 'Zielname',
    goalNamePlaceholder: 'z.B. Auto, Urlaub...',
    currency: 'Währung',
    quickSelect: 'Schnellauswahl:',
    createGoal: 'Ziel erstellen',
    errorName: 'Bitte geben Sie einen Zielnamen ein',
    errorAmount: 'Bitte geben Sie einen gültigen Zielbetrag ein',
    selectDate: 'Datum auswählen',
    errorDeadline: 'Bitte legen Sie eine Frist fest',
    errorInvalidDate: 'Bitte geben Sie ein gültiges Datum ein (JJJJ-MM-TT)',
    errorPastDate: 'Die Frist muss in der Zukunft liegen',
    errorCreate: 'Ziel konnte nicht erstellt werden. Bitte versuchen Sie es erneut.',
    noGoals: 'Noch keine Sparziele',
    noGoalsHint: 'Erstellen Sie Ihr erstes Ziel, um mit dem Sparen zu beginnen!',
    deleteTitle: 'Ziel löschen',
    errorDelete: 'Ziel konnte nicht gelöscht werden',
    errorRegenerate: 'Plan konnte nicht neu generiert werden',
    goalDetail: 'Ziel',
    notFound: 'Ziel nicht gefunden',
    progress: 'Fortschritt',
    remaining: 'verbleibend',
    onTrack: 'Im Plan',
    behindSchedule: 'Im Rückstand',
    projectedCompletion: 'Voraussichtlicher Abschluss',
    aiPlan: 'KI-Plan',
    monthly: 'Monatlich',
    weekly: 'Wöchentlich',
    category: 'Kategorie',
    regenerating: 'Neugenerierung...',
    regeneratePlan: 'KI-Plan neu generieren',
    deleteGoal: 'Ziel löschen',
    addFunds: 'Betrag hinzufügen',
    addFundsDescription: 'Wie viel haben Sie gespart?',
    addFundsConfirm: 'Hinzufügen',
    currentSaved: 'Bereits gespart',
    errorAddFunds: 'Betrag konnte nicht hinzugefügt werden',
    contributionsTitle: 'Einzahlungsverlauf',
    noContributions: 'Noch keine Einzahlungen erfasst.',
    contributions: 'Einzahlungen',
    contributionNote: '{{note}}',
  },
  fatFinder: {
    title: 'Ausgabenprüfung',
    tapToAnalyze: 'Tippen Sie, um Ihre Ausgaben zu analysieren',
    subtitle: 'Finde Sparmöglichkeiten',
    totalSavings: 'Potenzielle monatliche Ersparnisse',
    finding: 'Ergebnis',
    findings: 'Ergebnisse',
    typeSubscription: 'Abonnement',
    typeRecurringSplurge: 'Wiederkehrende Ausgabe',
    typeLargeOneOff: 'Große Einzelausgabe',
    typeCategoryExcess: 'Kategorieüberschuss',
    typeServiceOveruse: 'Übermäßige Nutzung',
    severityLow: 'Niedrig',
    severityMedium: 'Mittel',
    severityHigh: 'Hoch',
    currentMonthly: 'Aktuell',
    suggestedMonthly: 'Vorgeschlagen',
    potentialSavings: 'Du könntest sparen',
    regenerate: 'Bericht neu generieren',
    proRequired: 'Upgrade auf Pro für Ausgabenprüfungen',
    noFindings: 'Gut gemacht! Keine nennenswerten Einsparungen gefunden.',
    generating: 'Deine Ausgaben werden analysiert...',
    viewFullReport: 'Vollständigen Bericht anzeigen',
    relatedExpenses: 'Zugehörige Ausgaben',
    upgradeMessage: 'Upgrade auf Pro, um monatliche Ausgabenprüfungen freizuschalten',
    loading: 'Deine Ausgaben werden analysiert...',
    perMonth: 'pro Monat',
    emptyTitle: 'Sieht gut aus!',
    emptyDescription: 'Diesen Monat keine nennenswerten Sparmöglichkeiten gefunden.',
    findingsCount: '{{count}} Ergebnisse',
    generatedAt: 'Erstellt um {{time}}',
    trackSubscription: 'Abonnement verfolgen',
    severity: {
      low: 'Niedrig',
      medium: 'Mittel',
      high: 'Hoch',
    },
  },
  subscriptionManager: {
    title: 'Abonnements',
    settingsSubtitle: 'Wiederkehrende Zahlungen und Verlängerungen verfolgen',
    addTitle: 'Abonnement hinzufügen',
    editTitle: 'Abonnement bearbeiten',
    monthlyTotal: 'Monatliche Kosten',
    activeCount: '{{count}} aktiv',
    activeTitle: 'Aktive Abonnements',
    emptyTitle: 'Keine Abonnements verfolgt',
    emptyDescription: 'Verfolge deine wiederkehrenden Kosten und werde nie von einer Verlängerung überrascht.',
    addFirst: 'Erstes Abonnement hinzufügen',
    deleteTitle: 'Abonnement löschen',
    deleteMessage: '"{{name}}" aus dem Tracker entfernen?',
    inactive: 'Inaktiv',
    renewalToday: 'Verlängerung heute',
    renewalTomorrow: 'Verlängerung morgen',
    renewalInDays: 'Verlängerung in {{count}} Tagen',
    renewalOverdue: 'Verlängerung überfällig',
    notFound: 'Abonnement nicht gefunden',
    fieldName: 'Name',
    fieldNamePlaceholder: 'z.B. Netflix',
    fieldAmount: 'Betrag',
    fieldCycle: 'Abrechnungszeitraum',
    fieldNextRenewal: 'Nächstes Verlängerungsdatum (JJJJ-MM-TT)',
    fieldNotes: 'Notizen (optional)',
    fieldNotesPlaceholder: 'z.B. Familienplan',
    fieldActive: 'Aktiv',
    detectedFrom: 'Erkannt aus: {{source}}',
    errorName: 'Bitte einen Namen eingeben.',
    errorAmount: 'Bitte einen gültigen Betrag größer als 0 eingeben.',
    cycle: {
      monthly: 'Monatlich',
      yearly: 'Jährlich',
      quarterly: 'Vierteljährlich',
      weekly: 'Wöchentlich',
    },
    calendarView: 'Kalender',
    listView: 'Liste',
    noRenewalsThisMonth: 'Keine Verlängerungen diesen Monat',
    renewalsOnDay: 'Verlängerungen am {{day}}.',
  },
  tags: {
    title: 'Tags',
    addTag: 'Tag hinzufügen',
    createTag: 'Tag erstellen',
    editTag: 'Tag bearbeiten',
    deleteTag: 'Tag löschen',
    tagName: 'Tag-Name',
    tagColor: 'Tag-Farbe',
    noTags: 'Noch keine Tags',
    suggestedTags: 'Vorgeschlagene Tags',
    aiSuggestions: 'KI-Vorschläge',
    suggestTags: 'Tags vorschlagen',
    descriptionRequired: 'Bitte zuerst eine Beschreibung eingeben (mindestens 3 Zeichen)',
    recentTags: 'Letzte Tags',
    popularTags: 'Beliebte Tags',
    manageTags: 'Tags verwalten',
    confirmDelete: 'Möchten Sie diesen Tag wirklich löschen?',
    usedInExpenses: 'Verwendet in {{count}} Ausgaben',
  },
  projects: {
    title: 'Projekte',
    createProject: 'Projekt erstellen',
    editProject: 'Projekt bearbeiten',
    projectName: 'Projektname',
    description: 'Beschreibung',
    startDate: 'Startdatum',
    endDate: 'Enddatum',
    budget: 'Budget',
    activeProjects: 'Aktive Projekte',
    archivedProjects: 'Archivierte Projekte',
    noProjects: 'Noch keine Projekte',
    addToProject: 'Zum Projekt hinzufügen',
    suggestedProject: 'Vorgeschlagenes Projekt',
    totalSpent: 'Gesamtausgaben',
    budgetRemaining: 'Verbleibendes Budget',
    archiveProject: 'Projekt archivieren',
    deleteProject: 'Projekt löschen',
    confirmDelete: 'Möchten Sie dieses Projekt wirklich löschen?',
    confirmArchive: 'Möchten Sie dieses Projekt wirklich archivieren?',
    color: 'Farbe',
    noExpenses: 'Keine Ausgaben in diesem Projekt',
    expensesByCategory: 'Ausgaben nach Kategorie',
  },
  purchaseRequests: {
    title: 'Kaufanfragen',
    new: 'Neue Anfrage',
    approve: 'Genehmigen',
    reject: 'Ablehnen',
    abstain: 'Enthalten',
    approved: 'Genehmigt',
    rejected: 'Abgelehnt',
    pending: 'Ausstehend',
    purchased: 'Gekauft',
    expired: 'Abgelaufen',
    active: 'Aktiv',
    history: 'Verlauf',
    votes: 'Stimmen',
    addToPlan: 'Zum Kaufplan hinzufügen',
    convertSuccess: 'Als geplante Ausgabe hinzugefügt',
    noRequests: 'Noch keine Kaufanfragen',
    createFirst: 'Schlage einen Kauf für die Familie vor',
    voteComment: 'Kommentar hinzufügen (optional)',
    deadline: 'Abstimmungsfrist (optional)',
    approvalRule: 'Genehmigungsregel',
    majority: 'Mehrheitsbeschluss',
    unanimous: 'Alle müssen zustimmen',
    ownerOnly: 'Inhaber entscheidet',
    settingsTitle: 'Kaufanfragen',
    settingsSubtitle: 'Familien-Kaufabstimmung',
    cancelRequest: 'Anfrage abbrechen',
    markAsPurchased: 'Als gekauft markieren',
    plannedBanner: 'Geplanter Kauf · Als gekauft markieren',
    yourVote: 'Deine Stimme',
    of: 'von',
    membersVoted: 'Mitglieder abgestimmt',
    notifPurchaseRequests: 'Kaufanfragen',
    notifPurchaseRequestsDesc: 'Neue Anfragen, Stimmen und Entscheidungen',
    titleLabel: 'Titel',
    amountLabel: 'Betrag',
    editRequest: 'Anfrage bearbeiten',
    editTitle: 'Anfrage bearbeiten',
    deleteRequest: 'Anfrage löschen',
    deleteConfirm: 'Die Anfrage wird dauerhaft entfernt',
  },
  splits: {
    title: 'Kategorien aufteilen',
    splitExpense: 'Ausgabe aufteilen',
    selectCategory: 'Kategorie auswählen',
    addCategory: 'Kategorie hinzufügen',
    amount: 'Betrag',
    percentage: 'Prozentsatz',
    remainingAmount: 'Verbleibend: {{amount}}',
    totalMustMatch: 'Die Aufteilungsbeträge müssen der Gesamtausgabe entsprechen',
    suggestSplit: 'Aufteilung vorschlagen',
    aiSuggestion: 'KI-Vorschlag',
    confirmSplit: 'Aufteilung bestätigen',
    removeSplit: 'Aufteilung entfernen',
  },
  trip: {
    splitBetween: 'Aufteilen zwischen',
    splitEqual: 'Gleich',
    splitExact: 'Genaue Beträge',
    splitPercentage: 'Prozentsatz',
    splitShares: 'Anteile',
    splitSelectMember: 'Mindestens ein Mitglied auswählen',
    splitExactMismatch: 'Die Beträge müssen {{amount}} ergeben',
    splitPercentageMismatch: 'Die Prozentsätze müssen 100 % ergeben',
    createTrip: 'Reise erstellen',
    tripName: 'Reisename',
    tripNamePlaceholder: 'z. B. Sommer in Italien',
    nameRequired: 'Bitte einen Reisenamen eingeben',
    endDate: 'Enddatum',
    currency: 'Währung',
    settleUp: 'Abrechnen',
    balances: 'Salden',
    suggestedTransfers: 'Vorgeschlagene Überweisungen',
    allSettled: 'Alles ausgeglichen!',
    allSettledHint: 'Alle in dieser Reise sind ausgeglichen — keine Zahlungen nötig.',
    payVia: 'Zahlen',
    manualBlikInstructions: 'Diese Zahlung bitte manuell senden an:',
    markAsPaid: 'Als bezahlt markieren',
    markAsPaidBody: 'Teile der anderen Person mit, dass du die Zahlung gesendet hast. Sie bestätigt den Erhalt.',
    confirmReceived: 'Erhalt bestätigen',
    awaitingConfirmation: 'Bestätigung ausstehend',
    awaitingPayment: 'Zahlung ausstehend',
    paymentSettingsTitle: 'Zahlungseinstellungen',
    paymentMethod: 'Zahlungsmethode',
    paymentMethodRevolut: 'Revolut',
    paymentMethodPaypal: 'PayPal',
    paymentMethodBlik: 'BLIK',
    paymentMethodCash: 'Bargeld',
    paymentMethodOther: 'Andere',
    paymentHandle: 'Zahlungskennung',
    paymentHandleRequired: 'Bitte eine Zahlungskennung eingeben',
    paymentHandlePlaceholderRevolut: 'dein-revolut-benutzername',
    paymentHandlePlaceholderPaypal: 'dein-paypal.me-slug',
    paymentHandlePlaceholderBlik: '+48 XXX XXX XXX',
    paymentHandlePlaceholderGeneric: 'z. B. bar, persönlich',
    paymentHandleBlikHint: 'Gib deine Telefonnummer im internationalen Format ein, z. B. +48 XXX XXX XXX.',
    paymentInfoSaved: 'Deine Zahlungsinformationen wurden gespeichert.',
    daysLeft: '{{count}} Tage übrig',
    daysLeft_one: '{{count}} Tag übrig',
    tripEnded: 'Reise beendet — abrechnen',
    pastTrips: 'Vergangene Reisen',
    notifyTripSettleUp: 'Reise-Abrechnung',
    notifyTripSettleUpDesc: 'Erinnerungen zum Ausgleichen der Salden, wenn eine Reise endet',
    archiveTrip: 'Reise archivieren',
    archiveTripConfirm: 'Diese Reise archivieren? Sie wird zu Vergangene Reisen verschoben und schreibgeschützt.',
    archiveTripUnconfirmedWarning:
      'Es gibt unbestätigte Ausgleichszahlungen. Trotzdem archivieren?',
    archiveTripForce: 'Trotzdem archivieren',
    tripMap: 'Reisekarte',
  },
  gamification: {
    title: 'Erfolge',
    level: 'Stufe {{level}}',
    xpToNext: '{{xp}} XP bis zur nächsten Stufe',
    totalXp: '{{xp}} XP',
    streak: {
      title: 'Tägliche Serie',
      days_one: '{{count}} Tag',
      days_other: '{{count}} Tage',
      longest: 'Längste: {{count}} Tage',
      broken: 'Starte heute deine Serie!',
      keepGoing: 'Weiter so!',
    },
    badges: {
      unlocked: 'Erfolg freigeschaltet!',
      earned: 'Erhalten am {{date}}',
      locked: 'Gesperrt',
      progress: '{{percent}}% abgeschlossen',
      xpEarned: '+{{xp}} XP',
    },
    categories: {
      all: 'Alle',
      budget: 'Budget',
      tracking: 'Erfassung',
      streak: 'Serie',
      milestone: 'Meilenstein',
      savings: 'Ersparnisse',
      social: 'Sozial',
    },
    achievements: {
      firstExpense: { title: 'Erster Schritt', description: 'Erfasse deine erste Ausgabe' },
      expenses10: { title: 'Guter Anfang', description: 'Erfasse 10 Ausgaben' },
      expenses50: { title: 'Beständiger Erfasser', description: 'Erfasse 50 Ausgaben' },
      expenses100: { title: 'Ausgabenmeister', description: 'Erfasse 100 Ausgaben' },
      expenses500: { title: 'Budgetlegende', description: 'Erfasse 500 Ausgaben' },
      firstBudget: { title: 'Budgetplaner', description: 'Erstelle dein erstes Budget' },
      budgetMonthNoExceed: { title: 'Budgetchampion', description: 'Schließe einen Monat ab, ohne das Budget zu überschreiten' },
      budget3MonthsNoExceed: { title: 'Budgetmeister', description: 'Bleibe 3 Monate im Budget' },
      streak3: { title: 'Drei in Folge', description: '3-tägige Erfassungsserie' },
      streak7: { title: 'Wochenkrieger', description: '7-tägige Erfassungsserie' },
      streak30: { title: 'Monatliche Hingabe', description: '30-tägige Erfassungsserie' },
      streak100: { title: 'Jahrhundert-Serie', description: '100-tägige Erfassungsserie' },
      firstIncome: { title: 'Einkommenserfasser', description: 'Erfasse dein erstes Einkommen' },
      netPositiveMonth: { title: 'Im Plus', description: 'Beende einen Monat mit Einkommen über den Ausgaben' },
      referrals5: { title: 'Community-Builder', description: '5 aktive Freunde empfehlen' },
      ambassador: { title: 'Botschafter', description: '10 Freunde empfehlen — du bist eine Legende!' },
    },
    dashboardWidget: {
      viewAll: 'Erfolge anzeigen',
    },
  },
  categoryCreate: {
    title: 'Neue Kategorie',
    namePlaceholder: 'Kategoriename',
    nameRequired: 'Bitte geben Sie einen Kategorienamen ein',
    creating: 'Erstelle...',
    create: 'Erstellen',
  },
  investments: {
    portfolio: 'Portfolio',
    totalValue: 'Gesamtwert',
    dayChange: 'Unrealisierter Gewinn/Verlust',
    holdingsCount_one: '{{count}} Position',
    holdingsCount_other: '{{count}} Positionen',
    holdings: 'Positionen',
    addAsset: 'Asset hinzufügen',
    searchAsset: 'Assets suchen',
    searchPlaceholder: 'Aktien, Krypto, ETFs suchen...',
    noResults: 'Keine Assets gefunden',
    recommended: 'Empfohlen',
    buy: 'Kaufen',
    sell: 'Verkaufen',
    quantity: 'Menge',
    pricePerUnit: 'Preis pro Einheit',
    fee: 'Gebühr',
    total: 'Gesamt',
    date: 'Datum',
    notes: 'Notizen',
    notesPlaceholder: 'Optionale Notizen...',
    addTransaction: 'Transaktion hinzufügen',
    transactions: 'Transaktionen',
    noTransactions: 'Noch keine Transaktionen',
    currentPrice: 'Aktueller Preis',
    avgCost: 'Durchschnittspreis',
    totalInvested: 'Gesamtinvestition',
    marketValue: 'Marktwert',
    unrealizedPnl: 'Unrealisierter Gewinn/Verlust',
    pnl: 'Gewinn/Verlust',
    returnPercent: 'Rendite',
    allocation: 'Verteilung',
    analytics: 'Analysen',
    performance: 'Performance',
    topGainers: 'Top-Gewinner',
    topLosers: 'Top-Verlierer',
    benchmark: 'Benchmark',
    proRequired: 'Upgrade auf Pro für Benchmark-Vergleich',
    periods: {
      week: '1W',
      month: '1M',
      quarter: '3M',
      year: '1J',
      all: 'Alle',
    },
    assetTypes: {
      stock: 'Aktie',
      crypto: 'Krypto',
      etf: 'ETF',
      bond: 'Anleihe',
      commodity: 'Rohstoff',
    },
    removeHolding: 'Position entfernen',
    removeHoldingConfirm: 'Möchten Sie diese Position wirklich entfernen? Alle Transaktionen werden gelöscht.',
    deleteTransaction: 'Transaktion löschen',
    deleteTransactionConfirm: 'Möchten Sie diese Transaktion wirklich löschen?',
    refreshPrices: 'Preise aktualisieren',
    lastUpdated: 'Aktualisiert {{time}}',
    noHoldings: 'Noch keine Positionen',
    noHoldingsHint: 'Suchen und fügen Sie Assets hinzu, um Ihr Portfolio zu verfolgen',
    searchHint: 'Suche nach Ticker oder Firmenname',
    tryDifferentQuery: 'Versuchen Sie eine andere Suchanfrage',
    transactionType: 'Transaktionstyp',
    feeOptional: 'Optional',
    invalidQuantity: 'Bitte geben Sie eine gültige Menge ein',
    invalidPrice: 'Bitte geben Sie einen gültigen Preis ein',
    holdingNotFound: 'Position nicht gefunden',
    deleteHolding: 'Position löschen',
    deleteHoldingConfirm: 'Sind Sie sicher, dass Sie diese Position löschen möchten? Alle Transaktionen werden gelöscht.',
    pnlPercent: 'Rendite %',
    noHoldingsSubtitle: 'Suchen und fügen Sie Vermögenswerte hinzu, um Ihr Portfolio zu verfolgen',
    totalPnl: 'Gesamt-P&L',
    noPerformanceData: 'Noch keine Performance-Daten verfügbar',
    priceChartComingSoon: 'Preisdiagramm kommt bald',
    noPriceData: 'Keine Preisdaten',
    lastTxPrice: 'letzter Tx-Preis',
    allocationByType: 'Aufteilung nach Typ',
    noAllocationData: 'Keine Aufteilungsdaten verfügbar',
    benchmarkComparison: 'Benchmark-Vergleich',
    benchmarkProOnly: 'Upgrade auf Pro+ für Benchmark-Vergleich',
    noBenchmarkData: 'Keine Benchmark-Daten verfügbar',
    yourPortfolio: 'Ihr Portfolio',
    difference: 'Differenz',
    formulas: {
      formula: 'Formel',
      example: 'Ihre Daten',
      performanceTitle: 'Renditeberechnung',
      performanceDesc: 'Die Portfolio-Performance zeigt die prozentuale Wertveränderung Ihres Portfolios über den ausgewählten Zeitraum. Der aktuelle Wert wird mit dem Startwert verglichen.',
      performanceExample: 'Start: ${{start}} → Ende: ${{end}} = {{result}}% Rendite',
      allocationTitle: 'Allokationsberechnung',
      allocationDesc: 'Die Asset-Allokation zeigt, welchen Prozentsatz des Gesamtportfolios jeder Asset-Typ ausmacht. Dies hilft, die Diversifikation zu verstehen.',
      allocationExample: 'Der Wert jedes Typs wird durch den Gesamtportfoliowert geteilt und mit 100 multipliziert',
      gainersTitle: 'Gewinn/Verlust-Berechnung',
      gainersDesc: 'Der Gewinn/Verlust-Prozentsatz (P&L) zeigt, wie viel jede Position im Vergleich zum durchschnittlichen Kaufpreis gewonnen oder verloren hat.',
      gainersExample: 'Wenn Sie bei $100 gekauft haben und der aktuelle Preis $120 beträgt, ist Ihr P&L = +20%',
      benchmarkTitle: 'Benchmark-Vergleich',
      benchmarkDesc: 'Der Vergleich zeigt, wie Ihr Portfolio im Verhältnis zu einem Marktindex (wie S&P 500) abgeschnitten hat. Eine positive Differenz bedeutet, dass Sie den Markt übertroffen haben.',
      benchmarkExample: 'Portfolio: {{portfolio}}% vs Benchmark: {{benchmark}}% = {{diff}}% Differenz',
    },
    insights: {
      title: 'KI-Portfolio-Einblicke',
      loading: 'Ihr Portfolio wird analysiert...',
      error: 'Einblicke konnten nicht geladen werden',
      empty: 'Keine Einblicke verfügbar',
    },
  },
  help: {
    title: 'Hilfe',
    articleTitle: 'Hilfe',
    widgets: {
      title: 'Startbildschirm-Widgets',
      subtitle: 'Nur Android',
      intro: 'Fügen Sie Widgets zu Ihrem Android-Startbildschirm hinzu, um schnell auf Ausgabendaten zuzugreifen oder Ausgaben zu erfassen, ohne die App zu öffnen.',
      howToAdd: 'Wie füge ich ein Widget hinzu?',
      howToAddSteps: 'Startbildschirm lang drücken → Widgets → AI Budget Assistant → Widget wählen',
      available: 'Verfügbare Widgets',
      widgetSmallTitle: 'Budget – Heute',
      widgetSmallDesc: 'Heutige Ausgaben mit Vergleich zu gestern.',
      widgetMediumTitle: 'Budget – Woche',
      widgetMediumDesc: 'Balkendiagramm der Ausgaben der letzten 7 Tage plus heutiger Gesamtbetrag.',
      widgetLargeTitle: 'Budget – Übersicht',
      widgetLargeDesc: 'Budgetfortschrittsbalken und Top-Ausgabenkategorien für den aktuellen Zeitraum.',
      widgetQuickTitle: 'Budget – Schnell hinzufügen',
      widgetQuickDesc: 'Drei Schaltflächen: 🎤 Sprache, 📷 Bon scannen oder ✏️ manuell — öffnet die App direkt.',
      faqAndroid: 'Sind Widgets auf iOS verfügbar?',
      faqAndroidAnswer: 'Widgets sind nur für Android. iOS-Nutzer können dieselben Funktionen über die App nutzen.',
      faqUpdate: 'Wie oft werden Widget-Daten aktualisiert?',
      faqUpdateAnswer: 'Heute, Woche und Übersicht werden alle 30 Min. aktualisiert. Schnell hinzufügen ist statisch.',
    },
  },
  encryption: {
    title: 'Sicherheit',
    e2ee: 'Ende-zu-Ende-Verschlüsselung',
    e2eeDesc: 'Verschlüsseln Sie Ihre Finanzdaten, sodass nur Sie sie lesen können',
    setupE2EE: 'Verschlüsselung einrichten',
    setupDesc: 'Erstellen Sie eine Passphrase, um Ihre Daten mit Ende-zu-Ende-Verschlüsselung zu schützen',
    passphrase: 'Verschlüsselungs-Passphrase',
    passphrasePlaceholder: 'Geben Sie eine starke Passphrase ein...',
    passphraseConfirm: 'Passphrase bestätigen',
    passphraseConfirmPlaceholder: 'Passphrase erneut eingeben...',
    passphraseHint: 'Diese Passphrase ist unabhängig von Ihrem Anmeldepasswort. Bewahren Sie sie sicher auf — sie kann nicht wiederhergestellt werden.',
    passphraseMismatch: 'Passphrasen stimmen nicht überein',
    passphraseMin: 'Die Passphrase muss mindestens 8 Zeichen lang sein',
    setup: 'Verschlüsselung aktivieren',
    settingUp: 'Verschlüsselung wird eingerichtet...',
    setupSuccess: 'Verschlüsselung erfolgreich aktiviert!',
    recoveryKey: 'Wiederherstellungsschlüssel',
    recoveryKeyDesc: 'Speichern Sie diesen Schlüssel an einem sicheren Ort. Sie benötigen ihn, falls Sie Ihre Passphrase vergessen.',
    recoveryKeyCopied: 'Wiederherstellungsschlüssel in die Zwischenablage kopiert',
    recoveryKeySaved: 'Ich habe meinen Wiederherstellungsschlüssel gespeichert',
    unlock: 'Verschlüsselung entsperren',
    unlockDesc: 'Geben Sie Ihre Passphrase ein, um Ihre Daten zu entschlüsseln',
    unlockButton: 'Entsperren',
    unlocking: 'Wird entsperrt...',
    unlockFailed: 'Falsche Passphrase. Bitte versuchen Sie es erneut.',
    locked: 'Gesperrt',
    lockedDesc: 'Ihre verschlüsselten Daten sind gesperrt. Geben Sie Ihre Passphrase ein, um darauf zuzugreifen.',
    unlocked: 'Entsperrt',
    lock: 'Sperren',
    lockDesc: 'Verschlüsselte Daten sperren',
    accountEncryption: 'Kontoverschlüsselung',
    enableForAccount: 'Für dieses Konto aktivieren',
    tier1: 'Standard',
    tier1Desc: 'Verschlüsselt Beschreibungen, Notizen und Namen. Analysen und KI bleiben verfügbar.',
    tier2: 'Maximale Privatsphäre',
    tier2Desc: 'Verschlüsselt alle Daten einschließlich Beträge. Server-Analysen und KI werden deaktiviert.',
    enabled: 'Aktiviert',
    disabled: 'Deaktiviert',
    tierLabel: 'Verschlüsselungsstufe',
    keyRotation: 'Schlüsselrotation',
    keyRotationNeeded: 'Schlüsselrotation erforderlich (Mitglied wurde entfernt)',
    rotateKey: 'Schlüssel rotieren',
    rotating: 'Schlüssel wird rotiert...',
    rotateSuccess: 'Schlüssel erfolgreich rotiert',
    pendingGrants: 'Ausstehende Schlüsselfreigaben',
    pendingGrantsDesc: 'Diese Mitglieder benötigen den Verschlüsselungsschlüssel, um auf Daten zuzugreifen',
    grantKey: 'Schlüssel freigeben',
    granting: 'Wird freigegeben...',
    grantSuccess: 'Schlüssel erfolgreich freigegeben',
    recovery: 'Wiederherstellung',
    setupRecovery: 'Wiederherstellungsschlüssel einrichten',
    recoverAccess: 'Zugang wiederherstellen',
    recoverDesc: 'Geben Sie Ihren Wiederherstellungsschlüssel ein, um den Zugang zu Ihren verschlüsselten Daten wiederzuerlangen',
    recoveryKeyPlaceholder: 'XXXX-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX',
    newPassphrase: 'Neue Passphrase',
    recovering: 'Wiederherstellung läuft...',
    recoverSuccess: 'Zugang wiederhergestellt! Speichern Sie Ihren neuen Wiederherstellungsschlüssel.',
    noMembers: 'Keine ausstehenden Schlüsselfreigaben',
    resetTitle: 'Verschlüsselung zurücksetzen',
    resetConfirm: 'Dadurch wird Ihr Verschlüsselungsprofil gelöscht. Sie müssen die Verschlüsselung erneut einrichten. Zuvor verschlüsselte Daten, die nicht synchronisiert wurden, gehen verloren. Fortfahren?',
    resetButton: 'Verschlüsselung zurücksetzen',
    resetSuccess: 'Verschlüsselung wurde zurückgesetzt. Sie können sie erneut einrichten.',
  },
  reports: {
    title: 'Berichte',
    settingsTitle: 'Berichte & E-Mail',
    generateReport: 'Bericht erstellen',
    format: 'Format',
    dateRange: 'Zeitraum',
    lastWeek: 'Letzte Woche',
    thisMonth: 'Dieser Monat',
    lastQuarter: 'Letztes Quartal',
    thisYear: 'Dieses Jahr',
    specificMonth: 'Bestimmter Monat',
    customRange: 'Eigener Zeitraum',
    selectMonth: 'Monat auswählen',
    selectMonthHint: 'Wählen Sie oben einen Monat',
    from: 'Von',
    to: 'Bis',
    pickDate: 'Datum wählen',
    rangeIncomplete: 'Wählen Sie Start- und Enddatum — der Start darf nicht nach dem Ende liegen',
    monthlyDigestIndependent: 'Aktueller Monat, unabhängig vom Zeitraum oben',
    generate: 'Erstellen',
    generating: 'Wird erstellt...',
    reportGenerated: 'Bericht erfolgreich erstellt!',
    reportSavedTo: 'Bericht gespeichert in:\n{{location}}',
    recentReports: 'Letzte Berichte',
    noReports: 'Noch keine Berichte',
    monthlyDigest: 'Monatliche Zusammenfassung',
    monthlyDigestDesc: 'Monatliche Finanzübersicht per E-Mail erhalten',
    income: 'Einnahmen',
    expenses: 'Ausgaben',
    savingsRate: 'Sparquote',
    topCategories: 'Top-Kategorien',
    weeklyEmail: 'Wöchentliche E-Mail',
    weeklyEmailDesc: 'Wöchentliche Finanzzusammenfassung per E-Mail erhalten',
    weeklyEmailBusiness: 'Erfordert Business-Plan',
    sendOn: 'Senden am',
    sunday: 'So',
    monday: 'Mo',
    tuesday: 'Di',
    wednesday: 'Mi',
    thursday: 'Do',
    friday: 'Fr',
    saturday: 'Sa',
    exportBackup: 'Backup erstellen',
    backupExported: 'Backup erfolgreich erstellt!',
    backupSavedTo: 'Backup gespeichert unter:\n{{location}}',
    backupShared: 'Backup erstellt. Wähle im Menü einen Speicherort.',
    restoreBackup: 'Backup importieren',
    exporting: 'Wird erstellt...',
    restoring: 'Wird importiert...',
    backupRestored: 'Backup erfolgreich importiert!',
    restoreConfirmTitle: 'Backup importieren',
    restoreConfirmMerge: 'Mit vorhandenen Daten zusammenführen (Duplikate überspringen)?',
    restoreConfirmOverwrite: 'Alle vorhandenen Daten überschreiben? Dies kann nicht rückgängig gemacht werden.',
    merge: 'Zusammenführen',
    overwrite: 'Überschreiben',
    encryptionRestricted: 'Berichte sind für vollständig verschlüsselte Konten nicht verfügbar',
    deleteTitle: 'Bericht löschen',
    deleteConfirm: 'Möchten Sie diesen Bericht wirklich löschen?',
    share: 'Teilen',
    download: 'Herunterladen',
  },
  legal: {
    privacyPolicy: 'Datenschutzrichtlinie',
    termsOfService: 'Nutzungsbedingungen',
    agreeToTerms: 'Ich stimme den',
    and: 'und',
    bySubscribing: 'Mit dem Abonnement stimmen Sie unseren',
    mustAcceptTerms: 'Sie müssen die Nutzungsbedingungen und Datenschutzrichtlinie akzeptieren',
  },
  location: {
    sectionTitle: 'Standort',
    attachToggle: 'Standort an neue Ausgaben anhängen',
    attachToggleDesc: 'Speichert deine GPS-Position, wenn du eine Ausgabe vor Ort hinzufügst, und prüft deinen Standort, wenn du die Übersicht öffnest',
    permissionDenied: 'Standortberechtigung erforderlich. Aktiviere sie in den Systemeinstellungen.',
    title: 'Standort',
    addLocation: 'Standort hinzufügen',
    editLocation: 'Standort bearbeiten',
    removeLocation: 'Standort entfernen',
    myLocation: 'Mein Standort',
    tapToPlace: 'Tippe auf die Karte, um den Pin zu setzen',
    searchPlaceholder: 'Adresse oder Ort suchen',
    searchNoResults: 'Nichts gefunden',
    recentSearches: 'Zuletzt',
    pickerTitle: 'Ort der Ausgabe',
  },
  map: {
    mapView: 'Karte',
    listView: 'Liste',
    noLocationCount_one: '{{count}} Ausgabe ohne Standort',
    noLocationCount_other: '{{count}} Ausgaben ohne Standort',
    open: 'Öffnen',
  },
  debt: {
    debtsAndLoans: 'Schulden & Darlehen',
    debtDetails: 'Schuldendetails',
    lendMoney: 'Geld verleihen',
    borrowMoney: 'Geld leihen',
    contactNamePlaceholder: 'Name der Person',
    setDueDate: 'Fälligkeitsdatum (optional)',
    dueDate: 'Fälligkeitsdatum',
    contact: 'Kontakt',
    lent: 'Verliehen',
    borrowed: 'Geliehen',
    recordRepayment: 'Rückzahlung erfassen',
    repaymentHistory: 'Rückzahlungsverlauf',
    noRepayments: 'Noch keine Rückzahlungen',
    originalAmount: 'Ursprünglicher Betrag',
    totalRepaid: 'Gesamt zurückgezahlt',
    remaining: 'Verbleibend',
    status: 'Status',
    statusActive: 'Aktiv',
    statusPaid: 'Bezahlt',
    statusOverdue: 'Überfällig',
    peopleOweYou: 'Man schuldet Ihnen',
    youOwe: 'Sie schulden',
    moneyLent: 'Verliehenes Geld',
    moneyBorrowed: 'Geliehenes Geld',
    noDebts: 'Noch keine Schulden',
    noDebtsHint: 'Markieren Sie eine Ausgabe oder Einnahme als Schuld, um sie hier zu verfolgen',
    addDebt: 'Schuld hinzufügen',
    lendMoneyHint: 'Geld erfassen, das Sie verliehen haben',
    borrowMoneyHint: 'Geld erfassen, das Sie geliehen haben',
    filterAll: 'Alle',
    filterActive: 'Aktiv',
    filterOverdue: 'Überfällig',
    filterPaid: 'Bezahlt',
    isDebtRepayment: 'Schuldenrückzahlung',
    repaymentFor: 'Rückzahlung für Schuld',
  },
  calendar: {
    title: 'Kalender',
    income: 'Einnahmen',
    expenses: 'Ausgaben',
    netProfit: 'Nettogewinn',
    categories: 'Kategorien',
    wallets: 'Konten',
    transactions: 'Transaktionen',
    noTransactions: 'Keine Transaktionen für diesen Zeitraum',
    tapToViewDetails: 'Tippen für Details',
    allMonth: 'Ganzer Monat',
  },
  scenarioSimulator: {
    title: 'Szenario-Simulator',
    subtitle: 'Passen Sie Ausgaben & Einnahmen an, um Ersparnisveränderungen zu sehen',
    currentSavings: 'Aktueller Monat',
    scenarioSavings: 'Szenario',
    projectionTitle: 'Ersparnisprojektion',
    months: 'Mo.',
    expenseAdjustments: 'Ausgaben anpassen',
    incomeAdjustments: 'Einnahmen anpassen',
    addExtraIncome: 'Zusatzeinnahme hinzufügen',
    extraIncomeDescription: 'Beschreibung',
    extraIncomeAmount: 'Betrag pro Monat',
    resetAll: 'Alles zurücksetzen',
    perMonth: '/Mo.',
    unchanged: 'unverändert',
    noData: 'Nicht genug Daten',
    noDataHint: 'Fügen Sie Ausgaben und Einnahmen hinzu, um Projektionen zu sehen',
    currentPath: 'Aktuell',
    scenarioPath: 'Szenario',
    saveScenario: 'Szenario speichern',
    savedScenarios: 'Gespeicherte Szenarien',
    saveNamePlaceholder: 'z.B. Gastronomie 30% kürzen',
    scenarioSaved: 'Szenario gespeichert',
    noSavedScenarios: 'Noch keine gespeicherten Szenarien',
    loadScenario: 'Laden',
    deleteScenario: 'Löschen',
    shareScenario: 'Teilen',
    scenarioLimitFree: 'Sie haben das Limit von 5 Szenarien erreicht. Upgraden Sie auf Pro für unbegrenzte Speicherungen.',
  },
  widgets: {
    today: 'Heute',
    openApp: 'App öffnen',
    week: 'Woche',
    budgets: 'Budgets',
    topCategories: 'Top Kategorien',
    voice: 'Sprache',
    scan: 'Scannen',
    add: 'Hinzufügen',
    dayNames: ['So', 'Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa'],
  },
  settingsNav: {
    profile: 'Profil & Konto',
    profileDesc: 'Name, E-Mail, Zeitzone, Währung',
    appearance: 'Darstellung & Sprache',
    appearanceDesc: 'Design, Sprache',
    ai: 'KI-Einstellungen',
    aiDesc: 'Antwortstil, Modellauswahl',
    widgets: 'Dashboard-Widgets',
    widgetsDesc: 'Widgets auf dem Startbildschirm anzeigen oder ausblenden',
    notifications: 'Benachrichtigungen & Integrationen',
    notificationsDesc: 'Push-Benachrichtigungen, Telegram-Bot',
    security: 'Sicherheit & Verschlüsselung',
    securityDesc: 'Ende-zu-Ende-Verschlüsselung (E2EE)',
    data: 'Daten & Berichte',
    dataDesc: 'Synchronisierung, E-Mail-Berichte, Sicherung & Wiederherstellung',
    about: 'Über',
    aboutDesc: 'Version, Hilfe, Support, Rechtliches',
    wallet: 'Geldbörse',
    walletDesc: 'Salden, Schulden, Anfangssaldo',
    categories: 'Kategorien',
    categoriesDesc: 'Kategorien verwalten und löschen',
    merchants: 'Händler',
    merchantsDesc: 'Händler umbenennen, zusammenführen oder entfernen',
    referenceData: 'Stammdaten',
    referenceDataDesc: 'Kategorien, Händler, Tags und Projekte',
    accountsDesc: 'Konten wechseln, Mitglieder, Finanzmonat',
    tags: 'Tags',
    tagsDesc: 'Tags erstellen, umbenennen oder löschen',
    projects: 'Projekte',
    projectsDesc: 'Projekte verwalten',
    subscriptionDesc: 'Tarif, Nutzung, Upgrade',
    products: 'Produkte',
    productsDesc: 'Produktnamen und Gruppen verwalten',
    whatsNew: 'Neuigkeiten',
    whatsNewDesc: 'Entdecke, was diese App für dich tun kann',
  },
  categories: {
    title: 'Kategorien',
    expenseCategories: 'Ausgabenkategorien',
    incomeCategories: 'Einkommenskategorien',
    delete: 'Löschen',
    deleteConfirmTitle: 'Kategorie löschen',
    deleteConfirmMessage: 'Möchten Sie "{{name}}" wirklich löschen?',
    deleteErrorHasRecords: 'Kategorie kann nicht gelöscht werden. Sie wird in {{expenses}} Ausgaben, {{incomes}} Einnahmen, {{budgets}} Budgets und {{other}} weiteren Einträgen verwendet.',
    nameTaken: 'Eine Kategorie namens „{{name}}“ existiert bereits. Wählen Sie einen anderen Namen.',
    nameTakenByDeleted: 'Eine gelöschte Kategorie namens „{{name}}“ existiert noch. Wählen Sie einen anderen Namen.',
    deleteSuccess: 'Kategorie gelöscht',
    iconLabel: 'Symbol',
    empty: 'Keine Kategorien',
    system: 'System',
    edit: 'Kategorie bearbeiten',
    add: 'Kategorie hinzufügen',
    save: 'Speichern',
    expense: {
      foodAndDining: 'Essen & Trinken',
      transport: 'Transport',
      shopping: 'Shopping',
      entertainment: 'Unterhaltung',
      healthAndFitness: 'Gesundheit & Fitness',
      billsAndUtilities: 'Rechnungen & Versorgungsleistungen',
      education: 'Bildung',
      travel: 'Reisen',
      groceries: 'Lebensmittel',
      coffeeAndDrinks: 'Kaffee & Getränke',
      subscriptions: 'Abonnements',
      clothing: 'Kleidung',
      personalCare: 'Körperpflege',
    },
    income: {
      salary: 'Gehalt',
      freelance: 'Freelance',
      investments: 'Investitionen',
      gifts: 'Geschenke',
      otherIncome: 'Sonstiges Einkommen',
    },
  },
  merchants: {
    title: 'Händler',
    rename: 'Umbenennen',
    delete: 'Löschen',
    renameTitle: 'Händler umbenennen',
    renamePlaceholder: 'Händlername',
    nameRequired: 'Bitte Händlernamen eingeben',
    expensesCount: '{{count}} Ausgaben',
    deleteConfirm: 'Händler aus {{count}} Ausgaben entfernen?',
    renamed: '{{count}} Ausgaben aktualisiert',
    deleted: '{{count}} Ausgaben bereinigt',
    empty: 'Noch keine Händler',
    select: 'Auswählen',
    selected: '{{count}} ausgewählt',
    merge: 'Zusammenführen',
    mergeTitle: 'Händler zusammenführen',
    mergeInto: 'Zusammenführen in',
    mergeCount: '{{count}} Ausgaben werden verschoben',
    merged: '{{count}} Ausgaben zu {{name}} zusammengeführt',
    selectToMerge: 'Mindestens 2 Händler zum Zusammenführen auswählen',
    suggestionTitle: 'Sieht aus wie ein Verkäufer',
    suggestionMerge: 'Mit {{name}} zusammenführen',
    dismiss: 'Schließen',
    categoryRules: 'Kategorieregeln',
    noRules: 'Noch keine erlernten Regeln',
    ruleDeleteConfirm: 'Auto-Kategorieregel für „{{merchant}}" löschen?',
    ruleDeleted: 'Regel gelöscht',
    reapplyNothingToDo: 'Alle Ausgaben bei {{merchant}} sind bereits in der richtigen Kategorie.',
    reapplyTitle: 'Regel erneut anwenden',
    reapplyGroupLabel: '{{count}} aus {{category}}',
    reapplySummary: '{{count}} Ausgaben werden nach {{category}} verschoben',
    reapplyApplied: '{{count}} Ausgaben nach {{category}} verschoben',
    reapplyApply: 'Übernehmen ({{count}})',
  },
  aiUsage: {
    confirmTitle: 'AI-Nutzung',
    confirmMessage: 'Diese Aktion verwendet {{cost}} AI-Anfragen.\nVerbraucht: {{used}}/{{limit}} ({{remaining}} übrig)',
    continue: 'Fortfahren',
    dontAskAgain: 'Nicht erneut fragen',
    totalUsed: 'Verbraucht',
    byFeature: 'Nach Funktion',
    history: 'Verlauf',
    noUsage: 'Keine AI-Nutzung in diesem Monat',
    features: {
      chat: 'AI-Chat',
      voice: 'Spracheingabe',
      parse: 'Textanalyse',
      categorization: 'Kategorisierung',
      ocr: 'Belegscan',
      story: 'Ausgabengeschichte',
      fat_finder: 'Fat Finder',
      insights: 'AI-Einblicke',
      investment_insights: 'Investitionseinblicke',
      tag_suggestion: 'Tag-Vorschlag',
      project_suggestion: 'Projektvorschlag',
      split_suggestion: 'Aufteilungsvorschlag',
      goal_plan: 'Zielplanung',
    },
  },
  referral: {
    title: 'Freunde einladen',
    yourCode: 'Dein Empfehlungscode',
    share: 'Teilen',
    copy: 'Kopieren',
    copied: 'Kopiert!',
    codeOptional: 'Empfehlungscode (optional)',
    stats: {
      total: 'Gesamt',
      qualified: 'Aktiv',
      pending: 'Ausstehend',
    },
    bonusAiRequests: 'Bonus-AI-Anfragen',
    requests: 'Anfragen',
    milestone5: 'Kostenloser Monat Pro',
    milestone10: 'Ambassador-Abzeichen',
    status: {
      pending: 'Ausstehend',
      qualified: 'Aktiv',
      expired: 'Abgelaufen',
    },
    extendedTrial: '14-tägige verlängerte Testphase',
    inviteCardTitle: 'Für euch beide mehr drin',
    inviteCardBody: "Dein Freund hat dich gerade über die App zurückgezahlt. Schick ihm eine Einladung: er bekommt eine längere Testphase, du bekommst Bonus-KI-Anfragen.",
    inviteCardAction: 'Einladen',
    shareText: 'Tritt AI Budget Assistant mit meinem Code {{code}} bei! Erhalte eine 14-tägige Testphase.',
    settingsTitle: 'Freunde einladen',
    settingsSubtitle: 'Code teilen, Bonus-AI-Anfragen verdienen',
    referredUsers: 'Empfohlene Benutzer',
    empty: {
      title: 'Noch keine Empfehlungen',
      subtitle: 'Teile deinen Code, um Belohnungen zu verdienen',
    },
    reward: {
      free_pro_month: 'Kostenloser Monat Pro',
      ambassador_badge: 'Ambassador-Abzeichen',
    },
    howItWorks: {
      title: 'So funktioniert es',
      step1: 'Teile deinen Code mit Freunden',
      step2: 'Sie registrieren sich und nutzen die App 7 Tage lang',
      step3: 'Empfehlung wird aktiv — du erhältst +30 AI-Anfragen',
    },
    nextMilestone: 'Nächster Meilenstein',
    friendJoined: '{{name}} ist mit deinem Code beigetreten!',
    friendQualified: '{{name}} ist jetzt aktiv! +30 AI-Anfragen verdient.',
  },
  update: {
    titleAvailable: 'Update verfügbar',
    titleRequired: 'Update erforderlich',
    bodyAvailable: 'Eine neue Version von AI Budget Assistant ist verfügbar.',
    bodyRequired: 'Diese Version wird nicht mehr unterstützt. Bitte aktualisiere, um fortzufahren.',
    actionUpdate: 'Aktualisieren',
    actionLater: 'Später',
    releaseNotesLabel: 'Was ist neu',
  },
  whatsNew: {
    title: 'Neuigkeiten',
    spotlightNewBadge: 'Neu',
    tierPro: 'Pro',
    tierBusiness: 'Business',
    actionLearnMore: 'Mehr erfahren',
    actionGotIt: 'Verstanden',
    emptyState: 'Hier gibt es noch nichts – schau nach dem nächsten Update wieder vorbei.',
    entries: {
      'fat-finder': {
        title: 'Fat Finder',
        body: 'Eine KI-Prüfung Ihrer Ausgaben: findet vergessene Abos, Preiserhöhungen und einmalige Buchungen, die einen zweiten Blick verdienen.',
      },
      'financial-health-score': {
        title: 'Finanz-Gesundheitswert',
        body: 'Ein Wert von 0 bis 100 auf Ihrem Startbildschirm: Budgettreue, Sparquote, Zielfortschritt und Schuldenlage in einer Zahl.',
      },
      'safe-to-spend': {
        title: 'Sicher ausgeben',
        body: 'Wie viel Sie heute ausgeben können, ohne Rechnungen, Ziele oder Abos zu gefährden — die Zahl oben auf dem Startbildschirm.',
      },
      'personal-inflation-index': {
        title: 'Persönlicher Inflationsindex',
        body: 'Wie sich die Preise genau der Produkte entwickelt haben, die Sie kaufen — aus Ihren eigenen gescannten Belegen.',
      },
      'shopping-list-compare': {
        title: 'Wo ist es günstiger?',
        body: 'Vergleichen Sie Ihre Einkaufsliste über die Läden, in denen Sie wirklich einkaufen — mit echten Preisen aus Ihren Belegen.',
      },
      'community-price-map': {
        title: 'Community-Preiskarte',
        body: 'Wo in der Nähe der beste Preis für ein Produkt gefunden wurde: von der Community gesammelt und anonymisiert — auf niemanden zurückführbar.',
      },
      'financial-wrapped': {
        title: 'Finanzjahr im Rückblick',
        body: 'Ihr Geldjahr im Spotify-Wrapped-Stil: häufigster Händler, teuerster Monat, Sparquote und mehr.',
      },
      'inflation-shield': {
        title: 'Inflationsschutz',
        body: 'Sagt voraus, welche Ihrer regelmäßigen Einkäufe bald teurer werden, und schlägt vor, jetzt Vorrat anzulegen.',
      },
      'ai-chat-shopping-tools': {
        title: 'Einkaufsliste per Chat',
        body: 'Sagen Sie dem Chat „Milch und Eier hinzufügen” oder „Was sollte ich nachkaufen?” — er ergänzt, entfernt und schlägt vor.',
      },
      'receipt-price-check': {
        title: 'Preisprüfung für Belege',
        body: 'Jeder gescannte Beleg wird still mit dem verglichen, was Sie zuletzt im selben Laden bezahlt haben — kostet etwas mehr als sonst, sehen Sie einen Hinweis.',
      },
      'exchange-rate-alerts': {
        title: 'Wechselkurs-Alarme',
        body: 'Setzen Sie ein Ziel für ein Währungspaar und werden Sie in der Stunde benachrichtigt, in der es erreicht wird — kein Nachsehen mehr in der App.',
      },
      'shopping-list-auto-check': {
        title: 'Ihre Einkaufsliste hakt sich selbst ab',
        body: 'Scannen Sie den Beleg, und was Sie wirklich gekauft haben, wird von selbst abgehakt — mit „Rückgängig”, falls etwas danebenging.',
      },
    },
  },
  wrapped: {
    title: 'Jahresrückblick',
    notEnoughData: 'Noch nicht genug Daten',
    notEnoughDataDesc: 'Erfasse das Jahr über deine Ausgaben und Einnahmen, um deinen Jahresrückblick freizuschalten.',
    swipeHint: 'Wische weiter →',
    introTitle: 'Dein {{year}}\nin Zahlen',
    introSub: 'Lass uns auf dein Jahr zurückblicken',
    totalTracked: 'Du hast erfasst',
    spentLabel: 'ausgegeben dieses Jahr',
    earnedLabel: 'Eingenommen',
    transactions: 'Transaktionen',
    topMerchant: 'Dein Lieblingsort',
    merchantVisits: '{{count}} Besuche',
    merchantSpent: '{{value}} insgesamt',
    biggestMonth: 'Dein größter Monat',
    topCategory: 'Top-Kategorie',
    ofSpending: '{{percent}}% der Ausgaben',
    categoryMix: 'Wohin es floss',
    receiptsScanned: 'Gescannte Belege',
    receiptsScannedDesc: 'Jeder Scan macht deine Auswertungen genauer',
    saved: 'Du hast gespart',
    overspent: 'Du hast mehr ausgegeben als eingenommen',
    savingsRate: '{{percent}}% Sparquote',
    savedVsLastYear: '{{direction}} {{value}} im Vergleich zum Vorjahr',
    personalInflation: 'Deine persönliche Inflation',
    inflationDesc: 'Wie sich deine eigenen Preise dieses Jahr verändert haben',
    streakDays: '{{count}} Tage',
    longestStreak: 'Längste Tracking-Serie',
    outroTitle: 'Das war dein {{year}}!',
    outroDesc: 'Teile dein Jahr in Zahlen',
    hideAmounts: 'Beträge ausblenden',
    share: 'Teilen',
    shareTitle: '🎁 Mein {{year}} Jahresrückblick — AI Budget',
    shareSpent: '{{value}} an Ausgaben erfasst',
    shareMerchant: 'Lieblingsort: {{name}} ({{count}} Besuche)',
    shareMonth: 'Größter Monat: {{month}}',
    shareCategory: 'Top-Kategorie: {{name}} ({{percent}}%)',
    shareReceipts: 'Gescannte Belege: {{count}}',
    shareSaved: 'Gespart: {{value}}',
    shareInflation: 'Meine persönliche Inflation: {{percent}}%',
    shareStreak: 'Längste Serie: {{count}} Tage',
    shareCta: 'Verfolge deine → ai-budget.pl',
  },
  communityPrices: {
    sectionTitle: 'Community-Preise',
    contributeToggle: 'Anonyme Preise teilen',
    contributeToggleDesc: 'Teile anonymisierte Produktpreise von deinen Belegen, damit alle den günstigsten Laden finden. Es werden keine persönlichen Daten geteilt.',
    screenTitle: 'Wo ist es am günstigsten',
    entryTitle: 'Wo ist es am günstigsten?',
    entrySubtitle: 'Vergleiche Community-Preise zwischen Geschäften',
    searchPlaceholder: 'Produkt suchen...',
    searchHint: 'Suche nach einem Produkt, um Preise zwischen Geschäften zu vergleichen',
    noResults: 'Für dieses Produkt gibt es noch keine Community-Preise',
    regionsAvailable: '{{count}} Regionen',
    cheapest: 'Günstigster',
    backedBy: '{{receipts}} Belege · {{contributors}} Nutzer',
    week1: '1 Woche',
    week4: '4 Wochen',
    listView: 'Liste',
    mapView: 'Karte',
    mapEmpty: 'Noch keine Geschäfte auf der Karte',
    paywall: 'Wo ist es am günstigsten',
  },
  safeToSpend: {
    title: 'Sicher ausgeben',
    today: 'Heute sicher ausgebbar',
    subtitle: 'Nach Abdeckung deiner Verpflichtungen',
    breakdownTitle: 'So wird es berechnet',
    wallet: 'Kontostand',
    expectedIncome: 'Erwartetes Einkommen',
    subscriptions: 'Bevorstehende Abonnements',
    recurring: 'Bevorstehende Dauerausgaben',
    goals: 'Sparziel-Beiträge',
    buffer: 'Sicherheitspuffer',
    daysLeft: 'Verbleibende Tage',
    noIncomeAssumed: 'Kein weiteres Einkommen diesen Monat angenommen',
    approxRate: 'Ungefährer Wechselkurs angewendet',
    widgetLabel: 'Sicher ausgeben',
  },
  inflationShield: {
    title: 'Inflationsschutz',
    savedSoFar: 'Bisher gespart',
    estimated: 'geschätzt',
    basketForecast: 'Dein Warenkorb: ~{{pct}}% nächsten Monat',
    buyAheadTitle: 'Vorab kaufen & sparen',
    buyAheadTip: '{{product}} jetzt kaufen — spare ~{{save}}',
    units: '{{count}} Einheit',
    units_other: '{{count}} Einheiten',
    atStore: 'bei {{store}}',
    affordable: 'Heute erschwinglich',
    approxRate: 'Beträge basieren auf ungefähren Wechselkursen',
    empty: 'Gerade nichts zum Bevorraten',
    emptySub: 'Scanne ein paar Belege, damit wir steigende Preise für dich erkennen können',
    share: 'Teilen',
    hideAmounts: 'Beträge ausblenden',
    shareTitle: '🛡️ Mein Inflationsschutz — AI Budget',
    shareSaved: 'Bisher gespart: ~{{value}}',
    shareBasket: 'Meine Warenkorb-Prognose: +{{pct}}%/Monat',
    shareItem: '{{product}} steigt um +{{pct}}% — spare ~{{save}} durch Vorratskauf',
    shareTotal: 'Könnte ~{{value}} sparen durch Vorabkauf',
    shareCta: 'Inflation schlagen → ai-budget.pl',
  },
  affordability: {
    yes: 'Ja, erschwinglich',
    no: 'Derzeit nicht erschwinglich',
    tight: 'Erschwinglich, aber knapp',
    delaysGoal: 'Erschwinglich, verzögert aber ein Ziel',
    waitUntil: 'Erschwinglich nach {{date}}',
  },
  healthScore: {
    title: 'Finanzielle Gesundheit',
    tapForDetails: 'Tippen für Details',
    notEnoughData: 'Noch nicht genug Daten',
    breakdown: 'Score-Übersicht',
    note: 'Score wird lokal aus Budgets, Einkommen, Zielen und Schulden berechnet.',
    label: {
      green: 'Sehr gut',
      yellow: 'Befriedigend',
      red: 'Handlungsbedarf',
    },
    component: {
      budgetAdherence: 'Budget-Einhaltung',
      savingsRate: 'Sparquote',
      goalProgress: 'Zielfortschritt',
      debtHealth: 'Schuldengesundheit',
    },
    detail: {
      budgetsOnTrack: 'Alle Budgets im Rahmen',
      budgetsOverLimit: '{{overCount}} von {{totalCount}} Budgets überschritten',
      noBudgets: 'Keine aktiven Budgets',
      savingsRateGood: '{{pct}}% des Einkommens gespart – super!',
      savingsRateLow: '{{pct}}% des Einkommens gespart (Ziel: 20%)',
      noIncome: 'Kein Einkommen diesen Monat',
      goalsOnTrack: 'Alle Ziele im Plan',
      goalsBehind: '{{onTrackCount}} von {{totalCount}} Zielen im Plan',
      noGoals: 'Keine aktiven Sparziele',
      noOverdueDebts: 'Keine überfälligen Schulden',
      overdueDebts: '{{overdueCount}} überfällige Schuld(en)',
    },
  },
  autoCapture: {
    title: 'Automatische Erfassung aus Benachrichtigungen',
    subtitle: 'Kartenzahlungen automatisch aus Bank-Push-Benachrichtigungen erfassen',
    privacy: 'Der Benachrichtigungstext wird auf dem Gerät verarbeitet. Nur die resultierende Ausgabe wird synchronisiert – die Rohdaten verlassen Ihr Gerät nie.',
    enable: 'Automatische Erfassung aktivieren',
    grantPermission: 'Benachrichtigungszugriff erteilen',
    permissionGranted: 'Benachrichtigungszugriff erteilt',
    banks: 'Unterstützte Banken',
    androidOnly: 'Die automatische Erfassung ist nur auf Android verfügbar. Verwenden Sie auf iOS den Belegscanner.',
    captured: '{{amount}} {{currency}} erfasst · {{merchant}}',
    reviewLast: 'Zuletzt erfasste Ausgaben',
    testTitle: 'Noch keine automatisch erfassten Ausgaben. Aktivieren Sie die Erfassung und führen Sie eine Kartenzahlung durch.',
  },
  familyFeed: {
    title: 'Familien-Feed',
    showAll: 'Alle anzeigen',
    today: 'Heute',
    yesterday: 'Gestern',
    expenses: '{{count}} Ausgaben · {{amount}}',
    incomes: '{{count}} Einnahmen · {{amount}}',
    proposedPurchase: 'hat einen Kauf vorgeschlagen',
    purchaseApproved: 'Kauf genehmigt',
    purchaseMade: 'Gekauft!',
    purchaseShort: 'Anfrage',
    purchaseRejected: 'Abgelehnt',
    typeExpense: 'Ausgabe',
    typeIncome: 'Einnahme',
    typeRequest: 'Kaufanfrage',
    noActivity: 'Noch keine Aktivität',
    noActivityDesc: 'Hier erscheinen Ausgaben und Käufe aller Kontomitglieder',
    expand: 'Details anzeigen',
    collapse: 'Details ausblenden',
    reactAdded: 'Reaktion hinzugefügt',
    reactRemoved: 'Reaktion entfernt',
  },
  priceHistory: {
    title: 'Deine Inflation',
    headline: '+{{pct}}%',
    trackedProducts: '{{count}} Produkte · {{period}}',
    period3m: '3M',
    period6m: '6M',
    period12m: '12M',
    periodAll: 'Alle',
    notEnoughData: 'Scanne einige Belege, um deine Inflation zu sehen',
    noDataForPeriod: 'Nicht genug Verlauf für diesen Zeitraum. Versuche einen kürzeren Bereich.',
    scanReceiptCta: 'Beleg scannen',
    storeComparison: 'Filialvergleich',
    priceHistoryChart: 'Preisverlauf',
    cheapestStore: 'Günstigste',
    manageProducts: 'Produkte verwalten',
    renameProduct: 'Produkt umbenennen',
    mergeProducts: 'Produkte zusammenführen',
    mergeInto: 'Zusammenführen in',
    merged: 'Zusammengeführt',
    deleteAlias: 'Originalname wiederherstellen',
    ignoreProduct: 'Aus Verfolgung entfernen',
    ignoreConfirm: '"{{name}}" aus der Preisverfolgung entfernen? Du kannst es später durch Zurücksetzen des Alias wiederherstellen.',
    removePrice: 'Diesen Preis entfernen',
    removePriceConfirm: 'Preis vom {{date}} ({{price}}) entfernen? Nützlich für einmalige Rabatte, die den Inflationsindex verfälschen würden.',
    noProducts: 'Noch keine verfolgten Produkte',
    showMore: '+{{count}} mehr',
    productsHint: 'Produkte werden aus Belegscans erkannt. Tippe auf ein Produkt, um es umzubenennen, oder verwende Merge, um Produktvarianten zu kombinieren.',
    resetAliasTitle: 'Namen zurücksetzen',
    resetAliasBody: 'Zum Originalnamen "{{name}}" zurücksetzen?',
    resetAlias: 'Zurücksetzen',
    mergeSelected: 'Merge {{count}} Produkte',
    purchasesCount: '{{count}} Käufe',
    reanalyzeWithAi: 'Mit KI analysieren',
    reanalyzing: 'Analysiere...',
    reanalyzeSuccess: '{{count}} Produkte aktualisiert',
    reanalyzeConfirm: 'KI generiert neue Produktnamen aus deinen Bon-Beschreibungen. Das kann einen Moment dauern.',
  },
  shoppingList: {
    title: 'Einkaufsliste',
    settingsSubtitle: 'Plane deinen Einkauf, finde den günstigsten Laden',
    planAShop: 'Einkauf planen',
    addItem: 'Artikel hinzufügen',
    addFreeText: 'Hinzufügen „{{text}}"',
    frequentlyBought: 'Häufig gekauft',
    searchProducts: 'Produkte suchen',
    emptyList: 'Füge deinen ersten Artikel hinzu',
    noLists: 'Noch keine Einkaufslisten',
    createFirstList: 'Liste erstellen',
    clearChecked: 'Erledigte entfernen',
    compareCta: 'Preise vergleichen',
    compareTitle: 'Wo es am günstigsten ist',
    comparePaywall: 'Der Preisvergleich zwischen Geschäften ist eine Pro-Funktion.',
    cheapest: 'Am günstigsten',
    coverage: '{{covered}}/{{total}} Artikel',
    stalePrices: 'Einige Preise sind veraltet',
    missingCount: '{{count}} hier ohne Preis',
    cheapestPerItem: 'Günstigster Preis pro Artikel',
    notTracked: 'Noch nicht erfasst',
    noPriceData: 'Noch keine Preisverlaufsdaten — scanne zuerst ein paar Kassenbons',
    comparing: 'Vergleiche…',
    newList: 'Neue Liste',
    deleteList: 'Liste löschen',
    deleteListConfirm: 'Diese Liste und alle ihre Artikel löschen?',
    switchList: 'Liste wechseln',
    renameList: 'Liste umbenennen',
    archiveList: 'Liste archivieren',
    archiveListConfirm: 'Diese Liste archivieren? Sie wird ausgeblendet, aber nicht gelöscht.',
    listName: 'Listenname',
    manageLists: 'Listen verwalten',
    quantity: 'Menge',
    restockTitle: 'Zeit zum Nachkaufen',
    mapTitle: 'Geschäfte auf der Karte',
    sortCheapest: 'Günstigste',
    sortNearby: 'In der Nähe',
    noStoreLocations: 'Noch keine Standorte — deine Geschäfte erscheinen hier, sobald Kassenbons Adressen enthalten',
    findNearby: 'In der Nähe finden',
    distanceKm: '{{km}} km entfernt',
    dealsTitle: 'Angebote für dich',
    dealDrop: '-{{pct}}%',
    autoCheckSectionTitle: 'Einkaufsliste',
    autoCheckFromReceipts: 'Automatisch von Belegen abhaken',
    autoCheckFromReceiptsDesc: 'Wenn du einen passenden Beleg scannst, werden bereits auf deiner Einkaufsliste stehende Artikel automatisch abgehakt.',
    templates: 'Vorlagen',
    saveAsTemplate: 'Aktuelle Liste als Vorlage speichern',
    templateNamePlaceholder: 'Name der Vorlage',
    noTemplates: 'Noch keine gespeicherten Vorlagen',
    templateItemCount: '{{count}} Artikel',
    templateApplied: 'Vorlage angewendet',
    templateAllAlreadyOnList: 'Alles aus dieser Vorlage steht bereits auf deiner Liste',
    templateAppliedSomeSkipped: '{{added}} hinzugefügt, {{skipped}} bereits auf der Liste',
    renameTemplate: 'Vorlage umbenennen',
    deleteTemplate: 'Vorlage löschen',
    deleteTemplateConfirm: 'Diese Vorlage löschen? Das kann nicht rückgängig gemacht werden.',
    shareList: 'Liste teilen',
    shareListAction: 'Link teilen',
    revokeLinkAction: 'Link widerrufen',
    linkRevoked: 'Link widerrufen',
    linkCopied: 'Link in die Zwischenablage kopiert',
    shareLinkFailed: 'Der Freigabelink konnte nicht erstellt oder aktualisiert werden. Bitte versuche es erneut.',
  },
  receiptCheck: {
    cardTitle_one: '{{count}} Artikel kostet mehr als üblich',
    cardTitle_other: '{{count}} Artikel kosten mehr als üblich',
    cardSubtitle: 'Etwa {{amount}} mehr, als du hier normalerweise zahlst — ein Blick auf den Kassenbon lohnt sich.',
    usually: 'normalerweise',
    youPaid: 'du hast bezahlt',
    difference: 'Differenz',
    lowConfidence: 'basierend auf nur zwei früheren Käufen',
    foundTotal: 'Dieses Jahr {{amount}} über deinen üblichen Preisen gefunden',
  },
  receiptSplit: {
    title: 'Diesen Kassenbon aufteilen',
    entryAction: 'Aufteilen',
    addPerson: 'Person hinzufügen',
    recentPeople: 'Zuletzt verwendet',
    personName: 'Name',
    yourShare: 'Dein Anteil',
    shareByAmount: 'Betrag',
    shareReset: 'Gleichmäßig',
    shareYou: 'Ich',
    shareOverAllocated: 'Die Anteile dieser Position übersteigen 100%',
    assignEveryone: 'Weise jeder Person mindestens eine Position zu',
    assignHint: 'Tippe auf eine Position und dann auf alle, die sie geteilt haben',
    tapPersonHint: 'Tippe alle an, die sich diese Position geteilt haben',
    chipItemCount_one: '{{count}} Position',
    chipItemCount_other: '{{count}} Positionen',
    equalMode: 'Gleichmäßig aufteilen',
    equalHint: 'Diese Ausgabe hat keine Positionen, daher wird sie gleichmäßig aufgeteilt.',
    createSplit: 'Links erstellen',
    shareWith: 'An {{name}} senden',
    copyAll: 'Alle Links kopieren',
    statusSent: 'Gesendet',
    statusOpened: 'Geöffnet',
    statusClaimed: 'Sagt, bezahlt zu haben',
    statusSettled: 'Beglichen',
    confirmPaid: 'Erhalt bestätigen',
    confirmedToast: 'Als beglichen markiert',
    cancelSplit: 'Aufteilung abbrechen',
    cancelConfirm: 'Diese Aufteilung abbrechen? Die Links funktionieren dann nicht mehr und die Schulden werden entfernt.',
    tooMany: 'Du kannst mit bis zu 20 Personen aufteilen.',
    overBill: 'Die Anteile ergeben zusammen mehr als die Rechnung.',
    encrypted: 'Aufteilen ist bei Ende-zu-Ende-verschlüsselten Konten nicht möglich, da der Server den Kassenbon nicht lesen kann.',
    offline: 'Du benötigst eine Verbindung, um die Links zu erstellen.',
    showQr: 'QR-Code anzeigen',
    qrTitle: 'Zum Teilen scannen',
    qrHint: 'Jeder am Tisch kann das scannen, um seinen eigenen Anteil zu sehen',
    qrShare: 'Link teilen',
    qrShareMessage: 'Scanne das, um deinen Anteil der Rechnung zu sehen:',
    flagUnknownItem: 'Ein Artikel',
    flagWholeShare: 'Sein/ihr ganzer Anteil',
    flagResolve: 'Erledigt',
    flagFixHint: 'Um zu ändern, wer wie viel schuldet, storniere die Aufteilung und erstelle sie neu.',
    editAssignment: 'Zuordnung bearbeiten',
    editAssignmentTitle: 'Wer hatte das?',
    editAssignmentSave: 'Speichern',
    editBlockedConfirmed: 'Das kann nicht geändert werden — jemand hat bereits eine Zahlung für diese Aufteilung bestätigt. Storniere sie stattdessen und erstelle sie neu.',
  },
  receiptCategorySplit: {
    title: 'Nach Kategorie aufteilen',
    edit: 'Kategorien ändern',
    itemCategory: 'Kategorie für diesen Artikel',
    unassigned: 'Nicht zugewiesen',
    dropped: 'Kategorien konnten nicht zugeordnet werden — ohne Aufteilung gespeichert',
    notSplit: 'Nicht automatisch aufgeteilt — Sie können die Kategorien selbst zuweisen',
    newCategory: 'Neue Kategorie',
  },
  shortcuts: {
    helpTitle: 'Tastenkürzel',
    showHelp: 'Tastenkürzel anzeigen',
    searchFocus: 'Suche fokussieren',
    newExpense: 'Neue Ausgabe',
    navigateRows: 'Zwischen Zeilen wechseln',
    openRow: 'Zeile öffnen',
    toggleRow: 'Zeile aus-/abwählen',
    closeDialog: 'Dialog schließen',
  },
} as const;
