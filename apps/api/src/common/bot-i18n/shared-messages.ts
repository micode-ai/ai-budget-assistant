...
export const sharedMessages: Record<string, Record<string, string>> = {
  ...
  // /categorize (bot-categorize-command) — sequential Yes/Skip/Stop review of
  // CategorizeSuggestionsService's batched proposal. Identical copy across all
  // three bots post markup-conversion, so it lives here rather than being
  // tripled into each module's own helpers/i18n.ts.
  categorizeNothingToDo: {
    en: '🎉 No uncategorized expenses right now — you\'re all caught up!',
    ru: '🎉 Расходов без категории нет — всё разобрано!',
    ua: '🎉 Витрат без категорії немає — усе розібрано!',
    de: '🎉 Keine unkategorisierten Ausgaben — alles erledigt!',
    es: '🎉 No hay gastos sin categoría — ¡todo al día!',
    fr: '🎉 Aucune dépense sans catégorie — tout est à jour !',
    pl: '🎉 Brak wydatków bez kategorii — wszystko uporządkowane!',
    be: '🎉 Выдаткаў без катэгорыі няма — усё разабрана!',
    nl: '🎉 Geen ongecategoriseerde uitgaven — helemaal bij!',
  },
  categorizeIntro: {
    en: '🗂️ Found {{groups}} group(s) covering {{count}} uncategorized expense(s). I\'ll go through them one by one — reply with the button to file, skip, or stop anytime and finish the rest in the app.',
    ru: '🗂️ Найдено групп: {{groups}}, расходов без категории: {{count}}. Разберём их по одной — можно распределить, пропустить или в любой момент остановиться и доразобрать в приложении.',
    ua: '🗂️ Знайдено груп: {{groups}}, витрат без категорії: {{count}}. Розберемо їх по одній — можна розподілити, пропустити або будь-коли зупинитися й доробити в застосунку.',
    de: '🗂️ {{groups}} Gruppe(n) mit insgesamt {{count}} unkategorisierten Ausgabe(n) gefunden. Wir gehen sie einzeln durch — zuweisen, überspringen oder jederzeit stoppen und den Rest in der App erledigen.',
    es: '🗂️ Se encontraron {{groups}} grupo(s) con {{count}} gasto(s) sin categoría. Los revisaremos uno a uno — puedes archivar, omitir o detenerte cuando quieras y terminar el resto en la app.',
    fr: '🗂️ {{groups}} groupe(s) trouvé(s), couvrant {{count}} dépense(s) sans catégorie. On les passe en revue un par un — classez, ignorez, ou arrêtez à tout moment pour finir le reste dans l\'appli.',
    pl: '🗂️ Znaleziono grup: {{groups}}, wydatków bez kategorii: {{count}}. Przejdziemy je po kolei — możesz przypisać, pominąć albo zatrzymać się w dowolnym momencie i dokończyć w aplikacji.',
    be: '🗂️ Знойдзена груп: {{groups}}, выдаткаў без катэгорыі: {{count}}. Разбяром іх па адной — можна размеркаваць, прапусціць або спыніцца ў любы момант і дараабіць у праграме.',
    nl: '🗂️ {{groups}} groep(en) gevonden met {{count}} ongecategoriseerde uitgave(n). We gaan ze één voor één langs — toewijzen, overslaan, of op elk moment stoppen en de rest in de app afronden.',
  },
  categorizeLimitNote: {
    en: '\n\n⚠️ Today\'s AI sorting limit is reached — only matches found without AI are shown.',
    ru: '\n\n⚠️ Дневной лимит AI-разбора исчерпан — показаны только совпадения, найденные без ИИ.',
    ua: '\n\n⚠️ Денний ліміт AI-розбору вичерпано — показані лише збіги, знайдені без ШІ.',
    de: '\n\n⚠️ Das heutige KI-Sortierlimit ist erreicht — es werden nur ohne KI gefundene Treffer angezeigt.',
    es: '\n\n⚠️ Se alcanzó el límite diario de clasificación con IA — solo se muestran coincidencias encontradas sin IA.',
    fr: '\n\n⚠️ La limite quotidienne de tri IA est atteinte — seules les correspondances trouvées sans IA sont affichées.',
    pl: '\n\n⚠️ Osiągnięto dzienny limit sortowania AI — pokazane są tylko dopasowania znalezione bez AI.',
    be: '\n\n⚠️ Дзённы ліміт AI-разбору вычарпаны — паказаны толькі супадзенні, знойдзеныя без ІІ.',
    nl: '\n\n⚠️ De dagelijkse AI-sorteerlimiet is bereikt — alleen zonder AI gevonden overeenkomsten worden getoond.',
  },
  categorizeGroupPromptExisting: {
    en: '📂 {{count}} expense(s) look like <b>{{name}}</b> — file them there?',
    ru: '📂 Похоже, {{count}} расход(ов) относятся к категории <b>{{name}}</b> — распределить туда?',
    ua: '📂 Схоже, {{count}} витрат(и) належать до категорії <b>{{name}}</b> — розподілити туди?',
    de: '📂 {{count}} Ausgabe(n) sehen nach <b>{{name}}</b> aus — dort einordnen?',
    es: '📂 {{count}} gasto(s) parecen ser de <b>{{name}}</b> — ¿los archivamos ahí?',
    fr: '📂 {{count}} dépense(s) ressemblent à <b>{{name}}</b> — les classer là ?',
    pl: '📂 {{count}} wydatek(ów) wygląda na <b>{{name}}</b> — przypisać je tam?',
    be: '📂 Падобна, {{count}} выдатак(аў) належыць да катэгорыі <b>{{name}}</b> — размеркаваць туды?',
    nl: '📂 {{count}} uitgave(n) lijken op <b>{{name}}</b> — daar onderbrengen?',
  },
  categorizeGroupPromptNew: {
    en: '🆕 {{count}} expense(s) could form a new category <b>{{name}}</b> — create it and file them?',
    ru: '🆕 {{count}} расход(ов) могут образовать новую категорию <b>{{name}}</b> — создать её и распределить туда?',
    ua: '🆕 {{count}} витрат(и) можуть утворити нову категорію <b>{{name}}</b> — створити її й розподілити туди?',
    de: '🆕 {{count}} Ausgabe(n) könnten eine neue Kategorie <b>{{name}}</b> bilden — erstellen und einordnen?',
    es: '🆕 {{count}} gasto(s) podrían formar una nueva categoría <b>{{name}}</b> — ¿la creamos y los archivamos?',
    fr: '🆕 {{count}} dépense(s) pourraient former une nouvelle catégorie <b>{{name}}</b> — la créer et les classer ?',
    pl: '🆕 {{count}} wydatek(ów) mógłby utworzyć nową kategorię <b>{{name}}</b> — utworzyć ją i przypisać?',
    be: '🆕 {{count}} выдатак(аў) могуць утварыць новую катэгорыю <b>{{name}}</b> — стварыць яе і размеркаваць туды?',
    nl: '🆕 {{count}} uitgave(n) zouden een nieuwe categorie <b>{{name}}</b> kunnen vormen — aanmaken en onderbrengen?',
  },
  categorizeYesBtn: {
    en: '✅ Yes',
    ru: '✅ Да',
  ...
  categorizeApplied: {
    en: '✅ Filed {{count}} expense(s) under <b>{{name}}</b>.',
    ru: '✅ {{count}} расход(ов) отнесены к категории <b>{{name}}</b>.',
    ua: '✅ {{count}} витрат(и) віднесено до категорії <b>{{name}}</b>.',
    de: '✅ {{count}} Ausgabe(n) unter <b>{{name}}</b> eingeordnet.',
    es: '✅ {{count}} gasto(s) archivado(s) en <b>{{name}}</b>.',
    fr: '✅ {{count}} dépense(s) classée(s) dans <b>{{name}}</b>.',
    pl: '✅ {{count}} wydatek(ów) przypisano do <b>{{name}}</b>.',
    be: '✅ {{count}} выдатак(аў) аднесена да катэгорыі <b>{{name}}</b>.',
    nl: '✅ {{count}} uitgave(n) ondergebracht bij <b>{{name}}</b>.',
  },
  categorizeSkipped: {
    en: '⏭️ Skipped <b>{{name}}</b> — still uncategorized.',
    ru: '⏭️ Пропущено: <b>{{name}}</b> — по-прежнему без категории.',
    ua: '⏭️ Пропущено: <b>{{name}}</b> — досі без категорії.',
    de: '⏭️ Übersprungen: <b>{{name}}</b> — bleibt unkategorisiert.',
    es: '⏭️ Omitido: <b>{{name}}</b> — sigue sin categoría.',
    fr: '⏭️ Ignoré : <b>{{name}}</b> — toujours sans catégorie.',
    pl: '⏭️ Pominięto: <b>{{name}}</b> — nadal bez kategorii.',
    be: '⏭️ Прапушчана: <b>{{name}}</b> — па-ранейшаму без катэгорыі.',
    nl: '⏭️ Overgeslagen: <b>{{name}}</b> — blijft ongecategoriseerd.',
  },
  categorizeDone: {
    en: '🏁 Done. Filed {{appliedExpenses}} expense(s) across {{appliedGroups}} group(s). {{remaining}} still uncategorized — run the categorize command anytime, or finish them in the app.',
    ru: '🏁 Готово. Разобрано {{appliedExpenses}} расход(ов) в {{appliedGroups}} групп(ах). Осталось без категории: {{remaining}} — запустите разбор снова в любой момент или доразберите в приложении.',
    ua: '🏁 Готово. Розібрано {{appliedExpenses}} витрат(и) у {{appliedGroups}} груп(ах). Залишилося без категорії: {{remaining}} — запустіть команду знову будь-коли або доробіть у застосунку.',
    de: '🏁 Fertig. {{appliedExpenses}} Ausgabe(n) in {{appliedGroups}} Gruppe(n) eingeordnet. {{remaining}} weiterhin unkategorisiert — jederzeit erneut ausführen oder den Rest in der App erledigen.',
    es: '🏁 Listo. Se archivaron {{appliedExpenses}} gasto(s) en {{appliedGroups}} grupo(s). Quedan {{remaining}} sin categoría — vuelve a ejecutarlo cuando quieras, o termina en la app.',
    fr: '🏁 Terminé. {{appliedExpenses}} dépense(s) classée(s) dans {{appliedGroups}} groupe(s). {{remaining}} restent sans catégorie — relancez la commande à tout moment ou terminez dans l\'appli.',
    pl: '🏁 Gotowe. Przypisano {{appliedExpenses}} wydatek(ów) w {{appliedGroups}} grupie/grupach. Pozostało bez kategorii: {{remaining}} — uruchom ponownie w dowolnym momencie albo dokończ w aplikacji.',
    be: '🏁 Гатова. Разабрана {{appliedExpenses}} выдатак(аў) у {{appliedGroups}} групе(ах). Засталося без катэгорыі: {{remaining}} — запусціце разбор зноў у любы момант або дараабіце ў праграме.',
    nl: '🏁 Klaar. {{appliedExpenses}} uitgave(n) ondergebracht in {{appliedGroups}} groep(en). {{remaining}} nog ongecategoriseerd — voer het commando later opnieuw uit, of rond de rest af in de app.',
  },
  categorizeStoppedSummary: {
    en: '🛑 Stopped. Filed {{appliedExpenses}} expense(s) across {{appliedGroups}} group(s) so far. {{remaining}} left — run the categorize command anytime, or finish them in the app.',
    ru: '🛑 Остановлено. Успели разобрать {{appliedExpenses}} расход(ов) в {{appliedGroups}} групп(ах). Осталось: {{remaining}} — запустите разбор снова в любой момент или доразберите в приложении.',
    ua: '🛑 Зупинено. Встигли розібрати {{appliedExpenses}} витрат(и) у {{appliedGroups}} груп(ах). Залишилося: {{remaining}} — запустіть команду знову будь-коли або доробіть у застосунку.',
    de: '🛑 Gestoppt. Bisher {{appliedExpenses}} Ausgabe(n) in {{appliedGroups}} Gruppe(n) eingeordnet. {{remaining}} übrig — jederzeit erneut ausführen oder den Rest in der App erledigen.',
    es: '🛑 Detenido. Se archivaron {{appliedExpenses}} gasto(s) en {{appliedGroups}} grupo(s) hasta ahora. Quedan {{remaining}} — vuelve a ejecutarlo cuando quieras, o termina en la app.',
    fr: '🛑 Arrêté. {{appliedExpenses}} dépense(s) classée(s) jusqu\'ici dans {{appliedGroups}} groupe(s). {{remaining}} restantes — relancez la commande à tout moment ou terminez dans l\'appli.',
    pl: '🛑 Zatrzymano. Do tej pory przypisano {{appliedExpenses}} wydatek(ów) w {{appliedGroups}} grupie/grupach. Zostało: {{remaining}} — uruchom ponownie w dowolnym momencie albo dokończ w aplikacji.',
    be: '🛑 Спынена. Пакуль разабрана {{appliedExpenses}} выдатак(аў) у {{appliedGroups}} групе(ах). Засталося: {{remaining}} — запусціце зноў у любы момант або дараабіце ў праграме.',
    nl: '🛑 Gestopt. Tot nu toe {{appliedExpenses}} uitgave(n) ondergebracht in {{appliedGroups}} groep(en). {{remaining}} over — voer het commando later opnieuw uit, of rond de rest af in de app.',
  },
  categorizeAlreadyHandled: {
    en: 'That step already finished. Start a new pass with the categorize command.',
    ru: 'Этот шаг уже обработан. Запустите разбор заново командой categorize.',
    ua: 'Цей крок уже оброблено. Запустіть розбір знову командою categorize.',
    de: 'Dieser Schritt ist bereits abgeschlossen. Starte einen neuen Durchlauf mit dem categorize-Befehl.',
    es: 'Ese paso ya se completó. Inicia un nuevo pase con el comando categorize.',
    fr: 'Cette étape est déjà terminée. Relancez avec la commande categorize.',
