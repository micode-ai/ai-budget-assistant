# AI Budget Assistant — Benutzerhandbuch

> AI Budget Assistant ist dein intelligenter Finanzbegleiter. Erfasse Ausgaben manuell, per Spracheingabe oder durch Scannen von Belegen. Verwalte Budgets in mehreren Wahrungen, erhalte KI-gestutzte Ausgabeneinblicke und arbeite mit Familie oder Teammitgliedern an gemeinsamen Konten zusammen.

## Hauptfunktionen

- **Intelligente Ausgabenverfolgung** — Ausgaben manuell, per Spracheingabe oder durch Scannen von Belegen mit KI-gestutzter OCR hinzufugen
- **Multi-Wahrung Geldborse** — Salden in USD, EUR, PLN, GBP, UAH und RUB mit aktuellen Wechselkursen verfolgen
- **KI-Chat** — Fragen zu deinen Finanzen stellen und personalisierte Beratung erhalten
- **Ausgabengeschichte** — KI-generierte narrative Berichte mit Erfolgen und Einblicken
- **Budgets** — Ausgabenlimits mit Echtzeit-Fortschrittsverfolgung und Warnungen festlegen
- **Interaktive Analysen** — Diagramme mit Drill-Down, Kategorieaufschlusselungen und Trendanalyse
- **Mehrere Konten** — Personlich, Gemeinsam und Geschaftlich mit rollenbasierter Zugriffskontrolle (Eigentumer, Bearbeiter, Betrachter)
- **Offline-First** — Daten werden lokal gespeichert und automatisch synchronisiert, wenn du online bist
- **7 Sprachen** — English, Deutsch, Espanol, Francais, Polski, Русский, Українська

## Inhaltsverzeichnis

1. [Erste Schritte](./01-getting-started.md) — Registrierung, Anmeldung, erste Schritte
2. [Ubersicht](./02-dashboard.md) — Startbildschirm und Schnellaktionen
3. [Ausgaben & Einkommen](./03-expenses-and-income.md) — Transaktionen verwalten
4. [Spracheingabe & Beleg scannen](./04-voice-and-receipt.md) — KI-gestutzte Ausgabenerfassung
5. [Budgets](./05-budgets.md) — Ausgabenlimits erstellen und verfolgen
6. [Analysen](./06-analytics.md) — Diagramme, Einblicke und Datenexploration
7. [KI-Chat](./07-ai-chat.md) — Dein personlicher Finanzassistent
8. [Ausgabengeschichte](./08-spending-story.md) — KI-generierte Finanzberichte
9. [Konten](./09-accounts.md) — Personliche, gemeinsame und geschaftliche Konten
10. [Geldborse & Umtausch](./10-wallet-and-exchange.md) — Multi-Wahrungsverwaltung
11. [Einstellungen](./11-settings.md) — Profil, Erscheinungsbild und Praferenzen
12. [Abonnement-Plane](./12-subscription.md) — Free, Pro und Business Plane

## Unterstutzte Wahrungen

USD, EUR, PLN, GBP, UAH, RUB

## Unterstutzte Plattformen

iOS, Android, Web
