# AI Budget Assistant -- Guide Utilisateur

> AI Budget Assistant est votre compagnon financier intelligent. Suivez vos depenses manuellement, par la voix ou en scannant des recus. Gerez vos budgets dans plusieurs devises, obtenez des analyses de depenses alimentees par l'IA et collaborez avec votre famille ou votre equipe grace aux comptes partages.

## Fonctionnalites principales

- **Suivi intelligent des depenses** -- ajoutez des depenses manuellement, par saisie vocale ou en scannant des recus grace a l'OCR par IA
- **Portefeuille multi-devises** -- suivez vos soldes en USD, EUR, PLN, GBP, UAH et RUB avec des taux de change en temps reel
- **Chat IA** -- posez des questions sur vos finances et recevez des conseils personnalises
- **Histoire des depenses** -- rapports narratifs generes par l'IA avec realisations et analyses
- **Gamification** -- succes, series quotidiennes, XP/niveaux et collection de badges pour rester motive
- **Budgets** -- definissez des limites de depenses avec suivi en temps reel et alertes
- **Analyses interactives** -- graphiques avec exploration detaillee, repartition par categorie et analyse des tendances
- **Comptes multiples** -- comptes Personnel, Partage et Professionnel avec controle d'acces base sur les roles (Proprietaire, Editeur, Observateur)
- **Mode hors-ligne** -- les donnees sont enregistrees localement et synchronisees automatiquement une fois en ligne
- **7 langues** -- English, Deutsch, Espanol, Francais, Polski, Pyccкий, Yкpaїнcькa

## Table des matieres

1. [Premiers pas](./01-getting-started.md) -- inscription, connexion, premieres etapes
2. [Tableau de bord](./02-dashboard.md) -- apercu de l'ecran d'accueil et actions rapides
3. [Depenses et revenus](./03-expenses-and-income.md) -- gestion des transactions
4. [Saisie vocale et scan de recus](./04-voice-and-receipt.md) -- saisie de depenses assistee par l'IA
5. [Budgets](./05-budgets.md) -- creation et suivi des limites de depenses
6. [Analyses](./06-analytics.md) -- graphiques, aperçus et exploration des donnees
7. [Chat IA](./07-ai-chat.md) -- votre assistant financier personnel
8. [Histoire des depenses](./08-spending-story.md) -- recits financiers generes par l'IA
9. [Comptes](./09-accounts.md) -- comptes personnel, partage et professionnel
10. [Portefeuille et change](./10-wallet-and-exchange.md) -- gestion multi-devises
11. [Parametres](./11-settings.md) -- profil, apparence et preferences
12. [Abonnement](./12-subscription.md) -- plans Gratuit, Pro et Business
13. [Succes et Gamification](./13-gamification.md) -- series, XP, niveaux et badges

## Devises prises en charge

USD, EUR, PLN, GBP, UAH, RUB

## Plateformes prises en charge

iOS, Android, Web
