# AI Budget Assistant -- Przewodnik uzytkownika

> AI Budget Assistant to Twoj inteligentny towarzysz finansowy. Sledz wydatki recznie, glosowo lub skanujac paragony. Zarzadzaj budzetami w wielu walutach, otrzymuj analizy wydatkow oparte na AI i wspolpracuj z rodzina lub czlonkami zespolu na wspolnych kontach.

## Najwazniejsze funkcje

- **Inteligentne sledzenie wydatkow** -- dodawaj wydatki recznie, glosowo lub skanujac paragony dzieki OCR opartemu na AI
- **Portfel wielowalutowy** -- sledz salda w USD, EUR, PLN, GBP, UAH i RUB z aktualnymi kursami wymiany
- **Czat AI** -- zadawaj pytania o swoje finanse i otrzymuj spersonalizowane porady
- **Historia wydatkow** -- generowane przez AI raporty narracyjne z osiagnieciami i wnioskami
- **Budzety** -- ustalaj limity wydatkow ze sledzeniem postepu w czasie rzeczywistym i alertami
- **Interaktywna analityka** -- wykresy z drilldownem, podzialem na kategorie i analiza trendow
- **Wiele kont** -- konta Osobiste, Wspolne i Firmowe z dostepem opartym na rolach (Wlasciciel, Edytor, Obserwator)
- **Offline-first** -- dane zapisuja sie lokalnie i synchronizuja automatycznie po polaczeniu z siecia
- **7 jezykow** -- English, Deutsch, Espanol, Francais, Polski, Russkij, Ukrainska

## Spis tresci

1. [Pierwsze kroki](./01-getting-started.md) -- rejestracja, logowanie, pierwsze kroki
2. [Pulpit](./02-dashboard.md) -- ekran glowny i szybkie akcje
3. [Wydatki i przychody](./03-expenses-and-income.md) -- zarzadzanie transakcjami
4. [Wprowadzanie glosowe i skanowanie paragonow](./04-voice-and-receipt.md) -- dodawanie wydatkow z pomoca AI
5. [Budzety](./05-budgets.md) -- tworzenie i sledzenie limitow wydatkow
6. [Analityka](./06-analytics.md) -- wykresy, wnioski i eksploracja danych
7. [Czat AI](./07-ai-chat.md) -- Twoj osobisty asystent finansowy
8. [Historia wydatkow](./08-spending-story.md) -- narracje finansowe generowane przez AI
9. [Konta](./09-accounts.md) -- konta osobiste, wspolne i firmowe
10. [Portfel i wymiana walut](./10-wallet-and-exchange.md) -- zarzadzanie wieloma walutami
11. [Ustawienia](./11-settings.md) -- profil, wyglad i preferencje
12. [Plany subskrypcji](./12-subscription.md) -- plany Free, Pro i Business

## Obslugiwane waluty

USD, EUR, PLN, GBP, UAH, RUB

## Obslugiwane platformy

iOS, Android, Web
