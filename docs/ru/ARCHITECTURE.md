...
Веб-сборка мобильного приложения на Expo (`app.ai-budget.pl`) переключается на
десктопную раскладку при ширине окна **от 1024px** (`DESKTOP_MIN_WIDTH`, наряду с
`FACET_RAIL_MIN_WIDTH`, в `src/components/webLayout.constants.ts`). Это не второй
продукт — это те же экраны, те же хранилища, тот же API, но размещённые для мыши и
широкого окна, а не для пальца. Ниже этого порога, а также на нативных платформах,
отрисовка не меняется.

- **Разделение выполняется на уровне компонента и обеспечивается бандлером, а не
  веткой на уровне маршрута.** У каждого экрана остаётся один тонкий файл
  маршрута; соседний `*.web.tsx` решает, мобильная это раскладка или десктопная,
  исключительно по ширине, а `*Mobile.tsx` хранит JSX телефона в единственном
  определении, которое импортируют оба платформенных файла. Ничто под `src/` не
  может импортировать что-либо из `app/`.
- **`WebShell` / `WebTopBar` / `WebSidebar`** отвечают за обвязку (навигация в
  верхней панели, без левой колонки, один общий скролл страницы на экран). Ниже
  `DESKTOP_MIN_WIDTH` `WebShell` возвращает свои дочерние элементы без изменений —
  это механизм, который гарантирует, что отрисовка на телефоне не может разойтись,
  а не обещание вручную поддерживать её в актуальном состоянии.
- **Экраны с десктопной раскладкой на сегодня**: список транзакций (боковая
  панель фасетов со счётчиками, строки, сгруппированные по дням), главная панель,
  «Настройки» (двухпанельная оболочка — левая навигация плюс выбранный экран как
  правая панель) и чат (панель разговоров плюс ограниченная по ширине колонка для
  чтения).
- **Диалог размещает существующий компонент через ref-хендл и никогда не
  реализует редактирование заново.** Десктопный диалог «деталей» или «создания»
  вызывает тот же `triggerSave()` компонента, который уже вызывает мобильный
  маршрут, поэтому редактирование не может вести себя по-разному в зависимости от
  того, с какой платформы оно открыто.
- **Ни один компонент не рендерится в CI этого репозитория**
  (`react-test-renderer` отсутствует), поэтому любое правило, которое можно
  проверить численно — брейкпоинт, количество по фасету, дневной промежуточный
  итог — живёт в чистом, покрытом юнит-тестами модуле, и десктопный слой вызывает
  этот модуль, а не пересказывает правило инлайн.
...
│   ├── exchange-rate-alerts/    # «Сообщить, когда EUR/PLN дойдёт до 4.35» — личный ресурс
│   │   ├── exchange-rate-alerts.controller.ts
│   │   ├── exchange-rate-alerts.service.ts
│   │   └── exchange-rate-alert.cron.ts   # раз в час, группирует по fromCurrency
...
// Закрепление на каждого участника, а не колонка на ChatConversation — список
// разговоров неоднороден (в нём может быть чужой общий разговор), поэтому
// колонка либо лишала бы закрепления не-создателя, либо превращала бы
// закрепление одного участника в общую запись, которую никто другой не может
// отменить. Составной первичный ключ И ЕСТЬ уникальность
// «одно закрепление на человека на разговор» (отдельный уникальный индекс не нужен).
model ChatConversationPin {
  userId         String
  conversationId String
  pinnedAt       DateTime @default(now())  // миграция 20260907120000_add_chat_conversation_pins
...
- *залог* — за возвратную тару (`kaucja`), печатается отдельным блоком, никогда
  не является позицией, но входит в сумму к оплате. Он никогда не
  распределяется по группам, как скидка, и никогда не забирает резидуал — обе
  эти поправки касаются только товарных групп, ещё до того, как депозит
  добавляется отдельной группой (см. ниже).
...
**Депозит — тоже отдельная категория, а не только поправка в гейте.** Если чек
его сообщает, депозит добавляется отдельной точной группой — уже после того,
как скидка распределена по группам, а резидуал приписан крупнейшей, то есть
депозит не забирает долю ни от той, ни от другой поправки, — и засчитывается в
правило «минимум двух категорий» как при автоматической разбивке, так и при
ручной переразметке строки пользователем: чек, который иначе состоял бы из
одной категории (как правило, продуктов), теперь распадается на неё плюс
депозит. Название категории берётся из небольшой таблицы, зашитой прямо в API,
по одному слову на локаль (`Deposit`/`Kaucja`/`Pfand`/«Залог за тару»/…), и
определяется по языку **владельца аккаунта** — никогда сканирующего участника,
— так что общий аккаунт сходится на одной категории депозита независимо от
того, кто сканировал; если в аккаунте уже есть категория с таким же названием,
используется она, а не создаётся дубликат. Депозит намеренно никогда не идёт
через путь предложения категорий моделью, поэтому ему не нужно преодолевать
10%-й порог существенности, обязательный для предложенной категории: депозит
обычно составляет 1–2% чека и при таком пороге отбрасывался бы всегда, из-за
чего вся функция могла бы никогда не проявиться. Независимо от того, получилась
ли разбивка вообще, сумма депозита сохраняется прямо на расходе, поэтому она
видна даже на чеке, чьи позиции не сходятся с итогом достаточно точно для
разбивки.
