...
5. **Переиспользование**: чистая функция-формула (`computeSafeToSpend`) вынесена в `packages/shared-utils`, чтобы офлайн-фолбэк мобильного приложения мог вычислить то же число локально, не импортируя код API.
