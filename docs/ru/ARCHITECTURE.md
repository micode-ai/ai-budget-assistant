...
- **Типы аккаунтов**: `personal` (личный), `business` (бизнес), `shared` (общий)
...
- **Отслеживание AI-использования**: Каждый AI-запрос учитывается с единицами стоимости
...
- **Гарды**:
  ...
  - `AiUsageGuard` — проверяет, что пользователь не превысил лимит AI-запросов
...
- **Технология**: Android-виджеты через react-native-android-widget
- **3 размера**:
  - **Маленький** — итог расходов за сегодня
  - **Средний** — недельный график расходов
  - **Большой** — прогресс бюджетов + топ категории
- **Data bridge**: `widgetData.ts` сериализует данные из API/локального хранилища для виджетов
- **Фоновое обновление**: Данные виджетов обновляются каждые 30 минут через expo-background-fetch
