...
## Key concepts
- **JWT pair** — short-lived access token + long-lived refresh token; refresh is transparent to the user via `HttpClient` (in `services/http-client.ts`)
...
## Where to look first
Token lifecycle issues → `apps/mobile/src/services/http-client.ts` and `apps/mobile/src/stores/authStore.ts`. Server-side auth logic → `apps/api/src/modules/auth/auth.service.ts`.
