# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the "Mapa wydatków" (Expense Map / geolocation)
feature in AI Budget Assistant. Phone screenshots cycle inside a phone bezel
with vertical-slide transitions, warm-dark gradient + orange glow.

Scene order:
    0. map      — Expenses tab in Map mode, clickable pins + popup (DOZ Apteka)
    1. detail   — expense detail with mini-map + auto address from the receipt
    2. picker   — manual pin picker ("Dotknij mapy, aby ustawić pinezkę")

Outputs in docs/marketing/creatives/maps/renders/pl/:
    maps-reel.mp4 / .gif  (+ -4x5 variants)

Usage (from docs/marketing/scripts/):
    python build_maps_reel.py both
    python build_maps_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", "maps", "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")
DRAFT = os.path.join(MK, "creatives", "draft", "maps")

SRC = [
    os.path.join(DRAFT, "photo_1_2026-07-05_19-22-56.jpg"),  # map with pins
    os.path.join(DRAFT, "photo_2_2026-07-05_19-22-56.jpg"),  # detail mini-map + address
    os.path.join(DRAFT, "photo_3_2026-07-05_19-22-56.jpg"),  # manual pin picker
]
...
PHASES = {
    "A": ("MAPA WYDATKÓW",    "Zobacz, gdzie\nwydajesz pieniądze"),
    "B": ("PROSTO Z PARAGONU", "Adres sklepu\nz Twojego paragonu"),
    "C": ("TWOJA KONTROLA",    "Albo ustaw pinezkę\nsamodzielnie"),
}
SCENE_PHASE = ["A", "B", "C"]
PILLS = ["Mapa", "Z paragonu", "Pinezka"]
