# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the "Twoja inflacja" (Personal Inflation Index)
feature in AI Budget Assistant. Four phone screenshots cycle inside a phone
...
Scene order:
    0. index    — Analityka -> Twoja inflacja (-0.2%, 51 produktow, list)
    1. product  — "Mleko" price history chart + store comparison
    2. manage   — "Zarzadzaj produktami" (from receipts, Przeanalizuj z AI)
    3. rename    — rename-product bottom sheet (you stay in control)

Outputs in docs/marketing/creatives/inflation/renders/pl/:
    inflation-reel.mp4       1080x1920, 30fps
    inflation-reel.gif       600x1067
    inflation-reel-4x5.mp4
    inflation-reel-4x5.gif

Usage:
    python build_inflation_reel.py               # gif 9:16
    python build_inflation_reel.py mp4
    python build_inflation_reel.py both
    python build_inflation_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", "inflation", "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")

SRC = [
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_1_2026-07-03_14-45-57.jpg",  # index
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_2_2026-07-03_14-45-57.jpg",  # product / prices
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_3_2026-07-03_14-45-57.jpg",  # manage products
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_4_2026-07-03_14-45-57.jpg",  # rename sheet
]
...
PHASES = {
    "A": ("TWOJA INFLACJA",  "Policz ją\nz własnych paragonów"),
    "B": ("CENY I SKLEPY",   "Zobacz historię cen\ni gdzie taniej"),
    "C": ("AUTOMATYCZNIE",   "Wszystko\nz Twoich paragonów"),
}
SCENE_PHASE = ["A", "B", "C", "C"]
PILLS = ["Inflacja", "Ceny", "Automatyzacja"]
    ...
    for i in range(n):
        ...
        for t in (0.3, 0.55, 0.78, 1.0):
            frames.append(compose(bg, mask, shots, tops, i, t)
                          .convert("RGB").resize((gw, gh), Image.LANCZOS))
            durs.append(60)
    path = os.path.join(outdir, f"inflation-reel{suffix}.gif")
...
def render_mp4(suffix=""):
    ...
    path = os.path.join(outdir, f"inflation-reel{suffix}.mp4")
