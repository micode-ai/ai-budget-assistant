# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the "Podsumowanie roku" (Financial Wrapped) feature
in AI Budget Assistant. Six phone screenshots cycle inside a phone bezel with
vertical-slide transitions, warm-dark gradient + orange glow.

Icon layout (matches purchase-list / trip / family-feed style):
  - Top right : app icon with orange ring
  - Bottom    : MICODE badge + mi-code.pl

Scene order:
    0. entry  — Analytics tab, "Podsumowanie roku · Spójrzmy razem na twój rok"
    1. intro  — "Twój 2026 w liczbach" opening card
    2. place  — "Twoje ulubione miejsce · Biedronka · 88 wizyt"
    3. scans  — "229 zeskanowane paragony · Każde skanowanie precyzuje statystyki"
    4. streak — "21 dni · Najdłuższa passa śledzenia"
    5. share  — "Taki był twój 2026! · Ukryj kwoty · Udostępnij"

Outputs in docs/marketing/creatives/wrapper/renders/pl/:
    wrapped-reel.mp4       1080x1920, 30fps
    wrapped-reel.gif       600x1067
    wrapped-reel-4x5.mp4
    wrapped-reel-4x5.gif

Usage:
    python build_wrapped_reel.py               # gif 9:16
    python build_wrapped_reel.py mp4
    python build_wrapped_reel.py both
    python build_wrapped_reel.py both 4:5
"""
...
FEATURE = "wrapped"
FOLDER  = "wrapper"          # user-created screenshots folder
OUT   = os.path.join(MK, "creatives", FOLDER, "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")

SHOTS_DIR = os.path.join(MK, "creatives", FOLDER)
SRC = [
    os.path.join(SHOTS_DIR, "photo_1_2026-07-12_11-11-06.jpg"),  # analytics entry card
    os.path.join(SHOTS_DIR, "photo_2_2026-07-12_11-11-06.jpg"),  # intro "Twój 2026 w liczbach"
    os.path.join(SHOTS_DIR, "photo_3_2026-07-12_11-11-06.jpg"),  # ulubione miejsce Biedronka
    os.path.join(SHOTS_DIR, "photo_4_2026-07-12_11-11-06.jpg"),  # 229 zeskanowane paragony
    os.path.join(SHOTS_DIR, "photo_5_2026-07-12_11-11-06.jpg"),  # 21 dni passa śledzenia
    os.path.join(SHOTS_DIR, "photo_6_2026-07-12_11-11-06.jpg"),  # outro / Udostępnij
]

# Wrapped cards ARE the shareable content — nothing to blur.
BLUR_BANDS = {}
...
PHASES = {
    "A": ("PODSUMOWANIE ROKU", "Twój rok\nw liczbach"),
    "B": ("TWOJE STATYSTYKI",  "Ulubione miejsca\ni nawyki"),
    "C": ("PODZIEL SIĘ",       "Pochwal się\nswoim rokiem"),
}
SCENE_PHASE = ["A", "A", "B", "B", "B", "C"]
PILLS = ["Twój rok", "Statystyki", "Udostępnij"]
