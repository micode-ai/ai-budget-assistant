# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the Analytics / "Twoja historia wydatków"
feature in AI Budget Assistant (analytics overview + AI-narrated month-in-review:
totals, trend vs last month, category breakdown, income-vs-expense, warnings).
Four phone screenshots cycle inside a phone bezel with vertical-slide
transitions over a warm-dark gradient + orange glow.

Icon layout (matches gamification / inflation style):
  - Top right : app icon with orange ring
  - Bottom    : MICODE badge + mi-code.pl

Scene order:
    0. overview  — Analityka: 8 328,97 zł, "36% mniej", "Zobacz swoją historię"
    1. story      — Historia wydatków: AI narrative + total 12 332,88 PLN
    2. categories  — Wydatki wg kategorii: donut (Mieszkanie 38,7%)
    3. compare      — Przychody vs Wydatki + ostrzeżenie o przekroczeniu budżetu

Outputs in docs/marketing/creatives/analytics-spend-history/renders/pl/:
    spend-history-reel.mp4       1080x1920, 30fps
    spend-history-reel.gif       600x1067
    spend-history-reel-4x5.mp4
    spend-history-reel-4x5.gif

Usage:
    python build_analytics_spend_history_reel.py               # gif 9:16
    python build_analytics_spend_history_reel.py mp4
    python build_analytics_spend_history_reel.py both
    python build_analytics_spend_history_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", "analytics-spend-history", "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")

SH = os.path.join(MK, "creatives", "analytics-spend-history")
SRC = [
    os.path.join(SH, "photo_4_2026-07-16_12-21-53.jpg"),  # Analityka overview
    os.path.join(SH, "photo_3_2026-07-16_12-21-53.jpg"),  # historia: AI narrative + total
    os.path.join(SH, "photo_2_2026-07-16_12-21-53.jpg"),  # kategorie: donut
    os.path.join(SH, "photo_1_2026-07-16_12-21-53.jpg"),  # przychody vs wydatki + ostrzeżenie
]
...
PHASES = {
    "A": ("ANALITYKA",         "Ile wydajesz —\ni czy mniej niż zwykle"),
    "B": ("HISTORIA WYDATKÓW",  "AI opisze\nTwój miesiąc słowami"),
    "C": ("KATEGORIE",           "Zobacz, na co\nposzły pieniądze"),
    "D": ("PODSUMOWANIE",         "Przychody, wydatki\ni ostrzeżenia"),
}
SCENE_PHASE = ["A", "B", "C", "D"]
PILLS = ["Trendy", "Kategorie", "Historia AI"]
    ...
    for i in range(n):
        ...
        for t in (0.3, 0.55, 0.78, 1.0):
            frames.append(compose(bg, mask, shots, tops, i, t)
                          .convert("RGB").resize((gw, gh), Image.LANCZOS))
            durs.append(60)
    path = os.path.join(outdir, f"spend-history-reel{suffix}.gif")
...
def render_mp4(suffix=""):
    ...
    path = os.path.join(outdir, f"spend-history-reel{suffix}.mp4")
