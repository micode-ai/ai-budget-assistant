# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the Inflation Shield / "Tarcza antyinflacyjna"
feature in AI Budget Assistant (forecasts price rises from your receipts and
tells you what to stock up on NOW, which store, and how much you'll save). Two
phone screenshots cycle inside a phone bezel with vertical-slide transitions
over a warm-dark gradient + orange glow.

Icon layout (matches gamification / analytics / referral style):
  - Top right : app icon with orange ring
  - Bottom    : MICODE badge + mi-code.pl

Scene order:
    0. widget  — Pulpit: "Tarcza antyinflacyjna — Kup Marchew Luz teraz, oszczędź ~0,46 zł"
    1. screen   — Tarcza: zaoszczędzono + koszyk -0,5% + karta "Marchew Luz +20% Biedronka"

Outputs in docs/marketing/creatives/antinflation/renders/pl/:
    inflation-shield-reel.mp4       1080x1920, 30fps
    inflation-shield-reel.gif       600x1067
    inflation-shield-reel-4x5.mp4
    inflation-shield-reel-4x5.gif

Usage:
    python build_antinflation_reel.py               # gif 9:16
    python build_antinflation_reel.py mp4
    python build_antinflation_reel.py both
    python build_antinflation_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", "antinflation", "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")

AI_DIR = os.path.join(MK, "creatives", "antinflation")
SRC = [
    os.path.join(AI_DIR, "photo_1_2026-07-16_13-55-54.jpg"),  # Pulpit widget
    os.path.join(AI_DIR, "photo_2_2026-07-16_13-55-54.jpg"),  # Tarcza screen + buy-ahead card
]
...
PHASES = {
    "A": ("TARCZA ANTYINFLACYJNA", "Kupuj, zanim\nzdrożeje"),
    "B": ("KUP Z WYPRZEDZENIEM",    "Ile, czego i gdzie\nkupić z góry"),
}
SCENE_PHASE = ["A", "B"]
PILLS = ["Prognoza cen", "Kup taniej", "Oszczędności"]
    ...
    for i in range(n):
        ...
        for t in (0.3, 0.55, 0.78, 1.0):
            frames.append(compose(bg, mask, shots, tops, i, t)
                          .convert("RGB").resize((gw, gh), Image.LANCZOS))
            durs.append(60)
    path = os.path.join(outdir, f"inflation-shield-reel{suffix}.gif")
...
def render_mp4(suffix=""):
    ...
    path = os.path.join(outdir, f"inflation-shield-reel{suffix}.mp4")
