# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the "Budżety" (Budgets) feature in AI Budget
Assistant. Five phone screenshots cycle inside a phone bezel with
vertical-slide transitions, warm-dark gradient + orange glow.

Copied from build_purchase_requests_reel.py (the canonical template); only the
scene list, copy, pills and blur regions differ.

Icon layout (matches purchase-requests / family-feed style):
  - Top right : app icon with orange ring
  - Bottom    : MICODE badge + mi-code.pl

Scene order is NARRATIVE (see → drill down → catch the overrun → configure),
which is not the order the screenshots were handed over in:
    0. home      — dashboard with the "Budżet miesięczny" widget, 85% used
                   (safe-to-spend + income/expense figures blurred, see below)
    1. detail    — budget "Life", "W normie" badge, per-category progress bars
    2. detail2   — rest of the categories incl. Yoka over its limit (red),
                   plus days left / forecast / status
    3. edit      — budget mode switch "Ogólny / Według kategorii" + per-category limits
    4. edit2     — total budget, period chips, notification-threshold chips

Privacy: scene 0 has TWO blur bands — the safe-to-spend figure and the
Przychody/Wydatki row. Both reveal household income/balance and neither is
needed to sell the feature; the budget numbers themselves (1 248,94 zł of
8 200,00 zł, 85%) are deliberately left sharp because they ARE the pitch.
The category names visible in scenes 1-4 are the account's own custom
categories (family first names) — decide before publishing whether that is
acceptable, this script does not touch them.

Outputs in docs/marketing/creatives/budgets/renders/pl/:
    budgets-reel.mp4       1080x1920, 30fps
    budgets-reel.gif       600x1067
    budgets-reel-4x5.mp4
    budgets-reel-4x5.gif

Usage:
    python build_budgets_reel.py               # gif 9:16
    python build_budgets_reel.py mp4
    python build_budgets_reel.py both
    python build_budgets_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", FEATURE, "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")

_TG = r"C:\Users\perev\Downloads\Telegram Desktop"
SRC = [
    os.path.join(_TG, "photo_1_2026-07-31_12-49-58.jpg"),  # home + budget widget
    os.path.join(_TG, "photo_2_2026-07-31_12-49-58.jpg"),  # detail, categories
    os.path.join(_TG, "photo_3_2026-07-31_12-49-58.jpg"),  # detail, overrun + forecast
    os.path.join(_TG, "photo_4_2026-07-31_12-49-58.jpg"),  # edit, mode + limits
    os.path.join(_TG, "photo_5_2026-07-31_12-49-58.jpg"),  # edit, period + threshold
]
...
PHASES = {
    "A": ("BUDŻET MIESIĘCZNY", "Jedno spojrzenie —\nwiesz, ile zostało"),
    "B": ("LIMIT NA KATEGORIĘ", "Widzisz dokładnie,\ngdzie przekraczasz"),
    # Deliberately NOT "próg alertu": the "POWIADOM PRZY" chip visible in scene 4
    # is stored and shown on the detail screen, but budget-alert.service.ts loops
    # over a hardcoded THRESHOLDS = [50, 80, 100] and never reads
    # budget.thresholdPercentage — so the user does NOT choose when they are
    # warned, and no copy here may imply they do.
    "C": ("USTAWIASZ RAZ",      "Ogólny limit\nalbo na kategorie"),
}
SCENE_PHASE = ["A", "B", "B", "C", "C"]
PILLS = ["Limity", "Kategorie", "Alerty"]
FOOTER_URL = "mi-code.pl"

# Bands on scene 0 to blur, as (y_start, y_end) fractions of the screenshot
# height. Verified against the 591x1280 source: the first covers only the
# safe-to-spend figure (its label stays readable), the second only the
# income/expense amounts (their labels stay readable).
HOME_BLUR_BANDS = [(0.146, 0.192), (0.790, 0.833)]
...
def blur_bands(img, bands):
    """Gaussian-blur horizontal bands of an already-resized screenshot."""
    img = img.copy()
    for f0, f1 in bands:
        y1, y2 = int(SCR_H * f0), int(SCR_H * f1)
...
def _prep():
    ...
    for i, p in enumerate(SRC):
        ...
        if i == 0:
            img = blur_bands(img, HOME_BLUR_BANDS)
        shots.append(img)
    ...
    for i in range(n):
        ...
        for t in (0.3, 0.55, 0.78, 1.0):
            frames.append(compose(bg, mask, shots, tops, i, t)
                          .convert("RGB").resize((gw, gh), Image.LANCZOS))
            durs.append(60)
    path = os.path.join(outdir, f"{FEATURE}-reel{suffix}.gif")
...
def render_mp4(suffix=""):
    ...
    path = os.path.join(outdir, f"{FEATURE}-reel{suffix}.mp4")
