# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the "Lista zakupów" (Smart Shopping List) feature
...
Scene order:
    0. home     — home screen, "Zakupy → Lista zakupów" entry (balance blurred)
    1. list     — shopping list with restock chips ("Czas na uzupełnienie zapasów")
    2. compare  — "Gdzie najtaniej": per-store basket totals, cheapest highlighted
    3. per-item — cheapest store for each product ("Najniższa cena za produkt")
    4. map      — store map, "Najtaniej / Najbliżej" toggle + distance
    5. map-pin  — map pin popup: store, price, distance

Outputs in docs/marketing/creatives/purchase-list/renders/pl/:
    purchase-list-reel.mp4       1080x1920, 30fps
    purchase-list-reel.gif       600x1067
    purchase-list-reel-4x5.mp4
    purchase-list-reel-4x5.gif

Usage:
    python build_purchase_list_reel.py               # gif 9:16
    python build_purchase_list_reel.py mp4
    python build_purchase_list_reel.py both
    python build_purchase_list_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", "purchase-list", "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")

SHOTS_DIR = os.path.join(MK, "creatives", "purchase-list")
SRC = [
    os.path.join(SHOTS_DIR, "photo_1_2026-07-10_15-36-54.jpg"),  # home + Zakupy sheet
    os.path.join(SHOTS_DIR, "photo_2_2026-07-10_15-36-54.jpg"),  # shopping list + restock
    os.path.join(SHOTS_DIR, "photo_3_2026-07-10_15-36-54.jpg"),  # Gdzie najtaniej (per-store)
    os.path.join(SHOTS_DIR, "photo_4_2026-07-10_15-36-54.jpg"),  # cheapest per product
    os.path.join(SHOTS_DIR, "photo_5_2026-07-10_15-36-54.jpg"),  # store map + toggle
    os.path.join(SHOTS_DIR, "photo_6_2026-07-10_15-36-54.jpg"),  # map pin popup
]
...
PHASES = {
    "A": ("LISTA ZAKUPÓW",  "Twoja lista\nzawsze pod ręką"),
    "B": ("PORÓWNANIE CEN",  "Zobacz, gdzie\nkupisz taniej"),
    "C": ("MAPA SKLEPÓW",    "Najtaniej\ni najbliżej Ciebie"),
}
SCENE_PHASE = ["A", "A", "B", "B", "C", "C"]
PILLS = ["Lista zakupów", "Porównaj ceny", "Mapa sklepów"]
...
def blur_balance(img):
    """Blur only the safe-to-spend balance number on the home screen (scene 0)."""
    y1 = int(SCR_H * 0.135)
    y2 = int(SCR_H * 0.185)
    ...
    for i in range(n):
        ...
        for t in (0.3, 0.55, 0.78, 1.0):
            frames.append(compose(bg, mask, shots, tops, i, t)
                          .convert("RGB").resize((gw, gh), Image.LANCZOS))
            durs.append(60)
    path = os.path.join(outdir, f"purchase-list-reel{suffix}.gif")
...
def render_mp4(suffix=""):
    ...
    path = os.path.join(outdir, f"purchase-list-reel{suffix}.mp4")
