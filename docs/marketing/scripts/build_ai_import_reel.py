# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the AI universal bank-statement import in
AI Budget Assistant (ABA-390/391): when a bank has no ready-made parser, the
app sends the header row / a text fragment to an AI model, which infers which
column is the date, the amount and the description — so a statement from a bank
that is not on the list still imports.
...
Icon layout (matches fat-finder / gamification / analytics style):
  - Top right : app icon with orange ring
  - Bottom    : MICODE badge + mi-code.pl

Scene order (narrative: entry -> payoff). Deliberately just two shots, both of
them about THIS feature — the settings-hub, auto-capture and account-switcher
screens were cut on request, because they pull attention to other features and
make the loop drag.
    0. entry     — "SZYBKI IMPORT" list with the new "Rozpoznaj automatycznie
                   (dowolny bank)" row on top
    1. result    — preview of a foreign-bank statement: 77 transactions in BYN
                   recognised and ticked, "Importuj (77)"

COPY ACCURACY — do not "improve" these claims:
  * Column inference from CSV/XLSX is free; PDF extraction is a Pro feature.
    The pills therefore say "PDF w Pro", never "wszystko za darmo".
  * The app never promises it found every row — scene 1 deliberately keeps the
    app's own warning banner visible ("nie ma salda koncowego do porownania").
    Do not crop it out and do not write copy that claims a complete import.
  * "Dowolny bank" is the app's own wording for that menu row, so it is safe;
    "kazdy bank swiata" would not be.

Outputs in docs/marketing/creatives/ai-import/renders/pl/:
    ai-import-reel.mp4       1080x1920, 30fps
    ai-import-reel.gif       600x1067
    ai-import-reel-4x5.mp4
    ai-import-reel-4x5.gif

Usage:
    python build_ai_import_reel.py               # gif 9:16
    python build_ai_import_reel.py mp4
    python build_ai_import_reel.py both
    python build_ai_import_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", "ai-import", "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")

AI_DIR = os.path.join(MK, "creatives", "ai-import")
SRC = [
    os.path.join(AI_DIR, "photo_3_2026-08-11_13-56-34.jpg"),  # entry: Rozpoznaj automatycznie
    os.path.join(AI_DIR, "photo_5_2026-08-11_13-56-34.jpg"),  # result: 77 transakcji w BYN
]

# Privacy blur. Scene 1 is a REAL bank statement: the rows carry shop names, a
# pharmacy and a sole trader whose business name is a person's surname. Blur the
# text column (names, dates, amounts) but deliberately NOT the left checkboxes,
# the "Wybrano: 77" header or the "Importuj (77)" button — those three carry the
# whole message, and a blurred-content/crisp-chrome shot reads as an intentional
# privacy blur rather than a broken screenshot.
# Box is (x0, y0, x1, y1) as FRACTIONS of the source image, so replacing the
# screenshot with a different resolution does not silently shift the blur.
BLUR = {1: (0.22, 0.31, 0.98, 0.845)}
BLUR_RADIUS = 9
...
PHASES = {
    "A": ("NOWOŚĆ",          "Wyciąg z banku,\nktórego nie ma na liście"),
    "B": ("AI CZYTA WYCIĄG", "77 transakcji\ngotowych do importu"),
}
SCENE_PHASE = ["A", "B"]
PILLS = ["CSV, XLSX", "PDF w Pro", "Bez ręcznego wpisywania"]
...
def set_aspect(aspect):
    ...
    if aspect == "4:5":
        ...
        L = dict(ey=44, eye_sz=26, ul_dy=42, head_sz=54, head_gap=72, head_step=64,
                 glow=(540, 490), pills_dy=20, pill_sz=22, pill_h=50, pill_padx=18, pill_gap=12,
                 badge_sz=66, badge_by=1190, url_dy=8, url_sz=22)
    else:
        H, WIN_TOP, SCR_H, BEZEL = 1920, 440, 1220, 14
        L = dict(ey=78, eye_sz=31, ul_dy=50, head_sz=76, head_gap=96, head_step=88,
                 glow=(540, 650), pills_dy=26, pill_sz=25, pill_h=54, pill_padx=22, pill_gap=14,
...
def blur_region(img, frac, radius):
    """Gaussian-blur a fractional box of a screenshot, in place on a copy.

    Blurring happens at SOURCE resolution, before the shot is scaled into the
    phone viewport — blurring after the upscale would let the interpolator carry
    readable edges through, which is exactly what must not survive here.
    """
    w, h = img.size
...
def _prep():
    ...
    for i, p in enumerate(SRC):
        ...
        if i in BLUR:
            img = blur_region(img, BLUR[i], BLUR_RADIUS)
        shots.append(img.resize((SCR_W, SCR_H), Image.LANCZOS))
    ...
    for i in range(n):
        ...
        for t in (0.3, 0.55, 0.78, 1.0):
            frames.append(compose(bg, mask, shots, tops, i, t)
                          .convert("RGB").resize((gw, gh), Image.LANCZOS))
            durs.append(60)
    path = os.path.join(outdir, f"ai-import-reel{suffix}.gif")
...
def render_mp4(suffix=""):
    ...
    path = os.path.join(outdir, f"ai-import-reel{suffix}.mp4")
