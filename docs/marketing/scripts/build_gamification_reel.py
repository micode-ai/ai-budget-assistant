# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the Gamification / "Osiągnięcia" feature in
AI Budget Assistant (levels + XP, daily streak, unlockable achievements).
Three phone screenshots cycle inside a phone bezel with vertical-slide
transitions over a warm-dark gradient + orange glow.

Icon layout (matches inflation / purchase-requests / family-feed style):
  - Top right : app icon with orange ring
  - Bottom    : MICODE badge + mi-code.pl

Scene order:
    0. home       — Pulpit widget: Poziom 4, 10-dniowa seria, "Zobacz osiągnięcia"
    1. level       — Osiągnięcia: Poziom 4 / 385 XP + pierwsze osiągnięcia
    2. streak       — Osiągnięcia: seria 10/21 dni + odznaki budżetowe (+XP)

Outputs in docs/marketing/creatives/gamification/renders/pl/:
    gamification-reel.mp4       1080x1920, 30fps
    gamification-reel.gif       600x1067
    gamification-reel-4x5.mp4
    gamification-reel-4x5.gif

Usage:
    python build_gamification_reel.py               # gif 9:16
    python build_gamification_reel.py mp4
    python build_gamification_reel.py both
    python build_gamification_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", "gamification", "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")

GAM = os.path.join(MK, "creatives", "gamification")
SRC = [
    os.path.join(GAM, "photo_3_2026-07-16_12-02-25.jpg"),  # home widget (Pulpit)
    os.path.join(GAM, "photo_2_2026-07-16_12-02-25.jpg"),  # level + XP + achievements
    os.path.join(GAM, "photo_1_2026-07-16_12-02-25.jpg"),  # streak + budget badges
]
...
PHASES = {
    "A": ("TWÓJ POSTĘP",  "Poziom i seria\nprosto na pulpicie"),
    "B": ("XP I POZIOMY",  "Zdobywaj punkty\nza dobre nawyki"),
    "C": ("OSIĄGNIĘCIA",   "Utrzymuj serię,\nzbieraj odznaki"),
}
SCENE_PHASE = ["A", "B", "C"]
PILLS = ["Seria dni", "Poziomy XP", "Odznaki"]
    ...
    for i in range(n):
        ...
        for t in (0.3, 0.55, 0.78, 1.0):
            frames.append(compose(bg, mask, shots, tops, i, t)
                          .convert("RGB").resize((gw, gh), Image.LANCZOS))
            durs.append(60)
    path = os.path.join(outdir, f"gamification-reel{suffix}.gif")
...
def render_mp4(suffix=""):
    ...
    path = os.path.join(outdir, f"gamification-reel{suffix}.mp4")
