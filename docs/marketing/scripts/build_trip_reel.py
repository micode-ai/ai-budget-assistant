# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the "Portfel podróży" (Group Trip Wallet) feature
in AI Budget Assistant. Phone screenshots cycle inside a phone bezel with
vertical-slide transitions, warm-dark gradient + orange glow.

Scene order:
    0. create   — "Utwórz podróż" (name Camping, PLN)
    1. hub       — trip account: Rozliczenie / Ustawienia płatności / Mapa podróży + members
    2. invite    — "Zaproś" by search (found Aliona)   [email blurred]
    3. push      — partner's notification: "Mikhail zaprosił(a) Cię — Dołącz do Camping"
    4. split     — "Nowy wydatek": Podziel między, Po równo (100 zł Bilety)
    5. settle    — "Ustawienia płatności": Revolut / PayPal / BLIK

Outputs in docs/marketing/creatives/trip/renders/pl/:
    trip-reel.mp4 / .gif  (+ -4x5 variants)

Usage (from docs/marketing/scripts/):
    python build_trip_reel.py both
    python build_trip_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", "trip", "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")
DRAFT = os.path.join(MK, "creatives", "draft", "trip")

SRC = [
    os.path.join(DRAFT, "me-1.jpg"),      # create trip
    os.path.join(DRAFT, "me-2.jpg"),      # trip account hub
    os.path.join(DRAFT, "me-4.jpg"),      # invite by search (Aliona)  -> blur email
    os.path.join(DRAFT, "aliona-1.jpg"),  # partner push notification
    os.path.join(DRAFT, "end.jpg"),       # new expense with split
    os.path.join(DRAFT, "me-3.jpg"),      # settle-up payment method
]
# {scene_index: (y1frac, y2frac, x1frac, x2frac)} regions to Gaussian-blur
SCENE_BLUR = {2: (0.376, 0.449, 0.16, 0.72)}  # Aliona's e-mail on the invite-search row
...
PHASES = {
    "A": ("WSPÓLNY PORTFEL", "Budżet na wyjazd\ndla całej ekipy"),
    "B": ("ZAPROŚ EKIPĘ",    "Dodaj znajomych\ndo wyjazdu"),
    "C": ("PODZIEL KOSZTY",  "Dziel wydatki\nmiędzy wszystkich"),
    "D": ("ROZLICZCIE SIĘ",  "Kto komu ile –\napka policzy"),
}
SCENE_PHASE = ["A", "A", "B", "B", "C", "D"]
PILLS = ["Wspólny budżet", "Podział kosztów", "Rozliczenie"]
...
def fit_crop(img):
    """Center-crop the source to the target SCR_W:SCR_H ratio, then resize.
    Avoids distortion when screenshots come from phones of different ratios."""
...
def blur_regions(img, regions):
    ...
    for (y1f, y2f, x1f, x2f) in regions:
        x1, x2 = int(x1f * SCR_W), int(x2f * SCR_W)
        y1, y2 = int(y1f * SCR_H), int(y2f * SCR_H)
        crop = img.crop((x1, y1, x2, y2))
        img.paste(crop.filter(ImageFilter.GaussianBlur(16)), (x1, y1))
    return img
...
def _prep():
    ...
    for i, p in enumerate(SRC):
        img = fit_crop(Image.open(p))
        if i in SCENE_BLUR:
            img = blur_regions(img, [SCENE_BLUR[i]])
        shots.append(img)
    ...
    for i in range(n):
        ...
        for t in (0.3, 0.55, 0.78, 1.0):
            frames.append(compose(bg, mask, shots, tops, i, t)
                          .convert("RGB").resize((gw, gh), Image.LANCZOS))
            durs.append(60)
    path = os.path.join(outdir, f"{OUT_NAME}{suffix}.gif")
...
def render_mp4(suffix=""):
    ...
    path = os.path.join(outdir, f"{OUT_NAME}{suffix}.mp4")
