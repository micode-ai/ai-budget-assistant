# -*- coding: utf-8 -*-
"""
Instagram/Facebook Stories for the "Podziel paragon" (receipt split via guest
link) feature. Two kinds, same house style as the reel:

  1. Per-scene stories — the reel's six still frames as standalone PNGs, phone
     bezel + phase headline. These REUSE build_split_reel's own pipeline rather
     than re-implementing it, which matters for more than DRY: the reel module
     owns `sanitize()`, so the stories inherit the redaction of the payer's real
     BLIK phone number, Revolut handle and shop map for free. Re-implementing the
     compose step here would mean a second place that could forget to redact.

  2. A recap story — the whole flow as four numbered steps, no phone bezel, in
     the same shape as build_ai_chat_story_all.py.

Outputs in docs/marketing/creatives/split/renders/pl/:
    stories/01-wejscie.png … 06-potwierdzenie.png    1080x1920
    split-story-all.png                              1080x1920
    split-story-all-4x5.png                          1080x1344

Usage:
    python build_split_story.py              # scenes + recap 9:16
    python build_split_story.py scenes
    python build_split_story.py recap
    python build_split_story.py recap both
"""
import os, sys
from PIL import Image, ImageDraw, ImageFont, ImageFilter

import build_split_reel as R
...
EYEBROW  = "PODZIEL PARAGON"
HEADLINE = "Zapłaciłeś za wszystkich?"
SUBTITLE = "Rozlicz się bez proszenia się i bez Excela"
FOOTER_URL = "ai-budget.pl"

# (step number, what you do, the payoff in orange)
STEPS = [
    ("1", "Skanujesz paragon",            "pozycje wczytują się same"),
    ("2", "Dotykasz pozycję, potem osobę", "każdy płaci za swoje"),
    ("3", "Znajomy dostaje swój link",     "bez aplikacji, bez konta"),
    ("4", "Potwierdzasz wpłatę",           "dług zamyka się sam"),
]
...
def build_scenes():
    """The reel's six still frames, saved as standalone 9:16 story PNGs."""
    R.set_aspect("9:16")
...
def build_recap(aspect):
    ...
    # Two passes: measure the whole block, then centre it vertically in the band
    # between the subtitle and the footer. Drawing straight from `sy` left ~640px
    # of dead space under the last row at 9:16, which reads as an unfinished slide.
    ...
    # Justify rather than centre: the block stays anchored under the header (centring
    # it detached the rows from the headline and just moved the void to the middle)
    # and the leftover space is spread into the gaps. Stops at ~78% of the band —
    # on Stories the bottom strip is covered by the platform's own reply UI, so
    # filling it completely would be worse than leaving it clear.
