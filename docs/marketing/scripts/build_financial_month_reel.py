# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the "Miesiac finansowy" (Financial Month) feature
in AI Budget Assistant. Three phone screenshots cycle inside a phone bezel with
vertical-slide transitions, warm-dark gradient + orange glow.

The pitch: your salary lands on the 10th but the budget resets on the 1st. Set
the account's financial month to your payday and budgets run payday to payday.

Scene order:
    0. settings — account settings, MIESIAC FINANSOWY row (calendar month)
    1. picker   — the day sheet, with the explanation copy
    2. home     — home screen, "Budzet miesieczny" now labelled "2 sie - 1 wrz"

Outputs in docs/marketing/creatives/financial-month/renders/pl/.

Usage:
    python build_financial_month_reel.py both
    python build_financial_month_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", "financial-month", "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")

SRC = [
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_1_2026-08-03_18-04-46.jpg",  # settings
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_2_2026-08-03_18-04-46.jpg",  # day picker
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_3_2026-08-03_18-04-46.jpg",  # home, dated budget
]
...
PHASES = {
    "A": ("MIESIĄC FINANSOWY", "Wypłata 10-go?\nBudżet też od 10-go"),
    "B": ("JEDNO USTAWIENIE",  "Wybierz swój\ndzień wypłaty"),
    "C": ("GOTOWE",            "Budżet liczy się\nod wypłaty do wypłaty"),
}
SCENE_PHASE = ["A", "B", "C"]
PILLS = ["Miesiąc finansowy", "Budżety", "Raporty"]
    ...
    for i in range(n):
        ...
        for t in (0.3, 0.55, 0.78, 1.0):
            frames.append(compose(bg, mask, shots, tops, i, t)
                          .convert("RGB").resize((gw, gh), Image.LANCZOS))
            durs.append(60)
    path = os.path.join(outdir, f"financial-month-reel{suffix}.gif")
...
def render_mp4(suffix=""):
    ...
    path = os.path.join(outdir, f"financial-month-reel{suffix}.mp4")
