# -*- coding: utf-8 -*-
"""
Single "rewards" story (9:16 + 4:5) for the Referral / "Zaproś znajomych"
feature: the reward ladder (AI queries per friend, free month of Pro, how it
works) as bold stat cards on the house-style warm-dark gradient + orange glow.
No phone bezel — the rewards are the hero. No emoji (text-only so Windows fonts
render clean). Stacked card layout (values here are longer than one glyph).

Outputs in docs/marketing/creatives/referal/renders/pl/:
    referral-story.png        1080x1920
    referral-story-4x5.png    1080x1344

Usage:
    python build_referral_story.py          # 9:16
    python build_referral_story.py 4:5
    python build_referral_story.py both
"""
import os, sys
...
SUBTITLE = "Podziel się kodem — zgarniaj nagrody AI i Pro"
CTA      = "Poleć i zgarniaj nagrody"
FOOTER_URL = "ai-budget.pl"

# (tag, big value, descriptor)
CARDS = [
    ("ZA KAŻDEGO ZNAJOMEGO", "+30",     "bonusowych zapytań AI"),
    ("ZA 5 ZNAJOMYCH",       "1 mies.", "Pro całkiem za darmo"),
    ("PROSTO",               "3 kroki", "udostępnij · 7 dni · nagroda"),
]
...
def build(aspect):
    ...
    # reward cards (stacked layout — variable-length values)
    mx = 80
    cw = W - mx * 2
