# -*- coding: utf-8 -*-
"""
Single "stats" story (9:16 + 4:5) for the Analytics / "Twoja historia wydatków"
feature: the month-in-review headline numbers (total spend, trend vs last month,
top category) as bold stat cards on the house-style warm-dark gradient + orange
glow. No phone bezel — the numbers are the hero. No emoji (text-only so Windows
fonts render clean). Stacked card layout (values here are longer than one glyph).

Outputs in docs/marketing/creatives/analytics-spend-history/renders/pl/:
    spend-history-story.png        1080x1920
    spend-history-story-4x5.png    1080x1344

Usage:
    python build_analytics_spend_history_story.py          # 9:16
    python build_analytics_spend_history_story.py 4:5
    python build_analytics_spend_history_story.py both
"""
import os, sys
...
FEATURE_DIR = "analytics-spend-history"
BASENAME = "spend-history"
...
EYEBROW  = "ANALITYKA"
HEADLINE = "Twoja historia wydatków"
SUBTITLE = "AI opisuje Twój miesiąc: liczby, kategorie, ostrzeżenia"
CTA      = "Cała analityka w jednym miejscu"
FOOTER_URL = "ai-budget.pl"

# (tag, big value, descriptor)
CARDS = [
    ("WYDATKI · CZERWIEC", "12 333 zł", "całkowite wydatki miesiąca"),
    ("TREND",              "−36%",      "mniej niż poprzedni miesiąc"),
    ("TOP KATEGORIA",      "38,7%",     "poszło na Mieszkanie"),
]
...
def stat_card(draw, x0, y, cw, ch, tag, big, sub, tagf, bigf, subf):
...
def build(aspect):
    ...
    sy = hy + (96 if is916 else 76)
    centered(draw, sy, SUBTITLE, sf, GREY)

    # stat cards (stacked layout — variable-length values)
    mx = 80
    cw = W - mx * 2
    ...
    for tag, big, sub in CARDS:
        stat_card(draw, mx, y, cw, ch, tag, big, sub, tagf, bigf, subf)
        y += ch + gap
