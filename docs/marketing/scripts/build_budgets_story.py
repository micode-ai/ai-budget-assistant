# -*- coding: utf-8 -*-
"""
Instagram/Facebook Stories for the "Budżety" (Budgets) feature. Two kinds, same
house style as the reel:

  1. Per-scene stories — the reel's five still frames as standalone PNGs, phone
     bezel + phase headline. These REUSE build_budgets_reel's own pipeline rather
     than re-implementing it, which matters for more than DRY: the reel module
     owns HOME_BLUR_BANDS, so the stories inherit the redaction of the
     safe-to-spend figure and the Przychody/Wydatki amounts for free.
     Re-implementing the compose step here would mean a second place that could
     forget to redact.

  2. A recap story — the whole feature as four numbered steps, no phone bezel,
     same shape as build_split_story.py / build_ai_chat_story_all.py.

Copy discipline: no step claims the user picks when they are warned.
budget-alert.service.ts iterates a hardcoded THRESHOLDS = [50, 80, 100] and never
reads budget.thresholdPercentage, so the "POWIADOM PRZY" chip does not control
notifications — see the "Czego NIE obiecujemy" section of
copy/feature-budgets-reel-pl.md.

Outputs in docs/marketing/creatives/budgets/renders/pl/:
    stories/01-pulpit.png … 05-okres.png     1080x1920
    budgets-story-all.png                    1080x1920
    budgets-story-all-4x5.png                1080x1344

Usage:
    python build_budgets_story.py              # scenes + recap 9:16
    python build_budgets_story.py scenes
    python build_budgets_story.py recap
    python build_budgets_story.py recap both
"""
import os, sys
from PIL import Image, ImageDraw, ImageFont, ImageFilter

import build_budgets_reel as R
...
EYEBROW  = "BUDŻET MIESIĘCZNY"
HEADLINE = "Wiesz, ile Ci jeszcze zostało?"
SUBTITLE = "Limit osobno na każdą kategorię"
FOOTER_URL = "ai-budget.pl"

# (step number, what you do, the payoff in orange)
STEPS = [
    ("1", "Ustawiasz limit",              "ogólny albo na każdą kategorię"),
    ("2", "Każda kategoria ma swój pasek", "zielony, pomarańczowy, czerwony"),
    ("3", "Przekroczenia widzisz od razu", "na czerwono, bez szukania"),
    ("4", "Na pulpicie jedna liczba",      "ile jeszcze zostało"),
]
