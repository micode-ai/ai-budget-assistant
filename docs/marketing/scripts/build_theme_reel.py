# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the Appearance / accent-color personalization
feature in AI Budget Assistant (ABA-372): choose an accent color (13 presets
or your own HEX), pick light / dark / system, pick your language — and the
WHOLE app recolors to match. Three phone screenshots cycle inside a phone
bezel with vertical-slide transitions over the house warm-dark gradient +
orange glow.

Icon layout (matches referral / gamification / analytics style):
  - Top right : app icon with orange ring
  - Bottom    : MICODE badge + mi-code.pl

Scene order:
    0. entry  — Wygląd i język: motyw + siatka kolorów akcentu (fiolet)
    1. picker  — Wybierz kolor: suwak barwy + HEX #3B82F6 + Zastosuj (niebieski)
    2. applied — Pulpit w Twoim kolorze: cała aplikacja przemalowana

Outputs in docs/marketing/creatives/design/renders/pl/:
    theme-reel.mp4       1080x1920, 30fps
    theme-reel.gif       600x1067
    theme-reel-4x5.mp4
    theme-reel-4x5.gif

Usage:
    python build_theme_reel.py               # gif 9:16
    python build_theme_reel.py mp4
    python build_theme_reel.py both
    python build_theme_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", "design", "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")

REF = os.path.join(MK, "creatives", "design")
SRC = [
    os.path.join(REF, "photo_1_fixed.png"),  # Wygląd i język + kolory (nav-bar cleaned)
    os.path.join(REF, "photo_2_fixed.png"),  # picker koloru + HEX (przyciski z marginesem)
    os.path.join(REF, "photo_3_fixed.png"),  # Pulpit w niebieskim (top bleed + nav-bar cleaned)
]
...
PHASES = {
    "A": ("PERSONALIZACJA", "Dopasuj aplikację\ndo siebie"),
    "B": ("KOLOR AKCENTU",  "Wybierz swój\nkolor"),
    "C": ("GOTOWE",          "Cała aplikacja\nw Twoim kolorze"),
}
SCENE_PHASE = ["A", "B", "C"]
PILLS = ["Twój kolor", "Jasny / Ciemny", "9 języków"]
    ...
    for i in range(n):
        ...
        for t in (0.3, 0.55, 0.78, 1.0):
            frames.append(compose(bg, mask, shots, tops, i, t)
                          .convert("RGB").resize((gw, gh), Image.LANCZOS))
            durs.append(60)
    path = os.path.join(outdir, f"theme-reel{suffix}.gif")
...
def render_mp4(suffix=""):
    ...
    path = os.path.join(outdir, f"theme-reel{suffix}.mp4")
