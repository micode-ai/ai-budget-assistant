# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) + static story frames for the "Czat AI" (AI Chat)
feature in AI Budget Assistant. Five phone screenshots cycle inside a phone
bezel with vertical-slide transitions, warm-dark gradient + orange glow.

Icon layout (matches goals / purchase-list / trip / family-feed style):
  - Top right : app icon with orange ring
  - Bottom    : MICODE badge + mi-code.pl

Scene order:
    0. piwo        — "Ile wydałem na piwo w tym miesiącu?" -> 192.24 PLN, itemised
    1. biedronka   — "Ile wydałem w Biedronce w tym miesiącu?" -> 1159.50 PLN
    2. porownanie  — "Porównaj wydatki tego miesiąca z poprzednim" (podział %)
    3. kategorie   — comparison cd.: wzrost wydatków + podział na kategorie
    4. budzet      — "Ile zostało mi z budżetu w tym miesiącu?" -> 1051.47 PLN

Outputs in docs/marketing/creatives/ai-chat/renders/pl/:
    ai-chat-reel.mp4        1080x1920, 30fps
    ai-chat-reel.gif        600x1067
    ai-chat-reel-4x5.mp4
    stories/NN-<key>.png    1080x1920 static story frames

Usage:
    python build_ai_chat_reel.py               # gif 9:16
    python build_ai_chat_reel.py mp4
    python build_ai_chat_reel.py both          # gif + mp4 (reel)
    python build_ai_chat_reel.py stories       # static story PNGs
    python build_ai_chat_reel.py all           # stories + gif + mp4
    python build_ai_chat_reel.py both 4:5
"""
...
SHOTS_DIR = os.path.join(MK, "creatives", FEATURE)
STAMP = "2026-07-14_17-37-28"
SRC = [
    os.path.join(SHOTS_DIR, f"photo_1_{STAMP}.jpg"),  # piwo 192.24 (itemised from receipts)
    os.path.join(SHOTS_DIR, f"photo_2_{STAMP}.jpg"),  # Biedronka 1159.50
    os.path.join(SHOTS_DIR, f"photo_3_{STAMP}.jpg"),  # month comparison
    os.path.join(SHOTS_DIR, f"photo_4_{STAMP}.jpg"),  # comparison cd. + category split
    os.path.join(SHOTS_DIR, f"photo_5_{STAMP}.jpg"),  # budget status 1051.47 left
]
SCENE_KEYS = ["piwo", "biedronka", "porownanie", "kategorie", "budzet"]

# The chat answers ARE the demo (amounts computed by the app), so nothing is blurred.
BLUR_BANDS = {}
...
PHASES = {
    "A": ("CZAT AI",       "Zapytaj po ludzku\no każdy wydatek"),
    "B": ("ANALIZA AI",    "Porównaj miesiące\njednym pytaniem"),
    "C": ("TWÓJ BUDŻET",   "Ile zostało\ndo końca miesiąca?"),
}
SCENE_PHASE = ["A", "A", "B", "B", "C"]
PILLS = ["Wydatki", "Analiza", "Budżet"]
...
def blur_bands(img, bands):
    """Blur horizontal bands (given as SCR_H fractions) to hide real balances."""
    img = img.copy()
    for f1, f2 in bands:
        y1, y2 = int(SCR_H * f1), int(SCR_H * f2)
        region = img.crop((0, y1, SCR_W, y2))
        img.paste(region.filter(ImageFilter.GaussianBlur(16)), (0, y1))
    return img
...
def _prep():
    ...
    for i, p in enumerate(SRC):
        ...
        if i in BLUR_BANDS:
            img = blur_bands(img, BLUR_BANDS[i])
        shots.append(img)
...
def render_stories(suffix=""):
    bg, mask, shots, tops, outdir = _prep()
    sdir = os.path.join(outdir, "stories")
    os.makedirs(sdir, exist_ok=True)
    for i in range(len(shots)):
        frame = compose(bg, mask, shots, tops, i, 0.0).convert("RGB")
        path = os.path.join(sdir, f"{i+1:02d}-{SCENE_KEYS[i]}{suffix}.png")
        frame.save(path, "PNG")
...
def main(mode, aspect):
    ...
    if mode in ("stories", "all"):
        render_stories(SUFFIX[aspect])
    if mode in ("gif", "both", "all"):
        render_gif(SUFFIX[aspect])
    if mode in ("mp4", "both", "all"):
...
if __name__ == "__main__":
    args = sys.argv[1:]
    mode = "gif"
    if args and args[0] in ("gif", "mp4", "both", "stories", "all"):
