# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the "Wnioski o zakup" (Purchase Requests) feature
...
Icon layout (matches google-signin / family-feed style):
  - Top right : app icon with orange ring
  - Bottom    : MICODE badge + mi-code.pl

Scene order:
    0. home      — home screen with Rodzinna tablica widget (balance blurred)
    1. new-req   — new purchase request form (Lody 5 PLN, Zabka)
    2. push      — partner's lock screen push notification
    3. pending   — request detail, awaiting votes
    4. voted     — 1 vote received (Mikhail approved)
    5. approved  — both approved, Zatwierdzono + Dodaj do planu

Outputs in docs/marketing/creatives/purchase-requests/renders/pl/:
    purchase-requests-reel.mp4       1080x1920, 30fps
    purchase-requests-reel.gif       600x1067
    purchase-requests-reel-4x5.mp4
    purchase-requests-reel-4x5.gif

Usage:
    python build_purchase_requests_reel.py               # gif 9:16
    python build_purchase_requests_reel.py mp4
    python build_purchase_requests_reel.py both
    python build_purchase_requests_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", "purchase-requests", "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")

SRC = [
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_1_2026-06-30_18-22-20.jpg",  # home
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_2_2026-06-30_18-22-20.jpg",  # new request
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_2_2026-06-30_18-22-26.jpg",  # push notification
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_3_2026-06-30_18-22-20.jpg",  # pending
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_4_2026-06-30_18-22-20.jpg",  # 1 vote
    r"C:\Users\perev\Downloads\Telegram Desktop\photo_1_2026-06-30_18-22-26.jpg",  # approved
]
...
PHASES = {
    "A": ("WNIOSKI O ZAKUP",  "Zaproponuj zakup\ncałej rodzinie"),
    "B": ("GŁOSOWANIE",       "Rodzina głosuje\nna Twój wniosek"),
    "C": ("ZATWIERDZONO",     "Zakup zaakceptowany\nprzez rodzinę"),
}
SCENE_PHASE = ["A", "A", "B", "B", "C", "C"]
PILLS = ["Wnioski", "Głosowanie", "Zatwierdzenie"]
...
def blur_balance(img):
    """Blur only the balance number on the home screen (scene 0)."""
    y1 = int(SCR_H * 0.135)
    y2 = int(SCR_H * 0.185)
    ...
    for i in range(n):
        ...
        for t in (0.3, 0.55, 0.78, 1.0):
            frames.append(compose(bg, mask, shots, tops, i, t)
                          .convert("RGB").resize((gw, gh), Image.LANCZOS))
            durs.append(60)
    path = os.path.join(outdir, f"purchase-requests-reel{suffix}.gif")
...
def render_mp4(suffix=""):
    ...
    path = os.path.join(outdir, f"purchase-requests-reel{suffix}.mp4")
