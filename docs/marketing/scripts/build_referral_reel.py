# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the Referral / "Zaproś znajomych" feature in
AI Budget Assistant (share your code -> friends join -> you earn +30 AI queries
per active friend, and 5 friends = a free month of Pro). Three phone screenshots
cycle inside a phone bezel with vertical-slide transitions over a warm-dark
gradient + orange glow.

Icon layout (matches gamification / analytics style):
  - Top right : app icon with orange ring
  - Bottom    : MICODE badge + mi-code.pl

Scene order:
    0. entry   — Ustawienia: "Zaproś znajomych — Udostępnij kod, zdobądź bonus"
    1. code     — Twój kod polecenia 5GD2S4 + Kopiuj/Udostępnij + kamień milowy Pro
    2. rewards   — Jak to działa (3 kroki) + +30 zapytań AI / miesiąc Pro

Outputs in docs/marketing/creatives/referal/renders/pl/:
    referral-reel.mp4       1080x1920, 30fps
    referral-reel.gif       600x1067
    referral-reel-4x5.mp4
    referral-reel-4x5.gif

Usage:
    python build_referral_reel.py               # gif 9:16
    python build_referral_reel.py mp4
    python build_referral_reel.py both
    python build_referral_reel.py both 4:5
"""
...
OUT   = os.path.join(MK, "creatives", "referal", "renders")
BADGE = os.path.join(MK, "assets", "micode-badge.png")
ICON  = os.path.join(MK, "..", "..", "apps", "mobile", "assets", "icon.png")

REF = os.path.join(MK, "creatives", "referal")
SRC = [
    os.path.join(REF, "photo_1_2026-07-16_12-42-26.jpg"),  # Ustawienia entry
    os.path.join(REF, "photo_2_2026-07-16_12-42-26.jpg"),  # kod + udostępnij + kamień milowy
    os.path.join(REF, "photo_3_2026-07-16_12-42-26.jpg"),  # jak to działa + nagrody
]
...
PHASES = {
    "A": ("ZAPROŚ ZNAJOMYCH",  "Poleć aplikację\nznajomym"),
    "B": ("TWÓJ KOD",           "Udostępnij kod,\nzgarnij nagrody"),
    "C": ("NAGRODY",             "+30 zapytań AI\ni miesiąc Pro gratis"),
}
SCENE_PHASE = ["A", "B", "C"]
PILLS = ["Twój kod", "Nagrody AI", "Miesiąc Pro"]
    ...
    for i in range(n):
        ...
        for t in (0.3, 0.55, 0.78, 1.0):
            frames.append(compose(bg, mask, shots, tops, i, t)
                          .convert("RGB").resize((gw, gh), Image.LANCZOS))
            durs.append(60)
    path = os.path.join(outdir, f"referral-reel{suffix}.gif")
...
def render_mp4(suffix=""):
    ...
    path = os.path.join(outdir, f"referral-reel{suffix}.mp4")
