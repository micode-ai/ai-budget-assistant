# -*- coding: utf-8 -*-
"""
Single "benefit" story (9:16 + 4:5) for the Fat Finder / "Audyt wydatków"
feature: the three hooks (how many findings, potential monthly savings, a
concrete amount + next step at each finding) as bold stat cards on the
house-style warm-dark gradient + orange glow. No phone bezel — the message is
the hero. No emoji (text-only so Windows fonts render clean).

Wording is deliberate: the savings figure is POTENTIAL ("Potencjalne miesięczne
oszczędności" is the app's own label) and the audit is a Pro feature — the CTA
says so. Do not reword to promise savings.

Outputs in docs/marketing/creatives/fat-finder/renders/pl/:
    fat-finder-story.png        1080x1920
    fat-finder-story-4x5.png    1080x1344

Usage:
    python build_fat_finder_story.py          # 9:16
    python build_fat_finder_story.py 4:5
    python build_fat_finder_story.py both
"""
import os, sys
...
EYEBROW  = "AUDYT WYDATKÓW"
HEADLINE = "Gdzie uciekają pieniądze"
SUBTITLE = "AI przegląda Twój miniony miesiąc"
CTA      = "Zobacz swój audyt — w planie Pro"
FOOTER_URL = "ai-budget.pl"

# (tag, big value, descriptor)
# Keep the big value SHORT (<= ~9 glyphs at 100px) — a longer string crowds the
# descriptor line underneath it. That is why card 3 says "paragony", not
# "kwota + krok".
CARDS = [
...
def stat_card(draw, x0, y, cw, ch, tag, big, sub, tagf, bigf, subf, big_f=0.28):
    ...
    # fraction of the card: a value with descenders ("paragony") otherwise
...
def build(aspect):
    ...
    hf = ImageFont.truetype(HEAD_FONT, 72 if is916 else 58)
    ...
    # benefit cards (stacked layout — variable-length values)
    mx = 80
    cw = W - mx * 2
