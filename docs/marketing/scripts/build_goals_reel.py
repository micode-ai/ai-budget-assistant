# -*- coding: utf-8 -*-
"""
Animated reel (MP4 + GIF) for the "Cele oszczędnościowe" (Savings Goals) feature
in AI Budget Assistant. Five phone screenshots cycle inside a phone bezel with
...
Scene order:
    0. home    — home widget "Cele oszczędnościowe" with progress (balances blurred)
    1. list    — goals list: progress, status (Aktywny/Ukończony), difficulty tags
    2. detail  — goal detail: progress, "Dodaj kwotę", "W normie" forecast, history
    3. plan-ai — "Plan AI": monthly/weekly amount + AI description + milestones
    4. limits  — "Limity kategorii": suggested per-category cuts to reach the goal

Outputs in docs/marketing/creatives/goals/renders/pl/:
    goals-reel.mp4       1080x1920, 30fps
    goals-reel.gif       600x1067
    goals-reel-4x5.mp4
    goals-reel-4x5.gif

Usage:
    python build_goals_reel.py               # gif 9:16
    python build_goals_reel.py mp4
    python build_goals_reel.py both
    python build_goals_reel.py both 4:5
"""
...
SRC = [
    os.path.join(SHOTS_DIR, "photo_1_2026-07-12_11-10-01.jpg"),  # home widget (balances blurred)
    os.path.join(SHOTS_DIR, "photo_2_2026-07-12_11-10-01.jpg"),  # goals list + difficulty tags
    os.path.join(SHOTS_DIR, "photo_3_2026-07-12_11-10-01.jpg"),  # goal detail + forecast + history
    os.path.join(SHOTS_DIR, "photo_4_2026-07-12_11-10-01.jpg"),  # Plan AI + milestones
    os.path.join(SHOTS_DIR, "photo_5_2026-07-12_11-10-01.jpg"),  # category limits
]

# Fractional (y1, y2) bands to blur per scene index (protects real balances).
BLUR_BANDS = {
    0: [(0.148, 0.202), (0.832, 0.895)],  # safe-to-spend number + wallet balances row
}
...
PHASES = {
    "A": ("CELE OSZCZĘDNOŚCIOWE", "Wyznacz cel\ni śledź postęp"),
    "B": ("POSTĘP I PROGNOZA",    "Wiesz, kiedy\nosiągniesz cel"),
    "C": ("PLAN AI",              "AI ułoży plan\noszczędzania"),
}
SCENE_PHASE = ["A", "A", "B", "C", "C"]
PILLS = ["Wyznacz cel", "Śledź postęp", "Plan AI"]
