...
- Cluster: `saving` (pillar `how-to-save-money`/`jak-oszczedzac-pieniadze`), alongside siblings `inflation` and `wrapped`. `inflation-shield` is the explicit **forward-looking counterpart** to `inflation` (topic 16): topic 16 measures inflation that already happened, topic 26 forecasts a per-product trend from the same receipt data and recommends stocking up before the rise lands, rather than repeating topic 16's "how to calculate it by hand" structure. `wrapped` (topic 25) is linked as a second sibling since Financial Wrapped also surfaces the personal inflation number. `inflation-shield` was added to `CLUSTERS["saving"]` in `build_blog.py` as part of authoring this topic (the "no follow-up needed" pattern from Waves 7-8) and `build_pillar_links.py` was re-run so the `how-to-save-money` pillar links down to it in pl/en/ru.
- **Only 3 of 9 languages this wave** (pl, en, ru) — matches this project's `DC_ARTICLE_LOCALES` setting, the same reduced rollout Wave 8 (`wrapped`) used. `build_pillar_links.py` reports `MISSING inflation-shield for {de,es,fr,ua,be,nl}` on the other 6 pillar files, which is expected until (if ever) this topic is translated into the rest; it does not affect the pl/en/ru down-links or break the build.
- Positioning constraints (binding in all 3 languages): the forecast is described as deterministic trend statistics over the user's own receipt history, never as an "AI-powered" prediction — the feature is not an LLM call; recommended quantities and stock-up windows are described qualitatively (a month or two of normal consumption, capped), never with an invented number; the estimated saving is always framed as a conservative estimate (the underlying model halves the naive gap), never as an exact or guaranteed figure; the feature must not be described as detecting a failed promotion or an overcharge (that positioning belongs to the separate receipt price-check feature, topic 21's `receipts` pair) or as comparing prices against other users (no community layer exists for this feature yet); the free/no-tier-gate claim and the "needs a handful of real shopping trips first" honesty behavior are stated only as far as the shipped feature actually behaves.
- Blog now at 26 topics / 228 articles once this topic ships (was 25 topics / 225 articles after Wave 8, once `wrapped` was completed to 9 languages in a revision pass) — the 3 new files count against the topic total; the other 6 language slots stay open for a future translation pass.
