...
- **AI module** (`modules/ai/`): write actions (`create_*`) require user confirmation via `POST /ai/chat/confirm`. Read actions execute immediately. Don't add a new write action that bypasses confirmation.
  - **Before adding any ML/AI service**, run `ls apps/api/src/modules/ai/services/*.service.ts` and read any file whose name plausibly overlaps before adding a new one — the directory itself is the authoritative list (mirrors the module-list bullet above), not the roster below. The module is the fastest-growing area, duplicate services are costly to merge, and this roster has already drifted out of sync with the directory once (it was missing `chat.service.ts` and `geocoding.service.ts`) — treat it as a quick-reference starting point, not ground truth.
  - Roster (verify against the directory before trusting it): `categorization.service.ts` (auto-categorise transactions), `chat.service.ts` (lean chat orchestrator that dispatches to the tool/context/prompt services below), `embedding.service.ts` (vector embeddings), `ocr.service.ts` (receipt OCR), `whisper.service.ts` (voice transcription), `geocoding.service.ts` (Nominatim address/reverse geocoding, `geocode_cache`-backed — used by `OcrService`, `community-prices`, and `GET /ai/geocode/search`), `goal-planner.service.ts` (savings goal projections), `project-suggestion.service.ts`, `split-suggestion.service.ts`, `tag-suggestion.service.ts` (contextual suggestions), `ai-tools.service.ts` (function schemas + `executeAction` dispatcher — extend here when adding new function-calling tools), `user-context-builder.service.ts` (builds `UserContext` from Prisma; Redis-cached `uc:{accountId}` — extend when new context fields are needed for AI prompts), `prompt-builder.service.ts` (system prompt construction, language detection — extend here for prompt changes).
  - `model-resolver.ts` and `response-mode.helper.ts` are helpers, not services — no roster entry needed. `*.spec.ts` files alongside these are tests, not services.
