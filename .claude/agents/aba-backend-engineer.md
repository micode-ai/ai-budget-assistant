...
- `apps/api/src/modules/<feature>/` — authoritative list is `ls apps/api/src/modules/`; do not rely on any enumeration below (or anywhere else in this file) being current. New features either extend an existing module or create a new one.
  - Some modules deliberately split into two controllers so a public route can never inherit a class-level guard from a guarded one (and a guarded route can never quietly lose its guard) — `slack` (`SlackController`: webhook events + interactivity, both public/HMAC-verified, excluded from `/api/v1`, vs `SlackOAuthController`: multi-workspace install/callback, also excluded from `/api/v1`) and `restore-credentials` (`RestoreCredentialAuthController`, no class-level guard, vs `RestoreCredentialRegistrationController`, class-level `JwtAuthGuard`) are the current examples — see CLAUDE.md's restore-credentials entry ("Two controllers, deliberately not one") for the full rationale. Don't assume the usual single-controller module shape when touching either. When a module needs both a public and a guarded route, prefer this split over one controller with per-method guard overrides.
  - `whatsapp`'s webhook is public/unauthenticated by design (HMAC-verified, excluded from `/api/v1`) — this is a single-controller module (no guarded sibling route to protect from guard leakage), a different case from the dual-controller pattern above. See the WhatsApp bot cross-cutting rule below for the full detail before touching it.
...
- **Shared bot i18n** (`apps/api/src/common/bot-i18n/shared-messages.ts`): before adding a new string to any bot's own `helpers/i18n.ts`, check here first — if the string is (or plausibly will be) shared by 2+ of Telegram/WhatsApp/Slack, it belongs in this ~30-key, 9-language shared dictionary via the `createBotT(messages, { markup, defaultParams })` factory, not copy-pasted into each bot's own file. Each bot's `helpers/i18n.ts` now composes `const messages = { ...sharedMessages, ...platformMessages }` and should hold only genuinely platform-specific copy (e.g. Telegram's `/slash`-command strings vs WhatsApp/Slack's non-slash equivalents). `createBotT`'s `markup` option controls tag translation (`'html'` for Telegram's `<b>`/`<code>`, `'markdown'` for WhatsApp/Slack's `*bold*`/`` `code` ``), and `defaultParams` (e.g. `{ platform: 'Telegram' }`) lets one shared template like `notLinked` use a `{{platform}}` placeholder instead of three near-identical strings. This closed tech-debt `bot-i18n-triplication` (ABA-462) — don't reintroduce it by hand-duplicating a shared string across the three `platformMessages` objects.
...
- **Slack bot** (`modules/slack/`): system messages must be localized via `modules/slack/helpers/i18n.ts` (same locale set as Telegram/WhatsApp/mobile — currently 9 languages). Don't hard-code English strings. None of the bot i18n dictionaries (shared or platform-specific) enforce a compile-time "all keys present in all locales" check (unlike mobile i18n, which has the `i18n-add-strings` skill) — when adding a new bot string, manually verify it exists in every locale entry of whichever dictionary it lands in (shared dictionary if used by 2+ bots, otherwise the one bot's own `platformMessages`) — not "add it to all three bots' files."
