...
- **Local-first tab hydration**: list-bearing tabs (`(tabs)/index`, `expenses`, `budgets`, `analytics`) call their store's `loadXxx()` from `useEffect([currentAccountId])` only — **not** from `useFocusEffect`. Stores (`expenseStore`, `incomeStore`, `budgetStore`, `categoryStore`) read SQLite first, set `isLoading: false` immediately, then refresh from API in the background — so the tab paints from cache instantly and updates as the network call returns. The on-focus reload was removed (and `freezeOnBlur`/`lazy` dropped from `(tabs)/_layout.tsx`) because the "background refresh" inside `loadXxx` actually decrypts + upserts every server record and cascades into `loadCategories/loadProjects`, freezing the JS thread on every tab switch and making taps feel unresponsive. Pull-to-refresh + post-mutation updates handle staleness; the receipt-image fetch inside `expense/[id]` is also deferred via `InteractionManager` so opening a transaction is instant. Empty list + `isLoading=true` shows a centered `ActivityIndicator`; empty list + `!isLoading` shows the "Add your first…" empty state. Account switches re-trigger the fetch via the `currentAccountId` dep.
