...
- **Features**: `src/features/analytics/useAnalytics.ts` — analytics computations hook. Returns `AnalyticsSummary` (incl. `vsAverage: number` — signed % vs trailing 3-month rolling average, computed locally from SQLite expense store). `src/features/scenario/useScenarioProjection.ts` — scenario simulator projection hook (pure client-side, reads expense/income stores, projects savings over 3/6/12 months)
