 ...
 * `apps/api/src/common/utils/wallet-currencies.ts` — the API has no build step
 * for workspace packages and must not import `@budget/shared-utils` at runtime
 * (see the check-no-shared-utils-runtime-import.sh deploy guard), while the
 * mobile app computes its own wallet summary locally and needs the same answer.
 * Same rule table on both sides — change one, change the other.
 */
