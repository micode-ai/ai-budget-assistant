 ...
 *
 * MIRROR: apps/api/src/common/utils/financial-month.ts holds the canonical
 * copy used by the API. The API cannot import shared-utils at runtime (no
 * build step; prod Node throws ERR_UNSUPPORTED_DIR_IMPORT -- see ABA-252/253
 * and scripts/check-no-shared-utils-runtime-import.sh), so the duplication is
 * deliberate. Both copies are covered by the same case table; change one,
 * change the other.
 *
